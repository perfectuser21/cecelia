...
- [x] Brain 版本已 bump（patch）
      Test: manual:node -e "const p=JSON.parse(require('fs').readFileSync('packages/brain/package.json','utf8'));if(p.version==='1.129.0')process.exit(1);console.log('version:',p.version)"
