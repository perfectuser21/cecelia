...
{"task_bundle":{"run_id":"11111111-1111-4111-8111-111111111111","attempt_id":"22222222-2222-4222-8222-222222222222","role":"planner","inputs":{"task_id":"$TASK_ID","sprint_dir":"$SPRINT_DIR","planner_branch":"$PLANNER_BRANCH","workspace_spec":{"repo":"perfectuser21/zenithjoy-workspace"}}}}
JSON
printf '%s\n' \
...
if HARNESS_NODE=planner \
  CECELIA_TASK_ID="$TASK_ID" \
  SPRINT_DIR="$SPRINT_DIR" \
  PLANNER_BRANCH='cp-harness-prd-aaaaaaaa-a3' \
  WORKTREE_PATH="$WORKSPACE" \
  HARNESS_TASK_BUNDLE_FILE="$TASK_BUNDLE" \
    finalize_planner_output "$PROVIDER_RESULT" >/dev/null 2>&1; then
