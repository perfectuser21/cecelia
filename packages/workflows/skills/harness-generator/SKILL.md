...
updated: 2026-07-17
changelog:
  - 7.10.0: 恢复 workstream_index 定向（gear=segmented 专用）——Step 0 读 WORKSTREAM_INDEX/WORKSTREAM_COUNT；WS_COUNT>1 时定向实现指定段（task-plan.json tasks[WS_IDX-1] 的 files/dod 范围）；WS_IDX=1 自动进骨架模式；Step 1 按 IS_SEGMENTED 过滤测试文件；单段/默认模式行为不变（向后兼容）
...
Brain / controller 在 prompt 头部注入：
...
WORKSTREAM_INDEX={1..N}    # segmented gear 时注入；缺省=1（单段模式）
WORKSTREAM_COUNT={N}       # segmented gear 时注入；缺省=1
```
...
# workstream 定向（segmented gear 专用；缺省=单段模式）
WS_IDX="${WORKSTREAM_INDEX:-1}"
WS_COUNT="${WORKSTREAM_COUNT:-1}"
IS_SEGMENTED=$( [ "$WS_COUNT" -gt 1 ] && echo true || echo false )
IS_SKELETON=$( [ "$WS_IDX" = "1" ] && [ "$IS_SEGMENTED" = "true" ] && echo true || echo false )
echo "[generator] WS_IDX=$WS_IDX WS_COUNT=$WS_COUNT IS_SKELETON=$IS_SKELETON"
```

**Workstream 定向逻辑（segmented 多段模式）**：
- `WS_COUNT=1`（默认）：读全部 DoD，实现全部功能（与 v7.0 前行为一致）
- `WS_COUNT>1 && WS_IDX=1`：骨架段——读合同全貌，只实现结构/接口/stub；commit msg 用 `(Skeleton Red)/(Skeleton Green)`
- `WS_COUNT>1 && WS_IDX>1`：功能段——只读 task-plan.json 中 `tasks[WS_IDX-1]` 的 dod 范围；只实现该段 scope 内的文件

```bash
if [ "$IS_SEGMENTED" = "true" ]; then
  TASK_PLAN="${SPRINT_DIR}/task-plan.json"
  WS_TITLE=$(jq -r ".tasks[$((WS_IDX-1))].title // \"段$WS_IDX\"" "$TASK_PLAN" 2>/dev/null || echo "段$WS_IDX")
  WS_FILES=$(jq -r ".tasks[$((WS_IDX-1))].files // [] | .[]" "$TASK_PLAN" 2>/dev/null || echo "")
  WS_DOD=$(jq -r ".tasks[$((WS_IDX-1))].dod // [] | .[]" "$TASK_PLAN" 2>/dev/null || echo "")
  echo "[generator] 定向实现段 $WS_IDX: $WS_TITLE"
  echo "  files: $WS_FILES"
  echo "  dod: $WS_DOD"
fi
```

**Skeleton Task 检测（向后兼容）**：
- segmented 模式中 `IS_SKELETON` 由 workstream_index 自动推导（见上）
- 非 segmented 模式：`IS_SKELETON=$(echo "$TASK_PAYLOAD" | jq -r '.is_skeleton // false')`

若进入 Skeleton 模式：目标是让 E2E 测试从 Red 变 Green，中间层允许 stub。
...
# segmented 模式：只读本段的测试文件（tests/ws${WS_IDX}/ 或按文件名前缀过滤）
# 单段模式：读全部测试文件
if [ "$IS_SEGMENTED" = "true" ]; then
  git ls-tree -r "origin/${CONTRACT_BRANCH}" -- "${SPRINT_DIR}/tests/" | grep -E "ws${WS_IDX}[._-]|/ws${WS_IDX}/" || \
...
**只读 contract-draft.md，CONTRACT IS LAW。segmented 模式额外约束：只实现 task-plan.json `tasks[WS_IDX-1].files` 白名单内的文件；白名单外的文件禁止改动（由 evaluator 段验核查）。**
