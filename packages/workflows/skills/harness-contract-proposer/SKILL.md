...
|---|---|---|---|
| 整个 Sprint | `sprints/<本sprint目录>/tests/xxx.test.ts` | {行为列表} | → N failures |
````

**Test Contract 表「Test File」路径死规则（v9.26 — r33 run 7f939e7c 实证）**：

Test File 列必须写**完整真实路径**（`sprints/<本sprint目录>/tests/<文件名>`），禁止
