...
updated: 2026-07-28
changelog:
  - 9.17.0: Kernel raw result channel — channel version presence 判 managed，显式 result file 统一经 runner-owned raw-result-writer；managed 禁 stdout 猜测，channel/file 均 unset 时保留 WORKSPACE_PATH/.brain-result.json
...
`BRAIN_RESULT_CHANNEL_VERSION` 存在时是 managed Kernel，claimed JSON 必须经
stdin 交给固定 runner helper；空值、未知值或缺 result file 一律 fail closed。
channel version unset 但 `BRAIN_RESULT_FILE` 存在时是 headed/relay legacy
override，也交中央 helper 原子写。两者都 unset 才保留向
`${WORKSPACE_PATH:-/workspace}/.brain-result.json` 写入的角色 fallback：
