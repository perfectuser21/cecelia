...
```
开始 → Spec → [spec_review Gate] → Code → [Janitor 自检] → [code_review_gate Gate] → Push → PR → CI → Ship(Learning+合并+Clean) ✅
```
...
### Janitor 自检（Stage 2 末尾，code_review_gate 前必须完成）

代码写完、DoD 验证通过之后，在派发 code_review_gate 之前，对本次改动做一次自检：

```bash
# 脚本层（自动）
git diff origin/main...HEAD -- '*.js' '*.ts' | grep -E "^\+.*console\.(log|warn|error)" | grep -v "// " | head -20
# 如有 console.log → 删除（测试调试用的，不应进 PR）
```

**AI 自检层（读自己的 diff）**：

- 有没有注释掉的旧代码？→ 删除
- 有没有新旧逻辑并存（改了 A 但没删旧 A）？→ 删旧的
- 有没有重复实现同一功能（两处写了同样的逻辑）？→ 合并
- 有没有不再使用的 import？→ 删除

**规则**：只删、只合并，不新增功能。Janitor 不是 reviewer，是清洁工。
...
> **"停顿"含义澄清**：停顿指停止执行（等用户说"继续"）。
> "等 stop hook 放行"≠停顿——它是 Claude 在后台继续执行，由 stop hook 在会话结束时检查是否完成。
> 即：整个 Stage 1-4 是连续执行的，stop hook 只在每次"尝试结束会话"时触发检查。
...
- ✅ 完成 Stage 2 (Code) → **Janitor 自检** → 派发 code_review_gate → **等 stop hook 放行** → 执行 Stage 3
- ✅ 完成 Stage 3 (Integrate) → CI 通过 → **自动放行** → 执行 Stage 4
...
TaskCreate({ subject: "Stage 2: Code", description: "探索+DoD定稿+Generator subagent写代码+本地验证", activeForm: "写代码" })
TaskCreate({ subject: "Stage 3: Integrate", description: "push+创建PR+等CI+code_review_gate Gate", activeForm: "集成" })
...
```
开始 → Worktree → Stage 1 Spec → [spec_review] → Stage 2 Code → [code_review_gate] → Stage 3 Integrate → Stage 4 Ship → 完成
```
...
Stage 2: Code      → Generator subagent 写代码 + 自验证（逐条跑 DoD Test）→ Janitor 自检 → 派发 code_review_gate → 等 stop hook 放行
