...
const { escapeForJS, withRetry } = require('../../weibo-publisher/scripts/utils.cjs');
    ...
    const publishResult = await cdp.send('Runtime.evaluate', {
      expression: `(function() {
        ...
        const submitBtn = document.querySelector(
          'button[class*="publish"]:not([disabled]), button[class*="submit"]:not([disabled])'
        );
        ...
        return {
          clicked: false,
          error: '未找到发布按钮',
          btns: buttons.filter(b => b.offsetWidth > 0).map(b => b.textContent.trim()).slice(0, 8)
        };
      })()`,
      returnByValue: true,
    });
