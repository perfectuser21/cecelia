...
updated: 2026-07-28
changelog:
  - 8.17.0: Kernel raw result channel — Step 3 新增可执行 planner writer；真实 checkout 后立即从 git 回读并固定 BRANCH/PLANNER_BRANCH；channel version presence 判 managed，显式 result file 统一经 runner-owned raw-result-writer，二者 unset 保持 stdout-only；planner raw 强制含 status + review_required
...
# 分支名含实时分钟，不能靠前文重算；checkout 成功后以 git 真相回读给 writer。
...
`BRAIN_RESULT_CHANNEL_VERSION` 只要存在即为 managed Kernel（空值/未知值也必须
fail closed）；`BRAIN_RESULT_FILE` 存在但 channel version unset 时是
headed/relay legacy override，同样经中央 helper 原子写入。两者都 unset 才保持
planner 原 stdout-only 行为，不新建 workspace result 文件。
