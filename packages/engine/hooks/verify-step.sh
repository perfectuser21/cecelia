...
$whitelist_out
  ...
  修复：
    ...
    # Gate Generator: Generator seal 物理文件存在性检查
    # 原因：Generator subagent 必须独立提案测试方案，其 seal 文件是运行凭证
    local generator_seal_file="$PROJECT_ROOT/.dev-gate-generator-sprint.${BRANCH}"
    if [[ ! -f "$generator_seal_file" ]]; then
        _fail "Gate Generator: Generator seal 文件不存在
  期望路径: $generator_seal_file
  step_1_spec: done 前必须先运行 Generator subagent（Sprint Contract Gate）
  原因：.dev-gate-generator-sprint 是 Generator 实际运行的物理凭证
  修复：
    1. 运行 Stage 1 Sprint Contract Gate（Generator subagent）
    2. 等待 .dev-gate-generator-sprint.${BRANCH} 文件生成后再标记 done"
    fi
    echo "  ✅ [Gate Generator] Generator seal 已验证: .dev-gate-generator-sprint.${BRANCH}" >&2

    # Gate Evaluator: Evaluator seal 存在性 + verdict=PASS + divergence_count>=1（橡皮图章检测）
    # 原因：Evaluator 必须独立运行并发现至少 1 条分歧，证明非橡皮图章式审查
    local evaluator_seal_file="$PROJECT_ROOT/.dev-gate-spec.${BRANCH}"
    if [[ ! -f "$evaluator_seal_file" ]]; then
        _fail "Gate Evaluator: Evaluator seal 文件不存在
  期望路径: $evaluator_seal_file
  step_1_spec: done 前必须先运行 Evaluator subagent（spec_review）
  原因：.dev-gate-spec 是 Evaluator 实际运行的物理凭证
  修复：
    1. 运行 Stage 1 spec_review（Evaluator subagent）
    2. 等待 .dev-gate-spec.${BRANCH} 文件生成后再标记 done"
    fi

    local evaluator_verdict evaluator_divergence_count
    evaluator_verdict=$(node -e "
        try {
            const d = JSON.parse(require('fs').readFileSync('${evaluator_seal_file}', 'utf8'));
    ...
    if [[ "$evaluator_verdict" != "PASS" ]]; then
        _fail "Gate Evaluator: Evaluator seal verdict 不是 PASS
  文件: $evaluator_seal_file
  当前 verdict: $evaluator_verdict
  要求：Evaluator 必须通过（verdict=PASS）才能进入 Stage 2
  修复：解决 Evaluator 发现的问题后重新运行 spec_review"
    fi

    evaluator_divergence_count=$(node -e "
        try {
            const d = JSON.parse(require('fs').readFileSync('${evaluator_seal_file}', 'utf8'));
            const plans = d.independent_test_plans || [];
            const diverged = plans.filter(function(p){ return p.consistent === false; }).length;
            const nego = (d.negotiation_result || {}).divergence_count;
            process.stdout.write(String(nego !== undefined ? nego : diverged));
        } catch(e) { process.stdout.write('0'); }
    " 2>/dev/null || echo "0")

    # 橡皮图章检测：divergence_count=0 时需要额外验证
    # 豁免条件：若 sprint-contract-state round >= 2，说明经历多轮对抗后收敛（合法）
    # 若 round <= 1 且 divergence_count=0，则很可能是橡皮图章（一次过）
    if [[ "$evaluator_divergence_count" -eq 0 ]]; then
        # 读取 sprint-contract-state round（用于橡皮图章豁免判断）
        local contract_state_file_for_check="$PROJECT_ROOT/.sprint-contract-state.${BRANCH}"
        local pre_check_round=0
        if [[ -f "$contract_state_file_for_check" ]]; then
            pre_check_round=$(node -e "
                try {
                    const s = JSON.parse(require('fs').readFileSync('${contract_state_file_for_check}', 'utf8'));
                    process.stdout.write(String(s.round || 0));
                } catch(e) { process.stdout.write('0'); }
            " 2>/dev/null || echo "0")
        fi

        if [[ "$pre_check_round" -lt 2 ]]; then
            _fail "Gate Evaluator: 橡皮图章检测失败（divergence_count=0，round=${pre_check_round}）
  文件: $evaluator_seal_file
  当前 divergence_count: $evaluator_divergence_count
  sprint-contract-state round: $pre_check_round
  要求：Evaluator 必须在某一轮中发现至少 1 条分歧（divergence_count >= 1）
        或经历多轮对抗收敛（sprint-contract-state round >= 2）
  原因：divergence_count=0 且 round<=1 意味着 Evaluator 同意了主 agent 的所有测试方案，
        未进行独立分析，Sprint Contract 对抗机制形同虚设
  修复：
    1. 若这是第一次运行：确保 Evaluator 独立提案测试方案，与 Generator/主 agent 存在合理差异
    2. 若经历了多轮：检查 sprint-contract-state round 是否正确（应 >= 2）"
        fi
        echo "  ✅ [Gate Evaluator] 多轮收敛豁免: divergence_count=0 但 round=${pre_check_round} >= 2（合法收敛）" >&2
    fi
    echo "  ✅ [Gate Evaluator] Evaluator seal 已验证: verdict=${evaluator_verdict}, divergence_count=${evaluator_divergence_count}" >&2

    # Gate Sprint Contract State: .sprint-contract-state 存在性 + round>=1 检查
    # 原因：sprint-contract-loop.sh 必须运行并产生状态文件，证明收敛循环真实执行
    local contract_state_file="$PROJECT_ROOT/.sprint-contract-state.${BRANCH}"
    if [[ ! -f "$contract_state_file" ]]; then
        _fail "Gate Sprint Contract State: .sprint-contract-state 文件不存在
  期望路径: $contract_state_file
  step_1_spec: done 前必须先运行 sprint-contract-loop.sh
  原因：.sprint-contract-state 是收敛循环真实执行的物理凭证
  修复：
    1. 运行 bash packages/engine/scripts/devgate/sprint-contract-loop.sh ${BRANCH}
    2. 等待文件生成且 blocker_count=0 后再标记 done"
    fi

    local contract_round
    contract_round=$(node -e "
        try {
            const s = JSON.parse(require('fs').readFileSync('${contract_state_file}', 'utf8'));
            process.stdout.write(String(s.round || 0));
        } catch(e) { process.stdout.write('0'); }
    " 2>/dev/null || echo "0")

    if [[ "$contract_round" -lt 1 ]]; then
        _fail "Gate Sprint Contract State: round < 1（sprint-contract-loop.sh 未真实执行）
  文件: $contract_state_file
  当前 round: $contract_round
  要求：sprint-contract-loop.sh 必须至少执行 1 轮（round >= 1）
  修复：运行 bash packages/engine/scripts/devgate/sprint-contract-loop.sh ${BRANCH}"
    fi
    echo "  ✅ [Gate Sprint Contract State] 收敛循环已验证: round=${contract_round}" >&2
