...
## 2026-02-27: stop.sh 路由器层无头绕过 Bug (v12.30.9)

**问题**：`stop-dev.sh` 在 v12.11.1 删除了无头旁路，但路由器 `stop.sh` 忘记同步，
导致 `CECELIA_HEADLESS=true` 时直接 exit 0，Stop Hook 完全失效。

**教训**：修复多层架构中某一层的行为时，必须同时检查所有上层入口点。
路由器/门面模式中，上层封装往往有独立的"快速退出"逻辑，需要一并清理。
...
### [2026-03-04] 系统修复 Learning 顺序：5处矛盾一次清零

**失败统计**：CI 失败 0 次，本地测试失败 0 次

**错误判断记录**：
- 上次 PR #524 以为只改 SKILL.md 顶部描述就够了，但 09-ci.md 的状态表、正确行为列表、第9.4节仍有"CI 通过 → 合并 PR"，AI 读这些时直接走错

**预防措施**：
- 修改任何步骤执行顺序时，必须 grep 所有相关文件中的"合并"字样，确认全部一致
- 09-ci.md 里永远不出现 gh pr merge 命令（合并职责归 Step 10，不归 Step 9）
