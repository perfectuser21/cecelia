...
text_contains_token() {
    ...
    for pattern in "${TOKEN_PATTERNS[@]}"; do
        if echo "$text" | grep -qE "$pattern"; then
            ...
            if echo "$text" | grep -qiE '(^YOUR_|^PLACEHOLDER_|^EXAMPLE_|_YOUR_KEY$|_YOUR_SECRET$|_YOUR_TOKEN$|^example$|^placeholder$|^xxx+$|PLACEHOLDER|YOUR_API|YOUR_TOKEN|YOUR_KEY|YOUR_SECRET|CHANGE_ME|INSERT_HERE)'; then
                continue
            fi
            return 0
        fi
