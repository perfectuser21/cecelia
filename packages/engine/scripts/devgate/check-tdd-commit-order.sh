...
# ── Check 2: commit 1 message 含 (Red) / test( / chore(harness) ──────
# chore(harness): import contract 是合法的 contract import commit
if ! echo "$COMMIT_1_MSG" | grep -qE '\(Red\)|^test\(|^chore\(harness\)'; then
  echo -e "  ${RED}❌ commit 1 message 缺 (Red) 或 test( 前缀（或 chore(harness): 导入合同）${RESET}"
  VIOLATIONS=$((VIOLATIONS + 1))
fi
