...
created: 2026-04-15
changelog:
  - 1.1.0: 重塑为 Research Subagent 查询工具，decisions 作为推理输入非硬约束
  - 1.0.0: 初版 — autonomous_mode 查 Brain decisions 表作为约束源
---

> **v1.1.0 重塑说明 (autonomous-research-proxy 引入后)**:
> 本 skill 不再是 autonomous 主流程自动执行的 step, 改为 Research Subagent 按需调用的**查询工具**。
> Decisions 是 Subagent 调研时的**推理输入**（理由参考），不是**硬约束**。
> Subagent 读决策后做当下代码分析, first-principles 推导, 决策可被 override。

# Step 0.7: Decision Query — 查 Alex 历史决策作为约束

> Phase 1 统一后默认激活。在 00.5-enrich 之后、01-spec 之前执行。
...
# Phase 1 统一后永远激活，Research Subagent 可按需消费结果
```
