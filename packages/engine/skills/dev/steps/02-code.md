...
## 2.3.6：/simplify（代码简化 — 强制，不可跳过）

> **Cleanup 之后、本地验证之前执行。使用 Claude Code 内置 /simplify skill 让代码更简洁。**
> **与 Cleanup 的区别：Cleanup 扫垃圾（删无用代码），/simplify 做重构（让代码更简洁高效）。**

执行 `/simplify` skill，它会：
1. 审查本次改动的代码复用性、质量和效率
2. 发现可以简化的地方（过长函数拆分、嵌套展平、循环改高阶函数等）
3. **直接修改代码**（不是只报告，是实际改）
...
- 复用性：相似逻辑是否可以提取为公用函数
- 质量：命名是否清晰、逻辑是否直观
- 效率：是否有不必要的循环、重复计算、过度抽象
```

**执行方式**：调用 `/simplify` skill（Claude Code 内置，无需额外安装）

**完成后**：检查 /simplify 的改动是否影响 DoD 条目的测试结果。如果影响了，修复后继续。
