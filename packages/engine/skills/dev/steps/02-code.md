...
- `harness_mode = true` → 走 **Section 1**（Brain 派的 harness_generate 任务）
...
## 2. 主路径（Subagent 三角色全自动）

Implementer subagent 派遣由主 agent 做，触发时机来自 Superpowers `subagent-driven-development` skill。
Research Subagent 处理 Superpowers 链中的 user 交互（见 autonomous-research-proxy.md），Implementer 做实际实现。
...
**派发前记录 L2 Evidence**（契约 `subagent-driven-development` + `test-driven-development`）：
...
bash packages/engine/scripts/record-evidence.sh \
  ...
  --prompt packages/engine/skills/dev/prompts/subagent-driven-development/implementer-prompt.md \
  --return-status PENDING
```

Implementer 返回 DONE 后，取 `TDD_RED_LOG` / `TDD_GREEN_LOG` / `TDD_TEST_FILE` 字段补记：

```bash
bash packages/engine/scripts/record-evidence.sh \
  --event tdd_red \
  --test-file "$TDD_TEST_FILE" \
  --test-command "$TDD_TEST_COMMAND" \
  --exit-code 1 \
  --log-path "$TDD_RED_LOG"
...
**派发前记录 L2 Evidence**（契约 `subagent-driven-development` + `requesting-code-review`）：

```bash
bash packages/engine/scripts/record-evidence.sh \
  --event subagent_dispatched \
  --subagent-type spec_reviewer \
  --prompt packages/engine/skills/dev/prompts/subagent-driven-development/spec-reviewer-prompt.md \
  --return-status PENDING
```
...
**完成后记录 L2 Evidence**（契约 `verification-before-completion`）：

```bash
# Evidence: Pre-Completion Verification
bash packages/engine/scripts/record-evidence.sh \
  --event pre_completion_verification \
  --all-pass true \
  --checklist-json '[{"id":"test_pass","pass":true},{"id":"dod_verified","pass":true},{"id":"regression_clean","pass":true}]'
```
