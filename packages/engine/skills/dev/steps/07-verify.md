...
✅ 新流程：写完代码 → npm test + 逐条执行 DoD + 代码审查 → 全部 [x] → PR（省一轮 CI）
```
...
Reviewer 2 — 正确性 & Bug:
  ...
  prompt: |
    ...
    2. 错误处理缺失（try/catch、错误回调、Promise rejection）
    3. 竞态条件或并发问题
    4. 边界情况（空数组、null/undefined、超大输入）
...
Reviewer 3 — 项目规范 & 一致性:
  ...
  prompt: |
    ...
    聚焦审查：
    1. 是否遵循项目现有的命名规范（变量、函数、文件名）
    2. 是否遵循项目的架构模式（数据流、模块边界）
    3. 新增的抽象是否合理（不过度抽象，也不重复造轮子）
    4. 错误消息和日志是否有意义
    5. 是否与项目中类似功能的实现风格一致
...
以下情况可以跳过 7.4：
- 纯文档改动（只改 `.md` 文件）
- 纯配置改动（只改 `package.json` 版本号、`.yml` 文件）
- 变更文件 ≤ 2 个且 diff 行数 ≤ 30 行
