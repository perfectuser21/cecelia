...
LOG_FILE="${DEV_EXEC_LOG_FILE:-.dev-execution-log.jsonl}"
...
main() {
    ...
    # 生成报告（不使用 heredoc，避免 vitest worker thread 中 fd0 为 socket 导致写入失败）
    local efficiency_table
    efficiency_table=$(generate_efficiency_table)
    local quality_analysis
    quality_analysis=$(analyze_quality_with_llm)
    local improvement_suggestions
    improvement_suggestions=$(generate_improvement_suggestions)
    local log_content
    log_content=$(cat "$LOG_FILE" 2>/dev/null || echo "无日志")
    local expectations_content
    expectations_content=$(cat "$EXPECTATIONS_FILE" 2>/dev/null || echo "{}")
    local stability_retries
    stability_retries=$(echo "$stability_json" | jq -r '.total_retries')
    local stability_ci_rate
    stability_ci_rate=$(echo "$stability_json" | jq -r '.ci_pass_rate')
    local automation_rate
    automation_rate=$(echo "$automation_json" | jq -r '.automation_rate')
    local report_date
...

    {
        ...
        printf '%s\n\n' '## 稳定性维度'
        printf '```json\n%s\n```\n\n' "$stability_json"
        printf '%s\n' "- **重试次数**: $stability_retries"
        printf '%s\n\n' "- **CI 通过率**: $stability_ci_rate"
        printf '%s\n\n' '---'
        printf '%s\n\n' '## 自动化维度'
        printf '```json\n%s\n```\n\n' "$automation_json"
        printf '%s\n\n' "- **自动化程度**: $automation_rate"
        printf '%s\n\n' '---'
        printf '%s\n\n' '## 质量维度'
        printf '%s\n\n' "$quality_analysis"
        printf '%s\n\n' '---'
