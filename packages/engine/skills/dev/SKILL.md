...
description: |
  ...
  Stage 3: Integrate（push + CI）→ 派发 code_review_gate → 等 stop hook 放行
...
```
开始 → Spec → [spec_review Gate] → Code → Push → PR → CI → [code_review_gate Gate] → Ship(Learning+合并+Clean) ✅
```
...
4. CI 状态？
   ...
   - SUCCESS → code_review_gate PASS？（查 Brain API）
     ❌ PENDING → exit 2 → 等 Codex
     ❌ FAIL → exit 2 → 修代码
     ✅ PASS → 继续
...
- ✅ 完成 Stage 3 (Integrate) → CI 通过后派发 code_review_gate → **等 stop hook 放行** → 执行 Stage 4
...
```
开始 → Worktree → Stage 1 Spec → [spec_review] → Stage 2 Code → Stage 3 Integrate → [code_review_gate] → Stage 4 Ship → 完成
```
...
│   ├── 03-integrate.md     ← Stage 3: Push + CI + code_review_gate Gate
...
Stage 3: Integrate → push + 创建 PR + CI → 派发 code_review_gate → 等 stop hook 放行
...
| Stage 3: Integrate | Step 3 PR+CI | 删除 4 个 Codex 注册，改为 CI 后 1 个 code_review_gate |
