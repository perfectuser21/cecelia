...
description: |
  ...
  Stage 2: Code（写代码 + 本地验证）→ 派发 code_review_gate → 等 stop hook 放行
...
- ✅ 完成 Stage 1 (Spec) → spec_review subagent 同步审查（~10秒）→ PASS 后**立即**执行 Stage 2
- ✅ 完成 Stage 2 (Code) → code_review_gate subagent 同步审查（~10秒）→ PASS 后**立即**执行 Stage 3
- ✅ 完成 Stage 3 (Integrate) → CI 通过 → **自动放行** → 执行 Stage 4
- ✅ 完成 Stage 4 (Ship) → PR 合并 → 完成
- ✅ 一直执行到 PR 合并为止

### ⚠️ 关键澄清：「不要停顿」vs「等 stop hook 放行」

| 阶段 | 正确行为 | 原因 |
|------|----------|------|
| Stage 1 → Stage 2 | spec_review subagent **同步调用**，PASS 后立即继续（无需停顿） | subagent 是同步调用，直接返回结果 |
| Stage 2 → Stage 3 | code_review_gate subagent **同步调用**，PASS 后立即继续（无需停顿） | subagent 是同步调用，直接返回结果 |
| Stage 3 → Stage 4 | CI 状态轮询，通过后立即继续（无需停顿） | CI 结果可以实时查询 |
| 任何阶段遇到困难 | **分析 → 修复 → 继续**，不停顿等用户 | Stop Hook 循环机制保证不丢失进度 |

**Stop Hook 的作用**：不是"让 AI 停下来等"，而是在**会话尝试结束时**检查完成条件——若未完成则 `exit 2` 让 Claude 继续执行。AI 不需要主动"等待" stop hook，只需持续自动执行直到 PR 合并。
...
│   ├── 02-code.md          ← Stage 2: Code + 自验证 + code_review_gate Gate
