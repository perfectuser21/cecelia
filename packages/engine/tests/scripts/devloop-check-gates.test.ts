...
describe('devloop-check.sh — 4-Stage Pipeline 门禁条件（subagent 架构）', () => {
  ...
  describe('spec_review / code_review_gate 改为 Agent subagent（旧 Codex 路径已删除）', () => {
    ...
    // v16.0.0: spec_review gate 已从 devloop-check.sh 删除（Engine重构）
    it.skip('注释中说明 spec_review 由 Agent subagent 同步完成', () => {
  ...
  describe.skip('4-Stage Pipeline 条件完整性', () => {
  ...
  // v16.0.0: seal防伪机制及gate条件已删除（Engine重构）
  describe.skip('三阶段 seal 对齐检查（条件 1.6 / 2.8）', () => {
...
// v16.0.0: seal防伪机制及gate条件已删除（Engine重构）
describe.skip('stop-dev.sh — 15min 超时自动 pass 降级机制', () => {
