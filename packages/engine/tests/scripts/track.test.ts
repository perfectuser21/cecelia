...
describe('track.sh', () => {
  ...
  beforeEach(() => {
    ...
    // 复制脚本到测试目录（基于 __dirname 找到 skills/dev/scripts/track.sh）
    const engineRoot = path.resolve(__dirname, '../..')
    const scriptPath = path.join(engineRoot, 'skills/dev/scripts/track.sh')
...
`)
  ...
  describe('分支级别文件隔离', () => {
    ...
    it('should isolate run_id between branches', () => {
      ...
      // 在 branch-a 创建 run
      execSync('git checkout -b branch-a', { cwd: testDir, stdio: 'pipe' })
      execSync(`bash ${trackScript} start project-a branch-a .prd.md`, { cwd: testDir, stdio: 'pipe' })
      const runIdA = fs.readFileSync(path.join(testDir, '.cecelia-run-id-branch-a'), 'utf-8').trim()

      // 切换到 branch-b
      execSync('git checkout -b branch-b', { cwd: testDir, stdio: 'pipe' })

      // branch-b 没有自己的 run
      const statusB = execSync(`bash ${trackScript} status`, { cwd: testDir, stdio: 'pipe' }).toString()
      expect(statusB).toContain('No active run')

      // branch-a 的 run 仍然存在
      execSync('git checkout branch-a', { cwd: testDir, stdio: 'pipe' })
      expect(fs.existsSync(path.join(testDir, '.cecelia-run-id-branch-a'))).toBe(true)
      expect(fs.readFileSync(path.join(testDir, '.cecelia-run-id-branch-a'), 'utf-8').trim()).toBe(runIdA)
    })
  })
  ...
  describe('status 命令', () => {
    it('should show "No active run" when no run exists', () => {
      ...
      const output = execSync(`bash ${trackScript} status`, { cwd: testDir }).toString()
    ...
    it('should show run id when run exists', () => {
      ...
      // 创建 run
      execSync(`bash ${trackScript} start test-project test-branch .prd.md`, { cwd: testDir, stdio: 'pipe' })
  ...
  describe('done 命令', () => {
    it('should clear run_id after done', () => {
      ...
      // 创建 run
      execSync(`bash ${trackScript} start test-project test-branch .prd.md`, { cwd: testDir, stdio: 'pipe' })
      expect(fs.existsSync(path.join(testDir, '.cecelia-run-id-test-branch'))).toBe(true)

      // 完成 run
      execSync(`bash ${trackScript} done https://github.com/test/pr/1`, { cwd: testDir, stdio: 'pipe' })

      // run_id 应该被清理
      expect(fs.existsSync(path.join(testDir, '.cecelia-run-id-test-branch'))).toBe(false)
    })
  })
  ...
  describe('fail 命令', () => {
    it('should keep run_id after fail for retry', () => {
      ...
      // 失败
      execSync(`bash ${trackScript} fail "Test error"`, { cwd: testDir, stdio: 'pipe' })
  ...
  describe('向后兼容', () => {
    it('should read legacy .cecelia-run-id file', () => {
      ...
      // 创建旧格式文件
      fs.writeFileSync(path.join(testDir, '.cecelia-run-id'), 'legacy-run-id')
    ...
    it('should prefer branch-specific file over legacy file', () => {
      ...
      // 创建两种格式的文件
      fs.writeFileSync(path.join(testDir, '.cecelia-run-id'), 'legacy-run-id')
      fs.writeFileSync(path.join(testDir, '.cecelia-run-id-test-branch'), 'branch-run-id')
  ...
  describe('无效命令', () => {
    it('should show usage for invalid command', () => {
      try {
        execSync(`bash ${trackScript} invalid-cmd`, { cwd: testDir, stdio: 'pipe' })
