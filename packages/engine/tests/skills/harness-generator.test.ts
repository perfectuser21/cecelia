...
const SKILL_PATH = join(os.homedir(), '.claude', 'skills', 'harness-generator', 'SKILL.md');
...
describe('harness-generator v6.0 结构', () => {
  ...
  it.skipIf(!skillExists)('TDD Red 阶段：commit 1 只含测试文件 + DoD', () => {
  ...
  it.skipIf(!skillExists)('TDD Green 阶段：commit 2 含实现', () => {
  ...
  it.skipIf(!skillExists)('Verification 阶段：push 前贴测试证据', () => {
  ...
  it.skipIf(!skillExists)('Code Review 阶段：调 subagent 审 diff', () => {
  ...
  it.skipIf(!skillExists)('Mode 2 (harness_fix) 用 systematic-debugging', () => {
  ...
  it.skipIf(!skillExists)('changelog 有 5.0.0 条目说明 superpowers + TDD', () => {
