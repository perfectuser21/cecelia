...
export async function runStagingE2E(task, opts = {}) {
  ...
  try {
    ...
    // 刀3-3b：unknown 线（第三方 repo，非 cecelia/zenithjoy）不跑任何 deploy，
    // 直接置 pending_promote + 飞书通知，避免把 cecelia brain 错误部署到 :5222。
    if (line === 'unknown') {
      await recordResult(dbPool, { ...base, verdict: 'SKIP', reason: 'unknown_line_no_deploy' });
      await updatePromoteStatus(dbPool, prUrl, PROMOTE_STATUS.PENDING_PROMOTE);
      const notify = opts.notify || sendFeishu;
      try {
        await notify(
          `⏳ [Unknown 线 Pending] 非 cecelia/zenithjoy repo，跳过 staging deploy，等主理人确认\n`
          + `base_repo: ${baseRepo || '(缺失)'}\ninitiative: ${initiativeId || '?'}\nPR: ${prUrl || '?'}\n`
          + `confirm: POST /api/brain/harness/promote/<resultId>`
        );
      } catch (e) {
        console.warn(`[staging-e2e] unknown 线 pending 通知失败（忽略）: ${e.message}`);
      }
      await writeTaskResult(dbPool, task.id, {
        verdict: 'SKIP', reason: 'unknown_line_no_deploy',
        pr_url: prUrl, initiative_id: initiativeId,
        promote_status: PROMOTE_STATUS.PENDING_PROMOTE,
      });
      await updateTaskStatus(task.id, 'completed');
      return { success: true, taskId: task.id, verdict: 'SKIP', reason: 'unknown_line_no_deploy', promoteStatus: PROMOTE_STATUS.PENDING_PROMOTE };
    }
