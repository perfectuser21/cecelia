...
export async function generateCompareReport({ project_ids, format = 'json', include_tasks = false }) {
  ...
  // 查询项目基础信息（新 OKR 表：okr_projects UNION okr_scopes UNION okr_initiatives）
  const projectResult = await pool.query(
    `SELECT id, title AS name, 'project' AS type, status, created_at, updated_at FROM okr_projects WHERE id = ANY($1::uuid[])
     UNION ALL
     SELECT id, title AS name, 'scope' AS type, status, created_at, updated_at FROM okr_scopes WHERE id = ANY($1::uuid[])
     UNION ALL
     SELECT id, title AS name, 'initiative' AS type, status, created_at, updated_at FROM okr_initiatives WHERE id = ANY($1::uuid[])`,
    [project_ids]
  );
