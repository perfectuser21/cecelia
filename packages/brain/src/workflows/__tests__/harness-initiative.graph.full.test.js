...
describe('reportNode', () => {
  ...
  it('PASS → UPDATE initiative_runs phase=done', async () => {
    mockPool.query.mockResolvedValueOnce({ rows: [] }); // step_timing SELECT
    ...
    const sqlArgs = mockPool.query.mock.calls[1]; // calls[1] = UPDATE initiative_runs
  ...
  it('PASS → 第 4 次 query 是 INSERT INTO tasks (harness_report spawn)', async () => {
    ...
    // 4 queries: step_timing SELECT + UPDATE initiative_runs + UPDATE tasks + INSERT harness_report
    expect(mockPool.query).toHaveBeenCalledTimes(4);
    const insertCall = mockPool.query.mock.calls[3]; // calls[3] = INSERT harness_report
