...
describe('reportNode', () => {
  ...
  it('PASS → UPDATE initiative_runs phase=done', async () => {
    mockPool.query.mockResolvedValueOnce({ rows: [] }); // SELECT task_events (step_timing)
    mockPool.query.mockResolvedValueOnce({ rowCount: 1 }); // UPDATE initiative_runs
    ...
    const sqlArgs = mockPool.query.mock.calls[1]; // call[1] = UPDATE initiative_runs
  ...
  it('FAIL → UPDATE phase=failed + failure_reason', async () => {
    mockPool.query.mockResolvedValueOnce({ rows: [] }); // SELECT task_events (step_timing)
    mockPool.query.mockResolvedValueOnce({ rowCount: 1 }); // UPDATE initiative_runs
    mockPool.query.mockResolvedValueOnce({ rowCount: 1 }); // UPDATE tasks
    const delta = await reportNode({
  ...
  it('PASS → 同时 UPDATE tasks SET status=completed (B1)', async () => {
    ...
    mockPool.query.mockResolvedValueOnce({ rowCount: 1 }); // UPDATE initiative_runs
    mockPool.query.mockResolvedValueOnce({ rowCount: 1 }); // UPDATE tasks
    mockPool.query.mockResolvedValueOnce({ rowCount: 1 }); // INSERT harness_report
    await reportNode({
      initiativeId: 'i', sub_tasks: [{ id: 's1' }], final_e2e_verdict: 'PASS',
    });
    expect(mockPool.query).toHaveBeenCalledTimes(4);
    const taskUpdate = mockPool.query.mock.calls[2]; // call[2] = UPDATE tasks
  ...
  it('PASS → 第 4 次 query 是 INSERT INTO tasks (harness_report spawn)', async () => {
    ...
    expect(mockPool.query).toHaveBeenCalledTimes(4);
    const insertCall = mockPool.query.mock.calls[3]; // call[3] = INSERT harness_report
