...
export async function runSelfCheck(pool, opts = {}) {
  ...
  // 5. Schema version: DB must be >= EXPECTED_SCHEMA_VERSION (minimum acceptable).
  //    DB ahead of minimum  → forward-compatible, legal state → WARN only.
  //    DB behind minimum    → code requires migrations not yet applied → FAIL.
  //    Exact equality is NOT required; insisting on it would reject valid forward-compat DBs.
  try {
    const { rows } = await pool.query(
    ...
    if (versionOk && dbVerNum > expectedVerNum) {
      console.warn(`  [WARN] Schema Version: DB="${maxVer}" > minimum="${EXPECTED_SCHEMA_VERSION}" — DB is ahead of code minimum (forward-compat)`);
    }
    record(
      'Schema Version',
      versionOk,
      versionOk
        ? `DB="${maxVer}" >= minimum="${EXPECTED_SCHEMA_VERSION}"${dbVerNum > expectedVerNum ? ' (DB ahead — forward-compat)' : ''}`
        : `DB="${maxVer}" is behind minimum="${EXPECTED_SCHEMA_VERSION}" — run migrations`
    );
  } catch (err) {
