...
export async function compareGoalProgress(goalId = null) {
  // Get goals
  // 新 OKR 表：UNION ALL key_results + objectives（UUID 与旧 goals 相同，tasks.goal_id 可直接 JOIN）
  let goalsQuery = `
    SELECT g.id, g.title, g.status, g.progress, g.created_at, g.priority,
  ...
  goalsQuery += ' GROUP BY g.id, g.title, g.status, g.progress, g.created_at, g.priority ORDER BY g.priority';
