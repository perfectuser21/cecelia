...
当 event.type = "user_message" 或 "owner_intent" 时，根据意图创建任务：

| 意图类型 | task_type | 说明 |
|---------|-----------|------|
| coding / dev / 编程 / 实现 / fix / bug / 部署 / 自动化 / n8n / PR | dev | 编程、功能开发、bug修复、自动化流程 |
| review / 审查 / 看代码 / 巡检 / code review | code_review | 代码质量审查 |
| qa / test / 测试 / 质量检查 | qa | 测试、QA 验证 |
| audit / 安全扫描 / 安全审计 / security | audit | 安全扫描、代码安全审计 |
| research / 深度调研 / 分析报告 / 研究（需要完整报告） | research | 深度调研，输出完整分析报告 |
| explore / 查一下 / 快速了解 / what is / 轻量调研 | explore | 快速查询，轻量了解（MiniMax 快速版） |
| remember / note / 记下来 / 保存 / 想法 / 记录 | knowledge | 记录想法、保存认知到知识库 |
| okr / KR / goals / 目标 / 战略 / 规划 / 项目拆解 / scope / 范围 / 边界定义 | initiative_plan | OKR拆解、战略规划、Scope 范围界定（秋米） |
| 其他 / 不明确 | dev | 默认路由 |

路由方式：actions 中使用 create_task，task_type 按上表选择，title 来自用户消息。
