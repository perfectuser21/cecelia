...
// standard "all-ok" stubs for A2 / A3a / A3b
const OK_A2   = { rows: [{ count: '0' }] };
const OK_A3_1 = { rows: [] };  // A3a: no orphaned base features (empty rows = all have links)
const OK_A3_2 = { rows: [] };  // A3b: no steps without promise
...
describe('[S4-N7] 全部通过 → sentinel 写入，返回 failures=0', () => {
  it('writes sentinel and returns 0 failures when all pass', async () => {
    ...
    mockQuery
      ...
      .mockResolvedValueOnce({ rows: [] })               // A3a: no orphaned base features
      .mockResolvedValueOnce({ rows: [] })               // A3b: no steps without promise
...
// ── [S4-N10] A3 promise 缺失只查承诺地图域（存量旧步骤豁免）──
