...
describe('selectTargetProject - initiative priority', () => {
  it('should prefer project with active initiative when no queued tasks exist', async () => {
    // Create a KR directly in key_results（selectTargetProject 查 okr_projects WHERE kr_id）
    const krResult = await pool.query(
      "INSERT INTO key_results (title, status, priority) VALUES ('Test KR for select', 'active', 'P1') RETURNING *"
    );
    const kr = krResult.rows[0];
    testKRIds.push(kr.id);

    // Create project in okr_projects with kr_id（selectTargetProject 通过此字段找关联）
    ...
    // Initiative 仅在 state.projects 中手动构造（不需要写入 DB）
    ...
    const state = {
      projects: [
        ...
        { id: fakeInitiativeId, name: 'Test Initiative', status: 'active', type: 'initiative', parent_id: projWithInitResult.rows[0].id }
      ],
      activeTasks: [] // no queued tasks
    };
  ...
  it('should prefer project with queued tasks over project with only initiative', async () => {
    // Create a KR
    const krResult = await pool.query(
      "INSERT INTO key_results (title, priority, status) VALUES ('KR for project comparison', 'P1', 'in_progress') RETURNING *"
    );
    const kr = krResult.rows[0];
    testKRIds.push(kr.id);

    // Project 1: has queued task, linked to KR via kr_id
    ...
    // Project 2: has initiative but no queued task, linked to KR via kr_id
    ...
    // Use a fake initiative ID (in-memory state only, no DB insert needed)
    const fakeInitId = '00000000-0000-0000-0000-000000000002';
    ...
    const state = {
      projects: [
        ...
        { id: fakeInitId, name: 'Init Under Proj2', status: 'active', type: 'initiative', parent_id: proj2Result.rows[0].id }
      ],
      activeTasks: [
...
describe('generateArchitectureDesignTask - auto task generation', () => {
  it('should generate architecture_design task for project with active initiative and no tasks', async () => {
    // Create a KR in key_results (goal_id on tasks is a UUID column, accepts key_results IDs)
    const krResult = await pool.query(
      "INSERT INTO key_results (title, priority, status) VALUES ('KR for initiative plan', 'P1', 'in_progress') RETURNING *"
    );
    const kr = krResult.rows[0];
    testKRIds.push(kr.id);
  ...
  it('should return null when no active initiative exists under project (no task generated)', async () => {
    // Create a KR
    const krResult = await pool.query(
      "INSERT INTO key_results (title, priority, status) VALUES ('KR no initiative', 'P1', 'in_progress') RETURNING *"
    );
    const kr = krResult.rows[0];
    testKRIds.push(kr.id);
  ...
  it('should not create duplicate architecture_design task if one already exists', async () => {
    // Create a KR
    const krResult = await pool.query(
      "INSERT INTO key_results (title, priority, status) VALUES ('KR dedup test', 'P0', 'in_progress') RETURNING *"
    );
    const kr = krResult.rows[0];
    testKRIds.push(kr.id);
  ...
  it('should inherit KR priority for the generated task', async () => {
    // Create a P0 KR
    const krResult = await pool.query(
      "INSERT INTO key_results (title, priority, status) VALUES ('P0 KR priority test', 'P0', 'in_progress') RETURNING *"
    );
    const kr = krResult.rows[0];
    testKRIds.push(kr.id);
