...
describe('production routing authority migrations 413/414/417', () => {
  ...
  it('hardens both production anchors only in migration 417', async () => {
    const sql = await readFile(new URL('../../migrations/417_work_routing_anchor_hardening.sql', import.meta.url), 'utf8');
  ...
  it('hardens the mutable task projection after receipt creation', async () => {
    const sql = await readFile(new URL('../../migrations/421_work_routing_projection_guard.sql', import.meta.url), 'utf8');
