 ...
 * 3. digestLearnings 入口 rumination_run 心跳 INSERT
 * 4. fetchMemoryStreamItems (1 < MAX_PER_TICK → 补充)
 * 5. working_memory (notebook_id)
 * 6. synthesis_archive 历史查询（fallback 时）
 * 7+ remaining: dedup, memory_stream insert, cecelia_events, synthesis_archive write, digested update
 */
function setupForFallback(archiveRows) {
  mockQuery
    ...
    .mockResolvedValueOnce({ rows: [] })                                   // 5. working_memory (no notebook_id)
    .mockResolvedValueOnce({ rows: archiveRows })                          // 6. synthesis_archive history
    .mockResolvedValue({ rows: [] });                                      // 7+. all remaining
}
...
describe('Rumination fallback — synthesis_archive 历史上下文注入（P0 修复）', () => {
  ...
  it('NotebookLM 主路成功时，synthesis_archive fallback 查询不被触发（主路不变）', async () => {
    ...
    mockQuery
      ...
      .mockResolvedValueOnce({ rows: [{ value_json: 'nb-id-123' }] })       // 5. working_memory (has notebook_id)
      .mockResolvedValue({ rows: [] });                                      // 6+. remaining (no fallback synthesis_archive)
