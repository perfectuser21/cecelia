 ...
 * 1. isSystemIdle → idle check
 * 2. learnings fetch (LIMIT MAX_PER_TICK)
 * 3. fetchMemoryStreamItems (1 < MAX_PER_TICK → 补充)
 * 4. working_memory (notebook_id)
 * 5. [NEW] synthesis_archive 历史查询（fallback 时）
 * 6+ remaining: dedup, memory_stream insert, cecelia_events, synthesis_archive write, digested update
 */
function setupForFallback(archiveRows) {
  mockQuery
    ...
    .mockResolvedValueOnce({ rows: [] })                                   // 3. fetchMemoryStreamItems (empty)
    .mockResolvedValueOnce({ rows: [] })                                   // 4. working_memory (no notebook_id)
    .mockResolvedValueOnce({ rows: archiveRows })                          // 5. synthesis_archive history [NEW]
    .mockResolvedValue({ rows: [] });                                      // 6+. all remaining
}
...
describe('Rumination fallback — synthesis_archive 历史上下文注入（P0 修复）', () => {
  ...
  it('NotebookLM 主路成功时，synthesis_archive fallback 查询不被触发（主路不变）', async () => {
    ...
    mockQuery
      .mockResolvedValueOnce({ rows: [{ in_progress: '0', queued: '0' }] }) // 1. idle check
      .mockResolvedValueOnce({ rows: [sampleLearning] })                     // 2. learnings query
      .mockResolvedValueOnce({ rows: [] })                                   // 3. fetchMemoryStreamItems
      .mockResolvedValueOnce({ rows: [{ value_json: 'nb-id-123' }] })       // 4. working_memory (has notebook_id)
      .mockResolvedValue({ rows: [] });                                      // 5+. remaining (no fallback synthesis_archive)
