...
describe('tick-status', () => {
  ...
  describe('getTickStatus', () => {
    ...
    // 2026-07-06 staging 隔离硬关：env=false 必须无视 DB 值与「key 缺失默认 true」。
    // 真实事故：cecelia_staging 库无 tick_enabled key → ?? true → staging tick 越权跑，
    // 把任务调度到生产 bridge(3457)，Auto Staging Deploy 因此 12 连红。
    it('CECELIA_TICK_ENABLED=false + DB 无 key（缺省本为 true）→ enabled=false', async () => {
    ...
    it('CECELIA_TICK_ENABLED=false + DB enabled=true → env 硬关优先，enabled=false', async () => {
      process.env.CECELIA_TICK_ENABLED = 'false';
      try {
        mockQuery.mockResolvedValueOnce({
