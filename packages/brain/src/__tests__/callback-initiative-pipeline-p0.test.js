...
describe('断链#6: initiative_verify verdict 处理', () => {
  ...
  it('D5: initiative_verify APPROVED → project status 更新为 completed', async () => {
    ...
    expect(res.status).toBe(200);
    // execution.js APPROVED 分支：UPDATE okr_initiatives SET status = 'done'
    // （initiative 终态用 'done'，区别于 task/project 的 'completed'；execution.js:2545）
    const updateCall = mockPool.query.mock.calls.find(
      c => typeof c[0] === 'string' &&
        c[0].includes('UPDATE okr_initiatives') &&
        c[0].includes("'done'") && c[1]?.[0] === projectId
    );
