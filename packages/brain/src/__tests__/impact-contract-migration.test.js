...
describe('Impact Contract 正式迁移合同', () => {
  it('迁移只存在于 migrate.js 扫描的正式目录', () => {
    expect(existsSync(`${migrationsDir}408_impact_contracts.sql`)).toBe(true);
    expect(existsSync(`${migrationsDir}409_harness_gap_ledger.sql`)).toBe(true);
  ...
  it('Impact Contract 外键引用真实 tasks 表', () => {
    const sql = readMigration('408_impact_contracts.sql');
  ...
  it('Gap Ledger 加厚既有 task_dependencies 而不重建冲突表', () => {
    const sql = readMigration('409_harness_gap_ledger.sql');
