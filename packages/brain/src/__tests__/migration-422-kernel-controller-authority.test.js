...
describe('migration 423 Kernel Controller authority', () => {
  it('creates durable sessions and binds active runs to a generation', async () => {
    const sql = await readFile(
      new URL('../../migrations/423_kernel_controller_authority.sql', import.meta.url),
      'utf8',
    );
    ...
    expect(sql).toContain("VALUES ('423'");
  });

  it('has a complete rollback including the schema marker', async () => {
    const sql = await readFile(
      new URL('../../migrations/rollback/423_kernel_controller_authority.down.sql', import.meta.url),
      'utf8',
    );
    expect(sql).toContain('DROP TABLE IF EXISTS kernel_controller_sessions');
    expect(sql).toContain("DELETE FROM schema_version WHERE version = '423'");
  });
});
