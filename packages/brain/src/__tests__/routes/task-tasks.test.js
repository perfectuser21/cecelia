...
describe('task-tasks routes', () => {
  ...
  describe('POST /tasks', () => {
    ...
    it('[Bug2] 不传 location → INSERT params 第8个参数为 "us"（不是 null）', async () => {
      ...
      // params 顺序：title, description, priority, task_type, status, project_id, area_id, goal_id, location, payload, trigger_source
      const [, params] = mockPool.query.mock.calls[1];
      expect(params[8]).toBe('us'); // location 在第9个位置（index 8），status(initialStatus)作为$5插入后移位
    });
