...
describe('actions.js', () => {
  ...
  describe('createTask', () => {
    ...
    it('不传 domain 时 domain/owner_role 均为 null（不自动检测，13 个参数）', async () => {
      ...
      const insertCall = mockQuery.mock.calls[1];
      const params = insertCall[1];
      // 不传 domain 时写 null（向后兼容，PR #647 规范）
      expect(params.length).toBe(13);
      expect(params[11]).toBeNull(); // domain = null
      expect(params[12]).toBeNull(); // owner_role = null
    });
  });
