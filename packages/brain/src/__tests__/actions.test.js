...
describe('actions.js', () => {
  ...
  describe('createTask', () => {
    ...
    it('传 domain 时 SQL 包含 domain 字段，owner_role 自动推断', async () => {
      ...
      expect(params[12]).toBe('coding');    // domain (index 12, after learning_id at 11)
      expect(params[13]).toBe('cto');       // owner_role 自动推断
      expect(params[14]).toBe('code-only'); // delivery_type default
    });
    ...
    it('同时传 domain 和 owner_role 时使用传入的 owner_role', async () => {
      ...
      const insertCall = mockQuery.mock.calls[1];
      const params = insertCall[1];
      expect(params[12]).toBe('quality');   // domain (index 12, after learning_id at 11)
      expect(params[13]).toBe('vp_qa');
      expect(params[14]).toBe('code-only'); // delivery_type default
    });
    ...
    it('不传 domain 时，无匹配关键词则 domain 为 null，不写 owner_role（12 个参数）', async () => {
      ...
      const sql = insertCall[0];
      const params = insertCall[1];
      // 无匹配关键词时 domain=null，owner_role 列不写入（14 个参数：12 common + domain + delivery_type）
      expect(params.length).toBe(14);
      expect(params[12]).toBeNull(); // domain is null when undetected (index 12, after learning_id at 11)
