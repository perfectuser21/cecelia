...
describe('Path 6: startSelfDriveLoop safety timeout', () => {
  ...
  it('Path 6a: initial cycle hangs → safety-net fires → setInterval established (loop survives)', async () => {
    // Arrange: getConfig succeeds, loop_started INSERT succeeds,
  ...
  it('Path 6c: startSelfDriveLoop records loop_started event on startup', async () => {
    // Arrange: all queries succeed normally
    mockQuery.mockResolvedValue({ rows: [] });

    const { startSelfDriveLoop } = await import('../self-drive.js');

    await startSelfDriveLoop();

    // Find the loop_started INSERT call
    const insertCalls = mockQuery.mock.calls.filter(args =>
      typeof args[0] === 'string' && args[0].includes('INSERT INTO cecelia_events')
    );
    const loopStartedCall = insertCalls.find(args => {
      try {
        const payload = JSON.parse(args[1][0]);
        return payload.subtype === 'loop_started';
      } catch { return false; }
    });

    expect(loopStartedCall).toBeDefined();
  });
});
