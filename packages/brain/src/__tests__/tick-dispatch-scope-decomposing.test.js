...
describe('Tick Dispatch Scope: decomposing KRs', () => {
  ...
  it('应该包含 decomposing 状态的 KR（dispatch scope 查询）', async () => {
    ...
    const krResult = await pool.query(
      `INSERT INTO key_results (title, priority, status)
       VALUES ('测试 KR decomposing dispatch scope', 'P0', 'decomposing')
       RETURNING id`
    );
  ...
  it('不应该包含 pending 或 completed 状态的 KR', async () => {
    ...
    const pendingResult = await pool.query(
      `INSERT INTO key_results (title, priority, status)
       VALUES ('Pending KR scope test', 'P1', 'pending')
       RETURNING id`
    );
    ...
    const completedResult = await pool.query(
      `INSERT INTO key_results (title, priority, status)
       VALUES ('Completed KR scope test', 'P1', 'completed')
       RETURNING id`
    );
  ...
  it('decomposing KR 下的 queued decomp 任务应在 scope 内', async () => {
    ...
    const krResult = await pool.query(
      `INSERT INTO key_results (title, priority, status)
       VALUES ('KR with decomp task', 'P0', 'decomposing')
       RETURNING id`
    );
    testGoalIds.push(krResult.rows[0].id);
    const krId = krResult.rows[0].id;
