...
describe('recordLearnings writes to learnings table (FLC-3)', () => {
  ...
  function setupPoolMocks({ dedupResult = [] } = {}) {
    ...
    // 1. _persistReflectionEntry → INSERT/UPSERT working_memory
    mockPool.query.mockResolvedValueOnce({ rows: [] });
    ...
    mockPool.query.mockResolvedValueOnce({ rows: [] });
    // 7. logCortexDecision → INSERT INTO decision_log
    mockPool.query.mockResolvedValueOnce({ rows: [] });
    // 8. recordLearnings: content_hash dedup check
    mockPool.query.mockResolvedValueOnce({ rows: dedupResult });
    if (dedupResult.length === 0) {
      // 9. recordLearnings: INSERT INTO learnings
