/**
 * liveness-never-started.integration.test.js [BEHAVIOR]
 *
 ...
 *
 * 毕业自 sprints/08041147-relay-2c1a4771/tests/liveness-never-started.integration.test.ts，
 * 已登记 vitest.config.js POSTGRES_INTEGRATION_TESTS，永久回归测试入 CI（brain-integration job）。
 * 毕业仅改 import 相对路径与去除 TS 类型标注，describe/it 名与 expect 断言逐字不变。
 */
...
afterEach(async () => {
  ...
  while (tmpLogs.length) {
    try { rmSync(tmpLogs.pop()); } catch { /* already gone */ }
  }
});
...
describe('watchdog liveness 从未启动任务分类 [BEHAVIOR]', () => {
  ...
  it('从未启动任务的 failure learning 文本含真实根因标签 never_started 且不含 liveness_dead 假标签', async () => {
    ...
    const text = r.rows.map((row) => `${row.title} ${row.content}`).join('\n');
