...
const migration = readFileSync(
  new URL(
    '../../../migrations/377_codex_review_callback_idempotency.sql',
    import.meta.url,
  ),
  'utf8',
);
...
describe('migration 377 — real PostgreSQL callback idempotency', () => {
  ...
  it('is idempotent and records schema version 377 once', async () => {
    await expect(client.query(migration)).resolves.toBeTruthy();
    const { rows } = await client.query(
      `SELECT version FROM schema_version WHERE version = '377'`,
    );
