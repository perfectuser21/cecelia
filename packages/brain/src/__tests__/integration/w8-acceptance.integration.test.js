...
import {
  compileHarnessFullGraph,
  buildHarnessFullGraph,
} from '../../workflows/harness-initiative.graph.js';
import { buildHarnessTaskGraph } from '../../workflows/harness-task.graph.js';
...
describe('w8 acceptance — compileHarnessFullGraph 17 节点 + credentials 注入 + checkpoint resume', () => {
  ...
  // 镜像 ↔ 生产拓扑契约：DoD 要求的 12 顶层 + 5 sub-graph 节点必须都在生产 graph 里。
  // 生产新增节点不会 fail（subset 检查）；任一必走节点被生产删掉/改名才 fail —— 镜像方案
  // 的最关键 guard，否则 mirror 测试可能"自洽但跟生产脱钩"。
  it('生产 buildHarnessFullGraph 含 DoD 要求的 12 顶层节点（subset 契约）', () => {
    const prodNodes = Object.keys(buildHarnessFullGraph().nodes || {});
    const required12 = Object.keys(top12Spies);
    for (const name of required12) {
      expect(prodNodes, `production full graph must contain node '${name}'`).toContain(name);
    }
  });

  it('生产 buildHarnessTaskGraph 含 DoD 要求的 5 sub-graph 节点（subset 契约）', () => {
    const prodSubNodes = Object.keys(buildHarnessTaskGraph().nodes || {});
    const required5 = Object.keys(sub5Spies);
    for (const name of required5) {
      expect(prodSubNodes, `production sub graph must contain node '${name}'`).toContain(name);
    }
  });
