...
beforeAll(async () => {
  ...
  await pool.query(`DELETE FROM brain_modules WHERE id = ANY($1)`, [TEST_IDS]);
});
...
describe('brain_modules table (formerly features)', () => {
  it('inserts a module and reads it back', async () => {
    const { rows } = await pool.query(
      `INSERT INTO brain_modules (id, name, priority, status, smoke_status)
  ...
  it('filters by priority', async () => {
    const { rows } = await pool.query(
      `SELECT * FROM brain_modules WHERE id = $1 AND priority = $2`,
      ['test-feat-001', 'P0']
    );
  ...
  it('updates smoke_status without changing other fields', async () => {
    await pool.query(
      `UPDATE brain_modules SET smoke_status = $1, smoke_last_run = NOW(), updated_at = NOW()
       WHERE id = $2`,
      ['passing', 'test-feat-001']
    );
    const { rows } = await pool.query(`SELECT * FROM brain_modules WHERE id = $1`, ['test-feat-001']);
  ...
  it('seed upsert preserves existing smoke_status', async () => {
    ...
    await pool.query(
      `INSERT INTO brain_modules (id, name, priority, status)
       VALUES ($1, $2, $3, $4)
    ...
    const { rows } = await pool.query(`SELECT * FROM brain_modules WHERE id = $1`, ['test-feat-002']);
