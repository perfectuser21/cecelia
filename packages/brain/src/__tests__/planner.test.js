...
});reAll, afterAll, afterEach } from 'vitest';
import pg from 'pg';
import { DB_DEFAULTS } from '../db-config.js';
...
describe('Planner Agent', () => {
  ...
  describe('handlePlanInput - hard constraints', () => {
    ...
    it('should reject task whose project has no repo_path', async () => {
      // Create an okr_project without repo_path in metadata
      const projResult = await pool.query(
        "INSERT INTO okr_projects (title, status) VALUES ('no-repo-project', 'planning') RETURNING id"
      );
      testProjectIds.push(projResult.rows[0].id);
    ...
    it('should create task linked to project with repo_path', async () => {
      // Insert into okr_projects with repo_path in metadata
      const projResult = await pool.query(
        `INSERT INTO okr_projects (title, status, metadata) VALUES ('test-proj', 'planning', '{"repo_path":"/tmp/test"}') RETURNING id`
      );
      const projectId = projResult.rows[0].id;
      testProjectIds.push(projectId);
      const { handlePlanInput } = await import('../planner.js');
