/**
 * 迁移 474 结构断言（棒3a 判定，任务 33aa2bc4）：journey_assertion_receipts 放行
 * executor_kind='business_probe_runner'，并给它一套不要求 source_sha/machine_id 的 verdict 校验。
 * 374 的 brain_assertion_runner 分支原式保留。
 */
import { describe, it, expect } from 'vitest';
import { readFileSync, existsSync } from 'node:fs';
import { fileURLToPath } from 'node:url';

const up = fileURLToPath(new URL('../../migrations/474_business_probe_receipts.sql', import.meta.url));
const down = fileURLToPath(new URL('../../migrations/rollback/474_business_probe_receipts.down.sql', import.meta.url));
