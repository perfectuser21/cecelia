...
vi.mock('../seven-ring-audit.js', () => ({
  maybeRunSevenRingAudit: vi.fn().mockResolvedValue({ skipped: false, hard_defects: 0, pass: true }),
}));
...
import { maybeRunSevenRingAudit } from '../seven-ring-audit.js';
...
describe('scheduler-jobs 注册表', () => {
  ...
  it('JOBS 注册了 16 个 job（含 postdeploy-verifier 和 seven-ring-audit）', () => {
    expect(JOBS.map((j) => j.name)).toEqual([
      'arch-review', 'ci-patrol', 'strategy-trigger', 'conversation-digest', 'capture-digestion', 'daily-backup', 'line-dreaming', 'ledger-hygiene', 'battle-report', 'capture-triage', 'receipt-collector', 'gp-shelf-life', 'launchd-patrol', 'direction-proposer', 'postdeploy-verifier', 'seven-ring-audit',
    ]);
  });
  ...
  it('runSchedulerJobsOnce 调用全部 job，needsPool 决定传参', async () => {
    ...
    expect(maybeRunSevenRingAudit).toHaveBeenCalledWith(pool);
  ...
  it('哨兵用 ON CONFLICT upsert 写 working_memory，key 带前缀', async () => {
    ...
    expect(sentinelCalls).toHaveLength(16);
