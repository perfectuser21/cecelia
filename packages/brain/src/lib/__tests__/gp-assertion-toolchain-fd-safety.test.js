...
async function commandFor(bytes, sha256 = digest(bytes)) {
  return assertionCommand('scripts/smoke/gp.sh', '/repo', {
    ...
    toolchains: { bash: { path: '/tools/bash', ...(sha256 === null ? {} : { sha256 }) } },
  });
}
function fileSystem(bytes, { size = BigInt(bytes.length), regular = true, maxRead = 64 * 1024 } = {}) {
  const metadata = { dev: 1n, ino: 2n, size, ctimeNs: 3n, mtimeNs: 4n, isFile: () => regular };
  const handle = {
...
describe('GP assertion same-FD toolchain safety', () => {
  it('accepts only the original branded assertion command', async () => {
    const bytes = Buffer.from('tool-v1'), command = await commandFor(bytes), fs = fileSystem(bytes);
    for (const forged of [structuredClone(command), Object.freeze({ ...command })]) {
      await expect(createToolchainAttestation(input(forged), fs)).rejects.toMatchObject({ code: 'ASSERTION_COMMAND_UNTRUSTED' });
    }
  });
  it('opens the canonical tool with no-follow flags and hashes one FD', async () => {
    ...
    expect(fs.openFn).toHaveBeenCalledWith('/tools/bash', constants.O_RDONLY | constants.O_NONBLOCK | constants.O_NOFOLLOW);
  ...
  it.each([
    ...
    const command = await commandFor(bytes.length ? bytes : Buffer.from('expected')), fs = fileSystem(bytes, options);
    await expect(createToolchainAttestation(input(command), { ...fs, maxFileBytes: 4 })).rejects.toMatchObject({ code });
    expect(fs.handle.close).toHaveBeenCalledOnce();
  });
  it('hashes a greater-than-64KiB file across bounded short reads', async () => {
    const bytes = Buffer.alloc(70 * 1024, 7), command = await commandFor(bytes), fs = fileSystem(bytes, { maxRead: 9973 });
    const result = await createToolchainAttestation(input(command), { ...fs, maxFileBytes: bytes.length });
  ...
  it.each([
    ...
    await expect(createToolchainAttestation(input(command), fileSystem(bytes))).rejects.toMatchObject({ code });
  });
  it.each([
    ...
    await expect(createToolchainAttestation(input(command), fileSystem(bytes, options))).rejects.toMatchObject({ code });
  });
  it('rejects same-FD metadata changes', async () => {
    const bytes = Buffer.from('tool'), command = await commandFor(bytes), fs = fileSystem(bytes);
    fs.handle.stat.mockResolvedValueOnce(await fs.handle.stat())
      .mockResolvedValueOnce({ ...await fs.handle.stat(), mtimeNs: 99n });
    await expect(createToolchainAttestation(input(command), fs)).rejects.toMatchObject({ code: 'ASSERTION_TOOLCHAIN_CHANGED_DURING_READ' });
  });
  it('normalizes invalid bytesRead without retaining data', async () => {
    const bytes = Buffer.from('tool'), fs = fileSystem(bytes);
    fs.handle.read.mockResolvedValueOnce({ bytesRead: 1.5 });
    const error = await createToolchainAttestation(input(await commandFor(bytes)), fs).catch(reason => reason);
  ...
  it('normalizes close failures and preserves a primary safety error', async () => {
    const bytes = Buffer.from('tool'), command = await commandFor(bytes), createFs = fileSystem(bytes);
    createFs.handle.close.mockRejectedValue(secretError());
    const createError = await createToolchainAttestation(input(command), createFs).catch(reason => reason);
    ...
    const verifyError = await verifyToolchainAttestation(attestation, verifyFs).catch(reason => reason);
    ...
    await expect(createToolchainAttestation(input(await commandFor(Buffer.from('expected'))), primaryFs)).rejects.toMatchObject({ code: 'ASSERTION_TOOLCHAIN_FILE_EMPTY' });
  });
  it('checks the attestation brand before reading hostile objects', async () => {
    const bytes = Buffer.from('tool'), command = await commandFor(bytes);
    const attestation = await createToolchainAttestation(input(command), fileSystem(bytes));
    await expect(verifyToolchainAttestation(structuredClone(attestation))).rejects.toMatchObject({ code: 'ASSERTION_TOOLCHAIN_ATTESTATION_UNTRUSTED' });
    const getter = vi.fn(() => { throw secretError(); });
    const hostile = Object.defineProperties({}, {
      kind: { enumerable: true, get: getter }, actual_runner_digest: { enumerable: true, value: RUNNER },
      expected_runner_digest: { enumerable: true, value: RUNNER }, files: { enumerable: true, value: [] },
    });
    const trap = vi.fn(() => { throw secretError(); });
    for (const value of [hostile, new Proxy({}, { getPrototypeOf: trap })]) {
      const error = await verifyToolchainAttestation(value).catch(reason => reason);
