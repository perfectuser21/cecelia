 ...
 * HEAD /api/brain/pipelines/:id               检查任务是否存在（供 ZJ API 替代跨库直查 tasks 表）
...
/**
 * HEAD /:id
 * 检查 content-pipeline 任务是否存在（供 ZenithJoy API 替代跨库直查 tasks 表）
 * 200 = 存在，404 = 不存在
 */
router.head('/:id', async (req, res) => {
  ...
  try {
    ...
    res.status(result.rows.length > 0 ? 200 : 404).end();
  } catch {
    res.status(500).end();
  }
});
