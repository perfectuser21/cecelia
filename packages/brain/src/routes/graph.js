...
import { withConsistentSnapshot } from '../lib/consistent-read.js';
import { readFactSnapshotHeader } from '../lib/fact-snapshot-header.js';
import {
...
export async function loadGraphContext(repo = 'cecelia', dbPool = pool) {
  return withConsistentSnapshot(dbPool, async (client) => {
    const { rows: edges } = await client.query(
      `SELECT src_path, dst_path, edge_type FROM graph_edges WHERE repo = $1`, [repo]);
    const header = await readFactSnapshotHeader(client, 'graph', repo);
    ...
    return {
      ...
      freshness: { repo, ...computeFreshness(header) },
    };
  });
}
