...
根据 Review 报告中的具体建议进行修复。`;
      ...
      // 5c-harness. Harness v4.0+ 路由（已迁移到 harness-router.js）
      // 调用共享路由模块，让 HTTP 端点和 callback_queue worker 走同一路径。
      try {
        ...
        if (harnessType && harnessType.startsWith('harness_')) {
          ...
          await processHarnessRouting({
            task_id,
            harnessType,
            harnessPayload: harnessTask.payload || {},
            result,
            pr_url,
            newStatus,
            harnessTask: {
        ...
        console.error(`[execution-callback] harness routing failed (non-fatal): ${harnessErr.message}`, harnessErr.stack);
      }

      // 5c9. 断链 #5: dev 完成 → 检查同 project 所有 dev 是否全完成 → 创建 code_review (Initiative 级别)
      // ⚠️ Harness 模式的 Initiative 不走此断链（harness_evaluate PASS 时已处理）
      try {
