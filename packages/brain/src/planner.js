...
// Domain → task_type routing map
// Initiative 的 domain 字段决定自动生成的任务类型和对应 skill
const DOMAIN_TO_TASK_TYPE = {
  coding:      'architecture_design', // → /architect
  agent_ops:   'architecture_design', // → /architect
  operations:  'architecture_design', // → /architect
  security:    'architecture_design', // → /architect
  quality:     'qa',                  // → /code-review
  product:     'initiative_plan',     // → /decomp
  research:    'research',            // → /research
  knowledge:   'knowledge',           // → /knowledge
  growth:      'dev',                 // → /dev
  // finance → null（不自动生成，需人工介入）
};

/**
 * Resolve task_type based on Initiative domain.
 * finance → null (no auto task)
 * null/unknown → 'architecture_design' (backward compat)
 * @param {string|null} domain
 * @returns {string|null}
 */
export function resolveTaskTypeByDomain(domain) {
  if (!domain) return 'architecture_design';
  if (domain === 'finance') return null;
  return DOMAIN_TO_TASK_TYPE[domain] ?? 'architecture_design';
}
...
async function generateArchitectureDesignTask(kr, project) {
  try {
    ...
    // Include domain field for domain-aware task_type routing
    ...
    // Domain-aware task_type routing
    const taskType = resolveTaskTypeByDomain(initiative.domain);
    console.log(`[planner] domain-aware routing: domain=${initiative.domain ?? 'null'}, task_type=${taskType ?? 'null'} for initiative ${initiative.id}`);

    // finance domain → no auto task (requires manual intervention)
    if (taskType === null) {
      console.log(`[planner] domain=finance, skipping auto task generation for initiative ${initiative.id}`);
      return null;
    }

    // Check if a task of the resolved task_type already exists for this initiative (any active status)
    ...
    if (existingResult.rows.length > 0) {
      // Already has a matching task pending/in_progress, skip
      return null;
    }

    // Create task with domain-resolved task_type
    ...
    console.log(`[planner] 自动生成 ${taskType} 任务: ${newTask.title} (${newTask.id}) for initiative ${initiative.id} (domain=${initiative.domain ?? 'null'})`);
    return newTask;
  } catch (err) {
...
export {
  ...
  // Domain-aware routing (resolveTaskTypeByDomain is exported via 'export function' above)
  DOMAIN_TO_TASK_TYPE,
  // Learning penalty
