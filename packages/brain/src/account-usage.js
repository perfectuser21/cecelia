 ...
 * @param {string} source - 失败来源：'api_error'（Anthropic API 401）或 'token_expired'（token 文件过期）
 */
...
export async function proactiveTokenCheck() {
  for (const accountId of ACCOUNTS) {
    ...
    if (isExpired) {
      ...
      // token 有效：清除过期告警标记；只清除 source=token_expired 设置的熔断
      // api_error 设置的熔断（Anthropic API 返回 401）不清除，等 resetTime 自然过期
      ...
      if (isAuthFailed(accountId)) {
        ...
        if (entry?.source === 'token_expired') {
          ...
          console.log(`[account-usage] [proactive-check] ${accountId}: auth-failed 由 api_error 设置，不清除（等 resetTime 自然过期）`);
        }
      }
    }
  }
