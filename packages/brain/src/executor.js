...
async function triggerCodexBridge(task, forceBridgeUrl = null) {
  ...
  try {
    ...
    // crystallize_* 使用 runner 模式（playwright-runner.sh，via --task-id）
    // codex_qa 和其他类型使用 prompt 模式（单次 codex exec）
    const isCodexDev = task.task_type === 'codex_dev';
    const isCrystallize = task.task_type?.startsWith('crystallize');
    ...
    const response = await fetch(`${bridgeUrl}/run`, {
      ...
      body: JSON.stringify({
        ...
        // runner 模式参数（codex_dev: runner.sh，crystallize: playwright-runner.sh）
        runner: isCodexDev ? 'packages/engine/runners/codex/runner.sh' : isCrystallize ? 'packages/engine/runners/codex/playwright-runner.sh' : undefined,
        runner_args: isCodexDev ? ['--branch', taskBranch, '--task-id', task.id] : isCrystallize ? ['--task-id', task.id] : undefined,
        branch: taskBranch,
      }),
