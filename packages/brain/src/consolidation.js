 ...
 *   每天仅 6 个 5 分钟窗口共 30 分钟。Tick 间隔 ≈5 分钟，错过即等 4 小时；
 *   再叠加 hasTodayConsolidation 按"日"去重，一旦错过整天没有补救机会。
 *   现已替换为 elapsed-time 判断，参见 shouldRunByElapsed。
 */
...
export async function runDailyConsolidationIfNeeded(pool, now = new Date()) {
  ...
  if (!check.shouldRun) {
    return { skipped: true, reason: check.reason, last_run: check.last_run, hours_elapsed: check.hours_elapsed };
  }
...
export async function runDailyConsolidation(pool, opts = {}) {
  ...
  if (!forceRun) {
    const check = await shouldRunByElapsed(pool, now, CONSOLIDATION_INTERVAL_HOURS);
    if (!check.shouldRun) {
      console.log(`[consolidation] 跳过：${check.reason} (last_run=${check.last_run?.toISOString() ?? 'never'})`);
