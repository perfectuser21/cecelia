...
async function _enrichContextWithAnalyses(context, event) {
  try {
...
async function _processLLMResponse(event, prompt) {
  try {
...
async function analyzeDeep(event, thalamusDecision = null) {
  ...
  await _enrichContextWithAnalyses(context, event);
  ...
  return _processLLMResponse(event, prompt);
}
