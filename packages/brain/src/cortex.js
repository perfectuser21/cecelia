...
// ============================================================
// 系统简报去重（Report Context Hash）
// ============================================================

/**
 * 计算简报上下文的数据指纹
 * 对关键数值做量化处理（四舍五入到步长），实现 >80% 相似度折叠
 * @param {Object} context
 * @returns {string} 16字符 hex
 */
function _computeReportContextHash(context) {
  const q = (n, step = 5) => Math.round((Number(n) || 0) / step) * step;
  const stats = context.task_stats || {};
  const fingerprint = {
    completed: q(stats.completed, 5),
    failed: q(stats.failed, 2),
    queued: q(stats.queued, 10),
    in_progress: q(stats.in_progress, 2),
    tick_enabled: Boolean(context.system_health?.tick_enabled),
  };
  return crypto.createHash('sha256').update(JSON.stringify(fingerprint)).digest('hex').slice(0, 16);
}

/**
 * 检查近期是否已有相似简报（基于 context_hash 匹配）
 * @param {string} contextHash
 * @param {number} timeRangeHours
 * @returns {Promise<Object|null>} 已有简报，或 null
 */
async function _checkRecentSimilarReport(contextHash, timeRangeHours) {
  const windowHours = Math.max(Math.floor(timeRangeHours * 0.75), 6);
  try {
    const result = await pool.query(`
      SELECT id, content, metadata
      FROM system_reports
      WHERE type = '48h_summary'
        AND created_at > NOW() - ($1 || ' hours')::INTERVAL
        AND metadata->>'context_hash' = $2
      ORDER BY created_at DESC
      LIMIT 1
    `, [String(windowHours), contextHash]);
    return result.rows.length > 0 ? result.rows[0] : null;
  } catch (err) {
    console.warn('[cortex] generateSystemReport: 去重检查失败（非致命）:', err.message);
    return null;
  }
}
...
async function generateSystemReport({ timeRangeHours = 48 } = {}) {
  ...
  // 去重：计算数据指纹，检查近期是否已有相似简报（止住皮层回声）
  const _reportContextHash = _computeReportContextHash(context);
  const _recentSimilar = await _checkRecentSimilarReport(_reportContextHash, timeRangeHours);
  if (_recentSimilar) {
    console.log(`[cortex] generateSystemReport: 数据相似（hash=${_reportContextHash}），复用近期简报 ${_recentSimilar.id}`);
    const existingContent = typeof _recentSimilar.content === 'string'
      ? JSON.parse(_recentSimilar.content)
      : _recentSimilar.content;
    return {
      id: _recentSimilar.id,
      ...existingContent,
      time_range_hours: timeRangeHours,
      generated_at: context.generated_at,
      _deduped: true,
      _deduped_from: _recentSimilar.id,
    };
  }
  ...
  try {
    const metadata = {
      ...
      generated_at: context.generated_at,
      time_range_hours: timeRangeHours,
      context_hash: _reportContextHash,
    };
...
export {
  ...
  // 系统简报去重（测试用）
  _computeReportContextHash,
  _checkRecentSimilarReport,
