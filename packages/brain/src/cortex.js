...
async function _enrichWithDecisionHistory(context) {
  try {
...
async function _enrichWithSystemStatus(context) {
  try {
...
async function _enrichWithLearnings(context, event) {
...
async function _enrichWithHistoricalAnalyses(context, event) {
...
async function _enrichWithSelfModel(context) {
...
function _enrichWithRcaParams(context, event, thalamusDecision) {
...
async function _enrichWithCrossTaskPatterns(context) {
...
async function _processLLMResponse(event, response, timing) {
...
async function _handleAnalysisError(err, event) {
...
async function analyzeDeep(event, thalamusDecision = null) {
  ...
  await _enrichWithDecisionHistory(context);
  await _enrichWithSystemStatus(context);
  await _enrichWithLearnings(context, event);
  await _enrichWithHistoricalAnalyses(context, event);
  await _enrichWithSelfModel(context);
  _enrichWithRcaParams(context, event, thalamusDecision);
  await _enrichWithCrossTaskPatterns(context);
  ...
  try {
    ...
    return await _processLLMResponse(event, response, timing);
  } catch (err) {
    return await _handleAnalysisError(err, event);
  }
}
