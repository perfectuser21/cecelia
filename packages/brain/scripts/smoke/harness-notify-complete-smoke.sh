...
STATUS=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$BRAIN/api/brain/harness/complete" \
...
STATUS=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$BRAIN/api/brain/harness/complete" \
  ...
  -d '{"initiative_id":"smoke-test-initiative","pr_url":"https://github.com/org/repo/pull/1"}' 2>/dev/null)
[ "$STATUS" = "200" ] && ok "/harness/complete 正常请求 → 200" || fail "/harness/complete 正常 → $STATUS（期望 200）"

# 3. /harness/notify 缺 type → 400
STATUS=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$BRAIN/api/brain/harness/notify" \
  -H "Content-Type: application/json" -d '{"title":"test"}' 2>/dev/null)
...
STATUS=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$BRAIN/api/brain/harness/notify" \
  -H "Content-Type: application/json" \
  -d '{"type":"harness_complete","title":"smoke test 完成","initiative_id":"smoke-test-initiative"}' 2>/dev/null)
...
STATUS=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$BRAIN/api/brain/harness/notify" \
  -H "Content-Type: application/json" \
  -d '{"type":"general","title":"smoke 普通通知","message":"test"}' 2>/dev/null)
