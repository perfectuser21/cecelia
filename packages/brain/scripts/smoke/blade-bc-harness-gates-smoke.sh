...
#
# 验证：
#   1. POST /harness/judge — 无 exact run authority → 404（禁止脱离 Kernel run 裁决）
...
if [ "$JUDGE_STATUS" != "404" ]; then
  echo "[blade-bc smoke] FAIL — 无 exact run authority 期望 404，实际 $JUDGE_STATUS: $(cat /tmp/blade-bc-judge.json)"
  exit 1
fi
