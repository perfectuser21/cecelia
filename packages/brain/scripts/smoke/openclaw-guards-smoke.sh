...
const agentDrift = m.checkConfigDrift({agents:{defaults:{model:{primary:"openai/gpt-5.6-terra"}},entries:{media:{model:{primary:"openai/gpt-5.6-sol"}}}},plugins:{entries:{codex:{config:{appServer:{command:"/usr/bin/ssh",args:["session-runner"]}}}}}});
if (!agentDrift || !agentDrift.includes("media")) fail("agent级本机漏网未检出");
