...
#                reportNode 只在 computedVerdict!=='PASS' 派失败报告（PASS 不派）；
#                staging-e2e-runner auto_promoted / routes confirm promoted 各派成功报告；
#                生命周期闭合（UPDATE initiative_runs phase / UPDATE tasks）仍在 reportNode；
...
for f in "$SP" "$RUNNER" "$INIT_GRAPH" "$ROUTE" "$MIG307"; do
...
const g=fs.readFileSync('$INIT_GRAPH','utf8');
// reportNode 只在 !=='PASS' 派失败报告（PASS 不派）
if(!/computedVerdict !== 'PASS'/.test(g) || !/spawnHarnessReport/.test(g)){console.error('L1 FAIL: reportNode 未改为 FAIL-only 经 spawnHarnessReport 派');process.exit(1)}
// 生命周期闭合仍在 reportNode（没被挪走）
if(!/UPDATE initiative_runs SET phase/.test(g) || !/UPDATE tasks SET status/.test(g)){console.error('L1 FAIL: 生命周期闭合被挪走了（应保留 merge 时）');process.exit(1)}
...
console.log('[smoke] L1 PASS: 三态派发 + 幂等 + reportNode FAIL-only + 生命周期保留 + migration307 齐全');
" || exit 1
