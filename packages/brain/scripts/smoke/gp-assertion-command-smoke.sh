...
const lock = JSON.parse(await readFile('./packages/brain/package-lock.json'));
...
if (command.executable !== node || command.argv[0] !== vitest || lock.version !== JSON.parse(await readFile('./packages/brain/package.json')).version) process.exit(1);
