...
RESP=$(curl -sf -X POST "$BRAIN/api/brain/tasks" \
  ...
  -d '{"task_type":"harness_initiative","title":"headed-smoke-test","payload":{"orchestrator":"skill-relay","executor":"claude","mode":"headed","journey_id":"bb8cc561-b3ee-4fec-b74d-2255694bd963"}}' 2>/dev/null) || { fail "POST tasks(mode=headed) 不可达"; RESP="{}"; }
...
CODE=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$BRAIN/api/brain/tasks" \
  -H "Content-Type: application/json" \
  -d '{"task_type":"harness_initiative","title":"codex-headed-smoke","payload":{"orchestrator":"skill-relay","executor":"codex","mode":"headed","journey_id":"bb8cc561-b3ee-4fec-b74d-2255694bd963"}}' 2>/dev/null || echo "000")
[ "$CODE" = "200" ] || [ "$CODE" = "201" ] \
...
CODE2=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$BRAIN/api/brain/tasks" \
  -H "Content-Type: application/json" \
  -d '{"task_type":"harness_initiative","title":"headless-smoke","payload":{"orchestrator":"skill-relay","executor":"claude","mode":"headless"}}' 2>/dev/null || echo "000")
[ "$CODE2" = "201" ] || [ "$CODE2" = "200" ] \
