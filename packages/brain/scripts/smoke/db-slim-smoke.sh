#!/usr/bin/env bash
# db-slim 冒烟：8 条清理规则 SQL 对真实 schema 可执行 + check 模式双向验火。
set -euo pipefail
...
# 1) dry-run：8 条规则的 archiveWhere/preAssert 全部真跑一遍（对 schema 的存在性/列名/类型校验）
DRY_OUT="$("$NODE_EXECUTABLE" packages/brain/scripts/db-slim.mjs 2>&1)" \
  || fail "dry-run 执行失败: $DRY_OUT"
RULE_COUNT="$(printf '%s\n' "$DRY_OUT" | grep -c '命中 .* 行' || true)"
[[ "$RULE_COUNT" -eq 8 ]] || fail "dry-run 应报告 8 条规则，实际 $RULE_COUNT: $DRY_OUT"
pass "dry-run 8 条规则 SQL 对真实 schema 可执行"
