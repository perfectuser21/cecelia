...
FIX_ROUNDS=$(node -e "import('./src/orchestrator/constants.js').then(m => console.log(m.MAX_FIX_ROUNDS))" 2>/dev/null)
[[ "$FIX_ROUNDS" == "3" ]] \
  && ok "MAX_FIX_ROUNDS=3（Sprint 07231527 Blocking 1）" \
  || fail "MAX_FIX_ROUNDS 期望 3，实际 ${FIX_ROUNDS}"

HOPS=$(node -e "import('./src/orchestrator/constants.js').then(m => console.log(m.MAX_HOPS))" 2>/dev/null)
[[ "$HOPS" == "60" ]] \
  && ok "MAX_HOPS=60（Sprint 07231527 Blocking 1）" \
  || fail "MAX_HOPS 期望 60，实际 ${HOPS}"
