...
#   3. migration 332 已加 ledger_hygiene 类型
set -euo pipefail
...
echo "[ledger-hygiene-smoke] 3. migration 332 已加 ledger_hygiene 类型"
node -e "
const fs = require('fs');
const src = fs.readFileSync('packages/brain/migrations/332_design_docs_ledger_hygiene.sql', 'utf8');
if (!src.includes(\"'ledger_hygiene'\") || !src.includes('design_docs_type_check')) {
  console.error('FAIL: migration 332 缺 ledger_hygiene CHECK 值');
  process.exit(1);
}
console.log('migration 332 正确 ✓');
"
