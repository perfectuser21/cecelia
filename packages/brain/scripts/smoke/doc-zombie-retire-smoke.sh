...
# 验证：
#   1. seven-ring-audit.js 注释不再引用退役的状态更新脚本
#   2. quality.js 注释不再引用退役的状态更新脚本
#   3. 两处注释均已更新为说明现行数据源（CI test-pyramid-guard job）
set -euo pipefail

# 退役脚本名（拆分存储，避免自身被 grep 规则命中）
RETIRED_SCRIPT="write-current""-state.sh"

echo "[doc-zombie-retire-smoke] 1. seven-ring-audit.js 不含退役脚本引用"
node -e "
const fs = require('fs');
const retired = 'write-current' + '-state.sh';
const src = fs.readFileSync('packages/brain/src/seven-ring-audit.js', 'utf8');
if (src.includes(retired)) {
  console.error('FAIL: seven-ring-audit.js 仍含退役脚本引用');
  process.exit(1);
}
...
if (src.includes(retired)) {
  console.error('FAIL: quality.js 仍含退役脚本引用');
  process.exit(1);
}
if (!src.includes('2026-08')) {
...
node -e "
const fs = require('fs');
const retiredScript = 'scripts/' + 'write-current' + '-state.sh';
const retired = [
