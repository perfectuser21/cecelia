...
# A1: /api/brain/healthz → 200
HTTP=$(curl -s -o /dev/null -w "%{http_code}" "$BRAIN_URL/api/brain/healthz" 2>/dev/null || echo "000")
[ "$HTTP" = "200" ] && ok "A1: GET /api/brain/healthz → 200" || { fail "A1: GET /api/brain/healthz → $HTTP"; exit 1; }
...
# A5: 两步清理探针（queued→in_progress→failed）
CC1=$(curl -s -o /dev/null -w "%{http_code}" -X PATCH "$BRAIN_URL/api/brain/tasks/$TASK_ID" \
  -H "Content-Type: application/json" -d '{"status":"in_progress"}' 2>/dev/null || echo "000")
CC2=$(curl -s -o /dev/null -w "%{http_code}" -X PATCH "$BRAIN_URL/api/brain/tasks/$TASK_ID" \
  -H "Content-Type: application/json" -d '{"status":"failed"}' 2>/dev/null || echo "000")
[ "$CC1" = "200" ] && [ "$CC2" = "200" ] && ok "A5: 探针清理 queued→in_progress→failed 全 200" \
  || fail "A5: 清理失败（step1=$CC1 step2=$CC2）"
