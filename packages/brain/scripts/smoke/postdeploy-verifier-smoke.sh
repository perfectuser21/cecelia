...
TASK_ID=$(curl -s -X POST "$API/tasks" \
  ...
  -d "{\"task_type\":\"talk\",\"title\":\"smoke: pending_postdeploy test $RANDOM-$$\",\"status\":\"pending_postdeploy\",\"payload\":{\"postdeploy_check\":{\"command\":\"curl -s $BRAIN_URL/api/brain/health\",\"timeout_s\":10}}}" \
