...
const pairs = [
  ...
  ['test(brain): require map governed harness preflight', 'feat(brain): enforce map governed four-form harness runs', 'cd packages/brain && DB_URL="$DB_URL" BASELINE_SHA="$BASELINE_SHA" npx vitest run src/orchestrator/__tests__/change-kind-profiles.test.js src/orchestrator/preflight/map-impact-contract.test.js src/orchestrator/__tests__/map-recovery-contract.test.js --reporter=verbose'],
  ...
  ['test(brain): define unified router scratch acceptance', 'feat(cecelia): enforce unified work routing across all coding', 'DB_URL="$DB_URL" BASELINE_SHA="$BASELINE_SHA" bash packages/brain/scripts/smoke/unified-work-router-smoke.sh && cd packages/brain && npx vitest run src/__tests__/work-routing-observability.test.js --reporter=verbose'],
];
// A repair baseline may already contain an approved slice's immutable RED/GREEN
// ledger. Search only the candidate's first-parent ancestry; the baseline guard
// above still proves that the repair itself was appended to the frozen SHA.
const log = execFileSync('git', ['log', '--format=%H%x09%s', head], { encoding: 'utf8' })
  .trim().split('\n').map(line => line.split('\t'));
