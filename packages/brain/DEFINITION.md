...
## GitHub 凭据装载不依赖 GitHub 在线（Brain 1.273.77）

- `gh auth login` 在线校验不可用时回落直接写 hosts.yml（空 token 仍拒）；canonical Runner digest 重钉为 `sha256:bec4e67c1f91b6c0434c4ea49d079ef01f101a02aa03b45856b5e0e8e98b802f`。
...
- 裁判每条 coverage 回显 1-based `step_index`，服务端按序号对齐不比对措辞；越界/重复序号不采信并回落文字锚点。
...
- 冻结候选树断言拒绝时打印漂移文件清单（路径/状态/模式，无内容）；canonical Runner digest 重钉为 `sha256:bec4e67c1f91b6c0434c4ea49d079ef01f101a02aa03b45856b5e0e8e98b802f`。
...
- 断言所在包根目录补 a+w,+t 供 vite 写临时 bundled 配置；vitest 加 --no-cache；canonical Runner digest 重钉为 `sha256:bec4e67c1f91b6c0434c4ea49d079ef01f101a02aa03b45856b5e0e8e98b802f`。

## 可信断言执行器按包根跑 vitest（Brain 1.273.65）

- runner assertion-exec 以断言文件最近的 package.json 目录为 cwd 跑 vitest（根 vitest include 不含 packages/*/src），required assertion 不再恒报 No test files found；canonical Runner digest 重钉为 `sha256:bec4e67c1f91b6c0434c4ea49d079ef01f101a02aa03b45856b5e0e8e98b802f`。
...
- canonical Runner digest 重钉为 `sha256:bec4e67c1f91b6c0434c4ea49d079ef01f101a02aa03b45856b5e0e8e98b802f`，Fleet Worker 版本 pin 不变。
...
- Canonical Runner 更新为冻结来源重建的 `sha256:bec4e67c1f91b6c0434c4ea49d079ef01f101a02aa03b45856b5e0e8e98b802f`，NodeProfile、rollout、reconciler 与 smoke 共享同一不可变 pin。
