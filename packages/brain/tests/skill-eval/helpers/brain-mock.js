...
export function createMockBrainTick({
  db, MAX_CONCURRENT_SKILL_EVAL = 1, SKILL_EVAL_TIMEOUT = 30 * 60,
  SKILL_EVAL_PENDING_LIMIT = 20, dockerExecutor = null,
  feishuFn = async () => {}, quotaProvider = null, onAlert = null,
} = {}) {
  const feishuAlerts = [];
  ...
  const tick = {
    ...
    async schedule() {
      ...
      if (parseInt(running_count) >= MAX_CONCURRENT_SKILL_EVAL) return;

      if (quotaProvider) {
        const quota = await quotaProvider.getSonnetQuota();
        if (quota.remaining_5h_pct < 85 || quota.remaining_7d_pct < 90) {
      ...
      const pending = await db.oneOrNone(
        `SELECT task_id FROM skill_eval_tasks WHERE status='pending' ORDER BY created_at ASC LIMIT 1`
      );
      if (!pending) return;

      await db.none(
        `UPDATE skill_eval_tasks SET status='running', started_at=NOW(), updated_at=NOW() WHERE task_id=$1`,
        [pending.task_id]
      );
    },
    ...
    async checkTimeouts() {
      const timedOut = await db.manyOrNone(
        `SELECT task_id, container_id FROM skill_eval_tasks
         WHERE status='running' AND started_at < NOW() - INTERVAL '${SKILL_EVAL_TIMEOUT} seconds'`
      );
      for (const task of timedOut) {
        if (dockerExecutor && task.container_id) dockerExecutor.kill(task.container_id);
        await db.none(
          `UPDATE skill_eval_tasks SET status='failed', failure_reason='timeout', updated_at=NOW(), completed_at=NOW() WHERE task_id=$1`,
          [task.task_id]
        );
