-- Migration 464: 验证 463 里 NOT VALID 的 tasks_executor_kind_check
--
-- 秋米中文 GTD 表接入 Brain 统一调度·PR2 入口（task b7efdbff，决策 b8abd28c）。
-- 463 把 executor_kind 白名单扩到八值（补 'openclaw-agent'）时用了 NOT VALID——
-- 只做目录项登记，毫秒级，不扫存量行；tasks 是高频写表，DROP+ADD 在同一事务里扫全表
-- 会拿 ACCESS EXCLUSIVE 挡住派发/回写。本文件单独跑 VALIDATE CONSTRAINT：只需
-- SHARE UPDATE EXCLUSIVE 锁，与普通 DML 可并发；migrate.js 每文件一个事务，463 的
-- ACCESS EXCLUSIVE 在本文件开始前已随 463 提交释放。拆法与 PR1 的 461/462 一致。
--
...
VALUES ('464', 'VALIDATE tasks_executor_kind_check（463 的 NOT VALID 收尾，openclaw-agent 入白名单）')
