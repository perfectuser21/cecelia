CREATE TABLE IF NOT EXISTS work_routing_receipts (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(), task_id uuid NOT NULL UNIQUE REFERENCES tasks(id),
...
CREATE TABLE IF NOT EXISTS map_recovery_contracts (
  ...
  expires_at timestamptz NOT NULL, authorization_evidence jsonb NOT NULL, attempt_id uuid UNIQUE, created_at timestamptz NOT NULL DEFAULT now()
);
