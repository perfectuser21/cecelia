-- 463: 秋米中文 GTD 表与英文 Tasks 库登记进 notion_projection_map（三面模型，决策 297ffee5 / b8abd28c）
...
--
-- 照 PR1 461 的拆法：ADD CONSTRAINT ... NOT VALID 只做目录项登记（毫秒级，不扫存量行），
-- 新写入立即受约束；"确认全表已合规"挪到 464 的 VALIDATE CONSTRAINT（SHARE UPDATE
...
VALUES ('463', 'notion_projection_map 登记秋米中文 GTD 表 + 英文 Tasks 库（face=inlet）+ executor_kind 扩 openclaw-agent')
