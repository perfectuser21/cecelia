-- Migration 462: 验证 461 里 NOT VALID 的 tasks_task_type_check
--
-- 秋米中文 GTD 表接入 Brain 统一调度·PR1 地基（task 15f42776，决策 b8abd28c）。
-- 终审 Important #1：461 把重建 CHECK、回填 tenant_id、重建去重索引四类重活挤在
-- 同一个事务，其中"DROP+ADD CONSTRAINT 需要扫全表验证"这一步最重——尤其 tasks 是
-- 高频写表，长事务持锁会挡住派发/回写。461 已把 ADD CONSTRAINT 改成 NOT VALID
...
-- 独立的迁移文件、独立的事务（migrate.js 每文件一个 BEGIN/COMMIT），461 的
-- ACCESS EXCLUSIVE 锁（索引重建那步）在本文件开始前已经随 461 提交释放。
--
