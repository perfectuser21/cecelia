...
-- 3) 条件数据修复：仅在有数据时执行（sections 4-9）
DO $$
DECLARE
  total_goals int;
...
BEGIN
  -- 检查 goals 表是否有数据
  SELECT count(*) INTO total_goals FROM goals;

  IF total_goals = 0 THEN
    RAISE NOTICE 'Migration 036: Goals table is empty, skipping data repair (schema changes only)';
    RETURN;
  END IF;
  ...
  IF global_okr_count < 1 THEN
    RAISE NOTICE 'Migration 036: No Global OKR found, skipping data repair (fresh database)';
    RETURN;
  END IF;

  RAISE NOTICE 'Migration 036: Found % goals and % Global OKRs, proceeding with data repair',
    total_goals, global_okr_count;
  ...
  RAISE NOTICE 'Migration 036: Data repair completed';
END $$;
...
BEGIN
  ...
  IF total_goals = 0 THEN
    RAISE NOTICE 'Migration 036: Validation skipped (empty database)';
    RETURN;
  END IF;
  ...
  RAISE NOTICE 'Migration 036: Validation passed - No area_okr points to global_okr';
END $$;
...
BEGIN
  ...
  IF total_goals = 0 THEN
    RAISE NOTICE 'Migration 036 完成: Empty database (schema updated, no data to repair)';
    RETURN;
  END IF;
  ...
  RAISE NOTICE '  Total goals: %', total_goals;
