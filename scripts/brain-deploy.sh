...
if [[ "$DEPLOY_MODE" == "docker" ]]; then
    ...
    # 改为：green 临时端口(5230)起 + BRAIN_DEPLOY_CANARY 关 tick + health 全过 → 才删 blue。
    ...
    if [[ "$DRY_RUN" == true ]]; then
        ...
        drain_before_swap
        echo ""
        echo "  [dry-run] bluegreen_swap TARGET_VERSION=${VERSION} BLUE=cecelia-node-brain GREEN=cecelia-node-brain-green PORT=5230"
        ...
        # blue 是 bridge 网络（docker ps 显示 0.0.0.0:5221->5221），green 用 bridge + -p 5230:5221（5223 已归 dashboard staging 槽位，撞港=幽灵代理事故 2026-07-14），
        ...
        if ! TARGET_VERSION="${VERSION}" BLUE_NAME=cecelia-node-brain \
             GREEN_NAME=cecelia-node-brain-green TEMP_PORT=5230 HEALTH_TIMEOUT=90 bluegreen_swap; then
