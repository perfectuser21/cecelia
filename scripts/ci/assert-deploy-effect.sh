...
#
# 用法：
#   assert-deploy-effect.sh <brain_url> <expected_version> [max_uptime_s=300]
...
#
#   测试可注入 HEALTH_JSON_OVERRIDE=<文件路径> 跳过 curl。
#
# verdict / exit：
...
BRAIN_URL="${1:?用法: assert-deploy-effect.sh <brain_url> <expected_version> [max_uptime_s]}"
