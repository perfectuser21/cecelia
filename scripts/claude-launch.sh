...
_link_projects_dir() {
    ...
    if [[ -L "$link" ]]; then
        ...
        # rmdir 失败 = 仍有与主池同名的文件留在 $link（no-clobber 跳过了）。
        # 这些文件在主池均有副本，rename 备份后建软链；失败则放弃（sweep 下次处理）。
        if ! rmdir "$link" 2>/dev/null; then
            mv "$link" "${link}.orphan-$$" 2>/dev/null \
                || { echo "[claude-launch] ⚠️ 无法清理孤儿目录 $link，软链跳过" >&2; return 1; }
        fi
        ln -s "$target" "$link" || return 1
    else
        ln -s "$target" "$link" || return 1
...
_sweep_worktree_project_keys() {
    ...
    for pfx in "${scan_prefixes[@]}"; do
        for d in "$root"/${pfx}*; do
            ...
            echo "$active_wt_keys" | grep -qFx -e "$d_key" && continue
            # 并回主池（no-clobber）
            for f in "$d"/* "$d"/.[!.]*; do
                [[ -e "$f" ]] || continue
                fname="$(basename "$f")"
            ...
            # rmdir 成功 → 直接建软链；失败（冲突文件残留）→ rename 备份再建软链。
            # rename 也失败则跳过（该孤儿下次 sweep 再处理）。
            if rmdir "$d" 2>/dev/null || mv "$d" "${d}.orphan-sweep-$$" 2>/dev/null; then
