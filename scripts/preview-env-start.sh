...
# 该目录通常不单独跑 npm ci，node_modules 缺失或不完整（PR#3810 实测根因之一）。
...
# ── 3. 构建前端 ───────────────────────────────────────────────────────────────
# 注意：改为在 apps/dashboard 目录内单独跑 npm ci（非从 root --workspace=apps/dashboard）。
# 原因：从 root 跑 workspace 安装会删除 root node_modules symlink（npm reify 先清理）并重建，
# 在 ENOSPC（磁盘满）等情况下会留下不完整的 node_modules，导致 Brain 找不到 dotenv 崩溃
# （PR#3810 实测根因）。dashboard 无 workspace 内部依赖，在自己目录 npm ci 等价且安全。
log "Step 3: 构建 apps/dashboard（dashboard 目录内独立 npm ci，含 devDeps）..."
DASH_DIR="${WORK_DIR}/apps/dashboard"
DIST_DIR="${DASH_DIR}/dist"

# 容器 HOME=/Users/administrator，npm 默认 cache 在 $HOME/.npm，但该目录在容器内不可写。
# 显式指定 cache 到 /tmp 下避免 ENOENT（PR#3809 实测根因）。
...
if (cd "$DASH_DIR" && npm ci --cache "$NPM_CACHE_DIR" >> "$LOG_FILE" 2>&1 \
  ...
  DIST_DIR=""
  # 确保 root node_modules symlink 始终指向可用的生产依赖
  # （dashboard dir 内的 npm ci 不影响 root，此处只是保险恢复）
  if [ ! -L "${WORK_DIR}/node_modules" ] && [ -d "$BRAIN_NM_TARGET" ]; then
    rm -rf "${WORK_DIR}/node_modules"
    ln -sfn "$BRAIN_NM_TARGET" "${WORK_DIR}/node_modules"
    log "  → node_modules symlink 已恢复 → ${BRAIN_NM_TARGET}"
  fi
fi
