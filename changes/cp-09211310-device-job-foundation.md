...
- `migration 456`：`tasks_task_type_check` 纳入 `device_job`（安卓工作机的活），列表取自生产库而非抄旧 migration；`tasks` 加 `row_version INTEGER NOT NULL DEFAULT 0` 作乐观锁依据（`updated_at` 被 tick 定时 touch，不能当锁）。
