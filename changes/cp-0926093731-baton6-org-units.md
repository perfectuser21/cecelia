...
- 新增只读端点 `GET /api/brain/org-units`：返回 company→department 树 + 每个 unit 的成员计数（agent/human），供以后"AI 掌握公司信息"用；挂载于 `server.js`（`app.use('/api/brain', orgUnitsRouter)`）。
- 测试：`org-unit-promotion.test.js`（9 例，纯函数判定含数据不足/连续降级/滑动窗口边界）+ `routes/org-units.test.js`（3 例，mock pool，树形聚合/父子挂载/500 兜底）+ `migration-473-org-units.test.js`（6 例，结构断言）；新增 `org-units-smoke.sh` 登记入 `smoke-allowlist.txt`。
