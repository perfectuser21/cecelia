...
- [ ] [ARTIFACT] entrypoint.sh 定义 detect_structured_terminal() 结构化终态识别函数
...
- [ ] [ARTIFACT] ground-truth.js 导出 isInfrastructureErrorCode 与 isContractFaultCode 纯函数
...
- [ ] [ARTIFACT] dispatcher.js __test__ 含 filterBlacklistableTargets 且 failed_targets 构造消费之
...
- [ ] [ARTIFACT] attempt-store listFailedExecutionTargets 返回行携 error_code（供上层过滤）
...
- [ ] [ARTIFACT] 版本 bump 四处同步（brain package.json version 递增 + DEFINITION 同步链）
  Test: bash -c 'node scripts/facts-check.mjs && bash scripts/check-version-sync.sh'
- [ ] [ARTIFACT] 冻结合同测试落盘且真 import 被改模块（无 vi.mock）
...
- [ ] [BEHAVIOR] [L2] B-01: runner 结构化 BLOCKED + CONTRACT_* 保真透传（r69，不 provider_exit）
...
- [ ] [BEHAVIOR] [L2] B-02: runner 结构化 success 结果 JSON 被识别（r77，不当失败包装）
...
- [ ] [BEHAVIOR] [L2] B-03: kernel 病族边界——CONTRACT_* 家族不入基础设施病族
...
- [ ] [BEHAVIOR] [L2] B-04: kernel 派发——CONTRACT_* 不进 failed_targets 黑名单
...
- [ ] [BEHAVIOR] [L2] B-05: 负向——无结构化真崩溃仍 provider_exit / 超时仍 provider_timeout（语义不变）
