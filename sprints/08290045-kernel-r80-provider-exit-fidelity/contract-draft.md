...
|---|---|---|---|
| kernel 病族边界 + failed_targets 过滤（冻结主线） | `sprints/08290045-kernel-r80-provider-exit-fidelity/tests/provider-exit-fidelity.test.js` | isInfrastructureErrorCode；isContractFaultCode；failed_targets | → 10 failures（isInfrastructureErrorCode/isContractFaultCode 未导出、filterBlacklistableTargets 未定义） |
| runner/entrypoint 结构化终态保真透传（补充线） | `tests/gp/f1/step3-provider-exit-fidelity-r80.test.js` | __structured_success__；B1；负向语义不变 | → 9 failures（detect_structured_terminal 未定义、normalize_provider_failure 现写 provider_exit） |
