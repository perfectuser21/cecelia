...
> **R3 变更摘要**: 针对 R2 评审 7 个必须修改项 + 3 个可选改进全部整合。
> 核心修复：(1) 所有 regex 验证排除注释行 (2) 条件分支用结构校验替代距离启发式
> (3) fire-learnings-event 跳过验证 (4) ci-passed if:always() 保护
> (5) 核心 job 保护窗口 500→500 字符 + 注释排除 (6) devloop-check 回写窗口 500→2000
> (7) 04-ship.md 双路径结构校验 (8) health check 超时阈值验证
> (9) afterMerge 窗口 300→1000 (10) ci-passed DoD 条目新增
...
```bash
# 命令 1-2: 功能性结构校验（排除注释行）— R3 修复：过滤注释 + 移除 pkill.*node
node -e "
...
```bash
# 命令 1-3: Health check 超时阈值 <= 60 秒 — R3 新增（可选改进 1）
node -e "
...
```bash
# 命令 1-4: Dashboard 条件构建（if 分支结构校验）— R3 修复：替代距离启发式
node -e "
  const lines=require('fs').readFileSync('scripts/post-merge-deploy.sh','utf8')
    .split('\n').filter(l=>!l.trimStart().startsWith('#')).join('\n');
...
**行为描述**:
Harness 模式创建的 PR（通过 `harness` label 或 `cp-*` 分支前缀识别）在 CI 全部 pass 后自动 merge 到 main。非 Harness PR 不受影响，仍需人工 review。merge 失败时（如冲突），将失败状态回写 Brain 任务。
...
- `devloop-check.sh` 在 harness 模式下跳过交互确认
- `stop.sh` 在 harness 模式下跳过用户确认

**验证命令**:
```bash
# 命令 3-1: 04-ship.md harness 双路径结构校验 — R3 修复：要求代码变量 + 双路径
node -e "
...
```bash
# 命令 3-3: harness 模式明确跳过 fire-learnings-event — R3 新增（修复项 3）
node -e "
  const c=require('fs').readFileSync('packages/engine/skills/dev/steps/04-ship.md','utf8');
...
```bash
# 命令 3-4: devloop-check.sh harness 模式跳过交互
node -e "
...
```bash
# 命令 3-5: stop.sh harness 模式跳过用户确认
node -e "
...
**验证命令**:
```bash
# 命令 4-1: CI 包含 harness 条件跳过（排除注释行）— R3 修复
node -e "
  const lines=require('fs').readFileSync('.github/workflows/ci.yml','utf8')
    .split('\n').filter(l=>!l.trimStart().startsWith('#')).join('\n');
...
```bash
# 命令 4-2: 核心 job 不受 harness skip 影响（窗口扩大到 500 + 排除注释）— R3 修复
node -e "
  const lines=require('fs').readFileSync('.github/workflows/ci.yml','utf8')
    .split('\n').filter(l=>!l.trimStart().startsWith('#')).join('\n');
...
```bash
# 命令 4-3: ci-passed 有 if: always()（防止被跳过的 job 阻塞）— R3 新增（修复项 4）
node -e "
...
**硬阈值**:
- `devloop-check.sh` 包含 `curl -X PATCH /api/brain/tasks` 回写逻辑
- 回写逻辑在 harness 条件守卫内（_harness_mode 检查在 curl PATCH 之前 2000 字符内）
- `post-merge-deploy.sh` 部署失败时也有 Brain 回写

**验证命令**:
```bash
# 命令 5-1: devloop-check harness 失败回写（两步验证 + 2000字符窗口）— R3 修复
node -e "
  const c=require('fs').readFileSync('packages/engine/lib/devloop-check.sh','utf8');
...
**验证命令**:
```bash
# 命令 6-1: 部署成功后回写 deployed 状态（窗口扩大到 1000）— R3 修复（可选改进 2）
node -e "
  const lines=require('fs').readFileSync('scripts/post-merge-deploy.sh','utf8')
    .split('\n').filter(l=>!l.trimStart().startsWith('#')).join('\n');
  const healthIdx=lines.indexOf('health');
...
**范围**: `.github/workflows/ci.yml` 修改 — harness label 跳过非关键 job + auto-merge step + ci-passed 保护。不涉及 /dev skill 或部署脚本。
**大小**: S（<100行）
**依赖**: 无
...
- [ ] [BEHAVIOR] 04-ship.md 包含 harness_mode 变量检测 + 跳过 Learning 路径 + 保留非 harness 完整 Learning 流程
  Test: node -e "const c=require('fs').readFileSync('packages/engine/skills/dev/steps/04-ship.md','utf8');if(!/harness_mode|harness.mode|HARNESS_MODE/.test(c))throw new Error('FAIL: 无检测');const skip=/harness[\s\S]{0,500}(skip|跳过|不执行|do not)[\s\S]{0,200}(learning|Learning)/i.test(c);const normal=/docs\/learnings/.test(c)&&/fire-learnings-event/.test(c);if(!skip)throw new Error('FAIL: 无跳过指令');if(!normal)throw new Error('FAIL: 非harness路径不完整');console.log('PASS')"
- [ ] [BEHAVIOR] harness 模式明确跳过 fire-learnings-event 调用
  Test: node -e "const c=require('fs').readFileSync('packages/engine/skills/dev/steps/04-ship.md','utf8');if(!/harness[\s\S]{0,500}(skip|跳过)[\s\S]{0,200}fire-learnings-event/i.test(c)&&!/harness[\s\S]{0,500}(skip|跳过)[\s\S]{0,200}(learning|Learning)[\s\S]{0,200}fire-learnings/i.test(c))throw new Error('FAIL');console.log('PASS')"
- [ ] [BEHAVIOR] devloop-check.sh 包含 harness 模式检测
  Test: node -e "const c=require('fs').readFileSync('packages/engine/lib/devloop-check.sh','utf8');if(!/harness.*mode|_harness_mode|HARNESS_MODE/.test(c))throw new Error('FAIL');console.log('PASS')"
...
- [ ] [BEHAVIOR] stop.sh 包含 harness 模式检测
  Test: node -e "const c=require('fs').readFileSync('packages/engine/hooks/stop.sh','utf8');if(!/harness.*mode|_harness_mode|HARNESS_MODE/.test(c))throw new Error('FAIL');console.log('PASS')"
