...
**round 5 修订说明（合同修正轮，订正 round 1 host 白名单误判，呼应 Brain judge 独立复核 FAIL）**：round 4 版本 GP-STEP-3 host 判定只认 `skill-relay-claude-headed`（round 1 依据 `payload.dispatched_by_orchestrator=true` 误判本任务必为自动化 orchestrator 派发所致）。round 5 实测本任务实际执行路径是人工前台接管（`claimed_by="session:engine-patch"` 手动认领 fallback 值 + `initiative_runs` 唯一行 `orchestrator_host=foreground`，来自 Brain 官方 `POST /orchestrator/relay-runs/:initiative_id` 补建档端点），generator 已按 round 4 合同正确实现（PR #4109，CI 绿），但 round 4 合同本身遗漏了这一合法值，导致实测 `bash e2e-verify.sh` 报 `FAIL: host=foreground`，与 judge 独立复核 FAIL 结论吻合。本轮修正 GP-STEP-3 段 host 判定从二值扩展为三值合法枚举（`skill-relay-claude-headed`/`skill-relay-codex-headed`/`foreground`，每值均有 Brain 源码文件+行号+注释原文引用，见 contract-draft.md Risks R2），以下第 3 条 BEHAVIOR 的描述文字与「期望」同步更新；Test 命令本身（`awk` 提取 + `psql "$DB"`/`is_fresh` 内容级校验）不含 host 字面值断言，无需改动——host 判定逻辑的权威来源仍**唯一**落在 `e2e-verify.sh` 的 GP-STEP-3 段（本轮修正合同模板，`e2e-verify.sh` 生成物本体的同步更新留给后续 generator 轮次）。
...
- [ ] [BEHAVIOR] initiative_runs host 属于三值合法枚举（skill-relay-claude-headed/skill-relay-codex-headed/foreground，round 5 修正）且 phase 使用真实 DB 枚举拒绝 failed/unknown，且 started_at 新鲜度不早于 task.created_at（从唯一权威脚本 GP-STEP-3 段提取执行，真实 psql 定点查当前 task，round 2 新增新鲜度校验，round 5 host 白名单订正见 contract-draft.md Risks R2）
  ...
  期望: OK（`orchestrator_host` 为 `skill-relay-claude-headed`/`skill-relay-codex-headed`/`foreground` 三者之一即合法，round 5 修正）；若 FAIL 且信息提示「已知外部时序依赖」，见 contract-draft.md Risks R1——不代表脚本实现有误，需等落库后重跑；若 FAIL 且信息提示 `host=<非三值枚举内的值>`，见 contract-draft.md Risks R2——三值枚举以外的值才是真实故障
