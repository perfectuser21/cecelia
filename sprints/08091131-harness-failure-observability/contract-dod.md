...
- [ ] [ARTIFACT] 共享枚举 + fail-closed helper 模块存在
...
- [ ] [ARTIFACT] 机械闸 lint 脚本存在且支持 --fixture-files
...
- [ ] [ARTIFACT] failure-stats 路由已在 harness.js 注册
...
- [ ] [ARTIFACT] lint 已接入 brain-ci-deploy.yml
...
- [ ] [BEHAVIOR] [L2] B-01: harness terminal 写入点经真 helper 把 failure_class 真落 result 列 [接缝×2]
...
- [ ] [BEHAVIOR] [L2] B-02: GET /failure-stats 返回 failure_rate(number) 与 by_class(object)
...
- [ ] [BEHAVIOR] [L2] B-03: failure-stats schema keys 完整且无禁用字段名
...
- [ ] [BEHAVIOR] [L2] B-04: error path — 非法 days=0 返回 400 + error 字段
...
- [ ] [BEHAVIOR] [L2] B-05: 本 sprint 上线后新产生的 terminal harness 任务无一 result.failure_class IS NULL
...
- [ ] [BEHAVIOR] [L1] B-06 (INV-1 合同实跑): 机械闸自测 — 脏 fixture exit 1、干净 fixture exit 0、真实树 exit 0
...
- [ ] [BEHAVIOR] [L2] B-07: 空窗口失败率定义良好（0 条 terminal → failure_rate=0，不 NaN/null）
...
- [ ] [BEHAVIOR] [L2] B-08 (Invariant fail-closed): 非法枚举必抛错且绝不落库为 failed [接缝×2]
