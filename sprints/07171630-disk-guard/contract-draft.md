...
以下命令在生产容器（Brain 运行中）执行验收：

```bash
# === Step 1：触发一轮 disk-guard ===
curl -s -X POST localhost:5221/api/brain/cron/trigger \
...
BRAIN_CONTAINER=$(docker ps -q --filter name=brain)
LOG_HIT=$(docker logs "$BRAIN_CONTAINER" 2>&1 | grep '\[disk_check\]' | tail -3)
...
# === Step 5：验证 scheduler-jobs 注册（Brain API 查询）===
JOBS=$(curl -s localhost:5221/api/brain/scheduler/jobs)
echo "$JOBS" | grep -q 'disk-guard' \
  && echo "PASS: disk-guard 已注册到 scheduler" \
  || echo "WARN: scheduler jobs API 未返回 disk-guard（需确认 API 是否存在）"
