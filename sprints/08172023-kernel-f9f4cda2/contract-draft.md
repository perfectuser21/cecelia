...
**sprint**: 08172023-kernel-f9f4cda2
...
> **合同测试放置（r2 硬要求）**: 本合同冻结测试位于 `sprints/08172023-kernel-f9f4cda2/tests/`（kernel 只从此目录采集冻结产物）；永久回归测试由 Generator 在实现阶段复制到 `packages/brain/src/**/__tests__/`。测试内容与 run 1bc5cd92 合同（Proposer→Generator 已走通）逐字复用，仅换 sprint 目录路径。
...
**验证命令**:
```bash
npx vitest run sprints/08172023-kernel-f9f4cda2/tests/ground-truth-proposal-remote.test.js sprints/08172023-kernel-f9f4cda2/tests/regression-run-7a8e5319-rn.test.js --reporter=basic
```
...
**验证命令**:
```bash
npx vitest run sprints/08172023-kernel-f9f4cda2/tests/derive-observation-gate.test.js --reporter=basic
```
...
**验证命令**:
```bash
npx vitest run sprints/08172023-kernel-f9f4cda2/tests/work-routing-base-repo-backfill.test.js --reporter=basic
```
...
npx vitest run sprints/08172023-kernel-f9f4cda2/tests/ --reporter=basic 2>&1 | tee /tmp/e2e-sprint.log
...
#    9.25.0 死规则：packages/brain/src/** 的 vitest 必须在包根子 shell 内跑（根 vitest.config include 不覆盖 src/**，
#    从仓库根跑必 "No test files found" exit 1）；子 shell 退出后 cwd 仍在仓库根，不影响后续 psql/migration 步骤。
( cd packages/brain && npx vitest run --no-cache \
  ...
  ./src/__tests__/integration/work-routing-store.test.js \
