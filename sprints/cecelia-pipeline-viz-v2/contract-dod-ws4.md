...
- [x] [ARTIFACT] reportNode 返回 `report_path` 状态键，值为可 JSON.parse 的字符串（FullInitiativeState 字段）
  Test: node -e "const c=require('fs').readFileSync('packages/brain/src/workflows/harness-initiative.graph.js','utf8');const fn=c.slice(c.indexOf('reportNode'),c.indexOf('reportNode')+3000);if(!fn.includes('report_path')){console.error('FAIL: 缺 report_path 返回键');process.exit(1)}console.log('OK')"
