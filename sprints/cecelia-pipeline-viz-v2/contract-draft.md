...
| WS2 | `tests/ws2/harness-detail-route.test.ts` | HTTP 200 schema/error 404/step_timing 字段 | 4-5 failures（路由不存在）|
| WS3 | `tests/ws3/initiative-detail-panel.test.ts` | initiative-detail-panel/条件渲染/api brain harness/step-timeline-item | 3-4 failures（组件未写）|
| WS4 | `tests/ws4/report-node-fields.test.ts` | reportNode 含 step_timing/ws_issues/ws_costs + 元素结构 | 3 failures（字段未加）|
| WS5 | `tests/ws5/e2e-screenshot-chain.test.ts` | harness-screenshots/api brain harness detail | 2 failures（链路未建立）|
