# Sprint Contract Draft (Round 1) — 金标集 v0 + LLM 判定器 eval 通过率棘轮进 CI
...
> **前轮死因修复（validation_clock_required）**：上轮 E2E 用静态断言充数、未真跑出通过率，故校验时钟缺失。本轮 `## E2E 验收` 与全部 [BEHAVIOR] 均**真实执行** eval（node 逐条判读金标集 → 计算真实通过率 → 真实 exit code 驱动棘轮闸），非文本自证。
