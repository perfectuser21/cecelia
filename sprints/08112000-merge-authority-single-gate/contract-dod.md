...
- INV-3 [不回归/dev] Brain 明确 not-owned 的 cp-* 必 MERGE → **B-06**（+ 端点侧 B-02 owned:false）+ **A-05**（ci.yml 对 not-owned 置 harness-judge=success，防 required check 卡死 /dev，R1-2 补齐）
- INV-4 [归属凭 Brain 非标题] 只凭 initiative_runs 记录 → **B-01**（owned 判定来自 seed 的 initiative_runs 行，非标题/分支正则）

## ARTIFACT 条目

- [ ] [ARTIFACT] should-auto-merge.sh 判据改为 Brain 求证（含 fail-closed），不再以 feat(harness): 标题决定
...
- [ ] [ARTIFACT] Brain 归属求证端点 pr-ownership 已挂载
...
- [ ] [ARTIFACT] kernel merge_pr 置 harness-judge commit status
...
- [ ] [ARTIFACT] 通道3 engine-pr-watchdog 改造说明产出
...
- [ ] [ARTIFACT] A-05: ci.yml auto-merge job 对 not-owned(MERGE) PR 置 harness-judge=success（防 required check 永久卡死 /dev 红线兜底，R1-2）
...
- [ ] [BEHAVIOR] [L2] B-01: seed 的 harness run 被归属端点判为 owned:true（归属凭 initiative_runs，非标题）
...
- [ ] [BEHAVIOR] [L2] B-03: branch/pr_url 均缺失时端点返回 HTTP 400（error path）
...
- [ ] [BEHAVIOR] [L2] B-04: 通道1 fail-closed——Brain 不可达时 should-auto-merge.sh 输出 SKIP 且绝不 MERGE（红线）[接缝×2]
...
- [ ] [BEHAVIOR] [L1] B-05: kernel merge_pr 在 gh pr merge 之前先置 harness-judge=success（裁决唯一闸执行点）
...
- [ ] [BEHAVIOR] [L2] B-06: 通道1 owned=true → SKIP（用真实 stub Brain server，禁 mock HTTP 边）[接缝×2]
...
- [ ] [BEHAVIOR] [L2] B-07: 通道1 fail-closed——Brain 接受连接后超时无响应时脚本走 curl --max-time 超时路径输出 SKIP 且绝不 MERGE（红线，R1-1；与 B-04 连接被拒 exit7 是不同代码路径）[接缝×2]
