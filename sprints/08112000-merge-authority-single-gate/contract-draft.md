...
**可观测行为**: 脚本以 `$BRAIN_URL` + `branch` 求证：owned → `SKIP:...`；not-owned + cp-* → `MERGE`；非 cp-* → `SKIP`；Brain 任意故障 → `SKIP`（fail-closed），**绝不 MERGE**。标题不再参与判据（保留为日志参数）。

**fail-closed 三分支（PRD 验收断言第 4 条明列，缺一即漏）——三条是不同代码路径，各需独立断言**:
- **连接被拒**（`http://127.0.0.1:1` 无监听）：curl 立即 `exit 7` 快速失败 → SKIP。
- **超时**（Brain 接受连接后挂起不响应）：**必须** `curl` 求证显式带有限 `--max-time "${BRAIN_TIMEOUT:-8}"`（默认 8s，可 env 覆盖），触发 `exit 28` → SKIP。**漏配 `--max-time` 时真实 Brain 挂起会让 auto-merge job 无限死等 CI**，故超时须有独立用例逼出 `-m` 闸。
...
# 超时（exit28）— 真 stub 接受连接后永不响应，BRAIN_TIMEOUT=2 逼出 curl --max-time:
node -e 'const s=require("http").createServer(()=>{});s.listen(0,"127.0.0.1",()=>{const p=s.address().port;let o="";try{o=require("child_process").execFileSync("bash",[".github/workflows/scripts/should-auto-merge.sh","cp-x-abc","fix(brain): x"],{env:{...process.env,BRAIN_URL:"http://127.0.0.1:"+p,BRAIN_TIMEOUT:"2"},encoding:"utf8"})}catch(e){o=String(e.stdout||"")}s.close();if(!/^SKIP/.test(o.trim())||/MERGE/.test(o)){console.error("FAIL: 超时未 fail-closed",o);process.exit(1)}console.log("timeout-skip OK")})'
```
**硬阈值**: 连接被拒与超时两种脚本均输出以 `SKIP` 开头且不含 `MERGE`；超时用例证明 `curl --max-time` 已配置（否则 node 驱动会挂满 vitest/evaluator 超时预算）。
...
**可观测行为**: Brain 明确 `owned:false` 的 cp-* PR → 脚本 `MERGE`（照旧被通用 auto-merge）；且该 PR 不被 harness-judge 永久阻断——**因 harness-judge 一旦登记为分支保护 required check 会作用于所有 PR，/dev PR 拿不到 success 即被卡死**，故 `ci.yml` auto-merge job 在 `DECISION==MERGE`（not-owned）分支下必须先对该 PR head SHA 发出 `gh api repos/$GITHUB_REPOSITORY/statuses/<head_sha> -f state=success -f context=harness-judge`，令 required check 满足后再 `gh pr merge --auto`。这是 [不回归/dev] 红线的兜底代码路径（本轮新增 `[ARTIFACT]` 断言，见 contract-dod.md）。

**验证命令**:
```bash
# 脚本侧：not-owned → MERGE（见 E2E 段用真实 stub server 驱动）
bash .github/workflows/scripts/should-auto-merge.sh "cp-x-devpr" "fix(brain): 手动 dev" | grep -q '^MERGE'
# CI 侧兜底：ci.yml auto-merge job 含对 not-owned 置 harness-judge=success 的命令（防 required check 卡死 /dev）
node -e 'const c=require("fs").readFileSync(".github/workflows/ci.yml","utf8");if(!/context=harness-judge/.test(c)||!/state=success/.test(c)){console.error("FAIL: ci.yml 缺 not-owned 置 harness-judge=success 兜底");process.exit(1)}console.log("ci-fallback OK")'
```
**硬阈值**: not-owned + cp-* → MERGE（红线，误拦即卡死所有 /dev）；且 ci.yml 含 `context=harness-judge` + `state=success` 兜底命令。
...
- 关键字段: query `branch`（必），可选 `pr_url`；响应按上文 Response Schema 逐字段 `jq -e`。求证 `curl` 显式带 `--max-time "${BRAIN_TIMEOUT:-8}"`（有限超时，超时=fail-closed SKIP）。
- 生产调用方即 ci.yml auto-merge job（已注入 `BRAIN_URL` env，见 ci.yml:1942）——本 sprint 复用同一 env，不新增双路径。
- 该 job 在 `DECISION==MERGE`（not-owned）分支下另发一条 `gh api repos/$GITHUB_REPOSITORY/statuses/${{ github.event.pull_request.head.sha }} -f state=success -f context=harness-judge`（head_sha 走 GHA context env，非 LLM），令 /dev PR 拿到 required check success（R1-2 兜底）。
...
| **NFR（做得多好）** | | 归属求证有限超时（脚本侧 `curl --max-time "${BRAIN_TIMEOUT:-8}"`，默认 8s，超时 exit28=SKIP，防 Brain 挂起时 CI 死等）；置 check/合并走版本无关 GitHub REST；归属判定与 auto-merge 失败可追溯（脚本 `SKIP:<原因>`、失败回写 Brain task）。 |
...
# 5b. 通道 1 fail-closed（红线，R1-1）：Brain 超时（接受连接后挂起 exit28）→ 脚本 SKIP，绝不 MERGE
#     真 stub server 接受连接后永不响应，BRAIN_TIMEOUT=2 逼出 curl --max-time 超时路径（区别于 5. 的连接被拒）。
...
# 6b. /dev 不回归兜底（红线，R1-2）：ci.yml auto-merge job 对 not-owned(MERGE) PR 置 harness-judge=success
#     required check 一旦登记为分支保护会作用于所有 PR；无此兜底则 /dev PR 拿不到 success 被永久卡死。
...
|---|---|---|---|
| 通道1 归属换 Brain + fail-closed | `tests/should-auto-merge-brain.test.ts` | `owned=true` → 输出 SKIP；`owned=false` + cp-* → 输出 MERGE；`Brain 5xx → SKIP`；`非法 JSON → SKIP`；`Brain 不可达（连接被拒）→ SKIP`；`Brain 超时`（**R1-1 新增**，接受连接后挂起 → curl --max-time exit28 → SKIP）；`回归 #4755 分支`；`回归 #4759 分支` | 9 用例中 6 failed（`owned=true`/`5xx`/`非法JSON`/`不可达`/`超时`/`回归#4755` — 现脚本按标题判，Brain 求证全漏）；3 passed 属现脚本行为巧合一致（`owned=false→MERGE`、`回归#4759`标题恰为 feat(harness): 已 SKIP、`非cp-*→SKIP`），非伪绿 |
...
> BEHAVIOR 覆盖名均为对应 `it()` 名的字面子串。合并 RED 证据：10 failed | 3 passed（含 R1-1 新增超时用例，占 should-auto-merge 侧 1 条 red）。
