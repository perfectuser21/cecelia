...
// （+ 跨模块传递：runner 回执 → kernel 分类 → derive 路由）
//
// 覆盖父路 F1「工厂·开发闭环」第 3 步「造完真验」的 kernel 失败语义保真子路。
//
// 2026-08 生产实证（r69，attempt 56a09164）：generator 完成完整合同死锁分析
// （判定"无绿态可达" + 4 条修法），以结构化 BLOCKED + error.code=CONTRACT_SELF_CONTRADICTION
// 写下回执，但进程以非零码退出。runner 的 child.once('close') 分支对 code!==0 一律
// 用 failedHarnessResult(..., `provider_exit_${code}`) 覆盖，无视 resultPath 里已写下的
// 结构化合同故障 → 真实语义（应走 r40 重开 GAN）被埋没成 provider_exit → kernel 当基础
// 设施故障 → 进 failed_targets 黑名单 + infrastructure 重试，失败不留原因病族。
//
// 修法：runner 抽出纯函数 resolveProviderCloseResult({resultPath,attemptId,exitCode})——
// 非零退出下若 resultPath 存在合法结构化 result 且 error.code 匹配 ^CONTRACT_[A-Z0-9_]+$ 则
// 保真透传，否则回落 provider_exit_${code}；exit 0 与外层 provider_result_invalid 语义不变。
// 判定只凭结构化 result.error.code（PRD 边界：禁 grep stdout 文本）。
//
// 按产物闸规矩写在边上：真 import 被改模块（不 vi.mock 被改的边），真 fs 临时文件。
...
// Validation identity is late-bound: the executing role's attempt id is injected by the
// Runner as HARNESS_ATTEMPT_ID at run time. This pure-function fixture only needs *a*
// well-formed UUID to echo into failedHarnessResult / satisfy parseHarnessResult's
// UUID_PATTERN check; it must never hardcode a role attempt/capability UUID literal
// (validation-identity-policy.js#premature_validation_identity_binding). Synthetic
// fallback is used only for local runs when the Runner injects nothing.
...
function writeResult(obj: Record<string, unknown>) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'r77-close-'));
  const p = path.join(dir, 'result.json');
  fs.writeFileSync(p, JSON.stringify(obj));
  return p;
}

// —— derive(observed) 契约形状，对齐 packages/brain/src/orchestrator/__tests__/derive.test.js ——
...
const cb = (hop: number, patch: Record<string, unknown> = {}) => ({
  hop,
...
describe('generator 合同故障码保真透传（r69 埋没根除）[BEHAVIOR]', () => {
  ...
  it('全链复刻 r69：结构化 CONTRACT_* 非零退出 → 保真 → kernel 分类 → derive 路由重开 GAN', () => {
    ...
    // 2) kernel 分类（真 import execution-contract）
    ...
    // 3) derive 路由（真 import derive）：走合同故障仲裁 → 不进黑名单/infra 重试
  ...
  it('负向 真崩溃：无结构化 result 仍 provider_exit / failed，语义不变', () => {
    const missing = path.join(os.tmpdir(), 'r77-nonexistent-dir', 'none.json');
    ...
    // 分类为 runner_failure（非合同故障、非 infrastructure）；complete() 在真实链路补
    // provider_metadata，此处镜像其后置形状再分类。
  ...
  it('边界 非 CONTRACT_ 结构化 code：semantic_refusal 在非零退出下回落 provider_exit（只凭结构化 code，不 grep stdout）', () => {
    ...
    // 仅前缀无后缀的 CONTRACT_ 亦不匹配（^CONTRACT_[A-Z0-9_]+$ 要求 ≥1 后缀字符）
  ...
  it('纯函数可重放：同输入两次结果 deep-equal，且 exit 0 成功路径不受影响', () => {
    ...
    // exit 0 成功结果原样透传（非本 sprint 改动点，防回归）
