...
- [x] [ARTIFACT] `playground/server.js` 含 `/multiply` 路由注册
...
- [x] [ARTIFACT] `playground/server.js` 仍保留 `/health` 路由（防误删 W19 之前的基线）
...
- [x] [ARTIFACT] `playground/server.js` 实现 `/multiply` 时使用 `^-?\d+(\.\d+)?$` 完整匹配正则（防 Number() / parseFloat() 假绿）
  Test: manual:node -e "const c=require('fs').readFileSync('playground/server.js','utf8');if(!/\\^-\\?\\\\d\\+\\(\\\\\\.\\\\d\\+\\)\\?\\$/.test(c))process.exit(1)"

- [x] [ARTIFACT] `playground/tests/server.test.js` 含至少一个引用 `/multiply` 的测试用例
...
- [x] [ARTIFACT] `playground/tests/server.test.js` 同时含 `/multiply` happy（200 + product）与 error（400）断言
...
- [x] [ARTIFACT] `playground/tests/server.test.js` 仍保留现有 `/sum` 与 `/health` 用例（W19 + bootstrap 回归）
...
- [x] [ARTIFACT] `playground/README.md` 含 `/multiply` 字符串且给出至少 1 条 strict-schema 拒绝示例（`1e3` 或 `Infinity` 任一）
...
- [x] [ARTIFACT] `playground/README.md` 仍含 `/sum` 与 `/health` 段（防误删）
...
- [x] [ARTIFACT] `playground/package.json` 未新增 devDependencies（仅 `supertest` + `vitest`，无 zod/joi/ajv）
