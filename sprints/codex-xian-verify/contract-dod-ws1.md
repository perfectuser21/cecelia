...
- [x] [ARTIFACT] `packages/brain/src/routes/goals.js` /health handler 含 `codex_bridge_status` 字段写入
...
- [x] [ARTIFACT] `packages/brain/src/routes/goals.js` 含 bridge 探活 URL（XIAN_CODEX_BRIDGE_URL 或默认值）
...
- [x] [ARTIFACT] `packages/brain/src/routes/goals.js` 含 offline fallback（catch + 'offline' 字符串）
...
- [x] [BEHAVIOR] GET /api/brain/health 响应包含 codex_bridge_status 字段，类型为 string
...
- [x] [BEHAVIOR] codex_bridge_status 值为合法枚举 "online" 或 "offline"（不接受其他任何值）
...
- [x] [BEHAVIOR] schema 完整性 — 响应同时包含 codex_bridge_status、status、uptime_seconds（新字段与现有字段共存）
...
- [x] [BEHAVIOR] 禁用变体不存在 — codex_bridge_status 不等于 up/down/ok/reachable/active/unavailable（null 防护）
...
- [x] [BEHAVIOR] error path — bridge 探活 offline 时 health 仍返 HTTP 200（goals.js 含 catch-all + offline fallback 逻辑，由 WS2 单元测试验证探活失败路径）
