...
Evidence: 真实 PostgreSQL schema 与 SQL 合同 9/9 通过；两端 composite FK 和 partial unique index 均实查存在。
...
Evidence: 真库故意制造 edge FK 失败后 version1/旧 run 仍 active、version2 仍 draft，旧图仍为 20 节点/31 边。
...
Evidence: facts-check、version-sync 与精确 DoD mapping 均 exit 0；定向 10 files/73 tests 通过。
