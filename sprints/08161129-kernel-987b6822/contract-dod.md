...
- [ ] [ARTIFACT] 合同冻结测试落 sprints/<sprint_dir>/tests/（r2 硬约束，kernel 唯一采集路径）
  Test: node -e "require('fs').accessSync('sprints/08161129-kernel-987b6822/tests/contract-red.test.ts')"
...
- [ ] [ARTIFACT] E2E 种子脚本存在（真写已路由 task + receipt + active v2 run，禁伪造）
  Test: node -e "require('fs').accessSync('sprints/08161129-kernel-987b6822/e2e-seed-routed-task.mjs')"

- [ ] [ARTIFACT] 永久回归集成测试落 packages/brain/src/__tests__/integration/（Generator 复制，非冻结路径）
  Test: node -e "require('fs').accessSync('packages/brain/src/__tests__/integration/headed-attempts.pg.integration.test.js')"
...
- [ ] [BEHAVIOR] INV-1 [同闸门 c3617bdf / 逃生口唯一] 签发口不放松闸语义：非 worktree-manage Bash 无 lock 仍 block、inactive attempt validate 仍 409（B-01/B-02/B-04 覆盖）
  Test: manual:bash -c 'bash packages/engine/tests/integration/dev-mode-tool-guard.test.sh'
...
- [ ] [BEHAVIOR] INV-4 [真环境验证] psql 查 harness_attempts headed 行 + hook 真返回非 block（E2E 验收点④③覆盖，真 Brain+真 PG）
  Test: manual:bash -c 'grep -q "harness_attempts" sprints/08161129-kernel-987b6822/contract-draft.md'
...
- INV-6 [headed workspace_spec] 签发口写 attempt 时把 {branch,base_sha} 落 task_bundle.inputs.workspace_spec（复用 createHeadedKernelAttempt 既有 jsonb 写法，B-03 断言 workspace_spec）
- INV-7 [headed base_repo] cp-branch 名带 task short id（slug）、repo 记入 lock 与 workspace_spec（B-03 + E2E 六字段 jq 覆盖）
- INV-8 [planner分支] N/A：本 sprint 不涉及 planner 自行 checkout/switch，签发口只发 attempt 不动 planner 分支绑定
...
- [ ] [BEHAVIOR] REG-2 [不改validate] validate SQL 语义不改（含 attempt.callback_secret_hash = $7、读 x-harness-route-token）
