...
**sprint**: 08161129-kernel-987b6822
**journey_type**: dev_pipeline
**target_environment**: local_api（Brain 内部签发口 + psql 查 harness_attempts；curl localhost:5221）
**base_sha (implementation_baseline, frozen)**: bc4e864400daf7b4cdafec5af09e4b8b4767a13e

contract-gate: cecelia worktree（packages/brain/src/lib/contract-gate.js 存在）→ 代码层 Contract Gate 生效，断言按速查表惯用法写。
gp-anchor: skipped (product-map.json not found)

> **【r2 合同产物路径硬约束 — 本轮修复点】** 前一单（run 3cd1072a / 0ca4b234）合同冻结测试被放到 `packages/brain/src/**/__tests__/`，kernel 采集冻结产物只认 `sprints/<sprint_dir>/tests/` → run 以 `force_approve_but_contract_artifacts_missing` 终态。本轮把合同冻结测试落 **`sprints/08161129-kernel-987b6822/tests/contract-red.test.ts`**（kernel 唯一采集路径）。**永久回归测试**（`packages/brain/src/__tests__/integration/headed-attempts.pg.integration.test.js`、`packages/engine/tests/integration/dev-mode-tool-guard.test.sh` 扩展）由 Generator 在实现阶段落到各自 CI 目录，两者分工不混用（Invariant [合同产物路径]）。
...
**来源**: `[FROM_PRD]` — PRD Golden Path 第 6 步（「无 lock，hook 仅对 `bash <repo>/packages/engine/skills/dev/scripts/worktree-manage.sh …`（精确路径匹配）放行，其余一切照旧 fail-closed」）
...
```bash
# 见 B-03/E2E：init-or-check --task-id 后六字段齐全（jq -e）
...
```bash
cd packages/brain && DATABASE_URL="${DB_URL:?}" npx vitest run --config vitest.integration.config.js src/__tests__/integration/headed-attempts.pg.integration.test.js
```
...
**来源**: `[FROM_PRD]` — PRD Golden Path 第 7 步 + 边界情况
...
有头 worktree（真实调用方）→ Brain 签发口 / validate 的认证与字段，与生产 hook（`dev-mode-tool-guard.sh` scoped token 分支，实测 `curl -fsS --max-time 5 -X POST "$validation_url" -H 'Content-Type: application/json' -H "X-Harness-Route-Token: $scoped_token"`）逐字段一致：
...
- validate body 字段（hook 实测 jq 构造，逐字）：`{routing_receipt_id, task_id, run_id, repo, branch, base_sha}`。
...
- [回归] `packages/engine/tests/integration/dev-mode-tool-guard.test.sh` → 现有 lights 探活拦 ScheduleWakeup / Bash bg 等 case 必须保持通过。
- [回归] `packages/brain/src/routes/__tests__/work-routing.test.js` / `work-routing-validation-route.integration.test.js` → validate SQL 含 `attempt.callback_secret_hash = $7`、读 `x-harness-route-token`；**不改 SQL 语义**（Invariant [不改validate]）。
- [累积FR] context-manifest: journey e6f803f2 下 ability 仅 status=planned，无 done/working 已验收行为（PRD 已载明，无历史 FR 约束）；本 proposer 环境 Brain 不在运行（`runtime_resources.postgres=false`），端点不可达，记一行按 PRD「累积 FR：本 line 暂无历史」。
- [MAP] r2 说明：Map manifest 已升 v3（F1 认领仓库根 `DoD.md`），不再因 `DoD.md` 触发 `impact_anchor_missing`。本 proposer 环境 Brain 不可达无法拉 radius → `[MAP_NOT_CONFIGURED]`，无 must_run_assertions/fact_revisions 注入；不回退领域硬编码，按 PRD + 代码实测起草。

## 历史约束三源 → INV 映射（EVA v2 固定段）

| 来源 | 铁律 | 本 sprint 映射 |
|---|---|---|
| journey_feature | [同闸门] 有头无头共用同一闸门 c3617bdf 不变，补签发口不是放行口 | **INV-1**：非 worktree-manage Bash 无 lock 仍 block、inactive attempt validate 仍 409（DoD B-01/B-02/B-04 + INV-1） |
| journey_feature | [不改validate] 不改 validate 的 SQL 语义 | **REG-2**：validate SQL 回归全绿（work-routing.test.js / validation-route.integration.test.js 不变） |
| journey_feature | [逃生口唯一] hook 只加 worktree-manage.sh 精确路径 bootstrap 逃生口，禁任何按命令内容判读写白名单 | **INV-1 + B-01**：仅 `bash <绝对路径>/worktree-manage.sh …` 精确前缀放行，其它 Bash（含伪装路径）仍 block |
| area | [端点鉴权] 端点走既有鉴权，非裸开 | **INV-2**：签发口走 `workRoutingAuthorization`（internal token） |
| area | [禁写死环境假设值] | **INV-3**：worktree-manage 端点走 `CECELIA_ROUTING_HEADED_URL`/`BRAIN_URL` env，不写死 |
| area | [真环境验证才算done] | **INV-4**：psql 真查 harness_attempts headed 行 + hook 真返回非 block（E2E 真 Brain+真 PG） |
| area | [凭据安全]/[日志脱敏] | route_token/callback_secret 只签发时返回一次、库内只存 hash、不落日志明文（Response Schema + 判定点表） |
| area | [planner分支] planner 使用服务端签发的 PLANNER_BRANCH | **INV-8** N/A：签发口只发 attempt 不动 planner 分支绑定 |
| area | [租户隔离] | **INV-9** N/A：签发口/validate 为内部端点，无多租户业务数据面 |
...
| **Invariant** | 永不违反 | 同闸门不放松（c3617bdf）；validate SQL 语义不改；端点走既有鉴权；不写死环境假设值；route_token/callback_secret 只签发时返回一次、库内只存 hash、不落日志明文；合同冻结测试落 sprints/<sprint_dir>/tests/。 |
| **判定点** | 怎么知道 | 见判定点登记表 |
...
| ⚠️ headed attempt 用哪个 role/attempt_kind（不越 CHECK 约束） | A. role='headed_dev'/attempt_kind='headed'（PRD 建议）; B. role∈现有枚举 + attempt_kind∈现有枚举 + lease_owner='headed:%' 标记 | **B** | 357 迁移 role CHECK IN (planner,proposer,reviewer,generator,evaluator,judge,reporter) 不含 headed_dev；361/362 attempt_kind CHECK IN (initial,fix,retry,resume,recovery) 不含 headed。方法 A 触发 CHECK 违约插入失败。复用 `createHeadedKernelAttempt` 既有约定（lease_owner=`headed:<id>`）。 | 选 A 则 INSERT 抛 CHECK 违约，签发口 500，闸不解。**PRD [ASSUMPTION] role 建议与 CHECK 约束冲突，按约束为准。** |
...
> ⚠️ 两行判定点属「误判后果严重」级别；因 PrepPRD [ASSUMPTION] 与冻结 SQL/CHECK 约束**冲突**，本合同按代码真值裁定并在 notes 标 `judgment-pending-user: route_token=callback_secret_hash 摘要本身、headed 用 lease_owner 标记（非新 role/attempt_kind 枚举）` 待主理人确认。
...
> 由 evaluator 作为独立 task 在 local_api 上执行：真 Postgres（`$DB_URL`）+ 真 Brain（localhost:5221）+ 真 hook + 真 worktree-manage.sh。覆盖 PRD E2E 5 个验收点。种子（已路由 task + receipt + active v2 run）由 generator 产出的 `sprints/08161129-kernel-987b6822/e2e-seed-routed-task.mjs` 用现有 store（`createRoutedTask` + kernel 授权 fixture 同款）真写，禁伪造。
> journey_type=dev_pipeline → 无 staging 预览闸。
...
cleanup() {
  ...
  git worktree remove --force "$WORK_ROOT/wt" 2>/dev/null || true
  rm -rf "$WORK_ROOT" "$TOKEN_FILE" || true
}
trap cleanup EXIT

# 1. 空库 bootstrap：跑仓库真实 migrations，机检目标表存在
node packages/brain/scripts/run-migrations.mjs 2>/dev/null || node packages/brain/migrate.js 2>/dev/null || node packages/brain/src/migrate.js 2>/dev/null || npm --prefix packages/brain run migrate
...
SEED_JSON=$(node "$REPO_ROOT/sprints/08161129-kernel-987b6822/e2e-seed-routed-task.mjs")
...
# 7. 验收点④：psql 查 harness_attempts 存在该 run 的 headed 行（status=running + workspace_spec，带时间窗）
...
ROUTE_TOKEN=$(cat "$TOKEN_FILE" 2>/dev/null || echo deadbeef)
CODE=$(curl -s -o /tmp/vresp.json -w '%{http_code}' -X POST "$BASE_URL/api/brain/work-routing/validate" \
  -H 'Content-Type: application/json' -H "X-Harness-Route-Token: $ROUTE_TOKEN" \
...
|---|---|---|---|
| Golden Path 三处改动静态契约（冻结产物，kernel 采集） | `sprints/08161129-kernel-987b6822/tests/contract-red.test.ts` | `新增 headed-attempts 签发路由`、`支持 --task-id 调签发口并改 cp-branch`、`补 worktree-manage.sh 精确路径 bootstrap 逃生口` | 实现前三 it() 全 FAIL（字符串未出现）|
| Hook bootstrap 逃生口 + inactive block（永久回归） | `packages/engine/tests/integration/dev-mode-tool-guard.test.sh`（新增 case） | `worktree-manage.sh 精确路径放行`、`非 worktree-manage Bash block`、`run_attempt_inactive block` | 现 hook 无逃生口 → 新 case FAIL |
| Brain 签发口 + validate 往返（永久回归） | `packages/brain/src/__tests__/integration/headed-attempts.pg.integration.test.js` | `无 routing_receipt_id 返回 400`、`已路由 task 返回 route_token 且 harness_attempts running`、`validate 带 route_token 返回 valid true`、`completed 后 validate 409 run_attempt_inactive` | 端点未实现 → 404/无该行 FAIL |
