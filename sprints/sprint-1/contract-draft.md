...
**基于**: Planner 任务 3217cdf0（4 个断链点识别）+ R1/R2 review 反馈
**R2 修复点**:
1. `npm test | tail -20` → 改用 `bash -c 'set -o pipefail; ...'` 保证 exit code 传播
2. 裸 psql SELECT → 改为 node 脚本含 PASS/FAIL 判断
3. execution.js 文本位置检查过弱 → 改为正则验证控制流
...
- REVISION 分支有 `nextRound > MAX_CONTRACT_PROPOSE_ROUNDS`（或 `>=`）守卫逻辑
- 超限时不调用创建 `sprint_contract_propose` 的 `createHarnessTask`

**验证命令**：
```bash
# Happy path: 检查常量存在且值为 5
node -e "
...
# 边界验证: 正则检查 REVISION 分支含轮次守卫（控制流，非注释）
node -e "
  const c = require('fs').readFileSync('packages/brain/src/execution.js','utf8');
  const hasGuard = /nextRound\s*[>]=?\s*MAX_CONTRACT_PROPOSE_ROUNDS/.test(c) ||
                   /propose_round\s*[>]=?\s*MAX_CONTRACT_PROPOSE_ROUNDS/.test(c);
  if (!hasGuard) {
    console.error('FAIL: 未找到 nextRound/propose_round >= MAX_CONTRACT_PROPOSE_ROUNDS 控制流');
    process.exit(1);
  }
  console.log('PASS: REVISION 分支含轮次上限控制流守卫');
"
```
...
- `sprint-contract-reviewer/SKILL.md` 含 `git push origin HEAD`
- commit message 格式含 `contract draft round` 或 `contract review round`

**验证命令**：
```bash
# Proposer SKILL.md 含 git push 且顺序正确
node -e "
  ...
  if (addIdx === -1 || addIdx > pushIdx) {
    console.error('FAIL: git add 顺序错误或缺失'); process.exit(1);
  }
  console.log('PASS: proposer SKILL.md 含 git push，add→commit→push 顺序正确');
"

# Reviewer SKILL.md 含 git push
node -e "
  const c = require('fs').readFileSync(
  ...
  console.log('PASS: reviewer SKILL.md 含 git push origin HEAD');
"
```
...
node -e "
  ...
  console.log('PASS: ' + found + ' 含 propose_round 测试用例');
"

# 用 pipefail 运行测试（R2 修复：防止 tail 掩盖 exit code）
...
- **execution.js**：REVISION 分支顶部增加 `const MAX_CONTRACT_PROPOSE_ROUNDS = 5;`，用 `if (nextRound > MAX_CONTRACT_PROPOSE_ROUNDS)` 保底升格（复制 draft → sprint-contract.md，触发 sprint_generate）
- **sprint-contract-proposer/SKILL.md**：Phase 3 补充 `git add → git commit → git push origin HEAD`
- **sprint-contract-reviewer/SKILL.md**：同上，Reviewer 也 push feedback 文件
- **测试**：`harness-sprint-loop-v3.test.js` 追加 MAX_ROUNDS describe block（propose_round=5 和 4 两个 case）
