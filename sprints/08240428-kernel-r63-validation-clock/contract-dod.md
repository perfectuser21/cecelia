...
- [ ] [BEHAVIOR] [L2] B-05: 失败或被阻止的 fix intent 不刷新原点
  动作: 输入真实 loop shape 的 fix intent，但不给匹配的 effect:attempt_launched 回执，并加入 action/hop 错配回执
  预期观察: 未证实成功的 intent 被忽略，clock 保持初始 generator 原点
  等待预算: 0s
  留证: Vitest 对初始原点和 deadline 的精确断言输出
  Test: manual:bash -c 'npx vitest run --no-cache sprints/08240428-kernel-r63-validation-clock/tests/validation-clock-fix-extension.test.ts -t "失败或被阻止的 fix intent 不刷新原点"'
