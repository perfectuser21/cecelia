...
**范围**: 仅修改纯函数 `packages/brain/src/orchestrator/validation-clock.js` 的 `resolveValidationClock` 顺延与有界逻辑 + `tests/gp/f1/` 冻结回归测试。不改 `timeout_seconds` 默认值、不动人审 deadline、不做真库 loop E2E。
**大小**: S
...
- [ ] [ARTIFACT] 冻结测试文件存在且真 import 被改模块（禁 mock 被改的边）
  Test: node -e "const c=require('fs').readFileSync('tests/gp/f1/step3-validation-clock-fix-extend.test.js','utf8');if(!c.includes('validation-clock.js'))process.exit(1);if(/vi\.mock\(/.test(c))process.exit(1)"

## BEHAVIOR 条目（内嵌可执行 manual: 命令，journey_type=autonomous / 纯函数真 import）

- [ ] [BEHAVIOR] [L2] B-01: 冻结测试 6 用例全绿（r50 复刻存活 + 边界全覆盖）
  动作: 从仓库根真 import 真 `resolveValidationClock` 跑冻结测试 `tests/gp/f1/step3-validation-clock-fix-extend.test.js`
  预期观察: vitest 输出 `6 passed`、无 `failed`；r50 复刻用例断言 deadline 前移到第 3 个 fix 而存活
  等待预算: 0s
  留证: /tmp/frozen.log（vitest 末 8 行含 6 passed）
  Test: manual:bash -c 'npx vitest run tests/gp/f1/step3-validation-clock-fix-extend.test.js --no-cache 2>&1 | tee /tmp/frozen.log | grep -qE "6 passed" && ! grep -qE "[1-9][0-9]* failed" /tmp/frozen.log'
...
- [ ] [BEHAVIOR] [L2] INV-1: 既有 repo 回归 11/11 不退 + verified_existing_pr 采纳规则不破坏
...
- [ ] [BEHAVIOR] [L2] B-06: 合同外路径零写入（r68 死因守卫）
  ...
  预期观察: 无 `tests/regression/` 新增；所有变更文件 ∈ {validation-clock.js, 冻结测试, sprints/本目录/*}
  等待预算: 0s
  留证: stdout `OK`（越权时打印 FAIL-offcontract + 文件名）
  Test: manual:bash -c 'CH=$(git diff --name-only origin/main...HEAD 2>/dev/null || git diff --name-only HEAD~1 HEAD); if echo "$CH" | grep -qE "^tests/regression/"; then echo FAIL-regression; exit 1; fi; OFF=$(echo "$CH" | grep -vE "^(packages/brain/src/orchestrator/validation-clock[.]js|tests/gp/f1/step3-validation-clock-fix-extend[.]test[.]js|sprints/08241610-kernel-r69-validation-clock/)" || true); if [ -n "$OFF" ]; then echo "FAIL-offcontract $OFF"; exit 1; fi; echo OK'

### INV 覆盖（历史约束三源）

- INV-2 [vitest include 范围]：冻结测试落 `tests/gp/f1/`（根 vitest.config.js include `tests/**`），实跑非空匹配、红态 exit≠0 — 由 B-01 覆盖（已实证改前 3 failed / 改后 6 passed，exit code 如实反映）
