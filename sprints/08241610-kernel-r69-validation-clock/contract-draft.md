# Sprint Contract Draft (Round 2) — kernel validation clock 按 fix 轮有界顺延 [r69]
...
## 本轮修订说明（Round 2 — 封印闸 closure）

Round 1 合同 7 维 rubric 全 ≥7 被 Reviewer APPROVED，但封印机械闸 `collectApprovedContractArtifacts`（`packages/brain/src/orchestrator/contract-artifacts.js:146-151`）拒绝，代码 `FROZEN_CONTRACT_ARTIFACTS_MISSING:tests`：该闸只在 `${sprintDir}/tests/` 前缀下收集冻结测试，Round 1 把冻结测试落在 `tests/gp/f1/`（sprint 目录之外）→ `testPaths.length===0` → 拒绝封印。

**唯一修订**：把冻结测试从 `tests/gp/f1/step3-validation-clock-fix-extend.test.js` 迁到
`sprints/08241610-kernel-r69-validation-clock/tests/step3-validation-clock-fix-extend.test.js`（内容逐字不变，import 深度一致仍为 `../../../packages/brain/...`）。该位置：
- 满足封印闸 `requireTests`（落在 `${sprintDir}/tests/` 下）；
- 仍在根 `vitest.config.js` include（`sprints/**` 明列，见 INV-2），CI 实跑不受影响；
- 遵 PRD 要求 7「只写合同 claim 路径、严禁合同外副本」：**单一冻结测试**，不在 `tests/gp/f1/` 留副本（避免 r68 类合同外/重复文件）。此为对 PRD 要求 5「测试放 tests/gp/f1/」的必要偏离——该路径正是 r1 封印死因，封印闸位置铁律优先，已在 `case_file` closure 登记。

Golden Path / DoD / 八要素 / RED 逻辑与 Round 1 完全一致，仅测试路径 + 白名单随迁。
...
**来源**: `[FROM_PRD]` — sprint-prd.md「触发/背景」；loop.js:1552 调用 `resolveValidationClock({ action, decisionLog, intentAt, timeoutSeconds })`。
...
**来源**: `[FROM_PRD]` — sprint-prd.md「要求」；边界「0 fix 轮」。
...
**来源**: `[FROM_PRD]` — sprint-prd.md 要求 1「每次成功的 generator-fix 顺延」；`[AI_ADDED]` 顺延原点取「decision-log 中出现的 fix 行的持久化时间」（理由：纯函数不感知运行时派发副作用，只以 decision-log 行时序为准，保证可重放 — PRD 要求 3）。
...
**来源**: `[FROM_PRD]` — sprint-prd.md 要求 3「纯函数可重放」、要求 4「verified_existing_pr 语义不变」；INV-1 铁律。
...
| generator-fix 是否「成功派发」（应否顺延原点） | A. 感知运行时派发副作用; B. 以 decision-log 出现该 `spawn:generator-fix` 行为准 | B | 纯函数不感知运行时副作用，只以 decision-log 行时序为准，保证可重放（PRD 要求 3） | 若 fix 行进 log 但派发实际失败，可能多顺延一轮；有界 6 次封顶，且 loop.js 仅在派发成功路径 append fix 行，风险可控（非静默丢数据） |
...
- **INV-2 [vitest include 范围]**：测试须落 vitest include 范围内、实跑确认 exit code 语义 → 覆盖：冻结测试落 `sprints/08241610-kernel-r69-validation-clock/tests/`（根 vitest.config.js include 明列 `sprints/**`），B-01 `npx vitest run` 真匹配非空、红态 exit≠0（已实证：改前 3 failed，改后预期 6 passed）。
...
> 本 sprint 无 DB（postgres=false）、无 HTTP 端点、无浏览器；E2E = 从仓库根真 import 真模块跑 vitest（`sprints/**` 在根 vitest.config.js include 内），验 RED→GREEN + 既有回归不退 + 合同外路径零写入。

```bash
#!/bin/bash
set -euo pipefail
cd "${WORKSPACE_PATH:-/workspace}"
FROZEN="sprints/08241610-kernel-r69-validation-clock/tests/step3-validation-clock-fix-extend.test.js"

# 1. RED 已在 proposer 阶段实证（旧模块跑冻结测试 3 failed / 3 passed）；此处以最终交付态为准跑 GREEN。
...
ALLOWED='^(packages/brain/src/orchestrator/validation-clock\.js|sprints/08241610-kernel-r69-validation-clock/)'
OFF=$(echo "$CHANGED" | grep -vE "$ALLOWED" || true)
[ -z "$OFF" ] || { echo "FAIL: 合同外路径写入: $OFF"; exit 1; }

echo "OK validation clock 有界顺延 E2E 验收通过（RED->GREEN + 回归不退 + 零越权）"
```
...
|---|---|---|---|
| fix 轮有界顺延（r69） | `sprints/08241610-kernel-r69-validation-clock/tests/step3-validation-clock-fix-extend.test.js` | r50 复刻：3 个 generator-fix 后 deadline 前移到第 3 个 fix / 恰好 6 个 fix 轮：deadline 锚在第 6 个 fix / 7+ fix 轮超界 / 0 个 fix 轮时语义与今日完全一致 / 纯函数可重放 / verified_existing_pr | 旧模块跑：3 failed \| 3 passed（新行为用例真红——已实证） |

> Test File 为完整真实路径，落在 sprint 目录 `tests/` 下（封印闸 `requireTests` 要求）；冻结测试真 import `packages/brain/src/orchestrator/validation-clock.js`（禁 mock 被改的边）。既有 repo 测试 `packages/brain/src/orchestrator/__tests__/validation-clock.test.js` 作为**补充回归行**（INV-1，11/11 不退），非冻结测试主行。
