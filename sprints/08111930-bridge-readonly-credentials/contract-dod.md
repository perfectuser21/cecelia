...
- [ ] [ARTIFACT] 新增只读消费 helper 模块，导出 provision/cleanup/resolve 三函数
...
- [ ] [ARTIFACT] cecelia-bridge.cjs /llm-call 接入 helper，不再把 CLAUDE_CONFIG_DIR 直接指向权威账号目录
...
- [ ] [ARTIFACT] cecelia-bridge.js（同源第二实现）同步同一只读消费模式
...
- [ ] [ARTIFACT] INV-2 顺序不可颠倒：本 PR 改动文件集不含 infrastructure / refresh-claude-tokens / 让位逻辑
...
- [ ] [BEHAVIOR] [L2] INV-1 B-01: 经 bridge 派发一次 attempt 后权威文件 mtime+sha256 完全不变 [接缝×2]
...
- [ ] [BEHAVIOR] [L2] B-02: CLAUDE_CONFIG_DIR 指向临时副本（非权威目录）且 attempt 结束后临时目录被清理 [接缝×2]
...
- [ ] [BEHAVIOR] [L2] B-03: 临时目录创建失败时主流程抛错告警，不回退到权威目录
...
- [ ] [BEHAVIOR] [L2] B-04: 临时副本清理失败时仅记日志返回 false，不抛错不影响结果
...
- [ ] [BEHAVIOR] [L2] B-05: 并发两 bridge attempt 各用互不相同临时目录，权威文件仍未被写 [接缝×2]
...
- [ ] [BEHAVIOR] [L2] INV-1 B-06: 零回归——容器路径（docker-executor 挂载策略）与 Codex 信封/broker 既有单测全绿、行为不变
