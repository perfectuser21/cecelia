...
**范围**: 只新增 `docs/current/attempt-run-bridge-usage.md`；只逐路径排除明确冻结产物，绝不排除整个 sprint 目录，任何其他文件均失败。
**大小**: S
...
- [ ] [ARTIFACT] 产品 diff 相对权威基线仅含目标文档
  Test: bash -c 'S=sprints/coding-harness-20260831220855-sh8mp5; git diff --name-only 0f52356135922cf5031406dae629211837c3de92...HEAD | grep -Fvx -e "$S/contract-draft.md" -e "$S/contract-dod.md" -e "$S/tests/attempt-run-bridge-usage.test.ts" -e "$S/task-plan.json" -e "$S/.brain-result.json" -e ".brain-result.json" | diff -u <(printf "%s\\n" docs/current/attempt-run-bridge-usage.md) -'
...
- [ ] [BEHAVIOR] [L1] B-02: 读者获得恰当的九项角色白名单
  ...
  预期观察: 角色章节列表严格等于九项权威角色；重复、遗漏或任意额外项均失败。
  等待预算: 0s
  留证: 命令标准输出与白名单断言结果
  Test: manual:bash -c 'node -e "const s=require('"'"'fs'"'"').readFileSync('"'"'docs/current/attempt-run-bridge-usage.md'"'"','"'"'utf8'"'"');const e=['"'"'canary'"'"','"'"'planner'"'"','"'"'proposer'"'"','"'"'reviewer'"'"','"'"'generator'"'"','"'"'generator-fix'"'"','"'"'evaluator'"'"','"'"'evaluator-evidence-repair'"'"','"'"'judge'"'"'];const m=s.match(/^## 角色白名单\\s*$([\\s\\S]*?)(?=^## |\\s*$)/m);if(!m)process.exit(1);const a=[...m[1].matchAll(/^- `([^`]+)`\\s*$/gm)].map(x=>x[1]);if(JSON.stringify(a)!==JSON.stringify(e))process.exit(1)"'
...
- [ ] [BEHAVIOR] [L1] B-05: 候选产品 diff 严格保持单文档范围
  动作: 相对冻结实现基线列出候选变更，只逐路径排除明确冻结产物，并向同一过滤器输入任意 sprint 额外文件。
  预期观察: 真实 diff 仅剩目标文档；arbitrary-extra.txt 不被过滤并确定性导致空期望 diff 失败。
  等待预算: 0s
  留证: `/tmp/attempt-run-product-files` 与 diff 输出
  Test: manual:bash -c 'S=sprints/coding-harness-20260831220855-sh8mp5; F(){ grep -Fvx -e "$S/contract-draft.md" -e "$S/contract-dod.md" -e "$S/tests/attempt-run-bridge-usage.test.ts" -e "$S/task-plan.json" -e "$S/.brain-result.json" -e ".brain-result.json"; }; git diff --name-only 0f52356135922cf5031406dae629211837c3de92...HEAD | F > /tmp/attempt-run-product-files; printf "%s\\n" docs/current/attempt-run-bridge-usage.md | diff -u - /tmp/attempt-run-product-files; if printf "%s\\n" "$S/contract-draft.md" "$S/arbitrary-extra.txt" | F | diff -u /dev/null -; then exit 1; fi'
