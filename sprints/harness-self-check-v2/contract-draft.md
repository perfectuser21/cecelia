...
> **第四轮改动说明**: 根据 Round 3 Reviewer 证伪反馈的 4 条必须修改项 + 2 条可选改进，全面升级：
> (1) bash 块验证从"长度>=30字符"升级为**CI白名单工具调用检查**（必须含 `node -e`/`curl`/`bash`/`psql`/`npm` 之一）——修复三元组 1/7/9
> (2) DoD Test: 行从">=10字符"升级为**白名单工具前缀正则**——修复三元组 2
> (3) 三元组"命令："字段从">=10字符长度"升级为**与草案命令指纹模糊匹配**（前40字符子串匹配）——修复三元组 3/8
> (4) 结构性断言检查从"函数名存在"升级为**函数名+失败路径共存**（`.match(`/`.split(` 必须伴随 `throw`/`process.exit`）——修复三元组 5
> (5) Feature 描述文本阈值从 50 字符提升到 100 字符——可选改进
> (6) 去重 Feature 4 验证 2，改为验证最终合同中三元组 NO 的证伪分析质量——可选改进
...
blocks.forEach(block => {
  ...
  if (cmdMatch && hasLazy && hasBypass) {
    ...
    // 指纹匹配：三元组命令文本的前 20 字符须是某条草案命令指纹的子串
    const cmdFP = cmdText.replace(/\x60/g, '').slice(0, 20);
    const matched = fingerprints.some(fp => fp.includes(cmdFP) || cmdFP.includes(fp.slice(0, 20)));
...
if (fingerprintMismatches > validTriples * 0.3) throw new Error('FAIL: ' + fingerprintMismatches + '/' + validTriples + '个三元组命令与草案命令无指纹匹配，疑为编造');
const yesCount = (feedback.match(/能否绕过[：:]\s*YES/g) || []).length;
if (yesCount < 1) throw new Error('FAIL: 无YES三元组，证伪机制未触发');
console.log('PASS: ' + validTriples + '个有效三元组（>=' + minTriples + '），指纹不匹配=' + fingerprintMismatches + '，YES=' + yesCount);
"
```
...
**行为描述**:
GAN 结束后，可从产物文件中完整还原对抗过程。`sprint-contract.md` 是最终 APPROVED 合同，包含结构化内容（Feature 标题 + 验证命令块），每个 bash 命令块含 CI 白名单工具调用；`contract-review-feedback.md` 包含完整三元组；最终合同中不应包含"能否绕过: YES"。"能否绕过: NO"记录必须出现在结构化三元组块内。最终合同内三元组 NO 块必须同时含有"最懒假实现"字段（证明确实做了证伪构造，而非空壳 NO）。
...
- `sprint-contract.md` 中结构化三元组块内"能否绕过: NO"出现 >= 1 次（块内同时含"命令："+"最懒假实现"+"能否绕过：NO"）
- `contract-review-feedback.md` 中三元组命令字段与草案命令有指纹关联

**验证命令**:
```bash
# 验证 1：最终合同——Feature 结构 + bash 块白名单工具 + YES=0 + 结构化三元组 NO（含证伪构造）
node -e "
...
if (structuredNO < 1) throw new Error('FAIL: 无完整三元组内的NO记录（需同时含命令：+最懒假实现+能否绕过：NO）');
console.log('PASS: ' + featureBlocks.length + '个Feature（均含白名单工具），' + cmds + '条命令，YES=0，完整三元组NO=' + structuredNO);
"
```
...
**范围**: Reviewer 对草案执行证伪分析 -> 输出三元组反馈（命令字段与草案指纹匹配）-> Proposer 修订 -> Reviewer 再审 -> 最终 APPROVED 合同（断言+失败路径共存）
**大小**: M（验证涉及多个产物文件和多轮对抗记录）
**依赖**: Workstream 1 完成后

**DoD**:
- [x] [BEHAVIOR] `contract-review-feedback.md` 三元组覆盖率 >= 草案命令数 * 60%，命令字段与草案指纹匹配，至少 1 个 YES，判决格式为 `**判决**: X`
  Test: node -e "const fs=require('fs');const d=fs.readFileSync('sprints/harness-self-check-v2/contract-draft.md','utf8');const br=/\x60\x60\x60bash\n([\s\S]*?)\x60\x60\x60/g;const fps=[];let bm;while((bm=br.exec(d))!==null){const ct=bm[1].replace(/^#.*\n/gm,'').trim();if(ct.length>0)fps.push(ct.slice(0,40))};const min=Math.ceil(fps.length*0.6);const fb=fs.readFileSync('sprints/harness-self-check-v2/contract-review-feedback.md','utf8');const bks=fb.split('---');let t=0,mm=0;bks.forEach(b=>{const cm=b.match(/命令[：:]\s*(.+)/);if(cm&&b.includes('最懒假实现')&&/能否绕过[：:]\s*(YES|NO)/.test(b)){t++;const cf=cm[1].trim().replace(/\x60/g,'').slice(0,20);if(!fps.some(fp=>fp.includes(cf)||cf.includes(fp.slice(0,20))))mm++}});if(t<min)throw new Error('FAIL:三元组='+t+'<'+min);if(mm>t*0.3)throw new Error('FAIL:指纹不匹配='+mm+'/'+t);const y=(fb.match(/能否绕过[：:]\s*YES/g)||[]).length;if(y<1)throw new Error('FAIL:无YES');const hdr=fb.split('\n').slice(0,30).join('\n');if(!/\*\*判决\*\*[：:]\s*(REVISION|APPROVED)/.test(hdr))throw new Error('FAIL:判决格式错误');console.log('PASS:三元组='+t+'(>='+min+'),不匹配='+mm+',YES='+y)"
