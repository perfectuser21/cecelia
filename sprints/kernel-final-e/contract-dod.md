...
- [ ] [BEHAVIOR] [L2] B-01: GET /kernel-e 返回 200 + {result:"ok-e"}
...
- [ ] [BEHAVIOR] [L2] B-02: HTTP 状态码恰为 200（404=路由未注册=FAIL，禁 404-acceptable）
...
- [ ] [BEHAVIOR] [L2] B-03: 顶层 keys 完整性恰为 ["result"] 且禁用字段全不存在
...
- [ ] [BEHAVIOR] [L2] B-04: 边界——携带任意多余 query 参数仍稳定返回 200 + {result:"ok-e"}
...
- [ ] [BEHAVIOR] [L2] B-05: 非 GET 方法 → 404（沿用 /ping 约定，且现有端点 /ping 未回归）
...
- [ ] [BEHAVIOR] [L2] INV-1: 合同回归测试在 vitest include 范围内（playground/tests/）真跑转绿 exit 0
