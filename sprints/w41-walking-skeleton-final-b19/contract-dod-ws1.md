...
- [x] [ARTIFACT] seed 脚本存在且 node --check 通过
  Test: manual:bash -c 'node --check packages/brain/scripts/seed-w41-demo-task.js'

- [x] [ARTIFACT] drive 脚本存在且 node --check 通过
  Test: manual:bash -c 'node --check packages/brain/scripts/drive-w41-e2e.js'
...
- [x] [ARTIFACT] seed-output.json 含合法 demo_task_id (UUID v4) + injected_at (ISO 8601)
  Test: manual:bash -c 'node -e "const d=JSON.parse(require(\"fs\").readFileSync(\"sprints/w41-walking-skeleton-final-b19/evidence/seed-output.json\")); if(!/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/.test(d.demo_task_id)) throw new Error(\"bad uuid\"); if(!/^[0-9]{4}-[0-9]{2}-[0-9]{2}T/.test(d.injected_at)) throw new Error(\"bad iso\"); console.log(\"ok\")"'
