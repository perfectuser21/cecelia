...
**范围**: `packages/brain/src/orchestrator/required-context-contract.js` 新建 target-aware required-context 合同；`packages/brain/src/orchestrator/ground-truth.js` / `derive.js` 接入当前 head SHA 与 target-aware gate；`packages/brain/src/routes/preview.js` 或其 helper 保留 preview 启动失败证据；补齐真实 PG contract tests。
**大小**: L
...
- [ ] [ARTIFACT] `packages/brain/src/orchestrator/ground-truth.js` 含 target-aware required-context 接入点，读取服务端 `target_environment` 与当前 `headRefOid`
...
- [ ] [ARTIFACT] `sprints/07272219-kernel-e6ba6d09/tests/kernel-target-aware-required-context.pg.contract.test.ts` 存在且含 10 个以上 `it(` 契约用例
  Test: node -e "const fs=require('fs');const p='sprints/07272219-kernel-e6ba6d09/tests/kernel-target-aware-required-context.pg.contract.test.ts';const c=fs.readFileSync(p,'utf8');const n=(c.match(/\bit\(/g)||[]).length;if(n<10)process.exit(1);console.log('OK')"

## Invariant 覆盖条目

- [ ] [BEHAVIOR] [L2] INV-1 环境可信 - 服务端 `target_environment` 覆盖客户端 required contexts
  动作: 运行 real PG 契约测试并向任务 payload 注入客户端伪造 `required_contexts`
...
- [ ] [BEHAVIOR] [L2] INV-2 真环境验证 - GitHub checks 必须锚定当前 head SHA
  动作: 运行 stale SHA 契约测试
  预期观察: 非当前 SHA 的检查结果被拒绝
  验证命令: Test: manual:bash -c 'NODE_ENV=test npx vitest run sprints/07272219-kernel-e6ba6d09/tests/kernel-target-aware-required-context.pg.contract.test.ts -t "stale check SHA 必须拒绝当前 gate"'

- [ ] [BEHAVIOR] [L2] INV-3 租户隔离 N/A：本 sprint 不新增租户面查询或跨租户写路径
  动作: N/A
  预期观察: N/A
  验证命令: Test: manual:bash -c 'node -e "console.log(\"N/A: no tenant-scoped tables touched\")"'

- [ ] [BEHAVIOR] [L2] INV-4 凭据安全 - secrets 不硬编码、不进 git、不进日志
...
- [ ] [BEHAVIOR] [L2] preview 目标 preview failure 时必须硬阻断
  动作: 运行 preview-required gate 契约测试并注入 preview failure
  预期观察: `decision=block` 且 `reason=preview_failed`
  验证命令: Test: manual:bash -c 'NODE_ENV=test npx vitest run sprints/07272219-kernel-e6ba6d09/tests/kernel-target-aware-required-context.pg.contract.test.ts -t "preview 目标 preview failure 时必须硬阻断"'

- [ ] [BEHAVIOR] [L2] stale check SHA 必须拒绝当前 gate
  动作: 运行 stale SHA 契约测试
  预期观察: 旧 SHA checks 被拒绝并返回 `stale_check_sha`
  验证命令: Test: manual:bash -c 'NODE_ENV=test npx vitest run sprints/07272219-kernel-e6ba6d09/tests/kernel-target-aware-required-context.pg.contract.test.ts -t "stale check SHA 必须拒绝当前 gate"'

- [ ] [BEHAVIOR] [L2] wrong repo isolation 必须阻断
  动作: 运行 repo mismatch 契约测试
  预期观察: 错 repo checks 被拒绝并返回 `repo_mismatch`
  验证命令: Test: manual:bash -c 'NODE_ENV=test npx vitest run sprints/07272219-kernel-e6ba6d09/tests/kernel-target-aware-required-context.pg.contract.test.ts -t "wrong repo isolation 必须阻断"'

- [ ] [BEHAVIOR] [L2] wrong run isolation 必须阻断
  动作: 运行 run mismatch 契约测试
  预期观察: 错 run checks 被拒绝并返回 `run_mismatch`
  验证命令: Test: manual:bash -c 'NODE_ENV=test npx vitest run sprints/07272219-kernel-e6ba6d09/tests/kernel-target-aware-required-context.pg.contract.test.ts -t "wrong run isolation 必须阻断"'

- [ ] [BEHAVIOR] [L2] 缺失 required context 必须阻断并返回审计原因
  动作: 运行 missing required context 契约测试并构造空 checks
  预期观察: `decision=block` 且 `reason=missing_required_context`
...
- [ ] [BEHAVIOR] [L2] preview 启动失败保留 status body error evidence
  动作: 运行 preview failure evidence 契约测试并模拟 curl exit 22 + HTTP 503
  预期观察: 失败对象同时保留 `http_status`、`response_body`、`error`
...
- [ ] [BEHAVIOR] [L2] legacy rollout 不得覆盖服务端 required context contract
  动作: 运行 legacy rollout 兼容契约测试，注入旧字段与客户端 required list
  预期观察: 旧字段只记为 `legacy_inputs_observed=true`，不会改变服务端 required contexts
...
- [ ] [BEHAVIOR] [L2] generator fix 仅在真正 required failure 才触发
  动作: 比较 `local_api + preview fail` 与 `preview_env + preview fail` 两条路径
  预期观察: 前者 `generator_fix_required=false`，后者 `generator_fix_required=true`
...
- [ ] [BEHAVIOR] [L2] post merge staging production hard gate 与 review_required 单 SHA 审批
  ...
  预期观察: staging/production 任一 required failure、旧 SHA 或 review 未满足时 `merge_allowed=false`
