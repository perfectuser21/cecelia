# Sprint Contract Draft (Round 2) — 判定器金标集 v0 + eval 通过率棘轮进 CI

> **Round 2 修订说明**：Round 1 合同 7 维 rubric 全部 ≥7 已被 Reviewer APPROVED，但被封印机械闸 `assertTestContractResolvable` 以 `FROZEN_CONTRACT_TEST_CONTRACT_UNRESOLVABLE` 拒绝——## Test Contract 表第二行所列 `tests/gp/f1/step5-golden-eval-ratchet.test.js` 在 propose 分支上不存在（Round 1 只 commit 了 `sprints/.../tests/golden-eval-gate.test.ts`）。Round 2 **真实创建并 commit** 该测试文件（含可执行断言，实测 RED），使表中两条路径均可被封印闸同款解析链解析。合同其余内容维持 APPROVED 版本不变（净变化趋近 0，只补真实封印缺口，不加 PRD 之外内容）。
...
> 本 sprint 无 DB（runtime_resources.postgres=false）、无 HTTP server：交付物是纯 node eval 基建。local_api 模板退化为「node 测试 runner + eval 命令」（与 PRD E2E 占位一致）。sprints/** 与 tests/** 测试从仓库根 `npx vitest run` 由根 vitest include 覆盖，合法。
...
npx vitest run tests/gp/f1/step5-golden-eval-ratchet.test.js --reporter=basic 2>&1 | tee -a /tmp/golden-eval-e2e.log
...
| 永久 CI 回归（与既有 step3-judge-* 同域，随 root vitest include `tests/**` 常驻）| `tests/gp/f1/step5-golden-eval-ratchet.test.js` | 通过率可算出且 ≥ 入库阈值 / 棘轮降阈提交被拦截 / 缓存命中二次判定视觉调用计数为 0 / 视觉返回 null 必 fail-closed 判 FAIL / 契约完备 lint 缺段 FAIL / 判定步骤序列固化不漂移 / 金标集为空 | import `./eval/harness-visual-eval.mjs` 失败 → 1 failed（Failed to load url，实测已确认 RED）|

> **Test File 路径死规则合规（v9.26/v9.27）**：两条路径均为本 propose 分支**已 commit 的真实测试文件**（含可执行断言，实测 RED）—— 第一行 `sprints/09052347-kernel-a54539fd/tests/golden-eval-gate.test.ts` 是本 sprint 冻结合同测试；第二行 `tests/gp/f1/step5-golden-eval-ratchet.test.js` 是 Round 2 新建的永久 CI 回归（与既有 `step3-judge-*.test.js` 同域，满足 PRD「4 条纯代码用例落 tests/gp/f1/」+ bug-fix 死规矩「failing test 永久保留在 CI 作回归」）。二者均可被封印闸 `assertTestContractResolvable` 同款解析链解析（`git show <sha>:<path>` 命中），根除 Round 1 `FROZEN_CONTRACT_TEST_CONTRACT_UNRESOLVABLE`。
> BEHAVIOR 覆盖名均为对应 `it()` 名的字面子串（冻结测试 it 名如 `B-01 eval 金标集 v0 通过率可算出且 ≥ 入库阈值` ⊇ 覆盖名 `通过率可算出且 ≥ 入库阈值`；回归测试 it 名如 `缓存命中二次判定视觉调用计数为 0（防成本回归）` ⊇ 覆盖名 `缓存命中二次判定视觉调用计数为 0`）。
