...
- [x] [ARTIFACT] `routes/harness.js` 含 GET `/messages/:initiativeId/:subTaskId` 路由
...
- [x] [ARTIFACT] `routes/harness.js` 含 POST `/messages/:initiativeId/:subTaskId` 路由（返回 201）
...
- [x] [ARTIFACT] `harness-thread-lookup.js` 含 status 更新函数（UPDATE walking_skeleton_thread_lookup）
...
- [x] [BEHAVIOR] GET /messages 端点返回 200 + `messages` 数组字段（PRD: `messages` 必填 array；端点未注册时 curl -sf → nonzero → 真红）
...
- [x] [BEHAVIOR] GET 顶层 keys 严格等于 `["messages"]`（PRD 禁用字段 data/items/results/payload/list 不存在）
...
- [x] [BEHAVIOR] POST /messages 返回 201 + schema 字段 id/message/created_at + keys 完整性校验（PRD: keys == ["created_at","id","message"]）
...
- [x] [BEHAVIOR] POST 响应 keys 不含禁用字段（PRD 禁用: data/result/payload/body）
...
- [x] [BEHAVIOR] GET 不存在的 initiativeId 返回 `{"messages":[]}` 而非 404（PRD 边界：「查询到不存在的 initiativeId → 返 {"messages":[]}」）
...
- [x] [BEHAVIOR] GET 消息对象四字段完整性（PRD: id/message/created_at/consumed_at 必填 — 先 POST 创建消息再 GET 验证字段）
...
- [x] [BEHAVIOR] consumed 查询参数实现 + 禁用别名代码检查（PRD: 参数名 consumed，禁用 include_consumed/all/show_consumed）
...
- [x] [BEHAVIOR] thread_lookup status 生命周期更新代码存在（completed/failed，实现前 harness-thread-lookup.js 不含 UPDATE 语句 → 真红）
...
- [x] [BEHAVIOR] error path — POST body 无 `message` 字段时返回 4xx（输入校验）
