...
- [ ] [ARTIFACT] `harness-initiative.graph.js` 文件顶部 imports 包含 `spawnDockerDetached`
...
- [ ] [ARTIFACT] `runPlannerNode` 函数体内不再含 `reconnectOrSpawn` 调用（已移除阻塞）
...
- [ ] [ARTIFACT] `runPlannerNode` 函数体内含 `interrupt()` 调用
  Test: node -e "const c=require('fs').readFileSync('packages/brain/src/workflows/harness-initiative.graph.js','utf8'); const start=c.indexOf('export async function runPlannerNode'); const end=c.indexOf('\nexport ',start+1); const fn=c.slice(start,end>0?end:start+4000); if(!fn.includes('interrupt'))process.exit(1)"

## BEHAVIOR 条目

- [ ] [BEHAVIOR] harness-initiative.graph.js 导入并使用 spawnDockerDetached（WS2 实现前文件中不含此符号，FAIL → 真红）
...
- [ ] [BEHAVIOR] runPlannerNode 函数体移除阻塞调用（函数体不含 reconnectOrSpawn）
...
- [ ] [BEHAVIOR] runPlannerNode 函数体含 interrupt() 调用（挂起 graph，非阻塞返回）
...
- [ ] [BEHAVIOR] harness-initiative.graph.js 含 thread_lookup 写入逻辑（Planner 容器 ID 映射，实现前文件无此逻辑）
...
- [ ] [BEHAVIOR] error path — runPlannerNode 在 spawnDockerDetached 失败时返回 error 对象（不崩溃 graph）
