...
- [ ] [ARTIFACT] `harness-gan.graph.js` 文件顶部 imports 包含 `spawnDockerDetached`
...
- [ ] [ARTIFACT] `proposer` 函数体内不含 `reconnectOrSpawn`（已改为 detached）
...
- [ ] [ARTIFACT] `reviewer` 函数体内不含 `reconnectOrSpawn`（已改为 detached）
...
- [ ] [ARTIFACT] `detectConvergenceTrend` 函数仍然存在（收敛逻辑未被破坏）
...
- [ ] [BEHAVIOR] harness-gan.graph.js 导入并使用 spawnDockerDetached（实现前文件无此符号 → 真红）
...
- [ ] [BEHAVIOR] proposer 函数移除阻塞 reconnectOrSpawn（函数体内不含该符号）
...
- [ ] [BEHAVIOR] reviewer 函数移除阻塞 reconnectOrSpawn（函数体内不含该符号）
...
- [ ] [BEHAVIOR] detectConvergenceTrend 收敛逻辑仍存在（GAN 异步化不破坏收敛判断）
...
- [ ] [BEHAVIOR] error path — proposer/reviewer 含 interrupt() 调用（真正挂起，不只是空函数）
