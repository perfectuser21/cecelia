...
> 说明：唯一一行「BEHAVIOR 覆盖」的三个覆盖名均逐词取自冻结主测试 `runner-failure-role-window.test.js` 真实 `it()` 名的字面子串（`跨角色 runner_failure 不再互耗额度：evaluator 2 败后 publisher 首败仍可重派` / `缺 role 字段的历史 runner_failure 行不计入当前角色窗口` / `同角色 runner_failure 3 连败第 3 次仍进人审（窗口化不放宽负向语义）`），多值以 `/` 分隔。冻结主测试落 `sprints/08222322-kernel-69092ff0/tests/`，随本轮 commit 冻结进 HEAD 树。
>
> **为何本表不登记 repo 路径行**（Round 1 seal 拒绝 `FROZEN_CONTRACT_TEST_CONTRACT_UNRESOLVABLE:packages/brain/src/orchestrator/__tests__/derive.test.js` 的确定性根因）：封印闸 `contract-test-paths-seal.js`（line 76-98）对非 `sprints/` 前缀的 repo 路径行走 `readRepoFile(testFile)` = `git show <approvedSha>:<repo路径>`（loop.js:995）。Brain 封印环境是 partial clone（`blob:none`）——本轮新增的 `sprints/.../tests/*` blob 随 `git fetch <sha>` 落地可解析，但 `packages/brain/.../derive.test.js` 这类**未变更的旧 blob 被 filter 过滤、按需拉取失败**，`readRepoFile` 抛错 → 登记行判 UNRESOLVABLE（无 `#behavior:` 后缀 = 读文件即抛，非 BEHAVIOR 不匹配）。这是封印环境固有约束，proposer 无权改共享封印闸/git 拓扑。依 skill 9.27.0「repo 既有测试只能作为**补充行**」（可选，非必需），本表只登记必需的冻结主测试行；既有 `runner failure retries bounded` 有界重派回归改由 **contract-dod.md 的 DoD BEHAVIOR B-04**（非封印闸约束、evaluator 直接 `cd packages/brain && npx vitest run ... -t "runner failure retries bounded"`）承接，覆盖不丢；冻结主测试自身的「同角色 3 连败仍 exhausted」用例亦对同一负向语义构成独立 RED/GREEN 断言。
