...
- 文档使用简体中文（正文至少 100 个汉字且不得出现测试列出的繁体用词）；端点、字段、角色和状态等技术字面量保持源码拼写。
...
| NFR（做得多好） | 四个主题分别使用且只使用一次独立二级标题；正文通过简体中文机检；关键字可由自动测试逐字解析。 |
...
**可观测行为**：文档在独立的 `## 角色白名单` 章节中按此顺序、每行一个 Markdown 列表项逐字列出 `canary`、`planner`、`proposer`、`reviewer`、`generator`、`generator-fix`、`evaluator`、`evaluator-evidence-repair`、`judge`，不得多出第十项，并明确白名单之外角色会被拒绝。

**验证命令**：`node sprints/coding-harness-20260831174800-fxqhpa/tests/attempt-run-bridge-doc.test-helper.cjs roles`

**硬阈值**：从该章节解析出的列表必须与上述九项按顺序精确相等（封闭集合，无缺项、重复或额外项）；命令 exit 0。
...
|---|---|---|---|
| attempt-run 桥接中文说明 | `sprints/coding-harness-20260831174800-fxqhpa/tests/attempt-run-bridge-doc.test.ts` | `正文通过简体中文机检`；`四个主题各自具有独立章节`；`说明两个端点用途与鉴权方式`；`九项角色白名单是精确封闭集合`；`说明 payload 必填字段与 base_sha 省略语义`；`说明派发失败自动回滚状态` | 目标文档尚不存在，6 个用例均失败 |
