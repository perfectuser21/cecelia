...
- [ ] [BEHAVIOR] [L1] B-00：说明正文是简体中文且四个主题为独立章节
  动作：打开目标文档并解析二级标题与正文字符
  预期观察：正文至少包含 100 个汉字且无测试列出的繁体用词；四个指定二级标题各出现一次且各自正文非空
  等待预算：0s
  留证：helper 的 chinese 与 sections 模式标准输出
  Test: manual:bash -c 'node sprints/coding-harness-20260831174800-fxqhpa/tests/attempt-run-bridge-doc.test-helper.cjs chinese && node sprints/coding-harness-20260831174800-fxqhpa/tests/attempt-run-bridge-doc.test-helper.cjs sections'
...
- [ ] [BEHAVIOR] [L2] B-02：读者可从文档获得完整九项角色白名单
  ...
  预期观察：从该独立章节解析出的 Markdown 列表与源码九项角色按顺序精确相等，无缺项、重复或额外项，且文档明确名单外角色会被拒绝
  等待预算：0s
  留证：helper 命令标准输出
