...
- [ ] [ARTIFACT] `playground/server.js` /decrement 段含 strict-schema 整数正则 `^-?\d+$`
  Test: node -e "const c=require('fs').readFileSync('playground/server.js','utf8');if(!c.includes('^-?\\\\d+$'))process.exit(1)"
...
- [ ] [ARTIFACT] `playground/server.js` /decrement 算术表达式字面含减号（而非 `+ 1`）
  Test: node -e "const c=require('fs').readFileSync('playground/server.js','utf8');const m=c.match(/app\\.get\\('\\/decrement'[\\s\\S]*?(?:app\\.get\\(|app\\.listen\\(|export default)/);const body=m?m[0]:'';if(!/-\\s*1/.test(body))process.exit(1);if(/result:\\s*[^,}]*\\+\\s*1/.test(body))process.exit(1)"
...
- [ ] [BEHAVIOR] `GET /decrement?value=5` 返 `{result:4, operation:"decrement"}` 严 schema（值复算 + operation 字面字符串严格相等）
  Test: manual:bash -c 'cd playground && PLAYGROUND_PORT=3201 node server.js & SPID=$!; sleep 2; RESP=$(curl -fs "localhost:3201/decrement?value=5"); echo "$RESP" | jq -e ".result == 4 and .operation == \"decrement\"" > /dev/null; RC=$?; kill $SPID 2>/dev/null; exit $RC'
  期望: exit 0
...
- [ ] [BEHAVIOR] off-by-one 防盲抄 W26 increment：`value=0 → result=-1`、`value=1 → result=0`、`value=-1 → result=-2`（generator 抄 W26 `+1` 即 FAIL）
  Test: manual:bash -c 'cd playground && PLAYGROUND_PORT=3204 node server.js & SPID=$!; sleep 2; FAIL=0; curl -fs "localhost:3204/decrement?value=0" | jq -e ".result == -1" > /dev/null || FAIL=1; curl -fs "localhost:3204/decrement?value=1" | jq -e ".result == 0" > /dev/null || FAIL=1; curl -fs "localhost:3204/decrement?value=-1" | jq -e ".result == -2" > /dev/null || FAIL=1; kill $SPID 2>/dev/null; [ "$FAIL" = "0" ]'
  期望: exit 0

- [ ] [BEHAVIOR] 精度上下界 happy：`value=9007199254740990 → result=9007199254740989`、`value=-9007199254740990 → result=-9007199254740991`
  Test: manual:bash -c 'cd playground && PLAYGROUND_PORT=3205 node server.js & SPID=$!; sleep 2; FAIL=0; curl -fs "localhost:3205/decrement?value=9007199254740990" | jq -e ".result == 9007199254740989" > /dev/null || FAIL=1; curl -fs "localhost:3205/decrement?value=-9007199254740990" | jq -e ".result == -9007199254740991" > /dev/null || FAIL=1; kill $SPID 2>/dev/null; [ "$FAIL" = "0" ]'
  期望: exit 0
...
- [ ] [BEHAVIOR] strict-schema 拒：小数 / 前导 + / 双重负号 / 尾部负号 / 科学计数法 / 十六进制 / 千分位 / 空格 / 空串 / 字母 / Infinity / NaN / 仅负号 共 14 类输入全 400
...
- [ ] [BEHAVIOR] 错 query 名 + 缺参 + 多余字段：`/decrement`（无参）/ `?n=5` / `?a=5` / `?x=5` / `?value=5&extra=1` 全 400
...
- [ ] [BEHAVIOR] 前导 0 happy（十进制归一化，非八进制解析）：`value=01 → result=0`、`value=-01 → result=-2`、`value=007 → result=6`
  Test: manual:bash -c 'cd playground && PLAYGROUND_PORT=3209 node server.js & SPID=$!; sleep 2; FAIL=0; curl -fs "localhost:3209/decrement?value=01" | jq -e ".result == 0 and .operation == \"decrement\"" > /dev/null || FAIL=1; curl -fs "localhost:3209/decrement?value=-01" | jq -e ".result == -2" > /dev/null || FAIL=1; curl -fs "localhost:3209/decrement?value=007" | jq -e ".result == 6" > /dev/null || FAIL=1; kill $SPID 2>/dev/null; [ "$FAIL" = "0" ]'
  期望: exit 0
