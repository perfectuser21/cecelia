// 冻结合同测试（seal gate / runner finalizer 认这一份）—— 边：commander lease 过期有界自动重派 [r75]
//
// 背景（本主题第四次点火，r70–r74 案卷）：kernel 编排器 derive.js 的
// attemptCallbackRoute 对 infrastructure_blocked 失败按角色查 INFRA_RETRY_ACTION_BY_ROLE
// 重派，该表独缺 commander。故 commander attempt lease 过期被收割器 reconcile
...
//    （返回不阻塞的路由，主链继续；commander 是监理角色，其重派由 commanderCoordinator 在
//     下一 tick 独立负责，重派安全无副作用）。可观测契约：derive 结果 action ≠ wait:human_review
//     且 reason ≠ callback_infrastructure_route_unknown。
//  · 累计 ≥ 5（fail-closed 兜底）：仍 wait:human_review + reason=callback_infrastructure_route_unknown，
//    且决策对象带 callbackHop=Number(row.hop)（与 #5058 diagnostic 消费锚兼容，人 approve 后可消费出口）。
//
// 累计口径（ASSUMPTION）：该 run decisionLog 里 role=commander 且
// failure_class=infrastructure_blocked 的 effect:expired_attempt_reconciled 行、hop ≤ 当前行 hop 的条数
// （= 当前行在 commander infra 过期序列里的序号，保证有界判定单调）。
//
// GP 产物闸规矩：真 import 被改文件 derive.js，禁 mock 被改的边（commander infra 路由 ↔ decisionLog 行）。
import { describe, it, expect } from 'vitest';
import { derive } from '../../../packages/brain/src/orchestrator/derive.js';
// 与 tests/gp/f1/step3-commander-lease-expired-retry.test.js 是同一改动的两个 CI 闸产物：
// 本文件满足封印闸「sprints/<sprint_dir>/tests/ 至少一行冻结测试」+ finalizer HEAD 树校验；
// gp/f1 那份满足 F1 gp-anchor 闸。两者都真 import 被改文件 derive.js，禁 mock 被改的边。

const CAND_SHA = 'df13ca81519019e790d6387229f497ba78071cfc';
const IDENTITY = { contract_id: 'c75', manifest_sha256: 'm75', source_revision: 'r75' };
...
function baseObserved(overrides = {}) {
  return {
    ...
    pr: null,
    candidate: { branch: 'cp-route-api-fc96b088', head_sha: CAND_SHA },
...
describe('F1 step3 — commander lease 过期有界自动重派 [r75]', () => {
