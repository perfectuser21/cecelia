...
- [ ] [BEHAVIOR] POST /golden_path 用真实 owner_task_id + feature_id 建步返回 201 且回吐新模型字段，无旧列（Golden Path Step 1 用户可观察输出）
  Test: manual:bash -c 'set -e; DB_URL="${DB_URL:-postgresql://localhost/cecelia}"; TASK_ID=$(psql "$DB_URL" -t -c "INSERT INTO tasks (title) VALUES ('"'"'gp-dod-task-'"'"' || gen_random_uuid()) RETURNING id" | grep -Eo '[0-9a-f-]{36}' | head -1); FEATURE_ID=$(psql "$DB_URL" -t -c "INSERT INTO journey_features (name) VALUES ('"'"'gp-dod-feature-'"'"' || gen_random_uuid()) RETURNING id" | grep -Eo '[0-9a-f-]{36}' | head -1); STEP=$(curl -sf -X POST localhost:5221/api/brain/golden_path -H "Content-Type: application/json" -d "{\"owner_task_id\":\"$TASK_ID\",\"order_no\":1,\"feature_id\":\"$FEATURE_ID\"}"); echo "[Step1] POST /golden_path (owner_task_id=$TASK_ID) -> 201 body: $STEP"; echo "$STEP" | jq -e ".owner_task_id and .feature_id and (.order_no == 1) and (has(\"scope_type\") | not) and (has(\"ability_id\") | not)" || exit 1; echo OK'
  期望: OK
...
- [ ] [BEHAVIOR] POST /decisions target_type=golden_path 指向真实步 → 201 且 level=step/target_type=golden_path（Golden Path Step 2 用户可观察输出）
  Test: manual:bash -c 'set -e; DB_URL="${DB_URL:-postgresql://localhost/cecelia}"; TASK_ID=$(psql "$DB_URL" -t -c "INSERT INTO tasks (title) VALUES ('"'"'gp-dod-task2-'"'"' || gen_random_uuid()) RETURNING id" | grep -Eo '[0-9a-f-]{36}' | head -1); FEATURE_ID=$(psql "$DB_URL" -t -c "INSERT INTO journey_features (name) VALUES ('"'"'gp-dod-feat2-'"'"' || gen_random_uuid()) RETURNING id" | grep -Eo '[0-9a-f-]{36}' | head -1); STEP_ID=$(curl -sf -X POST localhost:5221/api/brain/golden_path -H "Content-Type: application/json" -d "{\"owner_task_id\":\"$TASK_ID\",\"order_no\":1,\"feature_id\":\"$FEATURE_ID\"}" | jq -r ".id"); DEC=$(curl -sf -X POST localhost:5221/api/brain/decisions -H "Content-Type: application/json" -d "{\"category\":\"nfr\",\"topic\":\"前后台\",\"decision\":\"后台静默\",\"level\":\"step\",\"target_type\":\"golden_path\",\"target_id\":\"$STEP_ID\",\"scope\":\"v1\"}"); echo "[Step2] POST /decisions (level=step, target=golden_path:$STEP_ID) -> 201 body: $DEC"; echo "$DEC" | jq -e ".level == \"step\" and .target_type == \"golden_path\"" || exit 1; echo OK'
  期望: OK
...
- [ ] [BEHAVIOR] GET /golden_path/:id/decisions?scope=v1 读回该步刚写决策 + DB 时间窗确认本轮写入（Golden Path Step 3 用户可观察输出）
  Test: manual:bash -c 'set -e; DB_URL="${DB_URL:-postgresql://localhost/cecelia}"; TASK_ID=$(psql "$DB_URL" -t -c "INSERT INTO tasks (title) VALUES ('"'"'gp-dod-task3-'"'"' || gen_random_uuid()) RETURNING id" | grep -Eo '[0-9a-f-]{36}' | head -1); FEATURE_ID=$(psql "$DB_URL" -t -c "INSERT INTO journey_features (name) VALUES ('"'"'gp-dod-feat3-'"'"' || gen_random_uuid()) RETURNING id" | grep -Eo '[0-9a-f-]{36}' | head -1); STEP_ID=$(curl -sf -X POST localhost:5221/api/brain/golden_path -H "Content-Type: application/json" -d "{\"owner_task_id\":\"$TASK_ID\",\"order_no\":1,\"feature_id\":\"$FEATURE_ID\"}" | jq -r ".id"); curl -sf -X POST localhost:5221/api/brain/decisions -H "Content-Type: application/json" -d "{\"category\":\"nfr\",\"topic\":\"前后台\",\"decision\":\"后台静默\",\"level\":\"step\",\"target_type\":\"golden_path\",\"target_id\":\"$STEP_ID\",\"scope\":\"v1\"}" > /dev/null; LIST=$(curl -sf "localhost:5221/api/brain/golden_path/$STEP_ID/decisions?scope=v1"); echo "[Step3] GET /golden_path/$STEP_ID/decisions?scope=v1 -> 200 body: $LIST"; echo "$LIST" | jq -e --arg s "$STEP_ID" "any(.[]; .target_id == \$s and .scope == \"v1\")" || exit 1; CNT=$(psql "$DB_URL" -t -c "SELECT count(*) FROM decisions WHERE target_type='"'"'golden_path'"'"' AND target_id='"'"'$STEP_ID'"'"' AND created_at > NOW() - interval '"'"'5 minutes'"'"'" | tr -d " "); [ "$CNT" -ge 1 ] || { echo "FAIL: 决策非本轮写入 CNT=$CNT"; exit 1; }; echo OK'
  期望: OK
...
- [ ] [BEHAVIOR] GET /tasks/:id/golden-path-decisions?category=nfr&scope=v1 按 owner_task_id join 出整条 golden path NFR 验收单含刚写决策（Golden Path Step 4 用户可观察输出）
  Test: manual:bash -c 'set -e; DB_URL="${DB_URL:-postgresql://localhost/cecelia}"; TASK_ID=$(psql "$DB_URL" -t -c "INSERT INTO tasks (title) VALUES ('"'"'gp-dod-task4-'"'"' || gen_random_uuid()) RETURNING id" | grep -Eo '[0-9a-f-]{36}' | head -1); FEATURE_ID=$(psql "$DB_URL" -t -c "INSERT INTO journey_features (name) VALUES ('"'"'gp-dod-feat4-'"'"' || gen_random_uuid()) RETURNING id" | grep -Eo '[0-9a-f-]{36}' | head -1); STEP_ID=$(curl -sf -X POST localhost:5221/api/brain/golden_path -H "Content-Type: application/json" -d "{\"owner_task_id\":\"$TASK_ID\",\"order_no\":1,\"feature_id\":\"$FEATURE_ID\"}" | jq -r ".id"); curl -sf -X POST localhost:5221/api/brain/decisions -H "Content-Type: application/json" -d "{\"category\":\"nfr\",\"topic\":\"前后台\",\"decision\":\"后台静默\",\"level\":\"step\",\"target_type\":\"golden_path\",\"target_id\":\"$STEP_ID\",\"scope\":\"v1\"}" > /dev/null; SHEET=$(curl -sf "localhost:5221/api/brain/tasks/$TASK_ID/golden-path-decisions?category=nfr&scope=v1"); echo "[Step4] GET /tasks/$TASK_ID/golden-path-decisions?category=nfr&scope=v1 -> 200 body: $SHEET"; echo "$SHEET" | jq -e --arg s "$STEP_ID" "any(.[]; .target_id == \$s and .category == \"nfr\" and .scope == \"v1\")" || exit 1; echo OK'
  期望: OK
