...
| 功能 | BEHAVIOR 覆盖 | `testFile` | 预期红证据 |
|---|---|---|---|
| 新 run 以 materializing 写入 | `writes the new run with materializing status` | `sprints/08231107-kernel-ccd99d19/tests/projection-two-phase.test.js` | 当前 INSERT 用 `'building'` → 该 it 红（实测 `1 failed`：`expected 'INSERT ... 'building') ...' to match /'materializing'/`） |
| 物化在翻转前完成 | `materializes all nodes and edges before the active flip` | `sprints/08231107-kernel-ccd99d19/tests/projection-two-phase.test.js` | 顺序锁（现行已满足，实现改 status 时防回退；随文件其余用例一并冻结） |
| 单事务翻转 superseded+active | `supersedes old active and activates new run in one flip` | `sprints/08231107-kernel-ccd99d19/tests/projection-two-phase.test.js` | 翻转锁（同上，随文件冻结） |
| active 选择排除 materializing 残行 | `selects only active runs never materializing residuals` | `sprints/08231107-kernel-ccd99d19/tests/projection-two-phase.test.js` | 读谓词锁（改 active 选择泄漏 materializing 即红） |
| revision 查找不返回 materializing 残行 | `never returns a materializing residual run` | `sprints/08231107-kernel-ccd99d19/tests/projection-two-phase.test.js` | 读谓词锁（同上；且仍须可读 superseded） |
