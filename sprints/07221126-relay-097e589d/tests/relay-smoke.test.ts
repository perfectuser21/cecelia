...
// 合同测试（TDD Red 阶段）：目标模块尚不存在时本文件必须全红。
// generator 实现后：本文件原样保留 + 在 packages/brain/src/utils/relay-smoke.test.js
// 落一份 CI 常跑副本（import 改 './relay-smoke.js'，用例语义一致）。
import { formatSmokeStamp } from '../../../packages/brain/src/utils/relay-smoke.js';
...
describe('formatSmokeStamp [BEHAVIOR]', () => {
  ...
  it('非字符串 taskId 抛 TypeError', () => {
    expect(() => formatSmokeStamp(12345678 as unknown as string, new Date('2026-07-22T00:00:00Z'))).toThrow(TypeError);
  });
  ...
  it('非 Date 参数抛 TypeError', () => {
    expect(() =>
      formatSmokeStamp('097e589d-ec53-4102-b8d1-9aa582b88ebd', '2026-07-22' as unknown as Date)
    ).toThrow(TypeError);
  });
