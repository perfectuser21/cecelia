...
- 不放宽 memory、disk、quota、global hard seat、review gate；这些闸门继续独立生效，只修 account free 与 candidate 交集判定。
- reviewed_contract_sha `b1aeccddb` 仅作历史证据，不继承审批。
...
Registry 可用但未提供比 PRD 更高优先级的 HTTP schema；字段继续逐字采用现有 Brain/Kernel 命名：`provider`、`account`、`sampled_at`、`cache_ttl_ms`、`status`、`reason`、`review_required`。
...
本任务入口不是新 API，而是 Brain 服务端拥有的任务行、run 行与 capability snapshot。真实调用方 shape 必须逐字保持如下，禁止测试伪造 `candidate.role` 快捷路径：

| 调用方 | 入口 | 关键字段 |
|---|---|---|
| dispatcher/tick | `dispatchNextTask()` 选出的真实 task 行 | `task.id`、`task.task_type`、`task.priority`、`task.payload.role_assignments`、`task.payload.review_required` |
| unified Controller / slot check | server-owned candidate / snapshot | `provider`、`account`、`sampled_at`、`cache_ttl_ms`、`vendors.<provider>.accounts[]`、`reason` |
| attempt 账本 | attempt/run 真实状态行 | `attempt_id`、`run_id`、`status`、`recovered_at`、`provider`、`account` |
| 完成闸门 | `PATCH /api/brain/tasks/:task_id` | `review_required=true` 时必须由服务端 review gate 拒绝未审批完成 |
...
| **保质期（何时过期）** | 该能力/数据/token 何时失效，谁负责退役 | capability snapshot 在 `sampled_at + cache_ttl_ms` 后立即失效；attempt active 集合只对当前非终态有效；legacy usage 边界一旦统一进 snapshot，旧路径不得再单独扣减。 |
...
| ⚠️ 本次 dispatch 的 role target 从哪里来 | A. synthetic `candidate.role`; B. server-owned task/run `role_assignments` | B. server-owned `role_assignments` | PRD 明示“tests must use the real task-row/dispatcher shape” | 无关空闲账号顶替 pinned 不可用账号，错误放行 |
...
| ⚠️ legacy usage 与 kernel usage 如何并存 | A. 双路径都扣；B. 统一归一到同一 provider-neutral snapshot 或完全隔离旧路径 | B. 明确单一路径，不允许双扣 | PRD 第 4 条要求显式定案 | 同一 attempt/account 被双扣，`free` 错误变负 |
...
[dispatcher/tick 选中真实 harness task] → [服务端角色锚定唯一 provider/account 候选] → [用 active attempt SSOT 与 provider/account free 计算 admission] → [快照 stale/missing/unknown 按 candidate fail-closed] → [Claude 满载但角色允许的 Codex/Grok 可派发] → [recovered terminal 释放 capacity 且 review gate 继续收口]
...
**验证命令**:
```bash
bash -c 'npx vitest run sprints/07272227-kernel-0f481d85/tests/kernel-dispatcher-chain.contract.test.ts -t "dispatcher tick 真实 role target 交集：Claude 满载但 pinned Codex account 有位时仍派发 Codex，未知 Grok 不得顶替"'
```

**硬阈值**: test exit code = 0；真实 task-row fixture 内不得出现 synthetic `candidate.role` 快捷路径；拒发/放行都带稳定 `reason`。
...
**可观测行为**: 选中 account 的 free 只扣 active attempts 一次；legacy/relay/kernel/attempt 账本对同 attempt 去重；`total=4, active=2, free=2` 且其他硬闸允许时 admission 放行。
...
### Step 4: missing/partial/stale snapshot、usage API 错误、candidate unknown 与 legacy boundary 全部 fail-closed，但只拒当前 candidate

**来源**: `[FROM_PRD]` — PRD Golden Path 第 4 步、边界情况第 1、2、5 条。

**可观测行为**: 缺失 snapshot、局部缺 provider/account、`sampled_at` 超过 `cache_ttl_ms`、usage API 错误、candidate unknown 时返回稳定 `reason` 并拒发；legacy usage 明确统一到 provider-neutral snapshot 单一路径，或明确隔离旧路径且不与 Kernel 规则矛盾。

**验证命令**:
```bash
bash -c 'npx vitest run sprints/07272227-kernel-0f481d85/tests/kernel-capacity-accounting.contract.test.ts -t "snapshot sampled_at cache_ttl 缺失陈旧 usage API 错误或 candidate unknown 都 fail closed 且只拒当前 candidate"'
```

**硬阈值**: test exit code = 0；`reason` 稳定可断言；unknown 不得扩大到整个 pool。

### Step 5: 真实 dispatcher/tick -> harnessSlotCheck -> unified Controller 证明 Claude 满载拒 Claude，但角色允许且有空位的 Codex/Grok 可派发；review gate 继续收口
...
**验证命令**:
```bash
bash -c 'npx vitest run sprints/07272227-kernel-0f481d85/tests/kernel-dispatcher-chain.contract.test.ts && npx vitest run packages/brain/src/routes/__tests__/tasks-completed-gate.test.js -t "Rule1: review_required=true + review_status=pending → 422 REVIEW_NOT_APPROVED"'
```
...
2. `harnessSlotCheck/unified Controller` ↔ provider/account usage snapshot：必须真走 provider/account 维度 free 计算与 stale 判定，不得只测聚合总数。
...
- `slot-allocator.js` ↔ provider/account usage snapshot（本单改 provider/account free 计算，不得把 snapshot/account ledger mock 成与实现脱钩的 synthetic role 候选）
- `orchestrator/execution-contract.js` ↔ attempt/run 状态语义（本单改 active/terminal SSOT，测试必须真走状态集合与释放语义）
...
|---|---|---|---|
| attempt 状态 SSOT 与释放 | `sprints/07272227-kernel-0f481d85/tests/kernel-attempt-status-ssot.contract.test.ts` | `attempt 状态 SSOT：active 仅 queued starting running，terminal 仅 completed completed_with_concerns needs_context blocked failed cancelled` / `same attempt 非终态转 recovered terminal 只释放一次容量` | 缺少 SSOT 常量或释放 API 时 fail |
| provider/account free 与 fail-closed snapshot | `sprints/07272227-kernel-0f481d85/tests/kernel-capacity-accounting.contract.test.ts` | `selected account free=max(0,safe_limit-active(provider account)) 且 total=4 active=2 free=2 仍放行` / `snapshot sampled_at cache_ttl 缺失陈旧 usage API 错误或 candidate unknown 都 fail closed 且只拒当前 candidate` | 缺少 provider-neutral accounting helper 或 stale/unknown rule 时 fail |
| 真实 dispatch 链交集与 review gate | `sprints/07272227-kernel-0f481d85/tests/kernel-dispatcher-chain.contract.test.ts` | `dispatcher tick 真实 role target 交集：Claude 满载但 pinned Codex account 有位时仍派发 Codex，未知 Grok 不得顶替` / `dispatcher tick -> harnessSlotCheck -> unified Controller 真实链路证明 Claude 满载拒 Claude 而 Codex Grok 可派发` | 缺少真实 task-row 交集 helper 或真实链路回归时 fail |
...
TASK_JSON=$(curl -fsS "http://localhost:5221/api/brain/tasks/$TASK_ID")
echo "$TASK_JSON" | jq -e '
