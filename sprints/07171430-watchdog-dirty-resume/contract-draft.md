...
|---|---|---|---|
| DIRTY+容器消失→重点火 | `../../tests/regression/watchdog-dirty-resume/harness-relay-watchdog-dirty-resume.test.js` | [BEHAVIOR-1] out.resumed===1，spawnFn 被调一次，日志含 resume_conflict | 修复前 ciStatus=pending 走 wait_ci_running → out.resumed=0 ≠ 1 → FAIL |
| BEHIND 回归保护 | `../../tests/regression/watchdog-dirty-resume/harness-relay-watchdog-dirty-resume.test.js` | [BEHAVIOR-2] out.resumed===1，日志含 BEHIND，不含 resume_conflict | BEHIND 分支被 DIRTY 补丁意外破坏 → resumed=0 → FAIL |
| CI全绿+CLEAN→不重点火 | `../../tests/regression/watchdog-dirty-resume/harness-relay-watchdog-dirty-resume.test.js` | [BEHAVIOR-3] out.resumed===0，spawnFn 未被调，日志含 skip_green_waiting_merge | 绿灯路径被重点火 → resumed=1 → FAIL |
| BLOCKED+CI_pending→等待 | `../../tests/regression/watchdog-dirty-resume/harness-relay-watchdog-dirty-resume.test.js` | [BEHAVIOR-4] out.resumed===0，spawnFn 未被调，日志含 wait_ci_running | BLOCKED 被误判 DIRTY → resumed=1 → FAIL |
| DIRTY+attempt_cap→封顶 | `../../tests/regression/watchdog-dirty-resume/harness-relay-watchdog-dirty-resume.test.js` | [BEHAVIOR-5] attempts>=MAX_RELAY_ATTEMPTS 时，out.resumed===0，spawnFn 未被调 | DIRTY 绕过 cap → resumed=1 → FAIL |
