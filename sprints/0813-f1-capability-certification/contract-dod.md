...
- [ ] [ARTIFACT] Mapper 认证闸落在真实读路径 map-state-resolver.js（新增读 gp_contract_id/impact_contract_id + signed 合同校验）
...
- [ ] [ARTIFACT] Evaluator receipt 写路径落 gp_contract_id/gp_contract_hash 绑定
...
- [ ] [BEHAVIOR] [L2] B-01: 四认证前提齐备时 F1 投影 green
...
- [ ] [BEHAVIOR] [L2] B-02: 无 signed GP contract 时 F1 非绿（fail-closed）
...
- [ ] [BEHAVIOR] [L2] B-03: receipt 身份绑定缺失（未绑 GP identity / 未绑 Impact / 陈旧 SHA）时 F1 非绿
...
- [ ] [BEHAVIOR] [L2] B-04: step link 未绑定 Feature/Assertion 时 F1 非绿
...
- [ ] [BEHAVIOR] [L2] B-05: 真实 Evaluator 落 PASS receipt 时绑定 gp_contract_id（写侧 GP identity）
...
- [ ] [BEHAVIOR] [L2] INV-1 [validation-clock]: Evaluator 落 receipt 的身份完整性 fail-closed 不回退
...
- [ ] [BEHAVIOR] [L2] INV-2 [judge-evidence]: N/A — 本 sprint 不触及 Judge evidence_insufficient vs 实现缺陷分支
