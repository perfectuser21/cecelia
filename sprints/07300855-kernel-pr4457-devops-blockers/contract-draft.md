...
**来源**: `[AI_ADDED]` — Reviewer R3 要求移除 evaluator 自我依赖，并把同 head evaluator/judge 证明与顺序 oracle 移到 judge 之后、发起人工复核之前。
...
## TDD Red 证据（Round 7 复跑）

依赖准备：`npm ci --workspace packages/engine --workspace packages/brain --workspace packages/quality --ignore-scripts`，成功安装锁文件指定依赖。Red 命令使用 Engine workspace 的已安装 Vitest，不调用会临时下载错误版本的裸 `npx vitest`：
