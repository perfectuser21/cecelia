...
- [ ] [ARTIFACT] post-judge controller gate 按 evaluator < judge < review-request < approval 排序并使用受认证回执
  Test: node -e "const c=require('fs').readFileSync('sprints/07300855-kernel-pr4457-devops-blockers/contract-draft.md','utf8');for(const s of ['Post-judge controller gate','evaluator hop < judge hop < review-request hop < approval hop','approved_by','1800'])if(!c.includes(s))process.exit(1)"
