// E2E 驱动（sprint 根，非 tests/ 冻结测试）— 由 evaluator 在 local_api(真 PG) 上执行；
// propose 阶段目标模块未实现故不运行。它是 evaluator/DoD 调用的辅助驱动，不含 it()/test()
// 用例，故刻意放在 sprint 根而非 tests/（tests/ 下的 .mjs 会被封印闸计入冻结测试集，而
// 该集合的登记项只接受 .test/.spec/.e2e/.sh，.mjs 驱动无法登记 → 结构性冲突，见 Test Contract 注）。
