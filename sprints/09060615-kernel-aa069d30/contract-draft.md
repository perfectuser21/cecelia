...
**验证命令**:
```bash
DB_URL="$DB_URL" node sprints/09060615-kernel-aa069d30/tests/gate-e2e-driver.mjs regression
...
**验证命令**:
```bash
DB_URL="$DB_URL" node sprints/09060615-kernel-aa069d30/tests/gate-e2e-driver.mjs pass 22222222-3333-4444-5555-666666666666
...
**来源**: `[AI_ADDED]` — GAN Round 1 加入，理由：PRD 边界情况「判不该做/边界过宽→拦截不放行」「decisions 落库失败 fail-closed 不静默放行」必须有可执行 oracle，否则 generator 可用静默放行假绿
...
**验证命令**:
```bash
DB_URL="$DB_URL" node sprints/09060615-kernel-aa069d30/tests/gate-e2e-driver.mjs reject 33333333-4444-5555-6666-777777777777
DB_URL="$DB_URL" node sprints/09060615-kernel-aa069d30/tests/gate-e2e-driver.mjs incomplete 44444444-5555-6666-7777-888888888888
...
DB_URL="$DB_URL" node sprints/09060615-kernel-aa069d30/tests/gate-e2e-driver.mjs pass "$STEP_PASS"
...
|---|---|---|---|
| 三镜头门禁控制逻辑（短路/拦截/fail-closed/落库编排） | `sprints/09060615-kernel-aa069d30/tests/capability-gate.test.ts` | 短路不调 adjudicate 不写 db / reject fail-closed / NFR 三数不完整 / decisions 落库失败 / 写 decisions(category=nfr,level=step,target_type=journey_step) | 目标模块 capability-gate.js 未实现，import 失败 → 全部 RED（已本地确认） |
| 真 PG decisions 写路径（禁 mock 边补充行，brain-integration CI） | `packages/brain/src/__tests__/integration/capability-gate.integration.test.js` | createRoutedTask new_capability 过闸落真行 / reject 不建 task 不放行 | 真 PG 断言 nfr 行；函数未实现 → RED（由 generator 落地，CI brain-integration 跑） |

> 注：`capability-gate.test.ts` 为本 sprint 冻结测试（v9.27 死规则强制，落盘且进 commit）；integration 行为 repo 既有测试位置的**补充行**（覆盖 createRoutedTask↔gate 接线的真 PG 断言，由 CI brain-integration job 起真 Postgres 跑）。
