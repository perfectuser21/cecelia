...
- [ ] [ARTIFACT] 永久回归用例已落 CI 文件（brain-integration 真 Postgres 跑，CLAUDE.md 铁律 20）
...
- [ ] [ARTIFACT] draft 分流实现落 contract-store.js（附着 draft → 原子换版，标记锚点便于审计）
...
- [ ] [ARTIFACT] RED 规格文件存在（移植来源，含 4 例真 PG 用例）
...
- [ ] [BEHAVIOR] [L2] B-01: reopen v1 draft 附着 + Round2 新证据 → 原子换版 v2 approved、v1 superseded、run.contract_id 切 v2
...
- [ ] [BEHAVIOR] [L2] B-02: 幂等重放相同 v2 证据返回同一 contract.id，不新建版本、不重复 supersede
...
- [ ] [BEHAVIOR] [L2] B-04: fail-closed —— 附着 approved 合同证据不一致（篡改 branch）仍抛 evidence mismatch
...
- [ ] [BEHAVIOR] [L2] B-05: run 首轮无 contract_id 不回归（draft 分流不误伤 null 附着的既有插入路径）
