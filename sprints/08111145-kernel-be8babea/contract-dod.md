...
- [ ] [ARTIFACT] 永久 Vitest CI 回归存在且使用 `describe/it/expect`，覆盖成功接力与失败传播
...
- [ ] [ARTIFACT] `scripts/deploy.sh` 是唯一修改的官方入口，复用既有 `promote-dashboard.sh`，不新增第三份前端
...
- [ ] [BEHAVIOR] [L2] B-01: Dashboard-only 成功路径必须调用既有双节点 promote 主链
...
- [ ] [BEHAVIOR] [L2] B-02: HK 同步或终验失败必须让 Dashboard-only 发布非零退出
...
- [ ] [BEHAVIOR] [L3] B-03: HK 与 US 四类生产资源等于真实 PR head [接缝×2]
...
- [ ] [BEHAVIOR] [L3] B-04: HK 生产入口 WebKit 私密新上下文等待刷新后保持 /workbench/tasks [接缝×2]
...
- [ ] [BEHAVIOR] [L3] B-05: 本轮真实入口日志 Referer 保持深链
...
- [ ] [BEHAVIOR] [L3] INV-1: 真环境验证必须真实访问 HK/US 与 WebKit
...
- [ ] [BEHAVIOR] [L2] INV-3: deploy 失败禁止 warning 降级
...
- [ ] [BEHAVIOR] [L3] INV-4: 判变使用生产自报 build-info
...
- [ ] [BEHAVIOR] [L3] INV-5: 判变端与终验端使用相同版本语义
...
- [ ] [BEHAVIOR] [L2] INV-6: 验证命令真实产生 Red exit code
