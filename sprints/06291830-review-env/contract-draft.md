...
**验证命令**:
```bash
# 验证 workflow 含 health check 步骤（检查健康检查相关关键字）
# gate-allow: weak-oracle/curl-no-jq 此处仅为 grep 静态内容扫描，"curl" 不在模式串中，无需 jq
grep -qiE "healthcheck|health.check|health_check" .github/workflows/preview-deploy.yml || \
  grep -qE "preview.*http|http.*preview|url.*200|200.*ok" .github/workflows/preview-deploy.yml || \
  { echo "FAIL: 缺 preview URL health check 步骤"; exit 1; }
