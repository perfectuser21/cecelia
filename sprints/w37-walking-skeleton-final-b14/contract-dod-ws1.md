...
---
# Contract DoD — Workstream 1: playground GET /decrement
...
- [ ] [BEHAVIOR] success 响应反向不含任一禁用字段名（`decremented`/`prev`/`predecessor`/`minus_one`/`sub_one`/`incremented`/`sum`/`product`/`quotient`/`power`/`remainder`/`factorial`/`negation`/`value`/`input`/`output`/`data`/`payload`/`answer`/`meta`）
...
- [ ] [BEHAVIOR] strict-schema 全部非法输入返 400：`value=1.5` / `value=1e2` / `value=abc` / `value=+5` / `value=` / 缺 value / `n=5`（错 query 名）
  Test: manual:bash -c 'cd playground && PLAYGROUND_PORT=3207 node server.js & SPID=$!; sleep 2; FAIL=0; for q in "value=1.5" "value=1e2" "value=abc" "value=+5" "value=" "" "n=5"; do CODE=$(curl -s -o /dev/null -w "%{http_code}" "localhost:3207/decrement?$q"); if [ "$CODE" != "400" ]; then echo "FAIL: $q expect 400 got $CODE"; FAIL=1; fi; done; kill $SPID 2>/dev/null; [ $FAIL -eq 0 ]'
  期望: exit 0
