...
**FIX 备注 (B14 4 fix)**: 原 [BEHAVIOR] 段违反 CI v5.0 DoD purity 规则（DoD 仅可含 ARTIFACT）。所有 BEHAVIOR 已落地为 `tests/ws1/decrement.test.js` 的 28 个 vitest test() 块（见 Sprint Tests 实跑 v5.0 已通过）。BEHAVIOR 段移除以满足 purity check。
