...
- `logic-done-pending` 文件系统接缝：真实世界接缝是“传入的 JSON 文件路径必须可读取”；真目标验证方式是在 `local_api` 机器上用 `mktemp` 生成临时 JSON 文件，再用 `node scripts/relay-demo/sort-json-keys.mjs <path>` 运行
- `logic-done-pending` Node CLI 接缝：真实世界接缝是“工具必须把合法 JSON 结果写到 stdout，且退出码反映成败”；真目标验证方式是 bash 中捕获 `stdout` 并交给 `jq -S .` / `node -e` 校验
- `logic-done-pending` 测试接缝：真实世界接缝是“vitest 在本地可跑并证明三类合同用例全部通过”；真目标验证方式是执行 `vitest run` 并从 CLI 输出中断言 `3 passed`

## Golden Path

[提供 JSON 文件路径] → [CLI 读取文件并递归排序对象键] → [stdout 输出稳定 JSON] → [vitest 证明嵌套对象/数组/空对象三个场景都成立]

### Step 1: 使用者提供一个可读取的 JSON 文件路径

**来源**: `[FROM_PRD]` — PRD Golden Path 第 1 条直接定义“使用者提供一个可读取的 JSON 文件路径，作为本工具唯一输入”

**可观测行为**: CLI 只接受一个 JSON 文件路径作为输入，命令可以在本地以该路径直接运行

**验证命令**:
```bash
TMP_JSON="$(mktemp)"
printf '{"b":1,"a":2}\n' > "$TMP_JSON"
node scripts/relay-demo/sort-json-keys.mjs "$TMP_JSON"
rm -f "$TMP_JSON"
```

**硬阈值**: 命令入口形态必须是 `node scripts/relay-demo/sort-json-keys.mjs <json-file-path>`；输入来源只能是文件路径参数

---

### Step 2: CLI 递归按字典序排序对象键并输出到 stdout

**来源**: `[FROM_PRD]` — PRD Golden Path 第 2 条直接定义“对象键按字典序递归稳定排列”

**可观测行为**: 对象层级中的键顺序稳定为字典序，嵌套对象同样递归排序；输出结果是合法 JSON 文本
...
echo "$OUT" | jq -e '. == {"a":{"c":3,"d":4},"m":{"a":1,"b":2},"z":1}'
rm -f "$TMP_JSON"
```

**硬阈值**: `stdout` 必须是合法 JSON；所有对象键递归按字典序排列；不得丢字段、增字段或改值

---

### Step 3: 数组顺序保持不变，数组中的对象元素也递归排序

**来源**: `[FROM_PRD]` — PRD Golden Path 第 2 条与边界情况第 2 条直接定义“数组顺序保持输入语义不变，数组中的对象元素同样满足递归键排序”

**可观测行为**: 数组元素数量与顺序保持不变；若数组元素是对象，则对象键被递归排序
...
**硬阈值**: 数组长度不变；数组顺序不变；数组内对象元素的键顺序符合字典序
...
**可观测行为**: 空对象不会被替换成数组、`null` 或其他值；本地运行 vitest 时，三个合同用例全部通过
...
EOF
npm exec --workspace packages/brain vitest -- --config "$TMP_CFG" run sprints/07071247-relay-demo-codex-r2/tests/sort-json-keys.contract.test.ts --reporter=verbose
rm -f "$TMP_CFG"
rm -f "$TMP_JSON"
```

**硬阈值**: 空对象语义保持不变；vitest 输出必须表明三个合同用例全部通过
...
git diff --name-only -- scripts/relay-demo sprints/07071247-relay-demo-codex-r2 >/dev/null
