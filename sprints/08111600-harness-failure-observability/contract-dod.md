...
- [ ] [ARTIFACT] 共享 helper 模块存在且导出枚举/规范化/终结函数
...
- [ ] [ARTIFACT] failure-stats 路由注册在 harness.js
...
- [ ] [ARTIFACT] brain-ci.yml 新增 lint 步骤（机械闸进 CI，非文档约定）
  Test: node -e "const c=require('fs').readFileSync('.github/workflows/brain-ci.yml','utf8');if(!c.includes('lint-terminal-failure-class'))process.exit(1)"
...
- [ ] [BEHAVIOR] [L2] B-01: 共享 helper 把 result.failure_class + failure_detail 真落库
...
- [ ] [BEHAVIOR] [L2] B-02: executor 真实写入点（缺 orchestrator flag）迁到 result.failure_class
...
- [ ] [BEHAVIOR] [L2] B-03: 受控枚举拒绝自由文本（规范化到 unclassified）
...
- [ ] [BEHAVIOR] [L2] B-04: GET /failure-stats?days=7 返回 failure_rate 数值 + by_class 分组对象
...
- [ ] [BEHAVIOR] [L2] B-05: error path — 非法 days 返回 400 + error 字段
...
- [ ] [BEHAVIOR] [L2] B-06: 机械闸 lint 干净树 exit 0、注入裸 terminal 写入 exit 1、还原后再 exit 0
...
- [ ] [BEHAVIOR] [L2] INV-2: 口径三源防线 — stats 不恒空且 by_class 求和 == terminal_failed_count
