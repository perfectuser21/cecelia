...
- [ ] [BEHAVIOR] GET /initiative/:id/ws-progress 返回 keys == ["initiative_id","workstreams"]（schema 完整性）
  Test: manual:bash -c 'ID=$(curl -sf -X POST localhost:5221/api/brain/tasks -H "Content-Type: application/json" -d '"'"'{"task_type":"harness_initiative","title":"_test_ws_progress_schema","status":"queued"}'"'"' | jq -r .id); curl -sf "localhost:5221/api/brain/harness/initiative/$ID/ws-progress" | jq -e '"'"'keys == ["initiative_id","workstreams"]'"'"' && printf "OK\n"'
  期望: OK

- [ ] [BEHAVIOR] initiative_id 字段值等于请求路径 id
  Test: manual:bash -c 'ID=$(curl -sf -X POST localhost:5221/api/brain/tasks -H "Content-Type: application/json" -d '"'"'{"task_type":"harness_initiative","title":"_test_ws_progress_id_match","status":"queued"}'"'"' | jq -r .id); curl -sf "localhost:5221/api/brain/harness/initiative/$ID/ws-progress" | jq -e ".initiative_id == \"$ID\"" && printf "OK\n"'
  期望: OK

- [ ] [BEHAVIOR] 无 WS checkpoint 时 workstreams 返回 []
  Test: manual:bash -c 'ID=$(curl -sf -X POST localhost:5221/api/brain/tasks -H "Content-Type: application/json" -d '"'"'{"task_type":"harness_initiative","title":"_test_ws_progress_empty","status":"queued"}'"'"' | jq -r .id); curl -sf "localhost:5221/api/brain/harness/initiative/$ID/ws-progress" | jq -e '"'"'.workstreams == []'"'"' && printf "OK\n"'
  期望: OK

- [ ] [BEHAVIOR] 不存在 initiative_id 返回 HTTP 404 + {error:"initiative not found"}
  Test: manual:bash -c 'CODE=$(curl -s -o /tmp/ws404.json -w "%{http_code}" "localhost:5221/api/brain/harness/initiative/00000000-0000-0000-0000-000000000099/ws-progress"); [ "$CODE" = "404" ] || { printf "FAIL: expected 404 got %s\n" "$CODE"; exit 1; }; jq -e '"'"'.error == "initiative not found"'"'"' /tmp/ws404.json && printf "OK\n"'
  期望: OK
