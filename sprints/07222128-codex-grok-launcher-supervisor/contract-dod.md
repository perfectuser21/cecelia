...
- [ ] [BEHAVIOR] `codex-launch.sh` exit 0 / exit 130（SIGINT）不重启（INV-3）
  Test: manual:bash -c 'bash /workspace/scripts/__tests__/codex-launch.test.sh 2>&1 | grep -E "PASS.*exit.0.no.restart|PASS.*SIGINT.no.restart"'
  期望: 至少 1 行 PASS

- [ ] [BEHAVIOR] `codex-launch.sh` SIGABRT/137/143 最多重试 3 次超限 exit 1（INV-4）
  Test: manual:bash -c 'bash /workspace/scripts/__tests__/codex-launch.test.sh 2>&1 | grep -E "PASS.*retry.*3|PASS.*max.*retries"'
  期望: 至少 1 行 PASS

- [ ] [BEHAVIOR] `codex-launch.sh` 重试时用 session-id 恢复（INV-5 + codex resume）
  Test: manual:bash -c 'bash /workspace/scripts/__tests__/codex-launch.test.sh 2>&1 | grep -E "PASS.*resume|PASS.*session.id"'
  期望: 至少 1 行 PASS

- [ ] [BEHAVIOR] `codex-launch.sh` 用凭据快照目录（INV-11，非真实 CODEX_HOME）
  Test: manual:bash -c 'bash /workspace/scripts/__tests__/codex-launch.test.sh 2>&1 | grep -E "PASS.*snapshot|PASS.*cred"'
  期望: 至少 1 行 PASS

- [ ] [BEHAVIOR] `grok-launch.sh` exit 0 / exit 130 不重启（INV-3）
  Test: manual:bash -c 'bash /workspace/scripts/__tests__/grok-launch.test.sh 2>&1 | grep -E "PASS.*exit.0.no.restart|PASS.*SIGINT.no.restart"'
  期望: 至少 1 行 PASS

- [ ] [BEHAVIOR] `grok-launch.sh` SIGABRT 最多重试 3 次超限 exit 1（INV-4）
  Test: manual:bash -c 'bash /workspace/scripts/__tests__/grok-launch.test.sh 2>&1 | grep -E "PASS.*retry.*3|PASS.*max.*retries"'
  期望: 至少 1 行 PASS

- [ ] [BEHAVIOR] `grok-launch.sh` 重试时用 grok --resume session-id（INV-5）
  Test: manual:bash -c 'bash /workspace/scripts/__tests__/grok-launch.test.sh 2>&1 | grep -E "PASS.*resume|PASS.*session.id"'
  期望: 至少 1 行 PASS

- [ ] [BEHAVIOR] `codex-supervisor.mjs` continue 时用同一 session-id 续跑（INV-5 + GP5）
  Test: manual:bash -c 'node /workspace/scripts/__tests__/codex-supervisor.test.mjs 2>&1 | grep -E "PASS.*continue.*session|PASS.*resume"'
  期望: 至少 1 行 PASS

- [ ] [BEHAVIOR] `codex-supervisor.mjs` complete 须外部验收（不信模型自称，INV-6）
  Test: manual:bash -c 'node /workspace/scripts/__tests__/codex-supervisor.test.mjs 2>&1 | grep -E "PASS.*external.*verif|PASS.*complete.*brain|PASS.*外部验收"'
  期望: 至少 1 行 PASS

- [ ] [BEHAVIOR] `codex-supervisor.mjs` blocked 写 Brain 不伪装 completed（INV-7）
  Test: manual:bash -c 'node /workspace/scripts/__tests__/codex-supervisor.test.mjs 2>&1 | grep -E "PASS.*blocked.*brain|PASS.*blocked.*不标"'
  期望: 至少 1 行 PASS

- [ ] [BEHAVIOR] `codex-supervisor.mjs` 超 MAX_TURNS 标 timed_out exit 1（FR-R5）
  Test: manual:bash -c 'node /workspace/scripts/__tests__/codex-supervisor.test.mjs 2>&1 | grep -E "PASS.*timed_out|PASS.*MAX_TURNS"'
  期望: 至少 1 行 PASS

- [ ] [BEHAVIOR] `grok-supervisor.mjs` continue 用 grok -p --resume session-id（GP6 + INV-5）
  Test: manual:bash -c 'node /workspace/scripts/__tests__/grok-supervisor.test.mjs 2>&1 | grep -E "PASS.*resume|PASS.*session"'
  期望: 至少 1 行 PASS

- [ ] [BEHAVIOR] `grok-supervisor.mjs` blocked 写 Brain 不伪装 completed（INV-7）
  Test: manual:bash -c 'node /workspace/scripts/__tests__/grok-supervisor.test.mjs 2>&1 | grep -E "PASS.*blocked"'
  期望: 至少 1 行 PASS

- [ ] [BEHAVIOR] `install-launchers.sh` 重复运行不产生重复 alias（INV-9 幂等）
  Test: manual:bash -c 'bash /workspace/scripts/__tests__/install-launchers.test.sh 2>&1 | grep -E "PASS.*idempotent|PASS.*幂等"'
  期望: 至少 1 行 PASS
...
- [ ] [BEHAVIOR:E2E] 路由单元测试（RED→GREEN）全部通过——harness-skill-relay.js 三分支 + entrypoint.sh 三分支 + unknown loud-fail
  Test: manual:bash -c 'bash /workspace/scripts/__tests__/codex-grok-launcher-routing.test.sh && echo "OK"'
  期望: OK（0 FAIL）

- [ ] [BEHAVIOR:E2E] codex-launch.sh fake binary 测试全部通过
  Test: manual:bash -c 'bash /workspace/scripts/__tests__/codex-launch.test.sh && echo "ALL_PASS"'
  期望: ALL_PASS

- [ ] [BEHAVIOR:E2E] grok-launch.sh fake binary 测试全部通过
  Test: manual:bash -c 'bash /workspace/scripts/__tests__/grok-launch.test.sh && echo "ALL_PASS"'
  期望: ALL_PASS

- [ ] [BEHAVIOR:E2E] codex-supervisor.mjs fake binary + fake Brain 测试全部通过
  Test: manual:bash -c 'node /workspace/scripts/__tests__/codex-supervisor.test.mjs && echo "ALL_PASS"'
  期望: ALL_PASS

- [ ] [BEHAVIOR:E2E] grok-supervisor.mjs fake binary + fake Brain 测试全部通过
  Test: manual:bash -c 'node /workspace/scripts/__tests__/grok-supervisor.test.mjs && echo "ALL_PASS"'
  期望: ALL_PASS

- [ ] [BEHAVIOR:E2E] entrypoint.sh bash 集成测试全部通过
  Test: manual:bash -c 'bash /workspace/scripts/__tests__/codex-grok-entrypoint-routing.test.sh && echo "ALL_PASS"'
  期望: ALL_PASS

- [ ] [BEHAVIOR:E2E] install-launchers.sh 幂等测试通过
  Test: manual:bash -c 'bash /workspace/scripts/__tests__/install-launchers.test.sh && echo "ALL_PASS"'
  期望: ALL_PASS
