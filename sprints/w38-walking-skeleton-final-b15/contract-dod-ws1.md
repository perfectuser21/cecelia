...
**FIX 备注 (W38 B15 verdict regex 修后 final happy 真验)**: 沿用 W37 /decrement Round-2 fix 的合同结构。BEHAVIOR 索引以 purity 安全格式恢复（无 `- [_]` 前缀，避开 DoD 纯度 Rule 1 正则；Test 字段 `manual:bash` 走 Rule 2 白名单），evaluator 直接 grep `[BEHAVIOR]` 提取仍可读到。
...
## BEHAVIOR 索引（evaluator 真启 server + curl + jq 实跑；purity-safe 无 `- [_]` 前缀）
