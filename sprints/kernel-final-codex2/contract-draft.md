...
- 单行命令：`NODE_ENV=test node -e "require.extensions['.ts']=require.extensions['.js'];require('./sprints/kernel-final-codex2/tests/kernel-ping2.test.ts')"`
- 实跑结果：exit code = 1；Node v20.20.2 内置 `node:test` 成功执行 4 个测试，其中 2 failed、2 passed，全程不加载 Vitest、Vite 或 Rollup。
- 可归因失败：测试解释器与测试文件均成功加载；`kernel-ping2.test.ts:9` 的 `assert.equal(res.status, 200)` 实收 404；`kernel-ping2.test.ts:15` 的 `assert.deepEqual(Object.keys(res.body), ['result'])` 实收空数组。TAP 终态为 `tests 4`、`pass 2`、`fail 2`、`duration_ms 18.071927`，失败发生在未实现路由的具体 HTTP 状态与响应 schema 断言，不是依赖加载、配置或测试收集故障。
- 永久回归命令：实现前后使用同一条 Node 内置测试命令；只有 GET 路由返回 HTTP 200、`result=ok2` 且仅含 `result` 时前两条才能转绿。
