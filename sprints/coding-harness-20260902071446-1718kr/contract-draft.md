...
[打开中文说明] → [理解端点与鉴权] → [按封闭角色和 payload 构造请求] → [识别失败回滚终态]

### Step 1: 找到中文桥接说明并理解两个端点
**来源**: `[FROM_PRD]` — PRD「Golden Path」第 1、2 项及「范围限定」。

**可观测行为**: `docs/current/attempt-run-bridge-guide.md` 为中文文档，分别解释 POST 创建与 GET 查询用途，并精确区分 loopback 与宿主/远端 Bearer 鉴权。

**验证命令**:
```bash
npx vitest run --no-cache sprints/coding-harness-20260902071446-1718kr/tests/attempt-run-bridge-guide.test.ts -t '中文文档说明 POST 创建与 GET 查询端点及鉴权边界'
```
**硬阈值**: 指定测试 1/1 通过，缺任一端点、鉴权名称、Bearer 占位符或中文正文均非零退出。
...
### Step 5: 保持 documentation-only 精确范围
...
|---|---|---|---|
| 端点与鉴权 | `sprints/coding-harness-20260902071446-1718kr/tests/attempt-run-bridge-guide.test.ts` | 中文文档说明 POST 创建与 GET 查询端点及鉴权边界 | 文档尚不存在，读取抛出 ENOENT |
