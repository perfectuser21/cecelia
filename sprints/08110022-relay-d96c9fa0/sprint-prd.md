...
- 新建正式 migration `408_impact_contracts.sql`（`harness_impact_contracts` 表）
...
**文件：**
- 正式 migration `409_harness_gap_ledger.sql`（新建 `harness_gaps` / `gap_events`，加厚既有 `task_dependencies`）
