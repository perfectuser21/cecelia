...
describe("Codex Slot 受控身份与自动路由 [BEHAVIOR]", () => {
  it("受控 SSH key 映射 actor 且忽略客户端 actor/host 自报", async () => {
    const { loadRootConfig, resolveActor } =
      await import("../../../packages/brain/src/codex-slot/identity.js");
  ...
  it("自动选择仅接纳身份、mmv、容量与新鲜度全部有效的 agent slot", async () => {
    const { selectAgentSlot } =
      await import("../../../packages/brain/src/codex-slot/selector.js");
    const now = Date.now();
