...
|---|---|---|---|
| 四档 change_kind Profile（proposer 红留档） | `sprints/08131104-kernel-77f19b77/tests/kernel-change-kind-profile.test.js` | bugfix change_kind 初始态跳 Planner、parameter_only change_kind 初始态跳 Planner、new_capability change_kind 走全链 | 见 tests/red-evidence.md（proposer round1 实测 bugfix 与 parameter_only 期望 generate 实得 planning） |
| 四档 change_kind Profile 分派 | `packages/brain/src/__tests__/kernel-change-kind-profile.test.js` | bugfix change_kind 初始态跳 Planner、parameter_only change_kind 初始态跳 Planner、new_capability change_kind 走全链 | derive 现不消费 change_kind → bugfix 与 parameter_only 现返回 planning（已实证 RED，见 tests/red-evidence.md） |
| Controller ownership + fail-closed + migration 413 | `packages/brain/src/__tests__/integration/kernel-controller-ownership.pg.integration.test.js` | createKernelRun 无 controllerSessionId fail-closed、harness_runtime=kernel-v1 直打不产生无 Controller run、migration 413 加 controller ownership 列 | 413 未建、createKernelRun 无 ownership 校验 → 列不存在且无主 run 可建（RED） |
| Controller/Kernel 生命周期隔离 | `packages/brain/src/__tests__/integration/kernel-controller-lifecycle.pg.integration.test.js` | Kernel fatal 只结束 Kernel Controller 存活、Controller fatal Kernel 不无主 无主历史 fail-closed、无主历史 Kernel Run fail-closed 进恢复 | Kernel detached 现无主，fatal 后 ownership 消失（RED） |
