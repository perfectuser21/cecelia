...
- [ ] [ARTIFACT] harness-report SKILL.md 含 "Step 3.5" 字面量
  Test: manual:bash -c 'grep -q "Step 3.5" packages/workflows/skills/harness-report/SKILL.md || exit 1'

- [ ] [ARTIFACT] SKILL.md Step 3.5 映射中含 "SprintPRD"（不是 "PRD"）
  Test: manual:bash -c 'grep -A30 "Step 3.5" packages/workflows/skills/harness-report/SKILL.md | grep -q "SprintPRD" || exit 1'

- [ ] [ARTIFACT] SKILL.md Step 3.5 映射中含 "PrepPRD"
  Test: manual:bash -c 'grep -A30 "Step 3.5" packages/workflows/skills/harness-report/SKILL.md | grep -q "PrepPRD" || exit 1'

- [ ] [ARTIFACT] SKILL.md Step 3.5 使用 "content" 字段（不是 "body"）发送 POST
...
- [ ] [BEHAVIOR] SKILL.md Step 3.5 中 sprint-prd.md 映射类型为 "SprintPRD" 而非 "PRD"
...
- [ ] [BEHAVIOR] SKILL.md Step 3.5 POST 请求体使用 content 字段（不是 body）
...
- [ ] [BEHAVIOR] SKILL.md Step 3.5 type 使用 DOC_TYPE 变量（不是硬编码 "Log"）
...
- [ ] [BEHAVIOR] SKILL.md Step 3.5 POST payload 含 initiative_id 字段
...
- [ ] [BEHAVIOR] SKILL.md Step 3.5 三种类型覆盖（PrepPRD/SprintPRD/Contract）在 Step 3 之后 Step 4 之前
