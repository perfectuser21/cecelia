...
- [x] [ARTIFACT] HarnessDetailPage.tsx 含 data-testid="docs-tab" 触发器
...
- [x] [ARTIFACT] HarnessDetailPage.tsx 含 data-testid="docs-tab-content" 渲染容器
...
- [x] [ARTIFACT] HarnessDetailPage.tsx 含 sprint-docs API 调用路径
...
- [x] [ARTIFACT] HarnessDetailPage.tsx 含 markdown 渲染（react-markdown 或 dangerouslySetInnerHTML）
...
- [x] [BEHAVIOR] HarnessDetailPage.tsx 同时含 docs-tab 和 docs-tab-content（WS 实现前两者都缺 → FAIL）
...
- [x] [BEHAVIOR] GET /api/brain/harness/sprint-docs 对已知 sprint_dir 返回 sprint_prd 字符串内容（依赖 WS3 已实现）
...
- [x] [BEHAVIOR] HarnessDetailPage.tsx 含 Tab 切换 state 逻辑（activeTab/selectedTab/tabState 等 state 变量）
...
- [x] [BEHAVIOR] react-markdown 依赖存在（package.json 或 node_modules）
...
- [x] [BEHAVIOR:E2E] 用户点击"文档"tab，docs-tab-content 可见，markdown 已渲染（Playwright 真实浏览器验证）
  Screenshots:
