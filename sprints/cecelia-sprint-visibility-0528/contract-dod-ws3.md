...
- [x] [ARTIFACT] packages/brain/src/routes/harness.js 含 /sprint-docs 路由注册
...
- [x] [ARTIFACT] harness.js /sprint-docs 路由返回含 prep_prd 字段
...
- [x] [ARTIFACT] harness.js /sprint-docs 路由返回含 sprint_prd 字段
...
- [x] [ARTIFACT] harness.js /sprint-docs 路由返回含 harness_report 字段
...
- [x] [BEHAVIOR] GET /api/brain/harness/sprint-docs?sprint_dir=... 返回 HTTP 200 + 含 sprint_dir 和 docs 字段
...
- [x] [BEHAVIOR] docs keys 完全等于 ["contract","harness_report","prep_prd","sprint_prd"]（sorted，不多不少）
...
- [x] [BEHAVIOR] sprint_dir 下存在的文件（sprint-prd.md）→ 对应字段为 string（非 null）
...
- [x] [BEHAVIOR] 不存在的 sprint_dir 路径下所有 docs 字段为 null（不报 404，返回 200+null）
...
- [x] [BEHAVIOR] 缺 sprint_dir 参数 → HTTP 400 + {error: string}（error path）
...
- [x] [BEHAVIOR] query 参数必须使用 sprint_dir（禁用 dir/path/d/p）— 若用错参数等效 400
