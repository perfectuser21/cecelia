...
**范围**: packages/brain 内 `diff-gate.js`（三类分流 + reason_code/detail 透传）、`harness-gates.js`（gateReceipt 透传 detail/unclaimed_files）、`loop.js`+`derive.js`（retryable:false 确定性出口 + 按 reason 路由 spawn:generator-fix / wait:human_review）。Brain semver 四处同步 + DevGate 三项。
**大小**: M
...
> **测试落位（r2 死因修复）**：合同冻结测试在 `sprints/08172017-kernel-37cf5c5f/tests/`（kernel 采集只认此路径，从仓库根 `npx vitest run` 直跑——root vitest.config include 覆盖 `sprints/**`）。Generator 实现阶段把 4 个测试文件复制到 `packages/brain/src/**/__tests__/` 作永久回归（CLAUDE.md 硬规则 20）。

## ARTIFACT 条目

- [ ] [ARTIFACT] diff-gate 三类分流冻结测试文件存在（sprints 冻结路径）
  Test: node -e "require('fs').accessSync('sprints/08172017-kernel-37cf5c5f/tests/diff-gate-reason-code.test.js')"

- [ ] [ARTIFACT] harness-gates gateReceipt 透传冻结测试文件存在
  Test: node -e "require('fs').accessSync('sprints/08172017-kernel-37cf5c5f/tests/harness-gates-reason-code.test.js')"

- [ ] [ARTIFACT] loop/derive 确定性出口路由冻结测试文件存在
  Test: node -e "require('fs').accessSync('sprints/08172017-kernel-37cf5c5f/tests/loop-impact-deterministic-route.test.js')"

- [ ] [ARTIFACT] 回归夹具（run d1360a48 录制件）+ 夹具冻结测试存在
  Test: node -e "require('fs').accessSync('sprints/08172017-kernel-37cf5c5f/fixtures/radius-d1360a48-impact-anchor.json');require('fs').accessSync('sprints/08172017-kernel-37cf5c5f/tests/diff-gate-regression-d1360a48.test.js')"

- [ ] [ARTIFACT] 永久回归测试已复制到 packages/brain/src/**/__tests__/（Generator 实现阶段交付，CI 永久回归）
  Test: node -e "const fs=require('fs');fs.accessSync('packages/brain/src/impact-contract/__tests__/diff-gate-reason-code.test.js');fs.accessSync('packages/brain/src/orchestrator/__tests__/loop-impact-deterministic-route.test.js')"

- [ ] [ARTIFACT] Final E2E harness 存在（真 runLoop 一跳 + 真 appendHop 落 orchestrator_decision_log）
  Test: node -e "require('fs').accessSync('sprints/08172017-kernel-37cf5c5f/e2e/impact-gate-e2e.mjs')"
...
- [ ] [ARTIFACT] Brain 版本四处同步 = 1.273.73（package.json / package-lock.json / .brain-versions / DEFINITION.md）
  Test: node -e "const v=require('./packages/brain/package.json').version;const l=require('./packages/brain/package-lock.json').version;const fs=require('fs');const bv=fs.readFileSync('.brain-versions','utf8').trim().split('\n').pop().trim();const def=/Brain[ \t]+版本[^:]*:[ \t]*([0-9.]+)/.exec(fs.readFileSync('DEFINITION.md','utf8'))[1];if(!(v===l&&v===bv&&v===def&&v==='1.273.73'))process.exit(1)"
...
- [ ] [BEHAVIOR] [L1] B-05: harness-gates beforeEvaluate gateReceipt 透传 reason/retryable/detail/unclaimed_files（INV-3）
  动作: 跑 harness-gates-reason-code 冻结测试（真 createHarnessImpactGates.beforeEvaluate + 注入 diffGate 确定性 blocked 结果）
  ...
  Test: manual:bash -c 'npx vitest run sprints/08172017-kernel-37cf5c5f/tests/harness-gates-reason-code.test.js'

- [ ] [BEHAVIOR] [L1] B-06: loop/derive impact_anchor_missing（retryable:false）→ intent 透传 + 路由 spawn:generator-fix（INV-1/INV-3）
  动作: 跑 loop-impact-deterministic-route 冻结测试 impact_anchor_missing 用例（真 runLoop + 真 derive，注入 beforeEvaluate 确定性 blocked receipt）
  ...
  等待预算: 30s
  留证: vitest 用例输出末 5 行（含 passed）
  Test: manual:bash -c 'npx vitest run sprints/08172017-kernel-37cf5c5f/tests/loop-impact-deterministic-route.test.js -t "impact_anchor_missing"'

- [ ] [BEHAVIOR] [L1] B-07: loop/derive capability_assertion_coverage_missing（retryable:false）→ 路由 wait:human_review
  动作: 跑 loop-impact-deterministic-route 冻结测试 coverage_missing 用例
  预期观察: 下一动作 dispatch 含 'wait:human_review'（Map 覆盖缺口需人补断言，不派 generator-fix）
  等待预算: 30s
  留证: vitest 用例输出末 5 行（含 passed）
  Test: manual:bash -c 'npx vitest run sprints/08172017-kernel-37cf5c5f/tests/loop-impact-deterministic-route.test.js -t "capability_assertion_coverage_missing"'

- [ ] [BEHAVIOR] [L1] B-08: 回归夹具 run d1360a48（真实 changed_files 含 DoD.md + radius 录制件）→ 新代码 blocked:impact_anchor_missing
  动作: 用录制件注入 evaluateDiffGate（changed_files 取自录制件 changed_files）跑夹具冻结测试
  ...
  等待预算: 30s
  留证: vitest 用例输出末 5 行（含 passed）
  Test: manual:bash -c 'npx vitest run sprints/08172017-kernel-37cf5c5f/tests/diff-gate-regression-d1360a48.test.js'
