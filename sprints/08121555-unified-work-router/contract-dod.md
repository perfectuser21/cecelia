...
- [ ] [BEHAVIOR] [L2] B-07: Generator frozen baseline 与 trust boundary 在真实命令链生效
  Test: manual:bash -c 'DB_URL="$DB_URL" BASELINE_SHA=7b6d3585522b9cacf70f39322abf69d54716927d bash docker/cecelia-runner/__tests__/entrypoint-generator-trust-boundary.test.sh --real-container && docker inspect --format "{{.State.Status}}" "$HARNESS_GENERATOR_CONTAINER_ID" | grep -qx exited'
  动作: 通过真实 Docker daemon 启动 runner 容器，检查 hook、setpriv、capability、pushurl 与敏感环境
  预期观察: baseline 后追加 commit 合法且 imported lineage 拒绝；Provider push 失败；callback/lease 凭据不可见；docker inspect 证明真容器已执行
  等待预算: 120s
