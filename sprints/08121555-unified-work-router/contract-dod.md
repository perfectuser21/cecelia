...
- [ ] [BEHAVIOR] [L2] B-01: 含凭据 origin 等价且日志脱敏、活跃 Kernel cwd 不被删除 [接缝×2]
  动作: 创建 bare Git remote 和真实 clone，将 origin 改成带 URL userinfo 的等价 GitHub 地址，登记活跃 detached Kernel workspace 后运行产品 reconcile
  预期观察: 两次均复用工作区，日志不含测试凭据，目录仍存在
  等待预算: 10s
  留证: Vitest 输出与临时目录状态
  Test: manual:bash -c 'npx vitest run sprints/08121555-unified-work-router/tests/worktree-origin-safety.test.ts --reporter=verbose && cd packages/brain && npx vitest run src/__tests__/harness-worktree-origin-safety.test.js --reporter=verbose'
...
- [ ] [BEHAVIOR] [L2] B-03: task 与 append-only Routing Receipt 在真实 PostgreSQL 原子创建并可追加后继 [接缝×2]
  动作: 对 attempt-scoped PostgreSQL 执行 migration，以真实 pg client 加锁后并发提交相同 source key、制造 receipt 写入失败，并直接 SQL 查询同一 task 的 superseding receipt 链
...
- [ ] [BEHAVIOR] [L2] B-04: Map/Impact preflight 在 Provider 前 fail closed [接缝×2]
  动作: 创建真实临时 Git repo/commit，在 PostgreSQL 持久化 16 分钟前 Map header，并在 preflight 前后直接查询 Provider attempt；产品集成组另覆盖 fresh/missing/revision/scanner/cross-repo/recovery
  预期观察: 仅 fresh 或合法单次 recovery 进入后续阶段；其余均稳定 reason_code 且 Provider attempt 为零
  等待预算: 30s
  留证: 测试输出、Map revision 与 attempt 计数
  Test: manual:bash -c ': "${DB_URL:?}"; DB_URL="$DB_URL" npx vitest run sprints/08121555-unified-work-router/tests/map-action-gates.integration.test.ts --reporter=verbose && cd packages/brain && DB_URL="$DB_URL" npx vitest run src/orchestrator/preflight/map-impact-contract.test.js src/orchestrator/__tests__/map-recovery-contract.test.js --reporter=verbose'
