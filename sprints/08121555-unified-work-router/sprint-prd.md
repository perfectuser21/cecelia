...
- **当前进度**：未配置
- **本次推进预期**：完成 Knife 0-5 与恢复前置回归的可执行验收闭环

## 背景

当前有头/无头 coding mutation 尚未全部由唯一顶层路由进入 Kernel Harness 2.0；前序 run 还因含凭据的 Git origin 被误判为孤儿并删除活跃 Kernel 工作区而失败。本 Sprint 恢复并完整交付统一 Work Router，且不得缩减既有批准设计。

## Golden Path（核心场景）

有头或无头 coding mutation 从任一任务创建入口进入 → Work Router 在动作前原子生成不可变 Routing Receipt 并正向确定四档 `change_kind` → 统一进入 Kernel Harness 2.0 → 每次读取 fresh Universal Map 并建立 Impact Contract → Generator 在 frozen baseline 与 trust boundary 内产出 → Evaluator/Judge、CI 与 merge fence 验证 → scratch 多入口真实验收通过。

具体：
1. 先以 RED/GREEN 永久复现并修复含凭据 Git origin 的归一化与日志脱敏，以及活跃 detached Kernel 工作区保护；回归测试永久进入 CI。
2. Knife 0-2 冻结路由合同、四档 `change_kind`、动态 task type 与任务创建入口事实清单，建立不可变 Routing Receipt，并让所有可执行业务任务入口只通过事务级统一创建边界。
3. Knife 3 强制每个 coding Harness 在计划或生成前校验明确 repo、fresh Map、baseline revision 与 active Impact Contract；异常失败关闭，仅允许受限且不可复用的 `map_recovery` 合同自修复。
4. Knife 4 让有头 mutation-capable tool 在动作前验证 live session、lock、receipt、run、repo、branch、worktree 与 base SHA；无头 Dispatcher 使用同一凭证闸，并完整落实 Generator frozen baseline、pushurl 熔断、降权和凭据剥离。
5. Knife 5 先 dry-run 再迁移旧任务，在 scratch 从 API、Intent、Capture 三个真实入口创建 coding mutation，并以 content、research、read-only review 为对照，验证路由、Map、Impact Contract、stale/resume、容器隔离与真实数据库产出。

## 边界情况

- coding 语义不确定时按写入处理；repo 未知或不唯一时失败关闭，禁止默认绑定 Cecelia。
- Map missing/stale、revision 或 scanner 非法、Impact Contract 无法建立时不得创建 Provider attempt；合法恢复仍保留 Structure/Diff Gate、Evaluator、Judge、CI 和 merge fence。
- Routing Receipt 缺失、过期、已 supersede，或 Brain API 不可达、worktree/HEAD 不匹配时，写操作在动作前 exit 2；只读诊断不误伤。
- 只读审查若发现需修改仓库，必须派生新的 coding mutation，不得原地取得写权限。
- 历史运行中 attempt 不切换执行模型；未开始旧任务保留 task id 与原 payload，经审计后新增 receipt 并重路由。

## 范围限定

**在范围内**：恢复前置回归；Knife 0-5；统一 Work Router、四档正向映射、不可变 Routing Receipt、全部建任务入口收敛；Map/Impact Contract 强制门禁；有头/无头动作闸；Generator frozen baseline 与 trust boundary；旧任务迁移、可观测性、Dashboard 审计视图及 scratch 多入口真实验收；每项先 RED commit 再 GREEN commit。

**不在范围内**：把全部内部 task type 改造成平级顶层入口；让纯只读审查进入完整 Harness；重写 Harness 内部状态机；人工维护 Map；修改运行中旧 attempt 的执行模型；以数据库 trigger、CI 或 merge fence 取代动作前主闸；保留或新增 Harness 1.0/`legacy_exempt` 逃生路径。

## 假设

- [ASSUMPTION: task payload 的 `anchor.step_id=3bf6c116-169c-46ec-bc7c-b335a22f80ec` 是本 Sprint 的 Golden Path 锚点。]
- [ASSUMPTION: `map_scope=cecelia` 已给出，但 `map_repo` 未配置；Planner 不猜测 repo 映射，Generator 开始前必须由路由合同补齐并验证。]
- [ASSUMPTION: 任务显式 `target_environment=local_api` 优先于路径推断；Dashboard 浏览器验收作为本地辅助子验收，不改变总体验收环境。]

## 预期受影响文件

- `packages/brain/src/`：统一路由、事务创建、入口收敛、Dispatcher 与 Kernel Map/Impact Contract 门禁。
- `packages/brain/migrations/411_work_routing_receipts.sql`：不可变 Routing Receipt 与受限恢复合同。
- `packages/engine/hooks/dev-mode-tool-guard.sh`、`packages/engine/skills/dev/scripts/worktree-manage.sh`：有头动作前 receipt/lock 验证。
- `docker/cecelia-runner/entrypoint.sh`：Generator frozen baseline、origin 归一化/脱敏、工作区保护与 trust boundary。
- `apps/dashboard/src/pages/warroom/WarRoomPage.tsx`：路由、Map 与 Impact Contract 审计视图。
- `packages/brain/scripts/smoke/unified-work-router-smoke.sh`：scratch 多入口真实验收。
- 对应单元、集成、Shell、容器及回归测试文件：每项先证明 RED，再实现 GREEN，并永久保留。
...
- 频控: 待定（PrepPRD 未指定）
- 版本要求: Kernel Harness 2.0；路由枚举、reason code 与 router version 必须稳定且可审计
- 可观测: 记录 `work_routed`、`work_route_blocked`、`route_violation`、Map/Impact Contract 与迁移事件；Git origin 与日志不得泄露凭据
...
- [ learning] learning: judge FAIL 先区分「证据压缩窗口截断」与「实现缺陷」：evidence_insufficient 时优先走 evaluator 补证轮（behavior_tests 扩容）而非改代码，避免对正确实现无谓返工 judge FAIL 先区分「证据压缩窗口截断」与「实现缺陷」：evidence_insufficient 时优先走 evaluator 补证轮（behavior_tests 扩容）而非改代码，避免对正确实现无谓返工（来源: area）
- [ learning] learning: 合同里的验证命令必须实跑确认 exit code 语义：vitest 对 include 范围外路径（如 sprints/**）绿态也 exit 1，写进合同前先跑一次 合同里的验证命令必须实跑确认 exit code 语义：vitest 对 include 范围外路径（如 sprints/**）绿态也 exit 1，写进合同前先跑一次（来源: area）
- [ learning] learning: judge 证据消费窗口为前 8 条 × 600 字符，evaluator 产 .brain-result.json 必须把一手证据（root-cause 输出、Red→Green 时序、exit_code 字段）排序进窗口前列，否则会因证 judge 证据消费窗口为前 8 条 × 600 字符，evaluator 产 .brain-result.json 必须把一手证据（root-cause 输出、Red→Green 时序、exit_code 字段）排序进窗口前列，否则会因证据截断被误打回（来源: area）
- [ learning] learning: 指标口径类告警先查口径三源失真（未接线恒空子指标、守卫自产回流自噬、双重计数）再当真实退化处理：m2 欠账 +5 实为冒烟噪声，真库 debt 462→288-290 指标口径类告警先查口径三源失真（未接线恒空子指标、守卫自产回流自噬、双重计数）再当真实退化处理：m2 欠账 +5 实为冒烟噪声，真库 debt 462→288-290（来源: area）
- [ learning] learning: 毕业步与 canonical 不可变 lint 存在结构性矛盾，涉 canonical 文件的收尾 commit 前先核对不可变清单（issue 8d2a9ff2） 毕业步与 canonical 不可变 lint 存在结构性矛盾，涉 canonical 文件的收尾 commit 前先核对不可变清单（issue 8d2a9ff2）（来源: area）
- [ learning] learning: controller 台账 .harness/progress.md 必须保持在 git 追踪之外，否则随 sprint PR 带入 repo 造成跨任务污染（issue 78016a5f） controller 台账 .harness/progress.md 必须保持在 git 追踪之外，否则随 sprint PR 带入 repo 造成跨任务污染（issue 78016a5f）（来源: area）
- [ learning] learning: judge 机械闸⑤（meta_verification_gap）对 local_api/无 UI smoke 任务会死锁：此类任务需在合同预先声明验证真相形态或对闸⑤放行（issue 0f586765） judge 机械闸⑤（meta_verification_gap）对 local_api/无 UI smoke 任务会死锁：此类任务需在合同预先声明验证真相形态或对闸⑤放行（issue 0f586765）（来源: area）
- [ learning] learning: Deploy Preview Environment check 跨 PR 失败是 Brain infra 既有故障（非 required check），功能 PR 遇到时应确认既有性并单独立案，不在功能 PR 里追修 Deploy Preview Environment check 跨 PR 失败是 Brain infra 既有故障（非 required check），功能 PR 遇到时应确认既有性并单独立案，不在功能 PR 里追修（来源: area）
- [ learning] learning: 高频合并 repo（如 cecelia）上 update-branch 后应立即挂 gh pr merge --auto 抢竞态，避免反复 BEHIND re-anchor 高频合并 repo（如 cecelia）上 update-branch 后应立即挂 gh pr merge --auto 抢竞态，避免反复 BEHIND re-anchor（来源: area）
- [ learning] learning: headed 前台点火任务必须在点火时用 Brain 同款 jsonb merge 把 worktree_path 写进 task payload，且路径必须在受控 Harness 根目录（DEFAULT_BASE_REPO/.claude headed 前台点火任务必须在点火时用 Brain 同款 jsonb merge 把 worktree_path 写进 task payload，且路径必须在受控 Harness 根目录（DEFAULT_BASE_REPO/.claude/worktrees/harness-v2/）内，否则 judge API 会因 filesystem authority mismatch 返回 409（来源: area）
- [ learning] learning: watchdog 对『从未启动的进程』必须走 never_started 分类兜底且不覆盖已有 error_message/failure_class，防止 process_disappeared→liveness_dead 假标签污染 u watchdog 对『从未启动的进程』必须走 never_started 分类兜底且不覆盖已有 error_message/failure_class，防止 process_disappeared→liveness_dead 假标签污染 urgent 学习流（来源: area）
- [ learning] learning: relay 单session 模式必须在各 phase 完成时调 POST /api/brain/harness/phase-event 写 node 级 done 事件并推进 run.phase，否则 finalize 收账闸报 no_e relay 单session 模式必须在各 phase 完成时调 POST /api/brain/harness/phase-event 写 node 级 done 事件并推进 run.phase，否则 finalize 收账闸报 no_evaluator_gate/pr_not_found 降级、harness/complete Dashboard 更新被拒，report 棒被迫手工补账（来源: area）
- [ learning] learning: PR 处于 CONFLICTING 状态时 GitHub 静默不触发 pull_request CI：不要按 CI 卡死空等，先 merge main 解冲突再等 CI PR 处于 CONFLICTING 状态时 GitHub 静默不触发 pull_request CI：不要按 CI 卡死空等，先 merge main 解冲突再等 CI（来源: area）
- [ learning] learning: capture_atoms urgent 路由建任务前必须按锚点/探针坐标查重：同根因已有 open 任务时合并而非裂变新单（实证：a6e6afc7 与 78e812c0 同 m7 探针双修复撞车，合流成本 5 轮 CI fix 中占 2  capture_atoms urgent 路由建任务前必须按锚点/探针坐标查重：同根因已有 open 任务时合并而非裂变新单（实证：a6e6afc7 与 78e812c0 同 m7 探针双修复撞车，合流成本 5 轮 CI fix 中占 2 轮）（来源: area）
...
- [ learning] learning: evaluator 临时脚本必须落会话独享路径（含 session id），禁止共享 /tmp 固定文件名——并发 sprint 互踩已实证导致首跑 FAIL evaluator 临时脚本必须落会话独享路径（含 session id），禁止共享 /tmp 固定文件名——并发 sprint 互踩已实证导致首跑 FAIL（来源: area）
...
```bash
# 占位：proposer 将按 local_api 填入真实脚本（真实 PostgreSQL + 临时 Git repo + 容器命令链）
# 期望验收点：API、Intent、Capture 三入口的 coding mutation 均产生不可变 receipt、进入 Harness、读取正确 fresh Map 并建立 active Impact Contract；stale Map 阻断后刷新可恢复且审计保留；content/research/review 对照不误路由；有头/无头写操作均在动作前受同一 receipt 合同约束；Generator 看不到 callback/lease 凭据且无法 push，获批 ref 仅由受信任 transport 发布。
# 必须保留并实跑：node scripts/facts-check.mjs
# 必须保留并实跑：bash scripts/check-version-sync.sh
# 必须保留并实跑：node packages/quality/scripts/devgate/check-dod-mapping.cjs
# 必须保留并实跑：bash packages/brain/scripts/smoke/unified-work-router-smoke.sh
```

## journey_type: autonomous
## journey_type_reason: 核心入口与执行门禁位于 packages/brain、packages/engine 和 runner 后端链路，Dashboard 仅为审计视图。
## target_environment: local_api
## target_environment_reason: task payload 显式指定 local_api；在本地 Brain API、真实测试 PostgreSQL、临时 Git repo 与 runner 容器完成主验收。
