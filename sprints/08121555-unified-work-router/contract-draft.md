...
- R1-1：已重写四个锁定测试，删除 optional helper、内存 Map 对象和 contract assert mode；测试现在创建 bare Git remote/真实 clone/detached cwd，直接查询 PostgreSQL receipt 链，持久化 stale Map 后查询 Provider attempt 行数，并真跑 Engine hook、Dispatcher 相邻模块与 runner 子进程。quote: `测试现在创建 bare Git remote/真实 clone/detached cwd，直接查询 PostgreSQL receipt 链，持久化 stale Map 后查询 Provider attempt 行数`。这些 oracle 在被改接缝未实现时必然失败，因此关闭“只检查导出符号或常量”的缺口。

Round 1 的 R1-2、R1-3、R1-4 已由 Round 2 Reviewer 裁定 closed，本轮不扩展其范围。
...
**验证命令**: `npx vitest run sprints/08121555-unified-work-router/tests/worktree-origin-safety.test.ts --reporter=verbose && cd packages/brain && npx vitest run src/__tests__/harness-worktree-origin-safety.test.js --reporter=verbose`
**硬阈值**: 锁定测试必须创建 bare remote、真实 clone 和 detached active cwd；同一 cwd 被复用、secret 日志命中 0、目录与 Git worktree 均仍有效；产品回归测试同步通过。
...
**验证命令**: `DB_URL="$DB_URL" npx vitest run sprints/08121555-unified-work-router/tests/routing-receipt.integration.test.ts --reporter=verbose && cd packages/brain && DB_URL="$DB_URL" npx vitest run src/__tests__/work-router.test.js src/__tests__/integration/work-routing-store.integration.test.js`
...
**验证命令**: `DB_URL="$DB_URL" npx vitest run sprints/08121555-unified-work-router/tests/map-action-gates.integration.test.ts --reporter=verbose && cd packages/brain && DB_URL="$DB_URL" npx vitest run src/orchestrator/preflight/map-impact-contract.test.js src/orchestrator/__tests__/map-recovery-contract.test.js`
...
|---|---|---|---|
| 恢复前置 | `tests/worktree-origin-safety.test.ts` | 含凭据 origin 等价且日志脱敏、活跃 Kernel cwd 不被删除 | bare remote/clone 后因缺 active registry/canonicalizer 而失败 |
| 路由与四档 | `tests/unified-work-router.test.ts` | 四档 change_kind 只正向映射、coding unknown 按 write | 模块不存在而失败 |
| 数据与入口 | `tests/routing-receipt.integration.test.ts` | task 与 Routing Receipt 原子创建且 append-only、入口委托统一边界 | 真 PostgreSQL 表/store 不存在而失败 |
| Map 与动作闸 | `tests/map-action-gates.integration.test.ts` | stale Map 在 Provider 前失败、有头无头 receipt 均在动作前验证 | 真 DB Map header、hook 或 Dispatcher 相邻模块未接线而失败 |
| 信任边界 | `tests/generator-trust-boundary.test.ts` | Generator 不持有 push callback lease 凭据 | runner 真子进程未形成四项真实回执而失败 |
