...
|---|---|---|---|
| Recovery Addendum | `tests/unified-work-router-contract.test.ts` | credential-bearing origin 归一化且日志脱敏 | canonicalizer/cleanup 合同尚未齐备 |
...
| Generator 边界 | `tests/unified-work-router-contract.test.ts` | Generator 环境剥离 callback 与 lease 凭据 | 全量 Generator 隔离尚未接线 |
