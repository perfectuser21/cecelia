...
- 冻结实现基线（权威值）：`22a62578f0aab77c58e1e0be25a6c321a78b35ad`，来源为本 attempt 的 `workspace_spec.base_sha` 与 `planner_head_sha`。task payload 中遗留的 `310ab9e...` 是修订前输入投影，不得进入本轮 Receipt/Map/Impact oracle；上一轮合同提交 `aa148cf...` 只是合同工件 SHA，也不是实现基线。该权威基线是血统与治理证据基点，不是完成态 HEAD。
...
[恢复前置 RED/GREEN] → [统一路由与不可变凭证] → [全入口收敛] → [Map/Impact Contract] → [有头/无头动作闸] → [Generator 信任边界] → [旧任务迁移] → [scratch 多入口、派生修复与观测真验]
...
### Step 7: 旧任务 dry-run、迁移与历史保留
**来源**: `[FROM_PRD]` — PRD §13、Knife 5。

**可观测行为**: dry-run 输出 queued/blocked/paused coding 分类、repo 解析率和阻塞清单但不写库；真实迁移保留 task id/原 payload 并追加 receipt，无法解析 repo 的任务阻塞；运行中 attempt 只写 `legacy_execution_audit`，不改执行模型。

**验证命令**:
```bash
cd packages/brain && DATABASE_URL="$DB_URL" npx vitest run src/__tests__/work-routing-migration.integration.test.js --reporter=verbose
```
**硬阈值**: dry-run 前后业务表 checksum 相等；未开始样本迁移 3/3 且历史字段逐字不变；运行中样本 1/1 task type/attempt 不变且新增本轮审计记录。

### Step 8: scratch 多入口、review 派生修复与可观测性真实验收
**来源**: `[FROM_PRD]` — PRD §16.6、Knife 5。

**可观测行为**: API/Intent/Capture 三个 coding 请求均有 receipt、Harness run、正确 repo Map 与 active Impact Contract；content/research/read-only review 对照不误入，且该 review 产生修复结论后派生一个新的 coding_mutation 子任务并进入 Harness；stale Map 阻断后刷新可 resume 并保留审计；规定事件、指标与 Dashboard 审计视图展示真实 receipt/Map/Impact 状态。

**验证命令**:
```bash
bash packages/brain/scripts/smoke/unified-work-router-smoke.sh
```
**硬阈值**: 直接 coding 入口 3/3、review 派生修复 1/1 进入 Harness；对照 3/3 不误入；七类事件均可按本轮 task id 查询；新 coding dev 直接派发=0、新 legacy_exempt=0、receipt coverage=100%、Map 查询率=100%；Dashboard 组件测试断言 work kind/Pipeline/repo/Map/Impact/route reason/阻塞 Gate 全部可见。
...
psql "$DB_URL" -v baseline="$BASELINE_SHA" -v started="$STARTED_AT" -tAc "SELECT count(*) FROM work_routing_receipts r JOIN tasks t ON t.id=r.task_id WHERE r.created_at >= :'started'::timestamptz AND r.work_kind='coding_mutation' AND r.pipeline='harness' AND r.canonical_task_type='harness_initiative' AND r.evidence->>'base_sha'=:'baseline'" | grep -qx 4
psql "$DB_URL" -v started="$STARTED_AT" -tAc "SELECT count(*) FROM work_routing_receipts WHERE created_at >= :'started'::timestamptz AND work_kind='coding_mutation'" | grep -qx 4
cd packages/brain
DATABASE_URL="$DB_URL" npx vitest run src/__tests__/work-routing-migration.integration.test.js src/__tests__/work-routing-observability.test.js --reporter=verbose
cd ../..
npx vitest run apps/dashboard/src/pages/warroom/WarRoomPage.test.tsx --reporter=verbose
...
|---|---|---|---|
| Recovery + Router + Gate | `tests/unified-work-router-contract.test.ts` | `credential-bearing origin 不得误判 orphan`；`冻结 baseline 必须是最终 HEAD 祖先`；`四种 change_kind 正向映射`；`coding mutation 必须原子创建 receipt`；`Map 与 Impact Contract 必须锚定 baseline`；`Generator trust boundary 必须 fail closed`；`review 派生修复必须进入 Harness`；`旧任务迁移必须保留历史并豁免运行中 attempt`；`路由事件指标与 Dashboard 审计必须可观察` | 当前缺少统一路由模块、receipt migration、迁移/观测接口与完整门禁而失败 |
