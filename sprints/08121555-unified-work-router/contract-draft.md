...
- 实现基线（唯一 SSOT）：`dd0dffac1774d92d8080ff4a4524e0ae8359d530`，来源为 task bundle 的 `inputs.implementation_baseline`。本角色 checkout 恰好位于同一 SHA；后续角色的 `workspace_spec.base_sha` 仍只选择角色 checkout，绝不得替换该实现基线。
- contract-gate: enabled (`packages/brain/src/lib/contract-gate.js` exists)
- gp-anchor: skipped (product-map.json not found)
- `[MAP_NOT_CONFIGURED]`：任务 payload 的 `map_scope` 为数组且 `map_repo` 缺失，无法按 Unified Map API 合同计算 radius；`must_run_assertions=[]`。不得回退到领域硬编码或伪造 freshness/fact revision；真实 Controller 验收必须证明 Router 在运行时取得 receipt.repo 对应的 fresh Map。
...
- Round 1 无上一轮 reviewer 案卷。当前冻结实现基线已包含历史 RED/GREEN 提交；本合同要求真实 Controller→Generator→Evaluator→Judge r11 运行以服务端机械闸字面 `PASS` 收口，并以候选产出和逐角色 provenance 作为验收真相。
...
| Knife 3 | `test(brain): require map governed harness preflight` | `feat(brain): enforce map governed four-form harness runs` | `cd packages/brain && DB_URL="$DB_URL" BASELINE_SHA=dd0dffac1774d92d8080ff4a4524e0ae8359d530 npx vitest run src/orchestrator/__tests__/change-kind-profiles.test.js src/orchestrator/preflight/map-impact-contract.test.js src/orchestrator/__tests__/map-recovery-contract.test.js --reporter=verbose` |
...
**验证命令**: `BASELINE_SHA=dd0dffac1774d92d8080ff4a4524e0ae8359d530 cd packages/brain && npx vitest run src/orchestrator/__tests__/change-kind-profiles.test.js src/orchestrator/preflight/map-impact-contract.test.js src/orchestrator/__tests__/map-recovery-contract.test.js --reporter=verbose`
