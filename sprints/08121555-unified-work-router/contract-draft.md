...
- 实现基线（唯一 SSOT）：`dd0dffac1774d92d8080ff4a4524e0ae8359d530`。本角色 checkout 仅选择起草工作区，不得替换该冻结基线，也不得把起草角色身份写入未来验收合同。
...
- Round 1 初稿：冻结 checkout 已包含 `tests/unified-work-router-contract.test.ts`；本轮必须实跑并保留真实 RED。TDD 账本的七行均为可从仓库根目录直接复制执行的完整命令，历史验证器必须逐 SHA 使用这些精确命令复跑。
...
**验证命令**: `cd packages/brain && BASELINE_SHA=dd0dffac1774d92d8080ff4a4524e0ae8359d530 npx vitest run src/orchestrator/__tests__/change-kind-profiles.test.js src/orchestrator/preflight/map-impact-contract.test.js src/orchestrator/__tests__/map-recovery-contract.test.js --reporter=verbose`
