...
const fromRepoRoot = (path: string) => pathToFileURL(resolve(process.cwd(), path)).href;

describe('Unified Work Router [BEHAVIOR] contract', () => {
  it('归一化含凭据 origin 且不泄露 secret', async () => {
    const mod = await import(fromRepoRoot('packages/brain/src/orchestrator/harness-worktree-recovery.js'));
  ...
  it('只接受四种 change_kind 正向映射', async () => {
    const { CHANGE_KINDS, selectPipeline } = await import(fromRepoRoot('packages/brain/src/work-router.js'));
  ...
  it('coding mutation 强制 receipt 与 required Impact Contract', async () => {
    const { routeWork } = await import(fromRepoRoot('packages/brain/src/work-router.js'));
  ...
  it('Generator Provider 不获得发布与回调能力', async () => {
    const mod = await import(fromRepoRoot('packages/brain/src/orchestrator/generator-trust-boundary.js'));
