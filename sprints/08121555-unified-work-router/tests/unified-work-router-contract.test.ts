...
describe('Unified Work Router contract [BEHAVIOR]', () => {
  ...
  it('review 派生修复必须进入 Harness', async () => {
    const module = await import('../../../packages/brain/src/work-routing-store.js');
    expect(module.createRoutedChildTask).toBeTypeOf('function');
  });

  it('旧任务迁移必须保留历史并豁免运行中 attempt', async () => {
    const module = await import('../../../packages/brain/src/work-routing-migration.js');
    expect(module.dryRunLegacyRoutingMigration).toBeTypeOf('function');
    expect(module.migrateLegacyRoutedTasks).toBeTypeOf('function');
  });

  it('路由事件指标与 Dashboard 审计必须可观察', async () => {
    const module = await import('../../../packages/brain/src/work-routing-observability.js');
    expect(module.getWorkRoutingMetrics).toBeTypeOf('function');
  });
});
