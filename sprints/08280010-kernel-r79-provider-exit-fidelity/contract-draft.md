# Sprint Contract Draft (Round 2) — 结构化上报保真透传，根除 provider_exit 语义埋没 [r79]
...
| F1 永久回归（补充） | `tests/gp/f1/step3-provider-exit-fidelity.test.js` | 保真透传：结构化 BLOCKED 的 CONTRACT_* / 合同故障重开 GAN / 归因口径：CONTRACT_* | baseline 2 failed \| 3 passed（同族 RED 补充锁） |
...
>
> **Round 2 修订（封印闸 closure）**：Round 1 被 seal 拒 `FROZEN_CONTRACT_TEST_CONTRACT_UNRESOLVABLE`——F1 行「BEHAVIOR 覆盖」名 `保真透传 CONTRACT_*` / `归因口径排除 CONTRACT_*` 不是 F1 测试文件对应 `it()` 名的连续子串（seal 双向小写子串匹配失败）。本轮把该两名改为真实 `it()` 名的连续子串：`保真透传：结构化 BLOCKED 的 CONTRACT_*`（子串于 `runner 保真透传：结构化 BLOCKED 的 CONTRACT_* 码不被埋没为 provider_exit`）、`归因口径：CONTRACT_*`（子串于 `归因口径：CONTRACT_* 不落 GENERATOR_RUNTIME_ERROR_CODES`）；`合同故障重开 GAN` 原已可解析（子串于 `kernel 分流：CONTRACT_* 家族 → 合同故障重开 GAN，不进 failed_targets`）。冻结行三名不变（Round 1 已全部可解析）。测试文件本体、被改接缝、DoD、task-plan 一字未改。
