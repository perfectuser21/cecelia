...
- [ ] [ARTIFACT] 冻结合同测试存在且真 import 被改模块（derive + ground-truth + 真 bash 抽取 entrypoint.sh 函数）
...
- [ ] [ARTIFACT] F1 永久回归测试存在（tests/gp/f1，避让既有 step3-* 文件名）
  Test: node -e "require('fs').accessSync('tests/gp/f1/step3-provider-exit-fidelity.test.js')"

- [ ] [ARTIFACT] ground-truth.js 导出 GENERATOR_RUNTIME_ERROR_CODES（归因口径可锁）
...
- [ ] [ARTIFACT] 版本四处同步（packages/brain/package.json / package-lock.json / .brain-versions / DEFINITION.md）
  Test: manual:bash -c 'bash scripts/check-version-sync.sh'

## BEHAVIOR 条目

- [ ] [BEHAVIOR] [L2] B-01: runner 保真透传结构化 BLOCKED 的 CONTRACT_* 错误码（r69 复刻）[接缝×2]
...
- [ ] [BEHAVIOR] [L2] B-02: runner 认可 commander-directive/v1 成功信封（r77 复刻）[接缝×2]
...
- [ ] [BEHAVIOR] [L2] B-03: kernel CONTRACT_* → 合同故障重开 GAN 路径，不进 failed_targets
...
- [ ] [BEHAVIOR] [L2] B-04: 归因口径 GENERATOR_RUNTIME_ERROR_CODES 排除 CONTRACT_* 家族
...
- [ ] [BEHAVIOR] [L2] INV-1: [负向不动] 真崩溃无结构化产出 → 仍 provider_exit，kernel 仍走 infrastructure 有界重派
...
- [ ] [BEHAVIOR] [L2] INV-3: [RED先行] F1 永久回归全绿（修复后永久保留在 CI 作回归，真 import 被改边）
