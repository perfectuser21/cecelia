...
|---|---|---|---|
| judge_feedback 注入两态 + 截断 | `packages/brain/src/orchestrator/__tests__/dispatcher-judge-feedback.test.js`（**红证据测量落点 = 此 CI 路径**；GAN 冻结原文见 `${SPRINT_DIR}/tests/dispatcher-judge-feedback.test.js`，逐字一致，import 为 `../dispatcher.js`，仅从此 `__tests__/` 落点解析正确） | `run 存在 judge FAIL verdict 时注入`；`只注入 hop 最大的最近一次`；`failure_class 非 evidence 类仍注入`；`不注入 judge_feedback 字段`；`judge 判 PASS 时不注入`；`超长 judge summary 截断后 bundle 不越 256KB` | 当前代码 4 failed \| 2 passed（注入相关 4 条 `judge_feedback undefined` 红；无 judge/PASS 两条守卫绿）|

> 「BEHAVIOR 覆盖」列每个名均为对应 `it()` 名的字面子串（可 `grep -F` 命中）。**红证据测量落点是 CI 路径 `packages/brain/src/orchestrator/__tests__/`（非 sprint tests/）**：冻结测试 import 写作 `../dispatcher.js`（对齐同目录既有 `dispatcher.test.js:8`），只从 `__tests__/` 解析到 `packages/brain/src/orchestrator/dispatcher.js`；sprint `tests/` 目录仅存档冻结原文，不在该目录跑。红证据实测（proposer 在 CI 落点验证）：`cp` 冻结原文到 `packages/brain/src/orchestrator/__tests__/dispatcher-judge-feedback.test.js` → `cd packages/brain && npx vitest run src/orchestrator/__tests__/dispatcher-judge-feedback.test.js` → `Tests 4 failed | 2 passed (6)`（4 条 `judge_feedback undefined` 断言红、2 条 `not.toHaveProperty` 守卫绿，无 module-not-found error）。
