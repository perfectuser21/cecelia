# 发版验收一体两面——F2 步2 加厚「一张表两列背靠背合着看」Golden Path v6

提案人：Cecelia（AI）。v6 = 对 solo 复审 r5（`.harness/verdicts/gp-r5-solo.json`）的逐条修订轮：**0 P0 / 1 P1 全部核销，0 REFUTE，5 P2 全部核销，P2 记账累计 24 条**。r2/r3/r4 的台账留痕在 `proposal-v3.md` / `proposal-v4.md` / `proposal-v5.md`，本版不重抄。
...
- **现状依据**：`.harness/explore-report.md` + v2/v3 实测 + v5 为核销 r4 findings 新增的直接读码/解析。**v6 是文本级收口轮，不新增现状标注**——v5 的全部现状结论已被 r5 独立复核逐条确认（见 `gp-r5-solo.json` 的 `code_verifications_this_round` 8 条，含 yaml 全表 56 格/建行 36/hard 8、`cells-map.mjs` 20 格与 6 个 `scenario_required`、`acceptance.js:88/174`、`369_acceptance_tables.sql:11` 等）

---

## 0. 相对 v5 的结构性变化

> v6 是**收口小修轮**：r5 判 0 P0 / 1 P1 / 5 P2，全部是文本级冲突与算术口径，无一条要求改结构。故本版不动 Gate A/B、不动九组合矩阵的骨架、不动 J1-J20 的 REC，只把互相打架的两处措辞对齐到同一个语义上。

| # | 变化 | 回应 |
|---|---|---|
| 1 | **A17④ 与闸② 的定义域冲突消解（以闸② 为准）**：v5 的 A17④ 断言「AI `scenario_not_triggered` + 人列通过 + 员工未勾该场景 → 绿（Q3）」，与闸②（人列给确定判定即证伪 → 拦）**定义域完全重合而期望相反**。本版把 A17④ 的适用域改写成正面定义——只覆盖 `verifiable_by='human_only'` 且 `scenario_class ∉ {opportunistic, unverifiable_this_version}` 的格；另拆出 **A17④′**（`opportunistic` 格的绿路径唯一 = **Q1 双绿**，证明双闸没把这 3 格锁死）与 **A17④″**（`opportunistic` 格 + 人列通过 + 员工未勾 → 必为 Q3″ 未定，**不得绿**）。同步改：§分类表第二行「享受 Q3 绿通道」列由「是」改「**否**」、九组合表 Q3 行写死定义域、§台账把 S10-c4 的「两条路都拦」上提为 3 个 `opportunistic` 格的**通则** | → r5-P1-1 |
| 2 | **Q3″ 收窄为「人列 = 通过」一种**：人列 = 不通过 时不再降级成「未定待裁决」，仍走 **Q6（红 + 自动进 bug 任务）**，reason 照样改写 `scenario_falsified` 并计入采证失准计数。人列独判本就足以定红，降级反而放松，且会让 A7 的 bug 任务分母漏格 | → r5-P2-3 |
| 3 | **口径切换表脚注算术修正**：Gate B 第6条通过时的增量由「`opportunistic` +1」改为「**`opportunistic` 不变（仍 3）、故障类 +1**」——S13-c4 升的是 `mandatory` 不是 `opportunistic` | → r5-P2-1 |
| 4 | **J17 对照表 B 列上限 20 → 19**：S13-c4 的不可制造与选哪个候选无关（超频发送在 B 之下同样违反 S12-c2），故 B 的 `machine_db` = 19、上限 19、常态 19−3 = **16**（常态数字与「13 vs 3 vs 16」的结论均不变） | → r5-P2-2 |
| 5 | **`unverifiable_this_version` 格的裁决绿不自动开 P0**：改为**只计数进 A12 第四项 + run 单头注记**；自动 P0 留给真正意外的 hard 格裁决绿。理由：S13-c4 本版每轮必走裁决绿，会固定产出内容相同的 P0，几轮后这条规则必被整体关掉，而它同时保护着另外 7 个 hard 格 | → r5-P2-4 |
| 6 | **A10 补「新 run 不得再被写成 `passed`/`failed`」的正向断言**，并在 D1 明写 `acceptance.js:88` 的旧三元式必须与状态机改造**同批替换** | → r5-P2-5 |

---

## R5 核销台账

> 处置只有两种：**核销@节名**（提案已改）或 **REFUTED+证据**。本轮 **1 条 P1 + 5 条 P2 全部核销，0 条 REFUTE**。r5 同时复核了 v5 对 r4 的 7 条核销，**全部裁 RESOLVED**，无退回项。

| id | 处置 |
|---|---|
| **r5-P1-1**（冻结断言 A17④ 与闸② 定义域重合而期望相反——照 A17④ 实现即在 S5-c4/S10-c4 两个 hard 红线格上重新打开 r4-P1-2 要堵的假绿口） | **核销@A17④ 重写并拆三条 + §分类表第二行 + 九组合表 Q3 行 + §台账通则段**。finding 属实，且它把矛盾的根挖得比我准：**闸① 保证任何能存活到 `human_complete` 的 `scenario_not_triggered` 必然出自「员工未勾」**（勾了就 400 拒收），所以闸② 的定义域 = A17④ 的定义域，二者不可能同时为真——A17④ 不是「少了一个例外」，是**整条与闸② 互斥**。我按 finding 的定夺方向以**闸② 为准**，并在两种修法里选了「定义最干净」的那种：**不在原断言上挂但书，而是把适用域整体改写成正面定义**——④ 的构造从「AI `scenario_not_triggered` + 人列通过」换成「`verifiable_by='human_only'` 且 `scenario_class ∉ {opportunistic, unverifiable_this_version}` 的格 + AI `reason='human_only'` + 人列通过」。理由是挂但书会留下一条字面仍是 `reason='scenario_not_triggered' → 绿` 的冻结断言，但书是注脚、容易被漏读；改成正面定义之后，**`scenario_not_triggered` 这个 reason 在全文再无任何通向绿的断言**，机械上一目了然。**并且我比 finding 多做一步**：finding 只要求「删除或改写 A17④」，但删掉它会连带删掉「Q3 绿通道仍然活着」这件事的唯一正向证据——一条只有拦、没有绿的规则集，实现期第一次撞上就会被当成过闸太严而整体放松。故本版拆成三条：④ 守住 `human_only` 的合法绿、**④′ 守住 `opportunistic` 格的绿路径确实存在（Q1 双绿）**、④″ 堵死假绿。finding 自己也写了这一点（「opportunistic 格并未被死锁——场景真发生时员工勾选 → AI 被闸① 逼出确定判定 → Q1 双绿」），④′ 就是把这句话变成可执行断言。代价按 finding 要求明示在 §台账通则段：场景未发生的轮次这 3 格每轮各需一次处置，这正是 J20 呈批项 1（`op` 加厚）的紧迫性来源 |
| **r5-P2-1**（口径切换表脚注把 S13-c4 的增量写成 `opportunistic` +1，与 Gate B 第6条/J20 的 `mandatory` 矛盾） | **核销@§口径切换表脚注**：改为「`human_only` −1 / `machine_db` +1 / **故障类 +1** / 上限 +1，`opportunistic` **不变（仍 3）**」，并把 finding 指出的后果写进脚注正文（写成 `opportunistic` +1 会让实现期把 S13-c4 放进 `opportunistic` 集合，红线8 又长出一条 `scenario_not_triggered` 声明口） |
| **r5-P2-2**（J17 对照表 B 列上限 20 未扣 S13-c4） | **核销@J17 对照表 + 常态口径注**：B 列上限 20 → **19**，并写明理由——候选 B 之下时限三格可由采证器自行计时故回 `machine_db`，但 S13-c4 的不可制造**与候选无关**（超频发送在 B 之下同样违反 S12-c2）。常态 16 = 19−3 不变，「13 vs 3 vs 16」的结论不翻 |
| **r5-P2-3**（Q3″ 覆盖「人列不通过」时把该格从 Q6 降级为未定，A7 的 bug 任务分母漏格） | **核销@闸② 定义 + §机械化落点 + 九组合表 Q3″ 行 + A17② + 出错路径表**。finding 给了两条路，我取**第二条（让 Q3″ 在人列 = 不通过时直接判红）**而不是第一条（A7 补一句「裁决为红后并入同一 bug 任务」）：第一条要在 bug 任务已经建完之后再回头补格号，等于给聚合式建单加一条时序依赖，而 A7 的谓词是「每 run ≤1 bug 任务」——裁决晚于建单，补格就得改已建任务的描述，实现期极易退化成「裁决完再手工加一句」。第二条是纯判据收窄：**人列不通过时不存在假绿风险**（AI 那一列无论说什么都不会让格变绿），红是比「未定待裁决」更强的处置，降级毫无收益。`scenario_falsified` 的 reason 改写与采证失准计数**照旧执行**，只是最终态走 Q6，故 P2-22 的横向观测不受影响 |
| **r5-P2-4**（S13-c4 每轮必走裁决绿 × 「hard 格裁决绿自动开 P0」= 每轮固定一个内容相同的 P0，几轮后规则必被关掉） | **核销@Step 6 挂片 + A12 + D4**：给 `unverifiable_this_version` 格的裁决绿单独一条处置——**只计数进 A12 第四项 + run 单头注记，不开 P0**；自动 P0 留给真正意外的 hard 格裁决绿。finding 的形状判断（同 P2-8「棘轮击穿但无人消费」）成立：一条每轮必然触发的告警等于没有告警，而它同时保护着另外 7 个 hard 格，被一起关掉的代价远大于 S13-c4 本身。根治仍指向 Gate B 第6条，已在处置文本里写死 |
| **r5-P2-5**（状态机扩 7 值但无断言禁止新 run 再被写成 `failed`，`acceptance.js:88` 现行三元式漏改即「员工判出不通过的那一轮永远达不到 `human_complete`」） | **核销@A10⑤ + D1**：A10 补正向断言「构造人列全填满且含『不通过』的 run，读回 `status` 必须 == `'human_complete'`，不得为 `'failed'`」，并补一条「新 run 的 `status` 不得取 `passed`/`failed`」的全表扫描；D1 明写 `acceptance.js:88` 的旧三元式必须与状态机改造**同批替换**。finding 指出的形状（与 r4-P1-1 同类：一个静默的、只在人侧显形的死端）成立——`failed` 达不到 `human_complete`，合看/裁决/员工回显全部 403 |
...
- **闸②（合看期交叉证伪）**——闸① 挡不住「员工漏勾」。而人列本身就是场景发生与否的第二证据：员工若能对该格给出**确定判定（通过/不通过）**，说明他看见了场景。故在 run 达 `human_complete` 那一刻服务端**重算**：凡 AI 声明 `scenario_not_triggered` 而人列为确定判定的格，AI 的 reason 一律被服务端改写为 **`scenario_falsified`** 并归**故障类**；最终态按人列分两支——
  - **人列 = 通过 → 落 Q3″**（未定，拦闸，处置 = 裁决而非重跑）；
  - **人列 = 不通过 → 落 Q6**（**红**，自动进 bug 任务，v6 收窄 ← r5-P2-3）。人列独判本就足以定红，AI 那一列无论说什么都不会让格变绿，此时没有假绿风险；若也降级成「未定待裁决」，反而比不做闸② 更松，还会让 A7 的 bug 任务分母漏掉这些格。reason 改写与「AI 场景采证失准」计数照常执行，只是最终态走 Q6。
...
| **S10-c4** | **是** | 两轮数据覆盖对照（红线11：旧评论不得覆盖新的） | 需同一目标有两轮采集数据；单轮规程下**常态不触发**。员工可勾（他知道自己跑了几次采集），人列可交叉证伪 → 双闸可执行 | `opportunistic`（**建议**把 S10 `op` 加一句「用同一关键词再发起一次采集，对照同一视频评论是否被覆盖」→ 升 `mandatory`；代价 = 仪式跨度再加约 8 分钟，见 P2-24） | 留在 `machine_db`，受双闸。**加厚前的诚实后果**见下方 §`opportunistic` 三格通则（v6 已把本行原有的「两条路都拦」上提为三格共用的通则，← r5-P1-1） |
...
### `opportunistic` 三格通则（v6 提升，← r5-P1-1）

v5 只在 S10-c4 那一行写了「两条路都拦」，而这条结论对 S5-c3 / S5-c4 / S10-c4 **三格一体适用**，且它就是 A17④ 与闸② 打架的根。本版把它写成通则，并明示代价：

**当 AI 声明 `scenario_not_triggered` 时，这 3 格没有任何通向绿的路径**——

| 人列 | 服务端重算结果 | 最终态 | 处置 |
|---|---|---|---|
| 通过 | 闸② 证伪，reason → `scenario_falsified` | 未定（**Q3″**） | 裁决（不重跑） |
| 不通过 | 闸② 证伪，reason → `scenario_falsified` | **红**（Q6） | 自动进 bug 任务 |
| 无法验证 | 不触发闸②（人列非确定判定），reason 保持 `scenario_not_triggered` | 未定（**Q9 双盲**） | 「补验证手段」任务 |
| 未填 | —— | 未定（Q0） | run 到不了 `human_complete` |

**为什么这不是死锁**：这 3 格的绿路径仍在，只是唯一——**场景真发生 → 员工在 Step 2 勾了该场景码 → 闸① 把 AI 的 `scenario_not_triggered` 当场 400 拒收 → AI 必须给确定判定 → 人列通过 + AI 通过 = Q1 双绿放行**。换句话说，双闸不是堵死这 3 格，是**把它们的绿从「AI 单方声明即可」改成「场景真发生且两列都判通过」**。A17④′ 就是这条路径的可执行断言。

**代价明示**：场景**未**发生的轮次，这 3 格每轮各需要一次处置（裁决 / 补验证手段任务），不会静默放行，但也确实占主理人时间。**这正是 J20 呈批项 1（把 S5 / S10 的偶发场景写进 `op` 升为规定动作）的紧迫性来源**——加厚后场景恒发生，三格全部走 Q1 双绿，每轮的固定处置消失。
...
| **合法 · scenario_not_triggered** | 该格 `scenario_class == 'opportunistic'`（**3 格**：S5-c3/S5-c4/S10-c4）**且**闸①（`detail.scenarios_observed[]` 不含该场景码）**且**闸②（`human_complete` 时人列非确定判定）双双成立 | **否**（v6 更正 ← r5-P1-1）。「合法」只表示服务端不 400 拒收、不归故障类，**不表示能绿**：人列非确定判定 → Q9 双盲拦；人列通过 → 闸② 证伪 → Q3″ 拦；人列不通过 → 闸② 证伪 → Q6 红。**这 3 格的绿只有 Q1 双绿一条路**（见 §`opportunistic` 三格通则）。v5 此格写「是」是空真命题——闸② 一旦成立，人列必非确定判定，而人列非确定判定就不可能是 Q3 的「人列通过」 |
...
- `human_complete` 触发时服务端重算（闸②）：`reason='scenario_not_triggered'` 且人列 ∈ (通过,不通过) → reason 改写 `scenario_falsified`、归故障类；**人列=通过 → 落 Q3″（未定）**；**人列=不通过 → 落 Q6（红，进 bug 任务）**（v6 分支 ← r5-P2-3）。
...
> **S13-c4 不参与本切换**（它的开关是 Gate B 第6条，不是第4条）。若第6条也通，在上表基础上再 **human_only −1 / machine_db +1 / 故障类 +1 / 上限 +1，`opportunistic` 不变（仍 3）**。**v6 更正（← r5-P2-1）**：v5 此处写「`opportunistic` +1」是错的——Gate B 第6条与 J20 呈批项 2 都写明 S13-c4 升的是 **`mandatory`**（受控注入让场景变成规定动作）而不是 `opportunistic`，它回 `machine_db` 后属于「非 `opportunistic` 的 machine_db 格」即故障类分母。照 v5 落地会让实现期把 S13-c4 放进 `opportunistic` 集合，红线8 又长出一条 `scenario_not_triggered` 声明口，正好抵消本 GP 对该格的 fail-closed 处置。
...
| Q3 | 通过 | 合法无法验证 | 仅人列绿 | **绿** | 无 | 放行。**定义域写死（v6 ← r5-P1-1）= `verifiable_by='human_only'` 且 `scenario_class ≠ unverifiable_this_version` 的格**；`opportunistic` 格**到不了这一行**——它们的合法无法验证只有 `scenario_not_triggered` 一种，而「人列通过」是确定判定，必被闸② 证伪翻成 Q3″。**例外：`scenario_class=unverifiable_this_version` 的格（S13-c4）不走此通道，绿必须经裁决** |
| Q3′ | 通过 | 故障无法验证 | 人绿·AI 哑火 | 未定 | 重跑打表器；重跑仍哑火→裁决 | **拦** |
| **Q3″**（v5 新增 ← r4-P1-2；**v6 收窄为「人列=通过」 ← r5-P2-3**） | **通过** | `scenario_not_triggered` **被闸② 证伪** | 人绿·AI 场景声明被证伪 | 未定 | **直接裁决，不重跑**（场景已过，重跑取不回来）；并记一次「AI 场景采证失准」计数 | **拦** |
| Q4 | 不通过 | 通过 | 分歧（AI 绿人红） | 未定 | 追查任务（优先信人） | **拦**，除非裁决绿 |
| Q5 | 不通过 | 不通过 | 双红 | **红** | bug 任务 | **拦**（裁决绿需写补偿措施并自动开 P0） |
| Q6 | 不通过 | 无法验证 | 人红独判 | **红** | bug 任务 | **拦**（**v6 扩容 ← r5-P2-3**：AI 列为被闸② 证伪的 `scenario_falsified` 时也落本行，仍是红、仍进 bug 任务，只是额外记一次「AI 场景采证失准」计数） |
...
> **Q3″ 为什么只收「人列=通过」（v6 ← r5-P2-3）**：v5 的 Q3″ 人列列写「通过/不通过」，于是「员工判红 + AI 恰好声明该场景没发生」这类格就从 Q6（红·自动进 bug 任务）降级成「未定·等主理人裁决」，A7 的 bug 任务分母漏掉它们。而人列判红时**根本不存在假绿风险**——AI 那一列无论写什么都不会让格变绿，此时把处置从「自动开 bug」换成「占用一次裁决」是纯损失。故人列=不通过 一律回 Q6；`scenario_falsified` 的 reason 改写与采证失准计数照旧，横向观测（P2-22）不受影响。
...
| **AI 说「本轮没这个场景」，员工却判了**（v5 新增；**v6 分两支 ← r5-P2-3**） | 合看页该格标「AI 说没场景，但你判了」，`ai_evidence.reason` 已被服务端改写为 `scenario_falsified` | **人列=通过 → Q3″**：不重跑（场景已过，取不回来）→ 主理人当场裁决。**人列=不通过 → Q6 红**：照常自动进本轮的 bug 任务，不占裁决。两支都记一次「AI 场景采证失准」计数，同格反复出现则开 P0 查该格采证方法（P2-22） |
...
> **分母口径写死在「定案后」（v6 明确 ← r5-P2-3）**：分流建任务的**触发时点 = run 转 `adjudicated` 之后**（Step 6 收尾），不是 `human_complete` 那一刻。故 A7 的 `final_state='红'` 分母取的是**裁决落定后的终态**——主理人把 Q2/Q3″/Q4/Q7/Q9 里任何一格判红，该格在同一次分流里自然进 bug 任务，不需要回头去改一个已经建好的任务。这条必须写死，否则实现者会在 `human_complete` 时就建单，此后所有由裁决产生的红格永远进不了 bug 分母。**被闸② 证伪且人列判红的格不依赖这条**——它在 `human_complete` 重算时就已是 Q6 红。
...
-- ⑤（v6 新增，← r5-P2-5）新 run 只走 7 值状态机，passed/failed 是只读的历史兼容值
--   ⑤-a 正向：构造一个人列全部 36 格填满、且其中至少一格为「不通过」的 run，读回
SELECT status FROM acceptance_runs WHERE id=:rid;        -- == 'human_complete'，不得为 'failed'
--   ⑤-b 对照：同样构造但人列全部「通过」的 run
...
```
> **为什么这条断言必须有（← r5-P2-5）**：`acceptance.js:88` 现行算法是 `status = pending>0?'in_review':fail>0?'failed':pass===total?'passed':'in_review'`，只要有一格人列「不通过」就把 run 写成 `failed`。D1 若只扩 CHECK 而漏改这一行，**员工判出不通过的那一轮永远达不到 `human_complete`**——合看页、裁决、员工回显（全部以 `human_complete` 为开门条件）一律 403，而闸侧照常跑。故障形状与 r4-P1-1 同类：静默、只在人侧显形、且发生在最需要看两列的那一轮（有格判红的轮次）。A10⑤ 是它的直接回归测试。
...
```
断言：gate summary 打印全部四个计数；构造「近 30 天 bypass >3 次」→ gate 直接 `exit 1`（棘轮生效）；构造「近 30 天 `force_reason` >3 次」→ 同样 `exit 1`（复盘逃生阀不得变日常）。**第四项（`unverifiable_this_version` 格裁决绿）只计数、无阈值**——它每轮必然发生，设阈值等于给发版设死锁。

**（v6 新增，← r5-P2-4）自动 P0 的定义域断言**：
```sql
...
```
> **为什么给它开例外**：S13-c4 在本版是 `unverifiable_this_version`，**每一轮都会走裁决绿**（这是 fail-closed 处置的必然结果，不是意外）。若照「hard 格裁决绿一律自动开 P0」执行，每次发版固定产出一个内容相同的 P0，几轮之后这条规则必被整体关掉——而它同时保护着另外 7 个 hard 格。一条每轮必然触发的告警等于没有告警（形状同 P2-8「棘轮击穿但无人消费」）。故把「自动 P0」留给真正意外的 hard 格裁决绿，S13-c4 走计数 + 注记。**根治仍是 Gate B 第6条**：受控注入一旦探明，该格升 `mandatory` 回 `machine_db`，本例外随之作废（实现期须把这条例外与 `scenario_class` 绑定，不得硬编码格号 `S13-c4`）。
...
**A17 · 场景证据指针与 hard 格保护（v5 新增 ← r4-P1-2；v6 重写 ②④ ← r5-P1-1 / r5-P2-3）**
```sql
...
--    触发 human_complete 后读回。v6 按人列取值分两支断言（← r5-P2-3）
--   ②-a 人列=通过（S10-c4）
...
--   ②-b 人列=不通过（S5-c4，同一 run 内）
SELECT ai_evidence->>'reason', final_state FROM acceptance_checks
WHERE run_id=:rid AND check_key='S5-c4';
-- 断言 reason == 'scenario_falsified' 且 final_state == '红'（Q6）；run 定案后再核一次，
--   该格号必须出现在本 run 的 bug 任务描述里（A7 的建单时点在 adjudicated 之后）
-- ③ 对照组：同格「AI scenario_not_triggered + 人列无法验证」→ reason 保持 scenario_not_triggered，
--    final_state == '未定'（Q9 双盲），同样不得为绿
-- ④ Q3 绿通道的合法定义域（v6 重写 ← r5-P1-1）：
--    适用域 = verifiable_by='human_only' 且 scenario_class ∉ {opportunistic, unverifiable_this_version}
--    构造：S12-c1（需安卓真机，两个口径下恒为 human_only，不随 Gate B 第4条切换）
--          「AI reason='human_only' 无法验证 + 人列通过」
SELECT final_state FROM acceptance_checks WHERE run_id=:rid AND check_key='S12-c1';
-- 断言 final_state == '绿'（Q3）——证明 Q3 绿通道没有被双闸误伤
-- ④′ opportunistic 格的绿路径唯一 = Q1 双绿（v6 新增 ← r5-P1-1）：
--    构造：单头 detail.scenarios_observed 含 S10-c4 的场景码（员工勾了）
--    ④′-a 此时 AI 提交 reason='scenario_not_triggered' → HTTP 400（闸① 拒收，AI 被逼给确定判定）
--    ④′-b AI 改交 ai_verdict='通过' + 人列'通过' → 读回
SELECT final_state FROM acceptance_checks WHERE run_id=:rid2 AND check_key='S10-c4';
-- 断言 final_state == '绿'（Q1）——证明这 3 格没被双闸锁死，绿路径确实存在
-- ④″ opportunistic 格的假绿口已堵死（v6 新增 ← r5-P1-1）：
--    构造：单头 scenarios_observed 不含该场景码（员工未勾）+ AI reason='scenario_not_triggered' + 人列'通过'
SELECT final_state FROM acceptance_checks WHERE run_id=:rid3 AND check_key='S10-c4';
-- 断言 final_state == '未定'（Q3″），**不得为 '绿'**；对 S5-c3 / S5-c4 各跑一遍，三格结果必须一致
...
> **v6 为什么给 A17④ 换适用域（← r5-P1-1）**：v5 的 ④ 断言「AI `scenario_not_triggered` + 人列通过 + 员工未勾该场景 → `final_state == '绿'`（Q3）」，而闸① 保证**任何能存活到 `human_complete` 的 `scenario_not_triggered` 必然出自「员工未勾」**（勾了就在 Step 3 被 400 拒收），于是闸② 的定义域与 ④ 的定义域**完全相等**，而两者期望的终态相反。二者不可能同时成立：要么闸② 是死代码，要么 ④ 恒失败。A 系列是「冻结后 AI 不可改」且按 J13 直接充当各 `/dev` 任务 DoD 的 `[BEHAVIOR]` 来源，实现者照 ④ 写测试就必须把闸② 削弱成「只在员工已勾时触发」——而那正是闸① 的职责，「闸① 挡不住漏勾」的整个立论坍塌，3 个 `opportunistic` 格（含 2 个 hard 红线格 S5-c4/S10-c4）回到 v4 的「AI 单方声明 + 人列通过即绿」形态。
> **修法选型**：可选「给 ④ 加一条排除 `scenario_class ∈ {opportunistic, unverifiable_this_version}` 的但书」，也可选「把 ④ 的适用域整体搬到 `human_only` 格」。**取后者**——加但书会留下一条字面仍是「`reason='scenario_not_triggered'` → 绿」的冻结断言，但书是注脚、容易被漏读；搬走之后 **`scenario_not_triggered` 这个 reason 在全文再无任何通向绿的断言**，机械上一目了然，与 §分类表第二行「否」、九组合表 Q3 行的定义域三处同向。
> **为什么不是直接删掉 ④**：删掉会连带删掉「Q3 绿通道仍然活着」这件事的唯一正向证据。一条只有拦、没有绿的规则集，实现期第一次撞上就会被当成「闸太严」整体放松。故 ④ 保留但换域（守 `human_only` 的合法绿），并补 **④′** 证明 `opportunistic` 三格的绿路径确实存在（场景真发生 → 员工勾 → 闸① 逼出确定判定 → Q1 双绿）、**④″** 堵死假绿口。三条合起来才是完整的定义域划分。
...
- **候选 B 呈主理人拍板（不由 AI 单方决定）**——**v5 按 r4-P2-22 把「上限」与「常态」拆开表述，不再拿上限充常态**：
  ...
  |---|---|---|---|
  | AI 确定判定**上限** | 3 格 | **16 格**（基线）／19（Gate B 第4条三格全通） | **19 格**（v6 更正 ← r5-P2-2） |
  ...
  > **常态 vs 上限的口径（← r4-P2-22；v6 修 B 列上限 ← r5-P2-2）**：「上限」= 该候选之下 `machine_db` 的全部格数，是 A4② 的断言上界；「常态」= 上限扣掉 3 个 `opportunistic` 场景格（S5-c3/S5-c4/S10-c4，场景未触发时本就是合法「无法验证」、不构成确定判定）。
  > **B 的上限是 19 不是 20**：候选 B 允许 AI 自己点火，故时限三格（S4-c2/S7-c2/S9-c2）可由采证器自行计时、回 `machine_db`；但 **S13-c4 的不可制造与选哪个候选无关**——制造「被频控限制」= 超频发送，在 B 之下同样违反 S12-c2 这条验收判据（yaml 原文「同设备10分钟内最多发3条」），换个执行体不会让它变得可制造。故 B 的 `machine_db` = 20−1 = **19**，上限 19，常态 19−3 = **16**。v5 此处写 20 是把「yaml 原始 machine_db 全格数」当成了 B 的上限。
  > A 的 3 格（S1-c3/S6-c4/S11-c4）不含场景格与时限格，不随口径变。**结论不翻：13 vs 3 仍是压倒性差距，13 vs 16 的 3 格差价换掉的是每轮一次真实抖音触达。**
...
- **v6 附注（← r5-P1-1 / r5-P2-3）**：双闸的机制本身不变，本版只把它的**终态口径**与冻结断言对齐到同一个语义上——①`opportunistic` 格在 AI 声明 `scenario_not_triggered` 时**没有任何通向绿的路径**（人列通过 → Q3″ 拦；人列不通过 → Q6 红；人列无法验证 → Q9 拦），其绿只有「场景真发生 → 员工勾 → 闸① 逼出确定判定 → Q1 双绿」一条，见 §`opportunistic` 三格通则与 A17④′/④″；②闸② 证伪后**人列=不通过 落 Q6 红而非 Q3″**，人列独判足以定红，降级成「等裁决」既无收益又漏 A7 的 bug 分母。**这也提高了呈批项 1 的分量**：不加厚 `op`，这 3 格在场景未发生的轮次每轮各要一次处置。
...
AI 四列 migration（J6-A，中文枚举 + CHECK）＋ `check_key` 改规程格号 ＋ `UNIQUE (run_id, check_key)`（J5-A）＋ `submitAcceptanceResults` 全链路加 `run_id` 作用域 ＋ **规程 yaml → 36 格建单生成器**（J10-B 的排除集：`na:true` ∪ `fixedNa` 步骤全格；含 J14 的 kind）＋ **静态属性收口（← r4-P2-23 措辞纠正）**：`scenario_required` 升级为 **`scenario_class`** 三值并**从 `cells-map.mjs` 迁进 yaml**（v5 复核：`cells-map.mjs` 全文有 `scenario_required` 6 处、**无** `verifiable_by`）；**`verifiable_by` 本就在 yaml 里**（36 个建行格全带，human_only 16 / machine_db 20），本项**只按 §口径切换表改 4 格取值**（S4-c2/S7-c2/S9-c2/S13-c4 → `human_only`），**不做迁移** ＋ `backend_sha`/`frontend_sha` 双源对账与 `spec_sha`/`version` 落库（J12）＋ **run 状态机改造**（`369_acceptance_tables.sql:11` 的 CHECK 由 4 值扩到 **7 值**：`pending`/`in_review`/`human_complete`/`adjudicated`/`stale`/`expired`/`abandoned`，`passed`/`failed` 退为历史兼容值；**三个非活跃终态是 status 取值不是 detail 旗标**，A10④ 断言这一点；**（v6 明写 ← r5-P2-5）`acceptance.js:88` 现行的 `status = pending>0?'in_review':fail>0?'failed':pass===total?'passed':'in_review'` 三元式必须与本次 CHECK 扩容同批替换——只扩 CHECK 不改这一行，人列出现「不通过」的 run 就会被写成 `failed`，永远达不到 `human_complete`，合看/裁决/员工回显全部 403；新 run 只允许落 7 值状态机，`passed`/`failed` 变成只读的历史值（A10⑤ 是回归测试）**；`status` 计算改按九组合矩阵算 `final_state` 与 `gate_verdict`，含 **Q0′ 缺格恒判「未定」**、**Q3″ 场景证伪（人列=通过）** 与 **闸② 人列=不通过 落 Q6 红**；`detail.ai_status` 记哑火并实现三条件判据）＋ **pending run 过期扫描器**（48h 未达 `human_complete` → `expired`）与显式作废端点（→ `abandoned`，留痕）＋ **建单期前置校验与逃生阀**（同 gp 上一轮 `review_closed_at` 非空，否则 409；带 `force_reason` ≥20 字则放行并留痕；单头 `tenant_account` ∈ 验收专用租户；单头 `mandatory` 场景码勾齐）＋ `review-closed` / `review-ack` 端点（含主体校验与「全员 ack 或满 24h」前置闸）。
...
九组合矩阵合看页（`apps/staff-hub/src/pages/`，含 device / `scenario_class` 标记 + 第14步灰带 + **Q3″ 与缺格图例**）＋ **闸②：`human_complete` 触发时服务端重算场景交叉证伪**（reason 一律改写 `scenario_falsified`；**人列=通过 落 Q3″ 走裁决不走重跑，人列=不通过 落 Q6 红照常进 bug 任务**，v6 ← r5-P2-3）＋ `adjudication` 裁决 API 与 hard 格裁决绿自动开 P0（**`scenario_class='unverifiable_this_version'` 的格例外：只计数进 A12 第四项 + 写 `detail.unverifiable_adjudicated[]` 注记，不开 P0；例外绑 `scenario_class` 不得硬编码格号**，v6 ← r5-P2-4）＋ **分流建任务的触发时点写死在 run 转 `adjudicated` 之后**（A7 分母口径 = 定案后终态）＋ **`unverifiable_this_version` 格绿必经裁决的强校验** ＋ 员工侧裁决回显视图 ＋ **员工侧「我已看过裁决」ack 与异议 note**、**发起人/主理人侧「关闭复盘」按钮**（v5 ← r4-P1-3；主体分离，员工打 `review-closed` → 403）＋ 侧边栏待办角标与仪式发起通知 ＋ **建单页承接现役网页的表头字段**（测试用客户账号（做成从验收专用租户下拉选择）/手机型号/客户端编号/本轮任务编号/本轮暗号 ＋ **本轮场景清单勾选 `scenarios_observed[]`**）＋ **`lib.mjs` 收编：generate 改产只读判据说明书，去掉三态按钮，第三态措辞统一**（J18-B）＋ 聚合式分流建任务（≤1 bug + ≤1 trace、**查重谓词加 `acceptance_bucket` 维度**、anchor 三件套、非绿格占比 >1/3 熔断、AI 整轮哑火走独立 P0）。
对应 Step 2 / Step 5 / Step 6 / Step 7；解锁断言 A6 / A7 / A12（自动 P0 定义域）/ A13 / A14 / A15 / A16① / A17②③④④′④″⑤。
