...
- [ ] [BEHAVIOR] [L1] B-01: 读者看到两个端点的用途
  ...
  Test: manual:bash -c 'node -e '\''const s=require("fs").readFileSync("docs/current/attempt-run-bridge-guide.md","utf8");for(const x of ["POST /api/brain/harness/attempt-run","创建","派发","GET /api/brain/harness/attempt-run/:id","查询"])if(!s.includes(x))throw new Error(x);console.log("OK endpoints")'\'''
...
- [ ] [BEHAVIOR] [L1] B-03: 读者看到九项角色白名单
  ...
  Test: manual:bash -c 'node -e '\''const s=require("fs").readFileSync("docs/current/attempt-run-bridge-guide.md","utf8");const a=["planner","proposer","critic","generator","generator-fix","evaluator","evaluator-fix","judge","reporter"];if(!s.includes("白名单"))throw new Error("白名单");for(const x of a)if(!s.includes(x))throw new Error(x);console.log("OK roles=9")'\'''
...
- [ ] [BEHAVIOR] [L1] B-04: 读者能区分 payload 必填与可选字段
  ...
  Test: manual:bash -c 'node -e '\''const s=require("fs").readFileSync("docs/current/attempt-run-bridge-guide.md","utf8");for(const x of ["sprint_dir","base_repo","branch","必填","base_sha","可省略","生产 Brain","自解析"])if(!s.includes(x))throw new Error(x);console.log("OK payload")'\'''
...
- [ ] [BEHAVIOR] [L1] B-05: 读者识别派发失败的完整自动回滚
  ...
  Test: manual:bash -c 'node -e '\''const s=require("fs").readFileSync("docs/current/attempt-run-bridge-guide.md","utf8").replace(/`/g,"").replace(/\\s+/g," ");for(const r of [/自动回滚/,/run\\s*(?:→|->)\\s*failed/,/session\\s*(?:→|->)\\s*closed/,/task\\s*(?:→|->)\\s*cancelled/])if(!r.test(s))throw new Error(String(r));console.log("OK rollback")'\'''
