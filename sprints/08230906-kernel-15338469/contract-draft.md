...
## 案卷 Closure（Round 2 — 修封印退回，非 Reviewer 打回）

**上一轮 Reviewer 台账**：Round 1 reviewer（attempt 50d34d91）7 维全 ≥7 判 APPROVED，`blockers: []`——无 Reviewer blocker 待关闭。

**本轮真实退回来源**：APPROVED 后**封印机械闸** `assertTestContractResolvable`（contract-test-paths-seal.js）拒绝冻结，`seal_rejection.code=FROZEN_CONTRACT_TEST_CONTRACT_UNRESOLVABLE`，detail=`packages/brain/src/orchestrator/__tests__/attempt-store.test.js`（bare 路径，**无** `#behavior:` 尾缀）。按封印协议属可重写缺陷，走 reopen 回本 proposer（非 failRun 终态）。

**根因定位（已代码核实，非猜测）**：
- 退回 detail 为 bare 路径（无 `#behavior:`）→ 命中 seal 的 `catch` 分支（`readRepoFile(testFile)` **抛异常**），不是 BEHAVIOR↔it() 不匹配。
- seal 对 Test Contract 表里 **repo 路径行**（非 `sprints/` 前缀）会调 `readRepoFile(testFile)` 读 approvedSha 下的 repo 文件校验 BEHAVIOR↔it()（seal r50 引入）。
- loop.js:996 传给 `readRepoFile` 的 `repo` 是 `task.payload.base_repo` **原值**；本 task `base_repo="https://github.com/perfectuser21/cecelia.git"`（**整串 URL**）。
- `readGitArtifact` → `normalizeArtifactRepo` → `assertSupportedRepo` 用 `GITHUB_REPO=/^[\w.-]+\/[\w.-]+$/` 校验；整串 URL 含 `://` 与多段 `/`，正则不匹配 → **确定性 throw**（本地 `node -e` 已复现 `test(URL)=false`）。
- 对照：sprint 产物解析路径（frozenContractArtifacts，loop.js:150）用 `parseBaseRepo(base_repo)` 先把 URL 归一成 `perfectuser21/cecelia` 再读 → 成功；两条读路径对 `repo` 处理不一致，故 sprint 行读得到、repo 行必抛。
- 结论：**只要 Test Contract 表含任何 repo 路径行，本 task 封印必再退**（确定性，与措辞/it() 名无关）。

**本轮 closure（做了什么 / 为什么足以关闭）**：
- **做了什么**：Test Contract 表**仅保留冻结 sprint 测试行**（`sprints/08230906-kernel-15338469/tests/failed-target-ttl.test.ts`），**移除 repo 路径行**（`packages/brain/.../attempt-store.test.js` 不再作为表行登记）。repo 既有测试回归改由 **DoD BEHAVIOR B-06/B-07 的 manual: 命令**承载（`cd packages/brain && npx vitest run … -t "…"`）——manual 命令不是 Test Contract 表行，封印时只被扫 fragile-grep（已用宽松式），**不触发** repo 行的 `readRepoFile` 读，故不再命中上述 throw。
- **为什么足以关闭**：seal 的 UNREGISTERED 硬闸只要求「artifacts 里每个冻结测试（`/tests/` + 测试扩展名）被表登记」——本 sprint 唯一 artifacts 冻结测试即上面 sprint 行，已登记；seal 的 UNRESOLVABLE 对 `sprints/` 行用**内存 artifact 内容**（existsSync=artifactPaths.has）+ 双向子串校验，可靠可解析。移除唯一触发 throw 的 repo 行后，封印两道闸均放行。repo 回归覆盖不丢：仍由 DoD B-06/B-07 在 evaluate 时（generator 产出后）真跑 repo 全套断言。
- **quote（本轮 Test Contract 段新增原文，用于 Reviewer 复核）**：`封印退回根因：seal 校验 repo 路径行时 readRepoFile 收到未归一的整串 URL base_repo，normalizeArtifactRepo 的 owner/repo 正则拒绝并抛异常 → bare-path UNRESOLVABLE；故本表移除 repo 路径行，repo 既有测试回归改由 DoD BEHAVIOR B-06/B-07 承载。`

> 说明：此 seal 的 raw-URL base_repo 未归一属 Brain 侧确定性缺陷（loop.js:996 应 `parseBaseRepo(base_repo)` 后再传，与 loop.js:150 对齐）。本 sprint 范围限定为 attempt-store 查询修复，不含 seal 代码改动；此处仅登记规避方式，Brain 侧修复另议。
...
- [packages/brain/src/orchestrator/__tests__/attempt-store.test.js → `按 hop 顺序暴露同 run/role 的终态失败执行目标`（第 1968 行 it()）] — 该测试当前断言 `pool.query` 第二参数为 `[input.runId, 'generator']`（**两参**）。本 sprint 给 SQL 加第三绑定参数后，该既有断言会变红——generator **必须**把它更新为含第三参数（`[input.runId, 'generator', 2]`），且既有 SQL 分支（status IN failed/cancelled、blocked+infrastructure_blocked、error_code NOT IN 豁免清单）不得回退。由 DoD B-06 承载。
...
- INV-1 [记仇窗口内不变]：由 BEHAVIOR B-05（窗口内失败记录仍映射为执行目标）+ B-06（既有终态失败执行目标 SQL 分支不回退，含更新后第三参数）覆盖。
...
> 说明：本 attempt 无真库，E2E 以 vitest 跑冻结 sprint 测试 + repo 既有 attempt-store 全套（后者含 generator 更新后的第三参数断言与新增 TTL 回归，在 evaluate 时 generator 产出上真跑）。
...
**表规则（本 task 封印退回后收敛）**：本表**仅登记 artifacts 冻结 sprint 测试行**。封印退回根因：seal 校验 repo 路径行时 readRepoFile 收到未归一的整串 URL base_repo，normalizeArtifactRepo 的 owner/repo 正则拒绝并抛异常 → bare-path UNRESOLVABLE；故本表移除 repo 路径行，repo 既有测试回归改由 DoD BEHAVIOR B-06/B-07 承载（manual: 命令非表行，evaluate 时对 generator 产出真跑）。seal UNREGISTERED 闸只要求 artifacts 冻结测试（`/tests/`+测试扩展名）被登记，本表已满足。

| 功能 | Test File | BEHAVIOR 覆盖 | 预期红证据 |
|---|---|---|---|
| 时效窗口豁免核心（冻结，强制） | `sprints/08230906-kernel-15338469/tests/failed-target-ttl.test.ts` | `默认 2 小时窗口经 created_at make_interval 过滤且第三参数为 2` / `HARNESS_FAILED_TARGET_TTL_HOURS 覆盖窗口小时数进第三参数` / `非法 HARNESS_FAILED_TARGET_TTL_HOURS 回退默认 2 小时` / `窗口边界采用窗口内含语义使用大于等于比较` / `窗口内失败记录仍映射为执行目标保持记仇语义不变` | 5 tests → 4 failed \| 1 passed（B-05 为记仇不变回归守卫，RED/GREEN 均绿；其余 4 条对当前无窗口/两参代码 RED，已本地实跑复现 4 failed \| 1 passed） |

> Test File 列为完整真实路径（无省略号）。BEHAVIOR 覆盖名逐词取自该测试文件真实 `it()` 名（全名，`grep -F '<覆盖名>' <test file>` 必命中；seal/CI 双向子串校验命中）。
> repo 既有测试回归（`packages/brain/src/orchestrator/__tests__/attempt-store.test.js`）**不作为本表行**，其覆盖由 DoD B-06（既有 `终态失败执行目标` 断言更新含第三参数）+ B-07（新增 `覆盖进 SQL 第三参数` TTL 回归）承载，evaluate 时真跑核对。
