...
- [ ] [ARTIFACT] 认证读回端点已注册（复用 capabilities 路由族）
...
- [ ] [ARTIFACT] 认证聚合复用现有 Mapper（禁平行系统）
...
- [ ] [ARTIFACT] 幂等 seed helper 存在（供 smoke/E2E/DoD 复用）
  Test: node -e "require('fs').accessSync('packages/brain/scripts/integration/seed-f1-cert-fixture.js')"

- [ ] [ARTIFACT] nightly 负向矩阵集成测试已登记进 POSTGRES_INTEGRATION_TESTS
...
- [ ] [BEHAVIOR] [L2] B-01: certification 端点对冻结身份返回 green
...
- [ ] [BEHAVIOR] [L2] B-02: 非 synthetic PASS receipt 精确落账
...
- [ ] [BEHAVIOR] [L2] B-04: 无 receipt 时不 green
...
- [ ] [BEHAVIOR] [L2] B-05: 错 SHA 时不 green
...
- [ ] [BEHAVIOR] [L2] B-06: 缺 Feature 时不 green
...
- [ ] [BEHAVIOR] [L2] INV-1 [validation-clock] 缺/错 merge SHA 一律 fail-closed 不 green
...
- [ ] [BEHAVIOR] [L2] INV-2 [evidence-vs-defect] 无 receipt(补证据) 与 receipt FAIL(缺陷) 语义区分
