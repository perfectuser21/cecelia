...
```
- `period_days` (number, 必填): 实际生效窗口天数，恒落在 **[1,365]** 闭区间。来源——PRD「days=N 窗口」+ PRD「边界情况：days 缺省/非数字/超大值口径」。确定性口径（generator 必须按此实现，与 DoD B-05/B-10 逐一对齐）：
  - **缺省 / 非数字**（`days` 未传、`days=`、`days=abc` 等 `parseInt` 得 NaN）→ 回落默认 **7**（本 sprint 目标窗口，非 `/harness/stats` 的 30）。
  - **数值 < 1**（含 `days=0`、负值 `days=-5`）→ clamp 到下限 **1**。
  - **数值 > 365**（如 `days=999999`）→ clamp 到上限 **365**。
  - **合法 [1,365]** → 原样。
  （对齐 `/harness/stats` 既有 `days<1 || days>365` clamp 惯例；默认值由 30 改为 7 因本端点专供「连续 7 天失败率」计量。任一分支返回 HTTP 200，脏参不 500。）
...
**Error / 边界**:
- `days` 非数字（如 `days=abc`）→ **不报错**，回落默认 7，返回 HTTP 200；`days` 越界（`<1` clamp 到 1、`>365` clamp 到 365）→ **不报错**，返回 HTTP 200（口径边界，proposer 定契约：计量端点对脏参数容错，避免日报消费方因脏参崩；见上方 `period_days` 确定性口径）。
...
- `persistTerminalFailure(dbPool, taskId, failureClass, failureDetail)` —— **钉死位于 `packages/brain/src/orchestrator/failure-class.js`**（与 tests import 源一致，不二选一、不散落别处），内部 `assertFailureClass` 后执行
...
# 7. Response Schema 完整 oracle（B-07..B-10）：分母字段类型 + 五字段完整性卡 + 禁用字段反向 + days 越界 clamp
RESP3=$(curl -sf "$BRAIN/api/brain/harness/failure-stats?days=7")
echo "$RESP3" | jq -e '(.total_terminal_failed|type=="number") and (.total_terminal_done|type=="number")' || { echo "FAIL: 分母字段 total_terminal_failed/done 非数值"; exit 1; }
echo "$RESP3" | jq -e 'has("period_days") and has("by_class") and has("total_terminal_failed") and has("total_terminal_done") and has("failure_rate")' || { echo "FAIL: schema 五必填字段缺失"; exit 1; }
echo "$RESP3" | jq -e '.fail_rate==null and .failureRate==null and .classes==null and .counts==null and .stats==null and .error_class==null' || { echo "FAIL: 禁用同义字段漂移"; exit 1; }
for D in 0 -5 999999; do
  curl -sf "$BRAIN/api/brain/harness/failure-stats?days=$D" | jq -e '.period_days>=1 and .period_days<=365' >/dev/null || { echo "FAIL: days=$D period_days 越界"; exit 1; }
done
echo "✅ Step7 Response Schema 完整 oracle 通过（分母字段/完整性/禁用字段/越界 clamp）"
