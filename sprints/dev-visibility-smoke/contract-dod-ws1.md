...
- [ ] [BEHAVIOR] smoke-verify.sh 内容含 `## Sprint PRD` 关键词检查逻辑（非空实现）
...
- [ ] [BEHAVIOR] smoke-verify.sh 内容含 sprint-prd.md 存在性验证（-s 或 existsSync 检查）
...
- [ ] [BEHAVIOR] bash smoke-verify.sh 执行退出码为 0（所有冒烟检查通过）
  Test: manual:bash -c 'bash sprints/dev-visibility-smoke/smoke-verify.sh && echo OK || { echo FAIL; exit 1; }'
  期望: OK

- [ ] [BEHAVIOR] smoke-verify.sh 内含 harness-utils.js prdContent 注入验证（两项关键词均须存在）
