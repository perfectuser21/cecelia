...
**范围**: 新增 `packages/brain/src/routes/time.js`（Express Router），实现三个分支；在 `packages/brain/server.js` 注册路由。路由从 WS1 `../utils/time-format.js` 导入，不重复实现时区逻辑。
**大小**: M（100-200 行）
**依赖**: Workstream 1
...
见 `sprints/tests/ws2/time-endpoint.test.ts`，覆盖：
