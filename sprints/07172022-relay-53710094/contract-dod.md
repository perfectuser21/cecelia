...
- [ ] [BEHAVIOR] INV-Headed环境 e2e wrapper 显式接受 harness 上下文变量
...
- [ ] [BEHAVIOR] INV-模板复用 已核对本次任务真实派发/执行历史而非复用历史 task id
...
- [ ] [BEHAVIOR] INV-禁写死假设 运行环境通过变量传入并带本地默认值
...
- [ ] [BEHAVIOR] INV-真验才done 接缝断言必须真目标验证
...
- [ ] [BEHAVIOR] INV-凭据安全 payload 不含常见 secret 字段
...
- [ ] [BEHAVIOR] INV-日志脱敏 合同不要求输出 token 或完整敏感 prompt
