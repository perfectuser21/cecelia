...
**可观测行为**: `GET /api/brain/agent-ops/model-accounts` 返回 `data.accounts` 恰好 8 条，每条含 11 字段（provider/plan/five_hour_pct/seven_day_pct/reset_at/host_alias/forwardable/forward_targets/status/last_checked_at/last_error）。

**验证命令**:
```bash
RESP=$(curl -sf "localhost:5221/api/brain/agent-ops/model-accounts")
echo "$RESP" | jq -e '.data.accounts | length == 8'
echo "$RESP" | jq -e '.data.accounts | all(has("provider") and has("five_hour_pct") and has("seven_day_pct") and has("reset_at") and has("host_alias") and has("forwardable") and has("forward_targets") and has("status") and has("last_checked_at") and has("last_error"))'
```
**硬阈值**: accounts 数 == 8；每条 11 字段齐全。
**验证命令（硬阈值→可执行）**:
```bash
echo "$RESP" | jq -e '[.data.accounts[] | select(.status=="ok" or .status=="unknown" or .status=="key_expired" or .status=="no_credential")] | length == 8'
```
...
- **collector ↔ ops_model_accounts 表（写路径）**: 本单新增账号快照落表。冻结测试 `[integration]` 段用真 Postgres（cecelia_test）验行落库，只允许 mock 最外层 provider 探针（ssh/usage 探针），禁 mock DB 边。
...
- INV-4 [幂等 UPDATE] → 采集 upsert 用 `INSERT ... ON CONFLICT (account_id) DO UPDATE`（DoD B-04 integration 验重复采集不产生重复行）
...
> 与 harness Sprint Tests job 同环境（真 PG cecelia + Brain live :5221 + `$DB_URL`）。脚本预置账号快照行（代表采集器输出——真实 mmv 采集不在 CI，见未覆盖清单），再 curl 端点验读路径 + 失败隔离 + key_expired，并静态断言 refresh_token 铁律。collector→表 写路径由冻结 `[integration]` 段真 PG 覆盖。
...
|---|---|---|---|
| 整个 Sprint（parser + 状态映射 + refresh 铁律 + collector→表→端点 + model_role） | `sprints/09190907-model-account-quota-projection/tests/model-accounts.test.ts` | 8 个账号 + 状态枚举、三家 parser 同 schema、classifyGrokUsageError 返回 key_expired、refreshToken 探针零调用、端点读真表 8 条含 last_error、agents 每条含 model_role primary_count | 模块 `ops-model-accounts-collector.js` 不存在 → 整个 suite 加载失败（Failed to load url ...ops-model-accounts-collector.js）→ 0 通过 |
| 补充（回归，真 PG，brain-integration 池） | `packages/brain/src/__tests__/ops-model-accounts.integration.test.js`（补充行，非冻结） | collector upsert 幂等 + 采集腿失败隔离 | 同因缺模块红 |
