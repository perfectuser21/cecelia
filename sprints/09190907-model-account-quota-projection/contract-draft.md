# Sprint Contract Draft (Round 1) — 模型账号配额+机器可达性投影（工厂·F5 指挥舱 刀2）
...
**可观测行为**: `buildOpsUnitNotionProperties`（`notion-push-sync.js:1124`）对有配额数据的 agent 行追加 5h%/7d%/更新时间 Notion 属性，随现有 notion-push-sync 推送（不新开同步管道）。

**验证命令**（Notion 真推送依赖 token，本地不可跑 → 验 properties builder 纯函数产出对应列）:
```bash
npx vitest run sprints/09190907-model-account-quota-projection/tests/ --no-cache -t "Notion" || echo "Notion 列由 properties builder 单测覆盖（见 DoD B-07）"
```
**硬阈值**: buildOpsUnitNotionProperties 对含配额的行产出 5h%/7d%/更新时间三属性。真 Notion 推送验证登记进未覆盖真实链路清单（依赖 token，logic-done-pending）。
