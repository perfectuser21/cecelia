...
- [ ] [BEHAVIOR] INV-4 [幂等 upsert] 同 account_id 重复采集不产生重复行
  动作: 真 PG 下对同一 account_id 连续两次 upsert（冻结测试 integration 段覆盖），查行数
  预期观察: ops_model_accounts 中该 account_id 行数恒为 1（ON CONFLICT DO UPDATE 幂等）
  等待预算: 0s
  留证: 冻结测试 integration 段断言（同 B-05 覆盖）+ psql count
