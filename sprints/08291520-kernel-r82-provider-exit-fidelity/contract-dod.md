...
|---|---|---|---|
| 回执保真 + failed_targets 排除 + 负向零回归（冻结主测） | `sprints/08291520-kernel-r82-provider-exit-fidelity/tests/r82-provider-exit-fidelity.test.js` | 保真透传不被包装成 provider_exit / failed_targets 采集排除 CONTRACT / 负向不回退 / success 结果 JSON | resolveProviderTerminalResult is not a function + SQL 无 CONTRACT 排除 → 9 failures |
| F1/step3 companion 守卫（PRD 指定位置补充行） | `tests/gp/f1/step3-contract-fault-fidelity-not-provider-exit.test.js` | 保真，error.code 不被埋没成 provider_exit / 真崩溃负向 / failed_targets 采集 SQL 排除 CONTRACT | 同上 → 3 failures |
