# Sprint Contract Draft (Round 2) — 结构化上报保真透传，根除 provider_exit 语义埋没 [r82]

> **Round 2 修订说明（封印机械闸打回复位）**：Round 1 经 Reviewer 7 维全部 ≥ 7 判 APPROVED，但被封印机械闸 `assertTestContractResolvable` 以 `FROZEN_CONTRACT_TEST_CONTRACT_UNRESOLVABLE` 拒绝（锚定 `#behavior:failed_targets 采集排除 CONTRACT`）。两处根因已修复：①冻结主测中失败目标采集 SQL 的 `it()` 名不含 Test Contract 表覆盖名 `failed_targets 采集排除 CONTRACT` 子串（v9.5 子串死规则违约），已改名使覆盖名成为 `it()` 名字面子串；②Test Contract 第二行 `tests/gp/f1/step3-contract-fault-fidelity-not-provider-exit.test.js` 从未真实落盘，已真实创建（真 import 两条被改边、含可执行断言、经实证 3/3 真红）并进 commit。合同规格（Golden Path/DoD/接缝清单/判定点）与 Round 1 一致，净变化仅"测试真落盘 + 覆盖名对齐"。
