...
describe('r82 边① 回执保真透传（kernel-attempt-handler resolveProviderTerminalResult）', () => {
  ...
  it('保真透传不被包装成 provider_exit：复刻 r69/r77 结构化 BLOCKED + CONTRACT_* 遇 provider 非零退出', () => {
...
describe('r82 边② failed_targets 采集排除 CONTRACT_* 家族（真 attempt-store SQL）', () => {
  it('failed_targets 采集排除 CONTRACT_* 家族：listFailedExecutionTargets 发往 Postgres 的 SQL 显式排除 CONTRACT 错误码，合同故障 target 不被拉黑', async () => {
    const pool = stubPool();
