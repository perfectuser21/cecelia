# Sprint Contract Draft (Round 2) — kernel validation clock 按 fix 轮自动顺延（有界）[r68]

> Round 2 唯一修订：修复 round-1 合同 APPROVED 后被封印机械闸拒绝（FROZEN_CONTRACT_TEST_CONTRACT_UNRESOLVABLE）——Test Contract 表第二行（`tests/gp/f1/…`）的「BEHAVIOR 覆盖」列曾写散文「同上五条（…tests/gp/f1/）」，被 parseTestContract 按 `[/,、;；]` 切成 `…tests`/`gp`/`f1` 等解析不到 `it()` 名的锚点。本轮把该列改成与第一行一致的干净 `it()` 名子串清单，并把两份冻结测试文件真实落盘 commit 进 propose 分支。实现契约（Golden Path / DoD / RED 断言）一字未改，Reviewer round-1 的 APPROVED 实质结论保持。
...
| fix 轮顺延（CI 常驻回归，补充行） | `tests/gp/f1/step3-validation-clock-fix-round-extension.test.js` | `B-01 顺延`、`B-02 有界`、`B-03 语义不变`、`B-04 Invariant`、`B-05 恰好 6 轮 fix` | 未改实现前 B-01/B-02/B-05 红，B-03/B-04 绿——与第一行同契约副本 |

> 「BEHAVIOR 覆盖」列每个名（如 `B-01 顺延`）均为对应 `it()` 名的字面子串，可 `grep -F` 命中；封印闸 parseTestContract 按 `[/,、;；]` 切词后逐词对回 `it()` 名（双向小写子串），故本列**只写干净的 `it()` 名子串**，禁止写含 `/`、`,`、`、` 的散文或「同上」指代（r68 round-1 封印被拒根因：第二行曾写「同上五条（…tests/gp/f1/）」被切成 `…tests`/`gp`/`f1` 三个解析不到的锚点 → FROZEN_CONTRACT_TEST_CONTRACT_UNRESOLVABLE）。冻结测试至少一行在 `sprints/<本sprint目录>/tests/`（第一行，封印闸解析锚点 + frozenTestFiles 登记）；`tests/gp/f1/` 为 PRD 要求的 CI 常驻回归副本行。
