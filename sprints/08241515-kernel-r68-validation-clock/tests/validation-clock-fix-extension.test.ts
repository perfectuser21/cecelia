...
// 冻结合同测试（RED 先行）——真 import 被改文件，禁 mock 被改的边。
// 覆盖父路：独立小路（无父路）——kernel validation clock 纯函数顺延逻辑。
...
function row(hop: number, action: string, createdAt: string, detail: unknown = undefined) {
