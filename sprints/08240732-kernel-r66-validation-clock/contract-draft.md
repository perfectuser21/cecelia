...
`spawn:generator` 建立原点 → `spawn:generator-fix` intent 获得匹配的 `effect:attempt_launched` 成功回执后，最多 6 次成为新原点 → 后续 Evaluator/Judge 使用有界新 deadline 判定存活；仅有 intent 或 `effect:dispatch_result` 阻断回执时不顺延。
...
**可观测行为**: 按 hop 排序的日志中，前 6 个同时具备 fix intent 与匹配成功派发回执的 `spawn:generator-fix` 可逐次取代原点；相同输入重放结果一致。成功证据唯一锁定为后续 `effect:attempt_launched` 行的 `detail.dispatch_hop === fix intent.hop` 且 `detail.dispatch_action === "spawn:generator-fix"`；该 effect 是 `loop.js` 仅在 dispatcher 返回 `control_status === "LAUNCHED"` 后持久化的可重放事实。
...
```
**硬阈值**: r50 新 deadline 晚于判定时刻；仅成功回执配对的 fix 计入最多 6 次；日志顺序变化但 hop 不变时结果深相等；上命令 exit 0。

### Step 2b: 失败或被阻断的 fix intent 不顺延
**来源**: `[FROM_PRD]` — PRD「要求」1 明确限定“派发成功时”，Reviewer R1-1 要求成对冻结验证。

**可观测行为**: 仅有 `spawn:generator-fix` intent，或该 intent 后只有 `effect:dispatch_result` 且 status 为 `BLOCKED`/`NEEDS_CONTEXT`，均保持上一有效原点；不得把派发意图当成成功事实。

**验证命令**:
```bash
npx vitest run --no-cache sprints/08240732-kernel-r66-validation-clock/tests/validation-clock-fix-extension.test.ts -t 'generator-fix 只有 intent 而派发失败或被阻断时不顺延'
```
**硬阈值**: 原点和 deadline 精确等于初始 generator clock，命令 exit 0。
...
- `orchestrator_decision_log` intent/effect 行序列 ↔ `resolveValidationClock` 原点选择（本单修改跨模块数据传递语义；测试必须传 `loop.js` 实际写入的 `spawn:generator-fix` 与 `effect:attempt_launched.detail.dispatch_hop/dispatch_action` shape 并真调用导出函数，禁止 mock 该函数或该输入边）。

## 真实调用方请求 shape

N/A — 本任务无设备、agent 或 HTTP 调用方；真实调用方是 `loop.js`，以 `{ action, decisionLog, intentAt, timeoutSeconds, allowEvaluatorOrigin }` 调用 `resolveValidationClock`。成功派发的生产日志 shape 为 intent `{hop,action:"spawn:generator-fix",created_at}` 加 effect `{action:"effect:attempt_launched",detail:{dispatch_hop:<intent hop>,dispatch_action:"spawn:generator-fix",attempt_id:<runtime id>}}`；运行时 attempt id 不固化为合同身份。
...
| 判定点（怎么知道） | 以 `effect:attempt_launched.detail.dispatch_hop/dispatch_action` 与 fix intent 精确配对，识别前 6 次成功 fix |
...
| fix intent 派发失败或被阻断 | 不顺延，继续使用上一有效原点 | 是 | 到既有 deadline 照常判死 |
...
|---|---|---|---|
| Sprint 冻结测试 | `sprints/08240732-kernel-r66-validation-clock/tests/validation-clock-fix-extension.test.ts` | `r50 场景在成功 generator-fix 后以 fix 行为新原点存活`; `generator-fix 只有 intent 而派发失败或被阻断时不顺延`; `第 7 次 generator-fix 不再顺延且仍以第 6 次 fix 为原点`; `无 generator-fix 时保持首个 generator 原点语义`; `相同乱序日志按 hop 重放得到相同 clock` | 当前实现固定首个 generator，成功顺延与有界顺延项 FAIL |
