...
> **修订说明**: 基于 Round 2 Reviewer 反馈修复全部 2 个必须修改项 + 2 个可选改进。
> 核心改进：F3 全部验证命令改为递归 walk 扫描子目录、F2-C1 过滤注释行后检查、F1-C2 增加块注释剥离。
...
- 组件代码（排除注释行和块注释）包含 API 调用（fetch/axios/useSWR/useQuery）
- 组件代码（排除注释行和块注释）包含列表渲染（.map）
- 组件代码（排除注释行和块注释）渲染 status、phase/stage、round 字段

**验证命令**:
```bash
# F1-C1: harness 目录包含有效组件文件（>50 字节）
node -e "
  const fs = require('fs');
...
# F1-C2: 组件包含真实 API 调用 + 列表渲染 + 阶段/轮次字段（修复 R2#B：增加块注释剥离）
node -e "
  const fs = require('fs');
...
# F1-C3: /harness 路由已注册（非注释代码，使用 Node API 递归扫描）
node -e "
  const fs = require('fs');
  const path = require('path');
...
**硬阈值**:
- `check-manual-cmd-whitelist.cjs` 的 ALLOWED_COMMANDS 集合（非注释代码）包含字符串 `'playwright'`
- 白名单仅做第一个词的校验（`npx playwright` 中 `npx` 已在白名单，`playwright` 作为独立命令时也需通过）

**验证命令**:
```bash
# F2-C1: ALLOWED_COMMANDS 非注释代码包含 playwright（修复 R2#2：过滤注释行再检查）
node -e "
...
**验证命令**:
```bash
# F3-C1: Brain 代码（递归扫描所有子目录）非注释行包含重试逻辑 + REPORT_FAILED（修复 R2#1：递归 walk 替代 readdirSync 顶层）
node -e "
  const fs = require('fs');
  const path = require('path');
  const walk = (dir) => {
...
node -e "
  ...
  for (const f of walk('packages/brain/src')) {
    ...
    if (/max.?retries\s*[:=]\s*3/i.test(code) && code.includes('REPORT_FAILED')) {
      found = true; console.log('PASS: max_retries=3 已确认 in ' + f); break;
    }
  }
  if (!found) { console.log('FAIL: 未找到非注释代码中 max_retries=3 + REPORT_FAILED 的赋值'); process.exit(1); }
"

# F3-C3: 递增间隔模式存在（递归扫描）（修复 R2#1）
node -e "
  const fs = require('fs');
  const path = require('path');
  const walk = (dir) => {
  ...
  for (const f of walk('packages/brain/src')) {
    ...
    if (/\[\s*\d+\s*,\s*\d+\s*,\s*\d+\s*\]/.test(code) && /retry|REPORT_FAILED/i.test(code)) {
      console.log('PASS: 找到递增间隔数组 in ' + f); process.exit(0);
    }
    if (/delay\s*\*=?\s*\d/.test(code) && /retry|REPORT_FAILED/i.test(code)) {
      console.log('PASS: 找到递增间隔乘法模式 in ' + f); process.exit(0);
    }
  }
...
- [ ] [BEHAVIOR] 组件非注释代码（含块注释剥离）包含 API 调用 + 列表渲染 + status/phase/round 字段
  Test: node -e "const fs=require('fs');const path=require('path');const dir='apps/dashboard/src/pages/harness';const tsxFiles=fs.readdirSync(dir).filter(f=>/\.tsx$/.test(f));let allCode='';for(const f of tsxFiles){let code=fs.readFileSync(path.join(dir,f),'utf8');code=code.replace(/\/\*[\s\S]*?\*\//g,'');const lines=code.split('\n').filter(l=>!l.trim().startsWith('//'));allCode+=lines.join('\n')+'\n'}const checks=[[/fetch|axios|useSWR|useQuery/,'API 调用'],[/\.map\s*\(/,'列表渲染(.map)'],[/status/,'status 字段'],[/phase|stage/,'phase/stage 字段'],[/round|gan_round/,'round 字段']];for(const[re,label]of checks){if(!re.test(allCode)){console.log('FAIL: 非注释代码中未找到 '+label);process.exit(1)}}console.log('PASS: 组件包含所有必需元素')"
- [ ] [BEHAVIOR] `/harness` 路由在 Dashboard 中注册（非注释代码，递归扫描）
...
**范围**: `packages/brain/src/` 中增加 harness_report 重试逻辑（max_retries=3、递增间隔、REPORT_FAILED verdict），递归扫描所有子目录
**大小**: S（<100行）
**依赖**: 无
