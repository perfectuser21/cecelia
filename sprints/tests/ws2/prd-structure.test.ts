...
describe('Workstream 2 — validatePrdStructure [BEHAVIOR]', () => {
  it('[ws2.t1] returns ok=true with sections=9 for the real Initiative B2 PRD', async () => {
  ...
  it('[ws2.t2] returns ok=false listing all 9 missing section names when given an empty document', async () => {
    const mod: any = await import(VALIDATOR_PATH);
  ...
  it('[ws2.t3] returns ok=false with emptySections naming the offending heading when a section body is whitespace-only', async () => {
    const mod: any = await import(VALIDATOR_PATH);
  ...
  it('[ws2.t4] treats sections separated only by code fences as empty', async () => {
    const mod: any = await import(VALIDATOR_PATH);
