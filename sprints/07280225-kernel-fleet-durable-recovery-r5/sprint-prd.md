...
## R46-R47 Reviewer 接口与 execution-target 恢复隔离

Reviewer-v2 必须沿用生产接口的七个字面 key：
`dod_machineability`、`scope_match_prd`、`test_is_red`、`internal_consistency`、
`risk_registered`、`verification_oracle_completeness`、`ci_workflow_alignment`。未知 key、
改名、缺维度、非整数或任一分数 `<7` 均 non-authorizing。临时 `verify` 脚本自己生成的
JSON/stdout 只是诊断；授权正控必须由 Controller 独立查询 append-only server store、重算
artifact/receipt/predecessor digest，并验证 issuer 与 observer 不同 trust domain。

execution target 排除必须绑定
`run_id+role+logical_cycle_id+provider+account+machine+failure_class`，并保留
`source_attempt_id,observed_at,expires_at/reset_at`。`execution_transport_unavailable`、
`machine_offline`、transient probe failure 带 TTL 和 fresh reprobe；auth/quota 带 reset/TTL；
product failure 不得污染 target health。每次选择必须写 append-only decision receipt，列出
完整 considered targets、逐项 exclusion reason/class/source/expiry、fresh probe snapshot 和
selected target，缺字段 fail closed。

`all_execution_targets_exhausted` 是 recoverable infrastructure blockage，只能进入有界
backoff、fresh reprobe 或 owner intervention；重复同字符串不得消耗 semantic/GAN budget 或
把 run 标 failed。只有达到独立持久化、可回放的 termination limit 或 owner 明确终止才可
terminal failed。标准恢复不得篡改 failed run；保留历史 run/Attempt/decision log，新 run 从
远端最新 contract branch exact SHA 建立 `recovery_of`。

永久 Red/Green 必须在生产 capability gate、dispatcher/loop 与真 PG seam 覆盖：
cycle1 team3/team4 transport unavailable、team5 quota unavailable，随后 cycle2 team4 fresh
probe 恢复并在同一 run 被重新选择；Brain restart/replay、TTL 未到、TTL 到期、auth reset、
...
: "${ATTEMPT_ID:?}" "${CONTRACT_SHA:?}" "${PR_HEAD_SHA:?}" "${TASK_BUNDLE_PATH:?}"
