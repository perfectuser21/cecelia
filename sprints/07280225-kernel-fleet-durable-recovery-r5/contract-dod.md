...
- [ ] [ARTIFACT] Sprint Red 测试库存按 realpath 去重后恰好一个文件、12 个唯一 `it()`，保留 migration parity 与 workflow bypass Red。
  Test: node -e "const fs=require('fs');const p='sprints/07280225-kernel-fleet-durable-recovery-r5/tests/durable-recovery.contract.test.ts';const c=fs.readFileSync(p,'utf8');if((c.match(/\\bit\\(/g)||[]).length!==12||!c.includes('fleet-worker transport with production upgrade rollback and source enum parity')||!c.includes('P0 workflows enforce owner merge staging production order'))process.exit(1)"
...
- [ ] [BEHAVIOR] [L3] Golden Path Step 10 — Draft CI Evaluator Judge are evidence-only
  动作: 在真实 Draft PR exact head 上运行统一 P0 workflow contract 与 Worker admission mutation。
  预期观察: 事件仅为 `ci,evaluator,judge`；PR 仍 Draft、auto-merge off；merge/staging/production mutation=0；Worker admission 失败时 Brain candidate receipt=0。
...
- [ ] [BEHAVIOR] [L3] Golden Path Step 11 — exact-head owner authorization precedes merge
  动作: 运行授权 owner exact-head 正控，以及 CI-only、non-owner、stale-head、missing-evidence、title/label auto-merge mutations。
  预期观察: 仅 controller 可在完整证据后 Draft→Ready/merge exact head；所有反事实 merge/staging/production count=0。
...
- [ ] [BEHAVIOR] [L3] Golden Path Step 12 — merge 后 staging 再 production 与 rollback
  动作: authorized merge 后真跑 US staging，再 production canary/rollback；分别 mutate main-push production、Fast Lane、schedule、skipped/idle staging、missing receipt。
  预期观察: 仅 `merge→staging_passed→production_canary_started→production_canary_passed` 成功；全部旁路非零；reverse cleanup residual/quarantine=0。
