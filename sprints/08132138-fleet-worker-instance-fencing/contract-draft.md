# Sprint Contract Draft (Round 2) — Fleet Worker 实例互杀防护 + quarantined attempt 终态闭环
...
> **R2 收敛摘要**：Round 1 Reviewer 打回 internal_consistency=6（唯一 blocker R1-1 = locked-test 与合同 prose 授予的 controller 实现自由度冲突）。本轮按推荐修法 (a)：删除 Step 5 / concern 中「controller 编排 / 实现自由度」措辞，把 expired fleet-worker attempt 的终态化+lineage **唯一入口** 钉死在 `reconcileExpiredKernelAttempt`，并显式点名 `harness-relay-watchdog.js:184-186` 的 fleet-worker deferral guard 必须改造；同时把 instance-fencing RED 测试单源化到 CI 可跑的 `.cjs`（删除 sprints/ 下 `.ts` 副本，Test Contract 表与 contract-dod 同指一路径）。
...
- [kernel-stale-attempt-reconcile.pg.integration.test.js] → 真 PostgreSQL 上 stale attempt 终态化 + 事务幂等（`scripts/kernel-stale-attempt-reconcile.mjs` 的 `reconcileStaleAttempts` 入口不得回退；注意：该入口只产 repair 提案，**不建 replacement**，与本 sprint 场景 C 的终态化+lineage 入口 `reconcileExpiredKernelAttempt` 是两条独立路径）
...
### Step 5: expired fleet-worker attempt 一次事务终态化 + append-only evidence + replacement restart_reason lineage + 幂等（场景 C）
...
**终态化入口（唯一，R2 收敛 — 消除 R1-1 locked-test 冲突）**:
expired fleet-worker attempt 的一次事务终态化 + replacement restart_reason lineage + 幂等 **必须** 落在
`packages/brain/src/harness-relay-watchdog.js` 的 `reconcileExpiredKernelAttempt` 内 —— 这是**唯一**终态化入口，**不**由 controller 编排，**不**在 `scripts/kernel-stale-attempt-reconcile.mjs`（后者只产 repair 提案、不建 replacement）。本轮**取消 R1 曾赋予的"watchdog vs controller 实现自由度"**：不存在可把该逻辑放进 controller 的分叉。

**现网必须改造的具体锚点**：`harness-relay-watchdog.js:184-186` 现对 `execution_transport==='fleet-worker'` 的 attempt **显式 early-return `{ ok:false, deferred_to_controller:true }`**（不终态化、不建 child）。本 sprint 必须改这段 fleet-worker deferral guard —— 对**已过 lease（`lease_expires_at < NOW()`）**的 fleet-worker attempt，不再 defer，而是继续走该函数**既有**的终态化+lineage 路径（reclaim → rotate → 建 `attempt_kind='resume'` child 且 `retry_of_attempt_id=parent.id`/`restart_reason` 非空 → parent `failClaimedAttempt` 终态化 `failed`）。lease 未过期的 fleet-worker attempt 仍可保持既有 defer 语义（本 sprint 不触碰非过期路径）。锁定测试 `kernel-instance-fencing-lineage.pg.integration.test.js` 正是把 attempt 置为 `execution_transport='fleet-worker'` + `lease_expires_at=NOW()-10min` 后**直接**调 `reconcileExpiredKernelAttempt` 断言此 observable —— 入口与合同承诺**逐字一致**，无实现自由度分叉，不构成 locked-test 死锁。

**可观测行为**（真 PostgreSQL，无 mock DB 边）:
1. 一个 active 且 `lease_expires_at < NOW()` 的 fleet-worker attempt 被 `reconcileExpiredKernelAttempt` 处理后，**一次事务**内 `status='failed'` 且 evidence 落库（append-only，不覆盖既有 evidence 历史）
...
```bash
# 见 ## E2E 验收 场景 C：migrate 空库 → 插入 expired fleet-worker parent → 直接调 reconcileExpiredKernelAttempt
# → 断言 failed+lineage → 二次驱动断言幂等（此处不重复脚本，避免与 E2E 段拼接冲突）
echo "Scenario C 真 PG 复演见 ## E2E 验收 场景 C"
```

**硬阈值**: parent `status='failed'`；child 存在且 `retry_of_attempt_id=parent.id AND restart_reason IS NOT NULL AND attempt_kind='resume'`；二次 reconcile 后 child 计数不变（=1）。**验证命令**：见 `## E2E 验收` 场景 C 的 psql/vitest 断言。
...
- **harness-relay-watchdog `reconcileExpiredKernelAttempt` ↔ `harness_attempts`（PostgreSQL）**：场景 C 的 failing 测试与 B-05/B-06 必须真 Postgres（`pg` + 真 `src/migrate.js` 空库），断言真实行落库/父子链/幂等，禁止 mock DB。测试直接调 `reconcileExpiredKernelAttempt` 驱动真终态化路径（含改造后的 fleet-worker guard），放 `packages/brain/src/__tests__/integration/kernel-instance-fencing-lineage.pg.integration.test.js` 并登记进 `packages/brain/vitest.config.js` 的 `POSTGRES_INTEGRATION_TESTS`（由 brain-integration job 起真 PG 跑）。
...
| 要素 | 说明 | 本次答案 |
|------|------|----------|
| **FR（做什么）** | 功能需求 | ①reconcile 按 instance namespace 隔离；②namespace 生成/持久化/重启稳定；③旧无 namespace 容器 fail-closed；④expired fleet-worker attempt 由 `reconcileExpiredKernelAttempt` 一次事务终态化+append-only evidence+replacement lineage+幂等；⑤contract_requirements.postgres→runtime_resources.postgres 投影真起 PG |
...
| 场景 | 失败行为 | 重试幂等？ | 降级策略 |
|------|----------|-----------|----------|
| expired 终态化事务中途失败 | 不留半开：parent+child 均落终态失败证据 | 是（幂等键=run_id+hop 唯一约束） | 交人工，append-only evidence 留痕 |
...
# ============ 场景 C：expired fleet-worker attempt 一次事务终态化 + lineage + 幂等（真 PG，禁 mock DB 边）============
# 唯一终态化入口 = reconcileExpiredKernelAttempt（harness-relay-watchdog.js）；
# 本 sprint 改造 line 184-186 fleet-worker deferral guard：对 lease 已过期的 fleet-worker attempt 走终态化+lineage，非 defer。
# 非 kernel-stale-attempt-reconcile.mjs（后者只产 repair 提案不建 replacement）。
# 真 PG oracle 落 PG 集成测试（注入 DB_URL 空库 + 真 migrate，直接驱动 reconcileExpiredKernelAttempt，无 DB mock）。
( cd "$BRAIN_DIR" \
  && DB_URL="$DB_URL" NODE_ENV=test \
...
- 重复提交: 同一 expired parent 并发两个 reconcile 进程（后台 `&` 同时起两次 `reconcileExpiredKernelAttempt` 驱动）→ 唯一约束应让其中一个 deduped，最终仍只 1 个 child
...
> 单源约定（R2 修 R1-1 附带项）：instance-fencing 逻辑层 RED 测试**单源**于 `packages/brain/scripts/fleet-worker/instance-fencing.test.cjs`（CI include glob `scripts/**/*.{test,spec}...` 可跑，brain-unit job）。已删除 sprints/ 下 `.ts` 同源副本，消除 .ts/.cjs drift；本表与 contract-dod B-01..B-04 同指该 `.cjs`。

| 功能 | Test File | BEHAVIOR 覆盖 | 预期红证据 |
|---|---|---|---|
| namespace 生成/持久化/重启稳定 | `packages/brain/scripts/fleet-worker/instance-fencing.test.cjs` | `persists a restart-stable namespace` | → FAIL（resolveInstanceNamespace 未实现） |
| reconcile 实例隔离 | 同上 | `does not reap other-instance` | → FAIL（reconcile 无 namespace 维度） |
| 旧无 namespace fail-closed | 同上 | `fail-closed on legacy no-namespace` | → FAIL（无 fail_closed 信号） |
| contract→runtime 投影 | 同上 | `projects contract_requirements.postgres` | → FAIL（projectContractRequirements 未导出） |
| expired 终态化+lineage+幂等（真 PG） | `packages/brain/src/__tests__/integration/kernel-instance-fencing-lineage.pg.integration.test.js` | `records restart_reason lineage` / `no second replacement` | → FAIL（fleet-worker deferral guard 导致 parent 恒 running、child=0） |
