...
|---|---|---|---|
| 跨角色窗口化核心（本 sprint 冻结） | `sprints/08230711-kernel-bae539c8/tests/step3-runner-failure-role-window.test.js` | 仍走 publish 重派 / 仍进人审 exhausted / 同角色计数只数自己 | RED: 2 failed（`仍走 publish 重派` + `同角色计数只数自己` 复现跨角色误耗，derive 返回 exhausted；本地实证 `Tests 2 failed \| 1 passed (3)`） |

> **本表只登记本 sprint 冻结产物**（唯一行，已落盘并进 commit）：`sprints/08230711-kernel-bae539c8/tests/step3-runner-failure-role-window.test.js`，3 条 it() 真 import real derive.js。「BEHAVIOR 覆盖」列每项均为对应 it() 名的字面子串（`仍走 publish 重派`⊂it#1「跨角色不误耗…仍走 publish 重派」/ `仍进人审 exhausted`⊂it#2「同角色有界语义不变…仍进人审 exhausted」/ `同角色计数只数自己`⊂it#3「同角色计数只数自己…」）。
>
> **R1→R2 修订：闭合 seal `FROZEN_CONTRACT_TEST_CONTRACT_UNRESOLVABLE`。** R1 曾把两条 repo 既有守卫（`tests/gp/f1/step3-runner-failure-retry.test.js`、`tests/gp/f1/step3-publisher-runner-failure-retry.test.js`）作为补充行登记进本表。封印时 `assertTestContractResolvable`（contract-test-paths-seal.js:89-111）对非 `sprints/` 前缀的 repo 行走 `readRepoFile(approvedSha, path)`（`git show <approvedSha>:<path>` 按批准 SHA 取 blob）；该读取在封印环境抛错，被 r50 逻辑判「登记了不存在的 repo 测试」→ 以裸路径入 unresolved → 拒封印（错误串正是这两条 repo 路径，无 `#behavior:` 后缀，证明是读取失败而非 BEHAVIOR↔it 不匹配）。R2 将这两条 repo 守卫**移出本表**、下沉到 `## 已知约束` 段与 DoD B-04/B-05/B-06（evaluator 直接 `npx vitest run tests/gp/f1/... -t "…"`，不经 readRepoFile，回归覆盖零削减）。本表只保留冻结产物行——彻底规避 repo 行的 readRepoFile 脆弱路径。repo 既有 9 条守卫仍在下方 `## E2E 验收` 全跑断言（`Tests 12 passed (12)`）与常规 CI 中执行，不回退。
