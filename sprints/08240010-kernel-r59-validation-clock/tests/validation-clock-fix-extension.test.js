...
describe('F1 validation clock 按 generator-fix 轮有界顺延', () => {
  ...
  it('乱序输入仍按 hop 重放并选第 6 次 fix 为新原点', () => {
    const fixes = Array.from({ length: 6 }, (_, index) => row(
      20 + index,
      'spawn:generator-fix',
      `2026-08-20T0${index + 1}:00:00.000Z`,
    ));

    expect(resolve([fixes[5], row(10, 'spawn:generator', generatorAt), ...fixes.slice(0, 5)]))
      .toEqual({
  ...
  it('第 7 次及以后 fix 不再顺延并沿用第 6 次期限', () => {
    const fixes = Array.from({ length: 8 }, (_, index) => row(
      20 + index,
      'spawn:generator-fix',
