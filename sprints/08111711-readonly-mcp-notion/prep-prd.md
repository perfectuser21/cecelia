...
- 资源隔离改用 macOS 原生机制：LaunchDaemon plist 的 `SoftResourceLimits`/`HardResourceLimits` 里的 `ResidentSetSize`（`oom_score_adj` 是 Linux 专属，目标机是 macOS 不适用；`MemoryLimit` 这个 key 不存在于 launchd schema，已用真实存在的 `ResidentSetSize` 替代）给 MCP 进程设 512MB 参考值——**这是 advisory 机制，不是确定性硬顶**（man page：系统内存紧张时才"优先"回收，不保证进程超限即被杀重启）；保护 Brain 靠的是纵深防御组合（连接池 max 5 + 限流 + statement_timeout 分级 + `ResidentSetSize` advisory + 崩溃限速重启 + 重启风暴告警），不是单靠这一层内存限制。**实现阶段验证方式相应调整**：确认 `ResidentSetSize` 配置存在且 `plutil -lint` 通过；实际回收行为依赖系统级内存压力，非确定性触发，不强求复现
...
- [ ] 确认 `ResidentSetSize` 配置存在且 `plutil -lint` 通过；如需验证实际内存回收行为需要在系统级内存压力下测试，非确定性触发（不作为可靠验收依据）
