...
- [ ] [ARTIFACT] 新增只读路由文件，含 `harness-selftest` 路径与固定 service 标识
...
- [ ] [ARTIFACT] 路由已挂载进 Brain 路由聚合（routes.js）
...
- [ ] [BEHAVIOR] 路由真注册：GET /api/brain/harness-selftest 返回 HTTP 200（404=未实现=FAIL，不接受 404-acceptable 旁路）
...
- [ ] [BEHAVIOR] 幂等：两次连续调用响应体逐字节一致（无状态、无副作用）
...
- [ ] [BEHAVIOR] 既有端点回归：/api/brain/health 仍返回 200 且含 status 字段（新增不破坏既有契约）
