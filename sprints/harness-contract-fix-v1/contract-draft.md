...
> 修订说明：针对 Round 1 Reviewer 反馈的 6 项必须修改 + 可选改进全部采纳。
> 主要变更：①验证命令改用精确代码块定位（indexOf 区间截取）替代 split；②新增 payload.contract_branch 赋值来源验证；③Feature 3 命令去除注释后再匹配；④required 字段补全 pr_url；⑤新增 retry_count >= 3 负向终止验证。
...
- execution.js 中 harness_fix→harness_report 的 createTask payload 包含 `contract_branch` 字段
- 所有 3 条路径中 `contract_branch` 赋值来源为 `payload.contract_branch`（不允许硬编码字面量）
...
# 路径3: execution.js harness_fix→harness_report payload 中包含 contract_branch（精确定位 harness_fix 代码块）
node -e "
  ...
  console.log('PASS: harness_fix→report payload 包含 contract_branch');
"
...
- retry_count >= 3 时**不再重试**，必须有 return/break/throw 终止语句阻止后续 createTask 执行
...
# 验证 retry_count >= 3 上限检查（去除注释后匹配，防止注释蒙混）
node -e "
  const code = require('fs').readFileSync('packages/brain/src/routes/execution.js', 'utf8');
  const idx = code.indexOf(\"harnessType === 'harness_report'\");
  const block = code.substring(idx, idx + 1500);
  ...
  console.log('PASS: 去除注释后 retry_count 上限检查 + createTask 均存在');
"

# 验证重试 payload 包含 4 个必要字段（含 pr_url）
node -e "
  const code = require('fs').readFileSync('packages/brain/src/routes/execution.js', 'utf8');
  const idx = code.indexOf(\"harnessType === 'harness_report'\");
  const block = code.substring(idx, idx + 1500);
...
# 负向验证: retry_count >= 3 后有 return/break/throw 终止（防止"打日志但继续重试"）
node -e "
  const code = require('fs').readFileSync('packages/brain/src/routes/execution.js', 'utf8');
  const idx = code.indexOf(\"harnessType === 'harness_report'\");
  const block = code.substring(idx, idx + 1500);
