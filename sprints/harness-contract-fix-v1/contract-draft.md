...
> 修订说明：R3 Reviewer 给出 REVISION 但 session 崩溃未留反馈。R4 自查后修复：
> ①Feature 1 新增第4条路径：execution.js harness_generate→harness_report（最后WS）缺少 contract_branch（R3 遗漏）；
> ②Feature 1 硬阈值从 3 条增至 4 条，覆盖 PRD 全部 5 条路径（其中 2 条已有 ✅）；
> ③Feature 3 验证命令增加 result===null 条件检查（确保重试仅在崩溃时触发，非正常完成）。
...
**行为描述**:
当 Harness pipeline 中任意环节创建下游任务时，`contract_branch` 字段必须从上游 payload 完整透传到下游 payload。PRD 定义 5 条路径：
...
node -e "
  ...
  if (!/contract_branch:\s*payload\.contract_branch/.test(failedBlock)) {
    console.error('FAIL: contract_branch 未从 payload.contract_branch 取值（可能硬编码）');
    process.exit(1);
  }
...
node -e "
  ...
  if (!/contract_branch/.test(reportBlock)) {
    console.error('FAIL: harness_generate(最后WS)→harness_report payload 缺少 contract_branch');
    process.exit(1);
  }
...
# 验证 result===null 条件存在（确保正常完成不触发重试）
node -e "
  const code = require('fs').readFileSync('packages/brain/src/routes/execution.js', 'utf8');
  const idx = code.indexOf(\"harnessType === 'harness_report'\");
  const block = code.substring(idx, idx + 1500);
