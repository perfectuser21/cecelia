...
> 修订说明：基于 R9 草案 + R9 Reviewer 反馈修复 2 个必修项 + 2 个可选改进。
> ① [必修1] Feature 1 Path 6 窗口从 600→块结束符定界（fallback 1200），同步修复 WS1 DoD 第4条 Test；
> ② [必修2] Feature 3 Command 2 增加结构性守护：验证 createTask 出现在 result null 判断之后（条件块内）；
> ③ [可选] Feature 1 Path 3/4/5 统一加注释剥离（与 Feature 3 一致）；
> ④ [可选] Feature 3 ARTIFACT 检查增加 describe/test 关键字验证。
...
**验证命令**:
```bash
# 路径3: CI通过→harness_report payload 中包含 contract_branch（精确定位 ci_passed 代码块，去除注释后匹配）
node -e "
  ...
  const nc = passedBlock.replace(/\/\/.*$/gm, '').replace(/\/\*[\s\S]*?\*\//g, '');
  if (!nc.includes('contract_branch')) {
    console.error('FAIL: CI通过→harness_report 的 createTask payload 中缺少 contract_branch');
    process.exit(1);
  }
  if (!/contract_branch:\s*payload\.contract_branch/.test(nc)) {
...
# 路径4: CI失败→harness_fix payload 中包含 contract_branch（精确定位 ci_failed 代码块，去除注释后匹配）
node -e "
  const code = require('fs').readFileSync('packages/brain/src/harness-watcher.js', 'utf8');
  const ciFailedIdx = code.indexOf(\"ciStatus === 'ci_failed'\");
  ...
  const nc = afterFailed.replace(/\/\/.*$/gm, '').replace(/\/\*[\s\S]*?\*\//g, '');
  if (!nc.includes('contract_branch')) {
...
# 路径5: execution.js harness_fix→harness_report payload 中 contract_branch 来自 harnessPayload（去除注释后匹配）
node -e "
  ...
  const nc = fixBlock.replace(/\/\/.*$/gm, '').replace(/\/\*[\s\S]*?\*\//g, '');
  if (!nc.includes('contract_branch')) {
    console.error('FAIL: harness_fix→harness_report payload 缺少 contract_branch');
    process.exit(1);
  }
  if (!/contract_branch:\s*harnessPayload\.contract_branch/.test(nc)) {
...
# 路径6: execution.js harness_generate（最后WS）→harness_report payload 中 contract_branch（块结束符定界，fallback 1200）
node -e "
  const code = require('fs').readFileSync('packages/brain/src/routes/execution.js', 'utf8');
  ...
  const rawBlock = afterGen.substring(lastWsIdx);
  const blockEnd = rawBlock.indexOf('} else {');
  const reportBlock = blockEnd > 0 ? rawBlock.substring(0, blockEnd) : rawBlock.substring(0, 1200);
  const nc = reportBlock.replace(/\/\/.*$/gm, '').replace(/\/\*[\s\S]*?\*\//g, '');
  if (!nc.includes('harness_report')) {
...
- createTask/createHarnessTask 调用必须在 result null 条件块**内**（不能在条件外无条件执行）
...
node -e "
  ...
  if (!nc.includes('createTask') && !nc.includes('createHarnessTask')) {
...
# 验证 result null 条件守护 createTask（结构性验证：createTask 必须在 result null 判断之后）
node -e "
  const code = require('fs').readFileSync('packages/brain/src/routes/execution.js', 'utf8');
  const idx = code.indexOf(\"harnessType === 'harness_report'\");
  const block = code.substring(idx, idx + 1500);
  ...
  console.log('PASS: createTask 在 result null 条件判断之后（条件守护有效）');
"
