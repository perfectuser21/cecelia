...
- **SC-002**（[ARTIFACT]）: `packages/brain/server.js` 通过 `app.use('/api/brain/build-info', ...)` 挂载该路由（注：PRD 原文写 `packages/brain/src/server.js`，实际工程结构为 `packages/brain/server.js`，无 `src` 目录前缀；合同按真实路径写以避免假阴性）
- **SC-003**（[BEHAVIOR]）: supertest GET `/api/brain/build-info` 返回 200，body 含且仅含三键
- **SC-004**（[BEHAVIOR]）: `body.package_version === JSON.parse(readFileSync('packages/brain/package.json')).version`
...
- 在 `packages/brain/server.js` 挂载路由
...
- `sprints/tests/ws1/build-info.test.js`: 新建，supertest 集成测试（注：PRD 原文写 `packages/brain/src/__tests__/build-info.test.js`，但 GAN 对抗下测试文件在 `sprints/tests/ws{N}/` 下由 Reviewer 跑红 → Generator commit 1 复制到生产路径；合同跟随 sprints/ 测试约定）
