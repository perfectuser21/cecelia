...
- **SC-002**（[ARTIFACT]）: brain `server.js` 通过 `app.use('/api/brain/build-info', ...)` 挂载该路由（注：实际路径为 `packages/brain/server.js`，PRD 原文 `packages/brain/src/server.js` 系路径手误）
...
- `packages/brain/server.js`: 修改，挂载新路由（PRD 原文写 `src/server.js` 系手误）
