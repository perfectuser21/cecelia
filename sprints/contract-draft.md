...
## Round 6 Reviewer 反馈回应（VERDICT: REVISION）

| # | Reviewer 指出 | 级别 | Round 7 回应 |
|---|---|---|---|
| 1 | Round 6 新增的 `hasBody` variant (i) `/expect\s*\(\s*[\w.]*\.body\.(iso\|timezone\|unix)/` 只锚定 `expect(res.body.iso)` 开头，不要求 `.toBe(...)` 等 matcher 后缀 → Generator 可以写 `expect(res.body.iso);` 裸 expect 通过检查却没做任何断言，重新打开 Round 5 本来要关的"行为锁半废"漏洞 | 阻塞 | 采纳"matcher 白名单强制"路径：`contract-dod-ws1.md` 第 8 条 variant (i) 收紧为 `/expect\s*\(\s*[\w.]*\.body\.(iso\|timezone\|unix)\s*\)\s*\.\s*(not\s*\.\s*)?<MATCHERS>\s*\(/`，其中 `<MATCHERS>` 是 vitest/jest matcher 白名单（`toBe` / `toEqual` / `toMatch` / `toMatchObject` / `toContain` / `toHaveProperty` / `toStrictEqual` / `toBeGreaterThan(OrEqual)?` / `toBeLessThan(OrEqual)?` / `toBeCloseTo` / `toBeDefined` / `toBeTruthy` / `toBeFalsy` / `toBeNull` / `toBeInstanceOf` 等）。允许 `.not.` 链式前缀。裸 expect 再无逃逸通道 |
| 2 | （Round 4 遗留未闭合）`hasSrv` 只要求 "import 语句存在"，Generator 可以 `import app from '../server.js'` 后自建 `const a = express(); request(a)` 绕开 server.js 的 app，从而 `server.js` 有没有写 `app.use('/api/brain/time', timeRoutes)` 对测试结果毫无影响 → 风险 2 真闭环始终未建立 | 阻塞 | 采纳"捕获变量名 + 强制 supertest 入参"路径：`contract-dod-ws1.md` 第 8 条从 `hasSrv` 升级为 "静态捕获 `import <SRV_NAME> from '../server.js'` 的 name + 静态捕获 `import <ST_NAME> from 'supertest'` 的 name + 校验 `<ST_NAME>(<SRV_NAME>)` 形态必须出现"。副作用：**禁动态 import**（Round 6 观察 C "hasSrv 动态 import 藏在 if(false) 死分支"规避通道一并关闭）。若 server.js 未真正注册路由，supertest 真跑会 404，DoD 第 11 条 vitest 运行时锁此刻才真正起作用 |
| D | （非阻塞观察）`Intl.DateTimeFormat` 对 `CET` / `Etc/GMT+0` 在 small-icu 与 full-icu Node 下行为可能不完全一致 | 非阻塞（Reviewer 明确标记） | 不动。多数 CI/dev 环境用 full-icu，目前 round-trip 未见 flaky。若后续出现相关 flaky 再显式声明 Node ICU 版本要求 |
...
**行为描述**:
Brain 暴露一个无鉴权、无副作用的只读 HTTP 端点，调用方通过 `GET /api/brain/time` 拿到 Brain 进程当前的标准化时间信息。每次调用独立计算当前时间，不缓存，字段严格为 `iso` / `timezone` / `unix` 三项。三个字段同源同一时刻，`iso` 与 `unix` 之间必须自洽。路由必须从 `packages/brain/server.js` 顶层入口挂载到 `/api/brain/time`，且必须通过"生产侧测试 **静态 import server.js default export 并用作 supertest 入参** + supertest GET + **200 状态码或带 matcher 的 body 三字段断言**"这一真实端到端路径可达，并在 vitest 下真实 exit 0（Round 4 入口锁 + Round 5 行为锁 + Round 6 断言形式收紧 + Round 7 "supertest 真用 server.js 的 app" + "matcher 白名单" 五层闭环）。
...
- **从 server.js 入口真实可达**（Round 4 双层锁 × Round 5 行为锁 × Round 6 断言形式收紧 × Round 7 supertest 入参锁 + matcher 白名单）：
  - **入口锁（Round 7 收紧）**：`packages/brain/tests/time.test.js` 必须 **静态** `import <SRV_NAME> from '../server.js'`（禁动态 import — 同步关闭 Round 6 观察 C），必须 `import <ST_NAME> from 'supertest'`，且必须出现 `<ST_NAME>(<SRV_NAME>)` 调用形态（supertest 真以 server.js 导入的 app 为入参）。过去 Generator 可 import 后自建 `const a = express(); request(a)` 绕开 server.js 注册，Round 7 关闭该通道
  - **行为锁**（Round 5 引入 × Round 6 收紧 × Round 7 matcher 白名单）：该 supertest 调用必须含 `.expect(200)` / `toBe(200)` / `toEqual(200)` 任一 200 断言，**或** 三选一带 matcher 的真实 body 断言：`expect(xxx.body.iso|timezone|unix).<MATCHER>(...)`（`<MATCHER>` 必须在 `toBe` / `toEqual` / `toMatch` / `toMatchObject` / `toContain` / `toHaveProperty` / `toStrictEqual` / `toBeGreaterThan(OrEqual)?` / `toBeLessThan(OrEqual)?` / `toBeCloseTo` / `toBeDefined` / `toBeTruthy` / `toBeFalsy` / `toBeNull` / `toBeInstanceOf` 白名单内；允许 `.not.` 前缀。裸 `expect(res.body.iso);` 不再命中）/ `expect(xxx.body).toHaveProperty('iso'|...)` / `toMatchObject({...iso/timezone/unix...})`
  - 运行时锁（DoD 第 11 条）：该生产侧测试在 vitest 下必须 exit 0 通过 —— 入口锁 + 行为锁 + 运行时三重保障。若 server.js 未真正注册路由，supertest 真跑必然 404 → vitest exit 非 0 → 红

**BEHAVIOR 覆盖**（落在 `sprints/tests/ws1/time.test.js` — Round 4 / 5 / 6 / 7 共 **8 个 it**，均走私有 express + supertest，Round 7 未改动 sprint 侧测试——修订仅发生在 DoD 第 8 条的正则形态）:
...
**ARTIFACT 覆盖**（写进 `contract-dod-ws1.md` — 共 **11 条**，Round 7 再度修订第 8 条）:
...
8. **Round 7 修订** — `packages/brain/tests/time.test.js` 必须满足**五层**：(a) **静态** `import <SRV_NAME> from '../server.js'` 捕获变量名（**禁动态 import**，同步关闭 Round 6 观察 C），(b) `import <ST_NAME> from 'supertest'` 捕获变量名，(c) **`<ST_NAME>(<SRV_NAME>)` 形态必须出现**（supertest 真用 server.js 导入的 app，Round 4 遗留的"import 存在 ≠ 被 supertest 使用"风险 2 闭环），(d) `.get('/api/brain/time')` 调用，(e) **`.expect(200)` / `toBe(200)` / `toEqual(200)` 任一 200 断言**，**或** 三选一带 **matcher** 的真实 body 断言：`expect(xxx.body.iso|timezone|unix).<MATCHER>(...)`（`<MATCHER>` 必须在白名单内：`toBe` / `toEqual` / `toMatch` / `toMatchObject` / `toContain` / `toHaveProperty` / `toStrictEqual` / `toBeGreaterThan(OrEqual)?` / `toBeLessThan(OrEqual)?` / `toBeCloseTo` / `toBeDefined` / `toBeTruthy` / `toBeFalsy` / `toBeNull` / `toBeInstanceOf`；允许 `.not.` 前缀）/ `expect(xxx.body).toHaveProperty('iso'|...)` / `toMatchObject({...iso/timezone/unix...})` —— Round 7 把 Round 6 的 `/expect\s*\(\s*[\w.]*\.body\.(iso|timezone|unix)/`（只锚定开头）收紧到必须带 matcher 后缀，封堵 Round 6 新开的"裸 expect" 规避通道（如 `expect(res.body.iso);`）
...
3. 新增 `packages/brain/tests/time.test.js`（必须既 import `../src/routes/time.js` 做私有 supertest 又 **静态** import `../server.js` default export 并用作 supertest 入参做真实 supertest，**且 supertest 调用必须含 `.expect(200)` 或带 matcher 的 body 三字段断言** —— Round 7 第 8 条 DoD 强制，裸 expect 不再命中；动态 import 不再允许）
...
**DoD 运行时锁附加运行命令**（Round 4 新增，Round 5/6/7 配合 `.expect(200)` / 带 matcher 的 body 断言 + supertest 入参锁强化行为）:

```bash
cd packages/brain && npx vitest run tests/time.test.js --no-coverage --reporter=basic
```

此命令在 Red 阶段也会失败（`packages/brain/tests/time.test.js` 尚不存在），在 Green commit 完成后必须 exit 0——Round 7 第 8 条 DoD 保证该测试本身必然是"supertest 以 server.js 导入的 app 为入参，且含 `.expect(200)` 或带 matcher 的 body 三字段断言"，因此 exit 0 等价于"HTTP 200 + 三字段 schema 在运行时被真实断言验证 + server.js 注册真的生效"。

---

## Red 证据说明（Round 7 本地实跑）
...
3. 新增 `packages/brain/tests/time.test.js`：
   ...
   - **且** 至少含 `.expect(200)` 或三选一带 **matcher 白名单** 的 body 断言：`expect(xxx.body.iso|timezone|unix).toBe/toEqual/toMatch/...(...)` / `expect(xxx.body).toHaveProperty('iso'|...)` / `toMatchObject({...iso...})` ——Round 7 第 8 条强制，裸 `expect(res.body.iso);` 不再命中
   ...
   - **职责分工（观察 B 显式说明）**：此文件与 `sprints/tests/ws1/time.test.js` 职责不同不得合并——
     ...
     - `packages/brain/tests/time.test.js` = 入口可达三层锁（静态 import server.js default export + 强制 supertest 入参 + 带 matcher 的 body/200 断言，由 brain 包 vitest 配置执行 + DoD 第 11 条直接 `npx vitest run` 拉起）
     - 两者跑不同 config、覆盖不同语义，试图合并会导致"路由注册正确但 server.js 注入失败"这类假绿回到视野之外
4. `CLAUDE.md` 第 7 节追加 `GET /api/brain/time` 一行

届时 `sprints/tests/ws1/time.test.js` 的 8 个 it 全部转 Green，DoD 11 条 ARTIFACT 命令全部 exit 0（含第 11 条 vitest 运行时通过，vitest 进程真实看到"supertest 用 server.js 的 app 发起 GET 拿到 200 + 三字段断言通过"）。
