...
**可观测行为**: `writeInitiativeRunEvent` helper 可直接调用；调用后 1 分钟内 `initiative_run_events` 有对应行；字段值正确
...
**范围**: `packages/brain/src/routes/harness.js` 新增 `GET /stream`（SSE，2s 轮询，30s keepalive）+ `packages/brain/src/events/initiativeRunEvents.js` 事件写入 helper
**大小**: M(100-300行)
**依赖**: Workstream 1 完成后
