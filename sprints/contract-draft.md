...
### Step 4: 禁用字段反向 + error path

**可观测行为**: response 中不存在 `value`/`answer`/`data` 等禁用字段；n=foo（非数字）→ HTTP 400 + `{"error":"..."}`
...
CODE=$(curl -s -o /dev/null -w "%{http_code}" "localhost:3093/abs?n=foo")
...
**描述**: 多步骤使用不同端口（3091-3095），避免残留进程造成假绿。
...
# 8. error path — 非数字 → 400
CODE=$(curl -s -o /dev/null -w "%{http_code}" "localhost:3095/abs?n=foo")
[ "$CODE" = "400" ] || { echo "FAIL: 非数字未返 400，实际=$CODE"; kill $SPID; exit 1; }

# 9. error path — 缺少 n 参数 → 400
CODE2=$(curl -s -o /dev/null -w "%{http_code}" "localhost:3095/abs")
...
|---|---|---|---|
| WS1 | `sprints/contract-dod-ws1.md` [BEHAVIOR]×6（manual:bash 内嵌命令） | result 字段值、operation 字段值、result 类型 number、schema keys 完整性、禁用字段 value/answer 反向、error path 400 | 修复前所有 6 条 manual:bash 命令 exit 1（server.js 无 /abs 路由） |
...
5. **[BEHAVIOR] 数量**: 6 条（≥4 阈值）✓ — 覆盖 schema 字段值、keys 完整性、禁用字段反向、error path 各至少 1 条
