...
**可观测行为**: 文档分别解释 POST 仅做异步派发并返回 `attempt_id`、GET 使用该 `attempt_id` 轮询结果；并明确无论从宿主还是远端调用，两个端点都必须携带 `Authorization: Bearer $CECELIA_INTERNAL_TOKEN`。

**验证命令**:
```bash
npx vitest run --no-cache sprints/coding-harness-20260831163508-4cvw64/tests/attempt-run-bridge-guide.test.ts -t "POST 仅异步派发并返回 attempt_id|GET 使用 POST 返回的 attempt_id 轮询|宿主和远端调用均携带 Bearer token"
```

**硬阈值**: 三个独立测试分别机检 POST 异步派发、GET 按 POST 返回的 attempt id 轮询、宿主与远端对两个端点均携带 Bearer token；任一测试失败则命令退出非 0。
...
|---|---|---|---|
| attempt-run 端点与鉴权 | `sprints/coding-harness-20260831163508-4cvw64/tests/attempt-run-bridge-guide.test.ts` | `POST 仅异步派发并返回 attempt_id`、`GET 使用 POST 返回的 attempt_id 轮询`、`宿主和远端调用均携带 Bearer token` | 目标文档尚不存在，3 个端点/鉴权测试均失败 |
| attempt-run 其余说明 | `sprints/coding-harness-20260831163508-4cvw64/tests/attempt-run-bridge-guide.test.ts` | `列出完整九项角色白名单`、`说明 payload 必填字段与 base_sha 省略规则`、`说明派发失败的三资源回滚状态` | 目标文档尚不存在，3 个内容测试均失败 |
