...
- MapPage ↔ 浏览器 `fetch` ↔ Brain map API（Final E2E 必须真请求，不得 `page.route()` 或替换响应）。
...
**验证命令**: `(cd apps/dashboard && npx vitest run --no-cache src/pages/map/MapPage.test.tsx)`
...
curl -sf 'http://localhost:5221/api/brain/map?scope=cecelia' > "$API_JSON"
jq -e '.scope_key=="cecelia" and (.nodes|length>0) and (.edges|length>0)' "$API_JSON" >/dev/null
cat > /tmp/map-final-e2e.mjs <<'JS'
import { chromium } from '@playwright/test';
import fs from 'node:fs';
const api = JSON.parse(fs.readFileSync(process.env.API_JSON, 'utf8'));
