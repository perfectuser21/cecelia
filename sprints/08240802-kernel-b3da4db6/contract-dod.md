...
- [ ] [BEHAVIOR] [L3] B-04: 非 fresh、真实网络失败与空投影均 fail closed [接缝×2]
  动作: 在真实浏览器中断网触发 HTTP 失败，并在 live 双 scope 响应存在 non-fresh/空 nodes 样本时逐一加载。
  预期观察: HTTP 失败清除旧节点与“新鲜”徽标；live non-fresh/空投影显示明确警示/空态；缺 live 样本的接缝保持 logic-done-pending。
  等待预算: 10s
  留证: `staging-map-http-error.png`、Playwright 输出及 live 样本的 freshness/scope 字段。
  Test: manual:bash -c 'awk '\''/^## E2E 验收/{f=1;next} f&&/^## /{exit} f&&/^```bash/{b=1;next} b&&/^```/{exit} b{print}'\'' sprints/08240802-kernel-b3da4db6/contract-draft.md >/tmp/map-contract-e2e.sh && bash /tmp/map-contract-e2e.sh'

- [ ] [BEHAVIOR] [L2] B-05: 冻结逻辑覆盖非 fresh、HTTP 失败、空 nodes、搜索空态与竞态
  动作: 执行本 sprint 冻结 Vitest，覆盖无法稳定由 live API 制造的异常输入与最后请求提交逻辑。
  预期观察: 五个字面用例启动且在实现前 RED；实现后全部通过，但不替代 B-04 真浏览器接缝。
  等待预算: 10s
  留证: 冻结 Vitest 逐用例输出。
