...
- 允许不真触发的更外层：`auto-learning.js`（fire-and-forget，requeue 路径下 status='requeued' 走 no-op 分支，真 import 真执行但无副作用；学习文本生成属 capture_atoms 路由，PRD 明确不在范围内）、dmesg（macOS 平台天然返回 null，无需注入）
...
| # | 接缝点 | 真目标验证方式 | 状态 |
|---|--------|----------------|------|
| 1 | 代码 ↔ 真 Postgres（watchdog_kill/error_message/failure_class 落库） | tests + E2E 真连 cecelia_test（db-config.js 同源解析），psql 直查断言 | 本合同内真验 |
...
## Golden Path
[从未启动任务出现] → [watchdog 双确认探针] → [分类 never_started + 字段不覆盖] → [下游分类保真] → [回归护栏 + 永久入 CI]
...
### Step 5: 下游分类保真——classifier 不落 transient 假通道
...
trap "psql \"$TEST_DB_URL\" -c \"DELETE FROM tasks WHERE id='$TID'\" >/dev/null 2>&1 || true" EXIT
rm -f "/tmp/cecelia-${TID}.log"
...
# 6. 下游分类保真：never_started 文本不落 transient 环境重试假通道
...
echo "OK Golden Path 验证通过: never_started 分类保真 + 字段不覆盖 + 回归护栏 + 永久入 CI"
```
...
**Red 实测证据（proposer 本机，2026-08-03）**：`NODE_ENV=test npx vitest run sprints/08041147-relay-2c1a4771/tests/` → **Tests 3 failed | 2 passed (5)**，失败原因与预期逐条一致（process_disappeared / transient）。
