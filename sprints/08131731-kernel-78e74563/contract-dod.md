...
- [ ] [ARTIFACT] 永久回归测试文件落在 brain CI 覆盖路径（VP3 永久保留要求）
...
- [ ] [ARTIFACT] platform-utils.js 导出内核 pressure 映射纯函数 resolveMemPressureRatio
...
- [ ] [ARTIFACT] 回归测试无 vi.mock/stub 被改的边（禁 mock 边清单执法：CONTRACT IS LAW）
...
- [ ] [ARTIFACT] 可观测留痕（NFR）：fleet 采集链在 online 机器 effective 归零 / physical 触发下限兜底时 warn 留痕，不静默无限退避
...
- [ ] [BEHAVIOR] [L2] B-01: 根因① 本机采集真实主机资源，physical 不再触底兜底 2
...
- [ ] [BEHAVIOR] [L2] B-02: 根因② macOS 内核 pressure 等级映射，darwin free% 路径不再参与
...
- [ ] [BEHAVIOR] [L2] B-03: 根因③ effective≥1 角色分配保底，消灭 floor 归零死区且 drained/offline 仍不可派
...
- [ ] [BEHAVIOR] [L2] B-04: 出口真验（接缝）——部署后 us-mac-m4 容量回正，proposer 真实可派 [接缝×2]
...
- [ ] [BEHAVIOR] INV-1 [口径三源失真] 修复后本机口径反映主机真实资源，不再产生假容量退化
