...
**任务性质**: 单文件模板升级（S-M 规模）
**修订说明**: 根据 Round 2 Reviewer 反馈修复 3 项——统一将 `includes()` 匹配范围从全文限定到目标区域（复用 REVISION#5 思路）

---

## Reviewer Round 2 反馈修复追踪

| # | 反馈类型 | 原始问题 | 修复方式 |
|---|----------|----------|----------|
| R3-1 | 命令太弱 | F1-C2 边界声明全文 `includes()` 可被 changelog 绕过 | 用 regex 提取 Step 0 区域后再检查 |
| R3-2 | 命令太弱 | F3-C1 9 类歧义全文 `includes()` 可被附录/术语表绕过 | 限定匹配到 `执行流程` 之后 |
| R3-3 | 命令太弱 | F4-C1 OKR 对齐全文短关键词可被 changelog 绕过 | 限定匹配到 `执行流程` 之后 |

**Round 1→2 修复保留**（Round 2 已验证通过的修复项不变）:
- REVISION#1: 版本号用 regex 匹配 frontmatter `version:` 行
- REVISION#5: F2-C1 六结构检查限定到执行流程区域
...
**验证命令**:
```bash
# F1-C1: 验证 Step 0 包含 Brain API 调用（无变化，Step 0 命令本身就限定了上下文）
node -e "
  const fs = require('fs');
...
# F1-C2: 验证 Step 0 区域内有边界声明 [R3-1: 限定到 Step 0 区域]
node -e "
  const fs = require('fs');
  const content = fs.readFileSync('packages/workflows/skills/harness-planner/SKILL.md', 'utf8');
...
**验证命令**:
```bash
# F2-C1: 验证执行流程区域内 6 个结构（REVISION#5 保留，无需修改）
node -e "
  const fs = require('fs');
  const content = fs.readFileSync('packages/workflows/skills/harness-planner/SKILL.md', 'utf8');
...
**硬阈值**:
- SKILL.md 执行流程区域内包含 9 类歧义检查的完整列表
- 执行流程区域内明确说明无法推断的项标记为 `[ASSUMPTION: ...]`
- 执行流程区域内明确说明"只有影响方向性决策的歧义才提问"的原则

**验证命令**:
```bash
# F3-C1: 验证执行流程区域内 9 类歧义 + ASSUMPTION + 方向性决策原则 [R3-2: 限定到执行流程区域]
node -e "
  const fs = require('fs');
  const content = fs.readFileSync('packages/workflows/skills/harness-planner/SKILL.md', 'utf8');
  const idx = content.indexOf('执行流程');
...
# F3-C2: 边界验证——ASSUMPTION 标记出现在执行流程区域内 [R3-2 一致性: 同步限定]
node -e "
  const fs = require('fs');
  const content = fs.readFileSync('packages/workflows/skills/harness-planner/SKILL.md', 'utf8');
  const idx = content.indexOf('执行流程');
...
**验证命令**:
```bash
# F4-C1: 验证执行流程区域内 OKR 对齐章节完整性 [R3-3: 限定到执行流程区域]
node -e "
  const fs = require('fs');
  const content = fs.readFileSync('packages/workflows/skills/harness-planner/SKILL.md', 'utf8');
  const idx = content.indexOf('执行流程');
...
# F4-C2: 验证版本号（REVISION#1 保留，无需修改）
node -e "
  const fs = require('fs');
  const content = fs.readFileSync('packages/workflows/skills/harness-planner/SKILL.md', 'utf8');
...
- [ ] [BEHAVIOR] 执行流程区域内包含 9 类歧义自检列表 + `[ASSUMPTION: ...]` 标记 + "方向性"决策提问原则 [R3-2: 限定到执行流程]
  Test: node -e "const c=require('fs').readFileSync('packages/workflows/skills/harness-planner/SKILL.md','utf8');const idx=c.indexOf('执行流程');if(idx===-1){console.error('NO 执行流程');process.exit(1)}const tpl=c.substring(idx);const k=['功能范围','数据模型','UX','非功能需求','集成点','边界','约束','术语','完成信号'];let m=k.filter(x=>!tpl.includes(x));if(m.length>0){console.error('MISS:'+m);process.exit(1)}if(!tpl.includes('ASSUMPTION')){console.error('NO ASSUMPTION');process.exit(1)}if(!tpl.includes('方向性')){console.error('NO 方向性');process.exit(1)}console.log('PASS')"
- [ ] [BEHAVIOR] 执行流程区域内 PRD 模板包含 `## OKR 对齐` 章节，含 KR + 进度 + 推进三字段 [R3-3: 限定到执行流程]
  Test: node -e "const c=require('fs').readFileSync('packages/workflows/skills/harness-planner/SKILL.md','utf8');const idx=c.indexOf('执行流程');if(idx===-1){console.error('NO 执行流程');process.exit(1)}const tpl=c.substring(idx);if(!tpl.includes('OKR 对齐')){console.error('NO OKR');process.exit(1)}if(!tpl.includes('KR')||!tpl.includes('进度')||!tpl.includes('推进')){console.error('INCOMPLETE');process.exit(1)}console.log('PASS')"
