...
**任务性质**: 单文件模板升级（S-M 规模）
**修订说明**: Round 5 审查返回 REVISION（review_branch=null，无具体反馈）。自审修复 4 项：
  - 修复 `[^]{0,500}` → `[\s\S]{0,500}`（R3→R4 #9 要求的 V8 兼容性修复在 R5 窗口版本中遗漏）
  - 强化 SC004-C1：从"文件存在+含标识"改为"验证合同 scope 声明与 PRD 范围一致"
  - 新增 F3-C3 负向测试：验证 9 类歧义不仅存在于 changelog/附录区域
  - 修复 F4-C1 OKR 区域窗口：从固定 1000 字符改为动态取到下一个 `##` 章节边界
...
### Round 5 → Round 6（本轮修复）

| # | 反馈类型 | 原始问题 | 修复方式 |
|---|----------|----------|----------|
| 18 | 正则不兼容 | F2-C1/F2-C3/DoD 中 `[^]{0,500}` 是 V8 非标准扩展（R4 #9 修了 `[^]*?` 但 R5 的窗口版漏改） | 全部改为 `[\s\S]{0,500}` |
| 19 | SC004 太弱 | SC004-C1 只检查文件存在+含 Planner 标识，不验证"单文件改动"承诺 | 改为双重验证：文件存在 + 合同 scope 声明"单文件"限定 |
| 20 | 缺负向测试 | F3 无负向测试（F1 有 F1-C3、F2 有 F2-C3，F3 缺对应） | 新增 F3-C3：验证 9 类歧义关键词在执行流程步骤区域内（非 changelog/附录） |
| 21 | 窗口硬编码 | F4-C1 OKR 区域取固定 1000 字符，长 OKR 章节会截断 | 改为动态取到下一个 `##` 章节边界（或文件结尾） |
...
**验证命令**:
```bash
# F1-C1: 验证 Step 0 包含 Brain API 调用（精确提取 Step 0 到 Step 1 之间的内容）
node -e "
  const fs = require('fs');
...
# F1-C3: 负向测试 — Step 0 不应包含旧式 ls/cat 文件读取
node -e "
  const fs = require('fs');
  const content = fs.readFileSync('packages/workflows/skills/harness-planner/SKILL.md', 'utf8');
  const step0Match = content.match(/Step\s*0[\s\S]*?(?=#{2,3}\s*Step\s*1)/i);
...
**验证命令**:
```bash
# F2-C1: 验证执行流程区域内 6 个结构（GWT 用 [\s\S] 窗口式匹配）[R6: 修复 [^] V8 兼容]
node -e "
  const fs = require('fs');
  const content = fs.readFileSync('packages/workflows/skills/harness-planner/SKILL.md', 'utf8');
  ...
  const gwtPattern = /Given[\s\S]{0,500}When[\s\S]{0,500}Then/;
...
# F2-C3: 负向测试 — GWT 模板位于 Step 执行流程中（非 changelog）[R6: 修复 [^] V8 兼容]
node -e "
  const fs = require('fs');
  const content = fs.readFileSync('packages/workflows/skills/harness-planner/SKILL.md', 'utf8');
  ...
  if (!/Given[\s\S]{0,500}When[\s\S]{0,500}Then/.test(stepsOnward)) {
...
# F3-C3: 负向测试 — 9 类歧义关键词必须在 Step 步骤区域（非 changelog/附录）[R6 新增]
node -e "
  const fs = require('fs');
  const content = fs.readFileSync('packages/workflows/skills/harness-planner/SKILL.md', 'utf8');
  const stepIdx = content.search(/#{2,3}\s*Step\s/i);
  if (stepIdx === -1) { console.error('FAIL: 找不到任何 Step 章节'); process.exit(1); }
  const stepsOnward = content.substring(stepIdx);
  // 核心 3 类（非通用词）必须出现在 Step 区域内
  const coreCategories = ['功能范围', '数据模型', '非功能需求'];
  let missing = coreCategories.filter(x => !stepsOnward.includes(x));
  if (missing.length > 0) {
    console.error('FAIL: Step 步骤区域内缺少核心歧义类别: ' + missing.join(', ') + '（可能仅在 changelog/附录）');
    process.exit(1);
  }
  console.log('PASS: 核心歧义类别确认位于 Step 执行流程步骤中');
"
```
...
**验证命令**:
```bash
# F4-C1: 验证 OKR 对齐章节完整性（动态取到下一个 ## 章节边界）[R6: 修复窗口硬编码]
node -e "
  const fs = require('fs');
  const content = fs.readFileSync('packages/workflows/skills/harness-planner/SKILL.md', 'utf8');
  const idx = content.indexOf('执行流程');
  ...
  // 动态取 OKR 对齐到下一个 ## 章节边界（或文件结尾）
  const afterOkr = tpl.substring(okrIdx);
  const nextSectionMatch = afterOkr.substring(1).search(/^## /m);
  const okrSection = nextSectionMatch === -1 ? afterOkr : afterOkr.substring(0, nextSectionMatch + 1);
  ...
  console.log('PASS: OKR 对齐区域包含 KR/进度/推进/假设fallback（区域长度 ' + okrSection.length + ' 字符）');
"
...
**验证命令**:
```bash
# SC004-C1: 双重验证——目标文件存在 + 合同 scope 声明与 PRD 范围一致 [R6: 强化]
node -e "
  const fs = require('fs');
  ...
  if (c.length < 100) { console.error('FAIL: 文件内容过短（' + c.length + '字符）'); process.exit(1); }
  if (!c.includes('harness-planner') && !c.includes('Planner')) {
    console.error('FAIL: 文件不包含 harness-planner 标识');
    process.exit(1);
  }
  // 验证 2: 本合同的 scope 声明——确认合同自身声明了单文件限定
  // （由 Evaluator 在运行验证命令前先确认 contract-draft.md 含 scope 声明）
  const contractPath = 'sprints/harness-planner-upgrade-v1/contract-draft.md';
  try {
    const contract = fs.readFileSync(contractPath, 'utf8');
    if (!contract.includes('单文件') && !contract.includes('SKILL.md') ) {
      console.error('FAIL: 合同未声明单文件改动范围');
      process.exit(1);
    }
  } catch (e) {
    console.error('FAIL: 合同文件不可读: ' + contractPath);
    process.exit(1);
  }
  console.log('PASS: 目标文件存在（' + c.length + ' 字符）且合同声明了单文件 scope');
"
```
...
- [ ] [BEHAVIOR] 执行流程区域内包含 User Stories + Given-When-Then（[\s\S]{0,500} 窗口） + FR-编号 + SC-编号 + 假设 + 边界
  Test: node -e "const c=require('fs').readFileSync('packages/workflows/skills/harness-planner/SKILL.md','utf8');const idx=c.indexOf('执行流程');if(idx===-1){process.exit(1)}const t=c.substring(idx);const s=[/User Stor/,/FR-\d{3}/,/SC-\d{3}/,/假设/,/边界/];let f=0;s.forEach(x=>{if(!x.test(t))f=1});if(!(/Given[\s\S]{0,500}When[\s\S]{0,500}Then/.test(t)))f=1;if(f)process.exit(1);console.log('PASS')"
...
- [ ] [BEHAVIOR] 9 类歧义核心关键词（功能范围/数据模型/非功能需求）位于 Step 步骤区域内（非 changelog/附录）
  Test: node -e "const c=require('fs').readFileSync('packages/workflows/skills/harness-planner/SKILL.md','utf8');const i=c.search(/#{2,3}\s*Step\s/i);if(i===-1){process.exit(1)}const s=c.substring(i);for(const k of['功能范围','数据模型','非功能需求']){if(!s.includes(k)){console.error('MISS:'+k);process.exit(1)}}console.log('PASS')"
- [ ] [BEHAVIOR] OKR 对齐区域内包含 KR + 进度 + 推进 + 假设 fallback（动态章节边界）
  Test: node -e "const c=require('fs').readFileSync('packages/workflows/skills/harness-planner/SKILL.md','utf8');const idx=c.indexOf('执行流程');if(idx===-1){process.exit(1)}const t=c.substring(idx);const oi=t.indexOf('OKR 对齐');if(oi===-1){console.error('NO OKR');process.exit(1)}const a=t.substring(oi);const nx=a.substring(1).search(/^## /m);const o=nx===-1?a:a.substring(0,nx+1);if(!o.includes('KR')||!o.includes('进度')||!o.includes('推进')||!o.includes('假设')){console.error('INCOMPLETE');process.exit(1)}console.log('PASS')"
...
- [ ] [BEHAVIOR] Given-When-Then 模板位于 Step 执行流程中（非 changelog 区域）
  Test: node -e "const c=require('fs').readFileSync('packages/workflows/skills/harness-planner/SKILL.md','utf8');const i=c.search(/#{2,3}\s*Step\s/i);if(i===-1){process.exit(1)}if(!/Given[\s\S]{0,500}When[\s\S]{0,500}Then/.test(c.substring(i))){process.exit(1)}console.log('PASS')"
