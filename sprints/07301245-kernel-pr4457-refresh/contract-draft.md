...
- Round 6 修订焦点：task bundle 未提供 Reviewer feedback、已批准的冻结来源或冲突计数拍板；继续维持 Round 5 的最小 fail-closed 合同，不在事实矛盾未解决时扩大合同或授权 generator 写入既有 PR。
- Round 5 修订焦点：本轮 task bundle 仍未附 Reviewer feedback 或已拍板的新冻结证据；维持上一轮最小合同与 fail-closed 前置闸，不把已知的 `32/33`、`77/761` 事实差异伪装成完成态。Generator 在拍板前仅可生成只读证据，禁止整合、处置告警或改动 PR #4457。
