...
- Round 7 采用 controller 的权威 machine facts，消除旧版 `32/33` 与 CodeQL 口径歧义：总冲突路径固定为 33（32 个 content + `DoD.md` 1 个 modify/delete）；77 固定指 check-run `90774353140` 的 PR-new-alert annotations。
...
| 2 | `.github/workflows/ci.yml` | content | semantic-merge | exact required contexts、CodeQL 扫描范围、workflow 语法与行为 |
...
| 8 | `packages/brain/config/fleet-node-profiles.json` | content | semantic-merge | profile schema + fleet-node 行为测试 |
...
| 25 | `packages/brain/vitest.config.js` | content | semantic-merge | test discovery 列表与实际执行集合相等 |
...
**可观测行为**: verifier 真读 Git object、check-run annotations 与 branch protection，逐字段核对上节全部身份和枚举。
**验证命令**:
```bash
...
**可观测行为**: resolution ledger 的路径集合与表格 exact-set 相等，33 行无重复、无未处置；每行运行指定内容/行为 oracle，禁止仅 JSON parse 或源码字符串存在检查。
**验证命令**:
```bash
...
**可观测行为**: disposition ledger 对 77 个 subject key exact-set 覆盖，每条有 rule/severity/path/line/classification/disposition/recheck；真实问题修复，范围外/误报仅留证且不得 dismiss。
**验证命令**:
```bash
...
**可观测行为**: QuickCheck fail-closed、node:test 双登记、OKR in-process `cecelia_test`、migration 369-381、上一轮四项 blocker 及表中受影响行为 oracle 真跑；atomic truth 不变。
**验证命令**:
```bash
...
**可观测行为**: 三个 exact context 的 check-run/check-suite head SHA 全等于同一 final PR head，结论 SUCCESS。
**验证命令**:
```bash
...
**可观测行为**: `evidence/audit-baseline.json` 在首次写目标分支前记录 `audit_start_utc`、actor、run/attempt/task、PR 列表、#4457 状态、main SHA、deployments；`audit-end.json` 在 evaluator 后记录 `audit_end_utc`。闭区间 `[audit_start_utc,audit_end_utc]` 内，以 target ref、final SHA、run/attempt/task/sprint marker 归因本任务；归因集合不得含 pull_request.created、merged/auto_merge、deployment。
**验证命令**:
```bash
node sprints/07301245-kernel-pr4457-refresh/scripts/verify-pr4457-evidence.mjs review-gate
```
**硬阈值**: #4457 `OPEN/isDraft=true/autoMergeRequest=null/mergedAt=null`，headRefName 固定；main 不包含 final head；归因的新 PR、merge、auto-merge、staging/production deployment 均为 0；所有 REST 分页完整且区间端点明确。上述命令执法。
