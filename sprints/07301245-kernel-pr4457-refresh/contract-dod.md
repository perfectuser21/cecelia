...
- [ ] [ARTIFACT] `evidence/freeze.json`、`conflict-resolution.json`、`codeql-freeze.json`、`codeql-disposition.json`、`regressions.json`、`exact-head.json`、`evaluator.json`、`audit-baseline.json`、`audit-end.json` 均按合同 schema 生成并绑定不可变 ID/SHA。
  Test: node -e "const fs=require('fs');const d='sprints/07301245-kernel-pr4457-refresh/evidence/';for(const f of ['freeze.json','conflict-resolution.json','codeql-freeze.json','codeql-disposition.json','regressions.json','exact-head.json','evaluator.json','audit-baseline.json','audit-end.json'])fs.accessSync(d+f)"
...
- [ ] [BEHAVIOR] [L3] B-01: 冻结身份与全部 subject 精确匹配 [接缝×2]
  ...
  预期观察: 33 个冲突路径、77 条 annotation、三个 required context 均 exact-set 匹配冻结身份与 hash
  等待预算: 120s
...
- [ ] [BEHAVIOR] [L2] B-02: 全部 33 个冲突路径完成内容与行为验证
  ...
  预期观察: 路径集合与合同表 exact-set 相等，32 content 加 1 modify/delete 全部唯一处置，无 unresolved，真实 oracle 全为 exit 0
  等待预算: 900s
...
- [ ] [BEHAVIOR] [L3] B-03: 全部 77 条 CodeQL annotation 收敛 [接缝×2]
  ...
  预期观察: 7 critical、59 high、11 medium 全部唯一覆盖，无 unclassified/dismissed/扫描缩小，final head 无同 subject 未解决项
  等待预算: 1800s
...
- [ ] [BEHAVIOR] [L2] B-04: 累计 Kernel Harness 行为与 atomic truth 保持
  ...
  预期观察: 每个命名命令由真实解释器执行且 exit 0；atomic truth 保持 true/false/false/0/99
  等待预算: 1200s
...
- [ ] [BEHAVIOR] [L3] B-05: 三个 required checks 绑定同一最终 SHA [接缝×2]
  ...
  预期观察: strict=true，context 集合与冻结 hash 一致，三者 head SHA 等于读取前后不变的 PR final head 且结论 SUCCESS
  等待预算: 1800s
...
- [ ] [BEHAVIOR] [L3] B-07: 审计窗内无新 PR 无 merge 无 deploy [接缝×2]
  动作: 用首次 mutation 前基线与 evaluator 后终点，分页真读 PR/events/check/deployments 并按 target ref/final SHA/run/attempt/task/sprint marker 归因
