...
- [ ] [BEHAVIOR] [L1] B-01: 读者能找到两个端点用途与鉴权要求
  ...
  预期观察: 文档分别说明 POST 创建/派发、GET 按 id 查询，并区分 loopback 与宿主/远端 Bearer 鉴权
  等待预算: 0s
...
- [ ] [BEHAVIOR] [L1] B-04: 读者能识别派发失败的完整回滚结果
  动作: 阅读派发失败自动回滚章节
  预期观察: 同一章节完整列出 `run→failed`、`session→closed`、`task→cancelled`
  等待预算: 0s
...
- [ ] [ARTIFACT] INV-1 规划分支：合同提交保持在 Harness 签发的 proposer 分支。
  Test: bash -c 'test "$(git branch --show-current)" = "cp-harness-propose-r1-2a748f96-r344f0e3d-a1"'
