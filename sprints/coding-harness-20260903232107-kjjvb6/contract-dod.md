...
- [ ] [BEHAVIOR] [L1] B-01: 两个端点用途封闭且无额外主端点
  动作: 读者打开文档的端点与用途章节。
  预期观察: POST 创建并异步派发，GET 按 attempt id 查询；现场计数恰为 2。
  等待预算: 0s
  留证: Vitest 详细输出中的端点集合相等与负向额外项断言。
  Test: manual:bash -c 'npx vitest run sprints/coding-harness-20260903232107-kjjvb6/tests/attempt-run-bridge-doc.contract.test.ts -t "文档准确说明两个端点用途且不存在额外主端点"'

- [ ] [BEHAVIOR] [L1] B-02: Bearer 鉴权正向说明且无泄密或远端免鉴权误导
  动作: 读者查看 loopback、宿主与远端鉴权说明。
  预期观察: 文档出现中间件名和 token 占位符；真实 token 与远端免鉴权表述均为 0。
  等待预算: 0s
  留证: Vitest 详细输出中的正向存在及两项负向匹配结果。
  Test: manual:bash -c 'npx vitest run sprints/coding-harness-20260903232107-kjjvb6/tests/attempt-run-bridge-doc.contract.test.ts -t "鉴权正向说明 Bearer 占位符且负向排除真实令牌和远端免鉴权"'
