...
# Contract DoD — 完整 Codex Slot 安全硬切换（Round 4）
...
- [ ] [BEHAVIOR] [L2] B02 GP4 四路 caller、退役 HTTP 与 executor/cache 全 consumer 都由 broker receipt 收口
  动作: 真触发四个生产 caller；让 broker 分别选 M1/M4 后各跑一次 `location=xian`；向真实 bridge POST `/execute`、`/execute-review`；再真跑 codex-usage/refresh 与 credentials health。
  预期观察: caller 与双 agent receipt/receiver 对账；executor `/health.accounts` reads=0、fixed M4 fallback=0；两个退役入口均 410 且 auth/process/lease 副作用=0；两个消费者只读安全 cache。
  ...
  期望: 真四 caller/双 agent/退役 HTTP/三 consumer/PostgreSQL；禁止静态 410、单次恰选 M4 或 mock 替代。
...
| INV-19 | 退役需生产消费者证据 | **B01 + B02 + A01**：旧脚本、四路 `/run`、`/execute*` 真 410、executor `/health.accounts`/固定 M4 fallback、`/accounts` 与 brain-meta/credentials-health 全部真跑闭环。 |
...
- [ ] [BEHAVIOR:E2E] local_api 控制面 + 真实 executor/user caller + xian-m1/xian-m4 fake-auth 完整生命周期
  ...
  期望: 4/4 caller + INV-19 全 consumer、七类 Error raw body exact、全局账号唯一、双机正常链与两支 mmv 变化、租户隔离、reaper/P0 receipt 与 DB 时间窗全部通过。
