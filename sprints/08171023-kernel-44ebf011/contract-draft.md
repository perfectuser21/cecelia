...
- [Invariant] generator_retry_identity / planner_role_branch —— 本单不触碰调度身份重试 / planner 分支，两铁律 N/A（见 DoD INV 条目）。
...
**验证命令**:
```bash
npx vitest run sprints/08171023-kernel-44ebf011/tests/ground-truth-proposal-remote.test.js sprints/08171023-kernel-44ebf011/tests/regression-run-7a8e5319-rn.test.js --reporter=basic
```
...
**验证命令**:
```bash
npx vitest run sprints/08171023-kernel-44ebf011/tests/derive-observation-gate.test.js --reporter=basic
```
...
**验证命令**:
```bash
npx vitest run sprints/08171023-kernel-44ebf011/tests/work-routing-base-repo-backfill.test.js --reporter=basic
```
...
npx vitest run sprints/08171023-kernel-44ebf011/tests/ --reporter=basic 2>&1 | tee /tmp/e2e-sprint.log
