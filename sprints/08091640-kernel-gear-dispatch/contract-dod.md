...
- [ ] [BEHAVIOR] [L2] B-01: gear=hotfix 初始态跳过 planner 直进 generate
  ...
  Test: manual:bash -c 'cd "${REPO_ROOT:-/workspace}"; L=$(npx vitest run packages/brain/src/orchestrator/__tests__/derive.test.js -t "不等于 spawn:planner" --reporter=dot 2>&1); echo "$L" | grep -qE "Tests +[0-9]+ passed" && ! echo "$L" | grep -qE "[1-9][0-9]* failed"'
...
- [ ] [BEHAVIOR] [L2] B-02: gear=hotfix 全程不派 planner/proposer/reviewer
  ...
  等待预算: 0s
  留证: vitest 输出末 5 行
  Test: manual:bash -c 'cd "${REPO_ROOT:-/workspace}"; L=$(npx vitest run packages/brain/src/orchestrator/__tests__/derive.test.js -t "全程不派" --reporter=dot 2>&1); echo "$L" | grep -qE "Tests +[0-9]+ passed" && ! echo "$L" | grep -qE "[1-9][0-9]* failed"'
...
- [ ] [BEHAVIOR] [L2] B-03: INV-1 gear=default 初始态零回归返回 spawn:planner
  ...
  Test: manual:bash -c 'cd "${REPO_ROOT:-/workspace}"; L=$(npx vitest run packages/brain/src/orchestrator/__tests__/derive.test.js --reporter=dot 2>&1); echo "$L" | grep -qE "Tests +[0-9]+ passed" && ! echo "$L" | grep -qE "[1-9][0-9]* failed"'
...
- [ ] [BEHAVIOR] [L2] B-04: gear=segmented 初始态照跑 planner（≠hotfix，对齐 controller segmented）
  ...
  等待预算: 0s
  留证: vitest 输出末 5 行
  Test: manual:bash -c 'cd "${REPO_ROOT:-/workspace}"; L=$(npx vitest run packages/brain/src/orchestrator/__tests__/derive.test.js -t "segmented 初始态 照跑 planner" --reporter=dot 2>&1); echo "$L" | grep -qE "Tests +[0-9]+ passed" && ! echo "$L" | grep -qE "[1-9][0-9]* failed"'
...
- [ ] [BEHAVIOR] [L2] B-05: INV-2 非法 gear（turbo）kernel 侧 fail-closed → mark_failed invalid_gear
  ...
  等待预算: 0s
  留证: vitest 输出末 5 行
  Test: manual:bash -c 'cd "${REPO_ROOT:-/workspace}"; L=$(npx vitest run packages/brain/src/orchestrator/__tests__/derive.test.js -t "fail-closed" --reporter=dot 2>&1); echo "$L" | grep -qE "Tests +[0-9]+ passed" && ! echo "$L" | grep -qE "[1-9][0-9]* failed"'
...
- [ ] [BEHAVIOR] [L2] B-06: gear 列 round-trip + observed.gear 注入（真 Postgres）[接缝×2]
  ...
  Test: manual:bash -c 'cd "${REPO_ROOT:-/workspace}"; export DATABASE_URL="${DB_URL:?}"; L=$(npx vitest run packages/brain/src/__tests__/integration/kernel-gear-dispatch.pg.integration.test.js -t "round-trip" --reporter=dot 2>&1); echo "$L" | grep -qE "Tests +[0-9]+ passed" && ! echo "$L" | grep -qE "[1-9][0-9]* failed"'
...
- [ ] [BEHAVIOR] [L2] B-07: hotfix run harness_attempts 角色分布 planner/proposer/reviewer=0 且 generator≥1（真 PG，时间窗防伪）[接缝×2]
  ...
  Test: manual:bash -c 'export PGURL="${DB_URL:?}"; B=$(psql "$PGURL" -tAc "SELECT count(*) FROM harness_attempts a JOIN initiative_runs r ON r.id=a.run_id WHERE r.gear='"'"'hotfix'"'"' AND a.role IN ('"'"'planner'"'"','"'"'proposer'"'"','"'"'reviewer'"'"') AND a.created_at > NOW() - interval '"'"'10 minutes'"'"'" | tr -d " "); G=$(psql "$PGURL" -tAc "SELECT count(*) FROM harness_attempts a JOIN initiative_runs r ON r.id=a.run_id WHERE r.gear='"'"'hotfix'"'"' AND a.role='"'"'generator'"'"' AND a.created_at > NOW() - interval '"'"'10 minutes'"'"'" | tr -d " "); [ "$B" = "0" ] && [ "$G" -ge 1 ]'
