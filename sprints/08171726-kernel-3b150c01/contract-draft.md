...
**sprint**: 08171726-kernel-3b150c01
**journey_type**: autonomous
**target_environment**: local_api
**base_sha (implementation_baseline, frozen)**: 0fec4503df85990a0a9b4452aa694fb63af89f2a
...
> [MAP_NOT_CONFIGURED]: task.payload 未注入 map_scope/map_repo（Step 1.0 radius 探测返回 not_configured），本轮无 must_run_assertions 注入；不回退领域硬编码，按 PRD + 代码实测起草。
> 冻结产物位置死规则（本单硬要求）: 合同冻结测试落 `sprints/08171726-kernel-3b150c01/tests/`（kernel 只从此目录采集冻结产物——前单 f9f943fc 因落 `packages/brain/src/**/__tests__/` 被判 `force_approve_but_contract_artifacts_missing`）；永久回归测试由 Generator 在实现阶段复制到 `packages/brain/src/**/__tests__/`。DoD 的 vitest 断言从仓库根跑 `sprints/**` 冻结测试（根 vitest.config.js include 覆盖 `sprints/**`，已实测可跑）。

## 锚定父路声明

独立小路（无父路）—— PrepPRD `step_id: aad25bdb-...`（journey `e6f803f2-8c48-4cce-a7a1-5b1bda5e9c29`）未锚定任何已存在 Golden Path 步骤；本 sprint 是 kernel 内部调度决策（impact-contract + orchestrator）的独立修复路径。

---

## 路径校正（PRD 文件路径 → 代码实测路径）

PRD「预期受影响文件」已给出真实路径，实测逐行核对当前代码（base_sha 0fec4503）如下（合同以实测为准）：
...
| `loop.js` | `packages/brain/src/orchestrator/loop.js` |
| `derive.js` | `packages/brain/src/orchestrator/derive.js` |
| `radius.js`（不改） | `packages/brain/src/map/radius.js` |

**实测关键事实（起草依据，已逐行核对当前代码）**：

1. `diff-gate.js` 步骤 3a（`evaluateDiffGate` 内，line 201-208）：
   ```js
...
2. `radius.js`（不改，仅作为 reason_code 来源事实）：`resolveImpactRadius` 返回
   `{ freshness:{ status, reason_code, ... }, unclaimed_files:[...], affected_nodes:[...], required_assertions:[...], fact_revisions:{...} }`。
   - 真新鲜度类（reason_code ∈ `fact_snapshot_stale`/`projection_revision_missing`/`projection_revision_mismatch`/`manifest_projection_mismatch`/`graph_projection_revision_mismatch`）。
   - 确定性类（`status:'unknown'` 且 reason_code ∈ `impact_anchor_missing`/`capability_assertion_coverage_missing`/`capability_not_in_active_projection`/`unsafe_assertion_ref`/`assertion_identity_ambiguous`）。
3. `harness-gates.js` `gateReceipt(stage, result, extra)`（line 26-36）当前只透传 `stage/gate/reason/retryable/contract_id/contract_hash` + extra，**不含 `detail`、不含 `unclaimed_files`**（实测：`beforeEvaluate` line 356 `gateReceipt('diff', result, {...})` 的 extra 无 detail/unclaimed_files）。
4. `loop.js`：
   - `DETERMINISTIC_IMPACT_ERROR_CODES`（line 84-90）当前含 `impact_contract_schema_invalid`/`impact_assertion_authority_invalid`/`mapper_evidence_invalid`/`mapper_evidence_missing`/`task_not_found`——**不含**本 sprint 的确定性 reason，需补集。
   - line 1441 `deterministic = DETERMINISTIC_IMPACT_ERROR_CODES.has(errorCode)`；line 1448/1543 `failure_class = ... ? 'impact_contract_invalid' : ...`（已按 deterministic/retryable 分流）。
   - line 1661-1664：`controlStatus === 'BLOCKED' && failureClass === 'impact_contract_invalid'` 分支**直接 `failRun('impact_gate_deterministic:...')` 并 return**，不经 derive 路由——本 sprint 要把它改成「记 BLOCKED 结果 + 由 derive 按 reason 路由 generator-fix / human_review」。
5. `constants.js`（line 60/68）：动作字面量 `SPAWN_GENERATOR_FIX='spawn:generator-fix'`、`WAIT_HUMAN_REVIEW='wait:human_review'`（PRD 验收里写的 `spawn:generator_fix` 下划线是笔误，合同一律用代码真值 `spawn:generator-fix`）。
...
**验证命令**:
```bash
npx vitest run --no-cache sprints/08171726-kernel-3b150c01/tests/diff-gate-reason-code.test.js -t 'impact_anchor_missing'
# 期望：该用例 PASS（gate=blocked / reason=impact_anchor_missing / retryable=false / detail.unclaimed_files=['DoD.md']）
```
**硬阈值**: 用例 PASS（exit 0）。验证命令（硬阈值→可执行）见 contract-dod.md B-01。
...
**验证命令**:
```bash
npx vitest run --no-cache sprints/08171726-kernel-3b150c01/tests/diff-gate-reason-code.test.js -t 'capability_assertion_coverage_missing'
# 期望：PASS
```
**硬阈值**: 用例 PASS（exit 0）。
...
**验证命令**:
```bash
npx vitest run --no-cache sprints/08171726-kernel-3b150c01/tests/diff-gate-reason-code.test.js -t 'fact_snapshot_stale'
# 期望：PASS（回归保护，行为不变）
```
**硬阈值**: 用例 PASS（exit 0）。
...
**说明**: `[AI_ADDED]`（分类骨架）——(c) 兜底分支确保未来新增枚举不被误当可重试或误放行；理由：防止 mapper 契约演进时静默漏判（既不无限重试也不误放行）。

**可观测行为**: 未知 reason_code → `gate='impact_unknown'`, `reason='mapper_contract_invalid'`, `retryable=false`（不 blocked、不放行、不可重试）。

**验证命令**:
```bash
npx vitest run --no-cache sprints/08171726-kernel-3b150c01/tests/diff-gate-reason-code.test.js -t 'mapper_contract_invalid'
# 期望：PASS
```
**硬阈值**: 用例 PASS（exit 0）。
...
**验证命令**:
```bash
npx vitest run --no-cache sprints/08171726-kernel-3b150c01/tests/harness-gates-reason-code.test.js
...
- `reason=impact_anchor_missing` → **不再 blanket failRun 空转**，下一动作 `spawn:generator-fix`（detail 携带 `unclaimed_files`）；同签名第二次仍失败 → `wait:human_review`。
...
**验证命令**:
```bash
npx vitest run --no-cache sprints/08171726-kernel-3b150c01/tests/loop-impact-deterministic-route.test.js
# 期望：PASS（generator-fix / human_review 路由 + intent 透传）
```
**硬阈值**: 用例 PASS（exit 0）。
...
**可观测行为**: scratch 库跑真实 gate 链一跳（真 `createHarnessImpactGates` + 真 `evaluateDiffGate` + 真 `appendHop`，仅注入 out-of-scope 的 mapper 录制响应），`orchestrator_decision_log` 新增行 `gate_verdict='deny:impact:impact_anchor_missing'` 且 `detail->'impact_gate'->>'retryable'='false'` 且 `detail->'impact_gate'->'unclaimed_files'` 非空。旧代码在同夹具下产出 `deny:impact:mapper_stale`（回归对照）。
...
- [回归] `packages/brain/src/impact-contract/__tests__/diff-gate.test.js` → pass/extend/drift 三类既有裁决（`freshness.status='fresh'` 路径）必须保持绿。
...
- [累积FR] context-manifest: unavailable（journey `e6f803f2` 本 line 无历史 golden_path；Brain 不在本 proposer 环境运行[runtime_resources.postgres=false]，端点不可达，按 PRD「累积 FR：本 line 暂无历史」记一行）。

## 历史约束三源 → INV 映射（EVA v2 固定段）

| 来源 | 铁律 | 本 sprint 映射 |
|---|---|---|
| PRD Invariant | [fail-closed] 未知 reason_code 必须 fail-closed（retryable=false） | **INV-2**：未知 reason_code 落 (c) fail-closed（B-04），禁静默放行 |
| PRD Invariant | [确定性不重试] Map 新鲜前提下确定性结论不得标 retryable=true | **INV-1**：确定性 reason `retryable:false`（B-01/B-02/B-04），真新鲜度 reason 仍 `retryable:true`（B-03） |
| PRD Invariant | [reason 保真] gate 结果必须透传原始 reason_code，不得覆写成 mapper_stale | **INV-3**：gateReceipt 透传 reason/retryable/detail，决策日志 `detail.impact_gate` 含 unclaimed_files（B-05/B-06/B-09） |
...
| area | [测试默认多租户] / [凭据安全] / [日志脱敏] / [端点鉴权] / [租户隔离] | N/A（无新凭据/新日志敏感字段/新端点/跨租户读写；决策日志按 run_id 隔离） |
...
| **Invariant（永不违反）** | | INV-1（确定性结论 `retryable:false`）、INV-2（fail-closed 禁静默放行）、INV-3（reason 保真 + 证据入决策日志）、INV-4（不新增 reason 枚举字面值）、INV-5（真环境落行验证），详见「历史约束三源」表 |
| **判定点（怎么知道）** | | 见「判定点登记表」 |
...
|--------|----------|----------|------|----------|
| ⚠️ 某 reason_code 属「可重试新鲜度」还是「确定性 fail-closed」 | A. 维护显式白名单集合（新鲜度集 / 确定性集）；B. 按 `freshness.status`（stale=可重试, unknown=确定性） | A. 显式白名单集合（新鲜度集含 fact_snapshot_stale/projection_revision_*/manifest_projection_mismatch/graph_projection_revision_mismatch；确定性集含 impact_anchor_missing/capability_assertion_coverage_missing/capability_not_in_active_projection/unsafe_assertion_ref/assertion_identity_ambiguous），白名单外→(c) fail-closed | 方法 B 不可靠：radius.js 里 graph_projection_revision_mismatch 是 `status:'unknown'` 却属新鲜度类，按 status 分会误判为确定性 blocked | 误判「新鲜度→确定性」会把真 stale 的可恢复重试掐死误报 human_review；误判「确定性→新鲜度」会退回本 sprint 要修的无限重试黑洞 |
...
> 被改的边全部真实执行（真 `createHarnessImpactGates` + 真 `evaluateDiffGate` + 真 `appendHop`）；仅 mapper（radius，范围外）用同形录制响应替身（已登记未覆盖真实链路清单）。
> Final E2E harness（`sprints/08171726-kernel-3b150c01/e2e/impact-gate-e2e.mjs`）由 Generator 在实现阶段交付；本段脚本 evaluator 原样执行。
...
cd "$WORKSPACE_PATH"
SPRINT_DIR="sprints/08171726-kernel-3b150c01"
...
# 3. 跑真实 gate 链落行：generator 交付的 E2E harness（真 impactGate 链 + 真 appendHop，注入 mapper 录制件）
