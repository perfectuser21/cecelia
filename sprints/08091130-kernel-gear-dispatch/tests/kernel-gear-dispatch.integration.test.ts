 ...
 *   segmented = 00000000-0000-4000-8000-00000000e6ed  (R2 新增，堵 R1-1)
...
const SEGMENTED_ID = '00000000-0000-4000-8000-00000000e6ed';
...
describe.skipIf(!process.env.DATABASE_URL)('kernel gear 三档分流（真 PG 集成）', () => {
  ...
  it('segmented run 合法持久化且不 fail-closed', async () => {
    // R1-1 修复：segmented 是合法档，必须落库 gear='segmented'、不被 finalize invalid_gear、
    // 且 planner 照跑（初始态 spawn:planner）——证 kernel 合法集校验把 segmented 当合法值透传，
    // 而非与 turbo 同路径 fail-closed。
    const { runId } = await driveKernelGearRun({ pool, gear: 'segmented', initiativeId: SEGMENTED_ID });
    const { rows } = await pool.query(
      'SELECT gear, failure_reason FROM initiative_runs WHERE id=$1', [runId],
    );
    expect(rows[0].gear).toBe('segmented'); // 合法落库，非归一 default 也非 fail-closed
    expect(String(rows[0].failure_reason || '')).not.toContain('invalid_gear');
    expect(await roleCount(runId, ['planner'])).toBeGreaterThanOrEqual(1); // planner 照跑
  });
