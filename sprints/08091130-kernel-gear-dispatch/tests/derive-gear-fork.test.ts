 ...
 *   - segmented → 合法档，初始态仍 spawn:planner（planner 照跑，非 fail-closed；R2 新增，堵 R1-1）
 * 当前 derive 完全忽略 gear（全目录 grep gear=0），prdExists=false 恒返回 spawn:planner，
 * 故 hotfix 两条断言现在必然 FAIL = RED。segmented 断言与 default 同结论（spawn:planner），
 * 但它是防「kernel 合法集校验把 segmented 漏出 GEAR_VALUES 当 turbo 同路径 fail-closed」的护栏。
 */
...
describe('kernel derive gear 分叉 [BEHAVIOR]', () => {
  ...
  it('gear=segmented 初始态 action 等于 spawn:planner', () => {
    // segmented 是 PRD 显式在范围内的第三合法档（GEAR_VALUES 第三值）——初始态 planner 照跑，
    // 分段语义在下游 proposer 多段 task-plan 实现。断言它被当作合法值透传、走 planner，
    // 而非被 kernel 合法集校验漏掉当作非法值 fail-closed（R1-1 修复护栏）。
    const r = derive(initialObserved({ gear: 'segmented' }));
