# Sprint Contract Draft (Round 2) — commander lease 过期自动重派（有界）[r73]
...
**验证命令**:
```bash
npx vitest run tests/gp/f1/step3-commander-infra-retry-r73.test.js -t '第 5 次' 2>&1 | grep -qE '1 passed' || exit 1
# 期望：4 次历史过期 infra + 当前第 5 次 → 仍重派 commander（-t '第 5 次' 仅命中 test2「未达上限（累计第 5 次失败）」，与 test3「第 6 次」不碰撞）
```

**硬阈值**: `prior(hop<current)=4 < 5` → `action === 'spawn:commander'`。
**验证命令**: `npx vitest run tests/gp/f1/step3-commander-infra-retry-r73.test.js -t '第 5 次' 2>&1 | grep -qE 'Tests.*1 passed' || exit 1`
...
**验证命令**:
```bash
npx vitest run tests/gp/f1/step3-commander-infra-retry-r73.test.js -t 'fail-closed' 2>&1 | grep -qE '1 passed' || exit 1
# 期望：prior=5 ≥ 5 → wait:human_review + callbackHop=111（-t 'fail-closed' 仅命中 test3「达上限（累计第 6 次失败）…fail-closed」，不与 test2「未达上限」子串碰撞）
```

**硬阈值**: `action === 'wait:human_review'` 且 `reason === 'callback_infrastructure_route_unknown'` 且 `callbackHop === 111`。
**验证命令**: `npx vitest run tests/gp/f1/step3-commander-infra-retry-r73.test.js -t 'fail-closed' 2>&1 | grep -qE 'Tests.*1 passed' || exit 1`
