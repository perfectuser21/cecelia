 ...
 *
 * r73 案卷（run da3aa553）：generator 报 CONTRACT_SELF_CONTRADICTION 触发合同重开 GAN
 * （设计内自愈）；第二版合同批准后 derive 见 proposeBranchRn>0 且无 PR/候选（no_pr），
 * 主链把 no_pr 路由到 spawn:generator-fix。但 fix 的源 workspace 属**第一版已作废合同**
 * → worker /prepare 报 workspace_source_attempt_unavailable → assembly_fault:
 * WORKSPACE_RESOLUTION_FAILED 终局。合同重开路径 100% 必死。
 *
...
// reopen 消费触发它的 generator callback（callback_hop），令其被 attemptCallbackRoute 跳过。
...
// 覆盖父路 factory/F1 造完真验 第 3 步（造完真验：合同重开后主链路由）
describe('derive no_pr：合同重开纪元派全新 generator，根除 WORKSPACE_RESOLUTION_FAILED（r73）', () => {
  it('B-01 重开后新合同批准 + no_pr（重开纪元内未派全新 generator）→ spawn:generator（contract_reopened_fresh_generator）', () => {
    // era1: 首版 generator 完成但报合同故障 → reopen 消费其 callback → era2 新合同批准，未派全新 generator。
    const log = [
    ...
    // RED 基线（修前）：derive 返回 spawn:generator-fix / no_pr —— 本断言必红。
  ...
  it('B-02 有界：重开后已派过全新 generator，再 no_pr → 回落既有 fix 语义（不无限重发 generator）', () => {
    // era2 已派 spawn:generator(hop15，纪元后)，其 callback 被 fix(hop17) 消费；再次 no_pr。
    const log = [
      genSpawn(10, 'contract_approved'),
      genCallback(11),
      reopen(12, 11),
  ...
  it('B-03 负向：无 REOPEN_GAN_CONTRACT 历史的 no_pr → 语义不变，仍 fix 路由', () => {
    // generator callback 被 fix(hop12) 消费；无任何 reopen 行。
    const log = [
      genSpawn(10, 'contract_approved'),
      genCallback(11),
      fixSpawn(12),
  ...
  it('B-04 纪元隔离：重开纪元起点之前的 spawn:generator 不算「重开后已派」（仍派全新 generator）', () => {
    // hop10 的 spawn:generator 在 reopen(hop12) 之前 → 不得被计入「重开后已派全新 generator」。
    const log = [
      genSpawn(10, 'contract_approved'),
      genCallback(11),
      reopen(12, 11),
