...
- [ ] [ARTIFACT] `playground/server.js` 的 /echo handler 响应中不含 `echo` key
...
- [ ] [ARTIFACT] `playground/server.js` 的 /echo handler 响应中含字面量 `msg` key
...
- [ ] [BEHAVIOR] GET /echo?msg=hello 返回 `{"msg":"hello"}` 严 schema 字段值
...
- [ ] [BEHAVIOR] GET /echo?msg=hello response keys 完整性恰好为 ["msg"]
...
- [ ] [BEHAVIOR] 空字符串边界 GET /echo?msg= 返回 `{"msg":""}` 非 null
...
- [ ] [BEHAVIOR] error path — GET /echo（缺少 msg 参数）返回 HTTP 400
