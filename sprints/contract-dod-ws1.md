...
- [ ] [ARTIFACT] `time-endpoints.js` 命名导出 3 个 handler（`getIsoHandler` / `getTimezoneHandler` / `getUnixHandler`），便于 BEHAVIOR 测试直接调用
  Test: node -e "const c=require('fs').readFileSync('packages/brain/src/routes/time-endpoints.js','utf8');for(const n of ['getIsoHandler','getTimezoneHandler','getUnixHandler']){if(!new RegExp('export\\\\s+(function|const|let|var)\\\\s+'+n+'\\\\b').test(c)){console.error('missing export',n);process.exit(1)}}"
