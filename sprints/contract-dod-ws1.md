...
**范围**: 新建 `packages/brain/src/routes/build-info.js`（Express Router）+ 在 `packages/brain/server.js` 挂载到 `/api/brain/build-info`，含 R2/R3/R4 + Cascade 加固
**大小**: S（< 100 行实现 + ~140-180 行测试）
**依赖**: 无
...
见 `tests/ws1/build-info.test.js`，覆盖（共 8 个 it）：
