...
**Round 10 变更**（Reviewer Round 9 反馈闭环 — 采纳"源级剥离 + 运行时强度提升"两条修复）：
- 风险 1（阻塞 — Reviewer Round 9 指出）：Round 9 链路游走器（walker）在**原始文件文本**上游走，**模板字面量 / 行注释 / 块注释里埋假链字面量**可令 walker 命中。例如 `` `supertest(server).get('/api/brain/time').expect(200)` `` 作为模板串字面量、或 `// supertest(server).get('/api/brain/time').expect(200)` 作为行注释 —— 三者皆在 raw text 中构成合法锚点 + 链，但代码真正执行时并未调用 supertest 至 time 端点。Round 10 采纳 Reviewer 明确建议的"源级剥离"：DoD 第 9 条 Test 在 `<ST>(<SRV>)` 锚定 + `.method(...)` 游走之前，先用三个 `.replace()` 清洗源文本 —— (i) `/\/\/[^\n]*/g` 剥离行注释，(ii) `/\/\*[\s\S]*?\*\//g` 剥离块注释，(iii) `/\x60[^\x60]*\x60/g` 剥离模板字面量（用 `\x60` 规避 shell 反引号冲突）。剥离后的字符流只含真实可执行代码，walker 在其上走才能保证"链路存在于真正运行的代码中"。**规则字符串字面量（`'...'` / `"..."`）保留不剥离** —— 因 `.get('/api/brain/time')` 的参数就是规则字符串，剥离它会连带破坏合法链路参数匹配；实操中在规则字符串里埋假链需 Generator 刻意为之且代码审查显眼异常，不在本轮守门职责内
- 风险 2（阻塞 — Reviewer Round 9 指出）：Round 9 DoD 第 11 条仅用 `npx vitest run ... --reporter=basic` 看 exit code。**Generator 可 `describe.skip` / `it.skip` / `it.todo` 让所有真测试变 pending，vitest 仍 exit 0** → 第 11 条运行时锁被伪造。Round 10 采纳 Reviewer 明确建议的"JSON 摘要硬门"：DoD 第 11 条 Test 从 `--reporter=basic` 升级为 `--reporter=json --outputFile=/tmp/time-vitest.json` + node 解析 JSON 的 **per-assertion status**（`testResults[*].assertionResults[*].status`），强制三条同时满足 —— (i) 存在至少一个 status === `"passed"` 的 assertion，(ii) 不存在 status ∈ {`"skipped"`, `"pending"`, `"todo"`} 的 assertion，(iii) 不存在 status === `"failed"` 的 assertion。任一违反即 exit 1。**本地实测已验证：vitest 的聚合字段 `numPassedTests` 会把 `.skip` 的 it 错误地计入 passed 数，不可靠；必须沿 per-assertion status 走每条 assertion 的实际状态才准**。配合 DoD 第 9 条同链 `.expect(200)`，运行时真正看到"supertest 以 server.js 的 app 为入参 → `.get('/api/brain/time')` → `.expect(200)` 单链命中 + 至少一个真绿 it 通过 + 无 skip/todo 伪装"
- 非阻塞观察 F（Reviewer Round 9 明示保留不计入判定）：walker 朴素括号计数不处理参数内字符串/正则字面量里的 `(` `)`。Round 10 源级剥离在干净流上运行后，Reviewer 明示"括号计数在剥离后的干净字符流上运行，这条顺带闭环"，本轮不单独处理
- 非阻塞观察 G（Reviewer Round 9 明示非旁路）：`supertest.agent(app)` / 中间变量 `const r = supertest(app); r.get(...)` 等合法 supertest 写法被同链锚点拒绝，属 Generator 侧摩擦成本（不是旁路）。Generator 直写单链即可规避，本轮不纳入
...
- [ ] [ARTIFACT] 生产侧测试以 `<ST_NAME>(<SRV_NAME>).get('/api/brain/time').expect(200)` **单一链式表达式**形态真实发起 supertest 请求 —— **Round 10 在 Round 9 同链游走器上追加"源级剥离"**（Reviewer Round 9 反馈闭环）：
  - **(a)/(b) 入参捕获（沿袭 Round 7/8/9 不变）**：先捕获 `import <SRV_NAME> from '../server.js'` 的变量名（**仅静态 import**，禁动态 import —— Round 6 观察 C 的规避通道"动态 import 藏在 if(false) 死分支"关闭），再捕获 `import <ST_NAME> from 'supertest'` 的变量名
  - **（Round 10 新增 — 源级剥离）**：在 walker 运行之前，先对测试文件源文本做三类剥离清洗 —— (i) 行注释 `/\/\/[^\n]*/g` 剥离（防 `// supertest(server).get(...).expect(200)` 假链），(ii) 块注释 `/\/\*[\s\S]*?\*\//g` 剥离（防 `/* supertest(server).get(...).expect(200) */` 假链），(iii) 模板字面量 `/\x60[^\x60]*\x60/g` 剥离（防 `` `supertest(server).get('/api/brain/time').expect(200)` `` 作为模板串假链；用 `\x60` 规避 shell 反引号冲突）。**规则字符串字面量保留不剥离** —— 因 `.get('/api/brain/time')` 的参数本就是规则字符串，剥离会破坏合法参数匹配；实操中在规则字符串里埋假链需 Generator 刻意为之且代码审查显眼异常
  - **(c)(d)(e) 链路游走器（Round 9 核心 — Round 10 起运行于剥离后的干净字符流）**：脚本以 `<ST_NAME>(<SRV_NAME>)` 为锚点，沿 `.<method>(<balanced-parens>)` 游走链路（每个 method 必须紧跟 `(` 并收到匹配 `)`）：
  ...
  - **为何要源级剥离（Round 10 新增论证）**：Reviewer Round 9 指出 Round 9 walker 在**原始文件文本**上游走，Generator 可把 `supertest(server).get('/api/brain/time').expect(200)` 写在行注释 / 块注释 / 模板字面量里，raw text 仍命中 walker 但代码执行时并未真正调用。剥离这三类"可读但不执行"的源后 walker 再走，确保命中的链路必然是真正会执行的代码。Reviewer 明示"非阻塞观察 F（walker 朴素括号计数不处理参数内字符串/正则字面量里的 `(` `)`）在剥离后干净流上运行，这条顺带闭环"
  - **（旁路关闭）**：Round 8 允许的 `toBe(200)` / `toEqual(200)` 替代路径**已关闭**（Round 9 建立） —— 这两种断言即便出现也无法证明挂在 time 端点的响应上。硬门仅接受链上字面量 `.expect(200)`
...
- [ ] [ARTIFACT] **生产侧测试在 vitest 下真实运行通过且无 skip/todo 伪装** —— Round 4 Reviewer 风险 2 闭环锁 × Round 9 同链 × **Round 10 运行时强度升级（Reviewer Round 9 反馈闭环）**。运行 `packages/brain/tests/time.test.js`，**不再仅看 exit 0**（Reviewer Round 9 指出 exit 0 可被 `describe.skip` / `it.skip` / `it.todo` 让所有真测试变 pending 但 vitest 仍 exit 0 伪造运行时绿）：改用 `--reporter=json --outputFile=/tmp/time-vitest.json` + node 解析 JSON 的 **per-assertion status**（`r.testResults[*].assertionResults[*].status`），强制三条同时满足 —— (i) 存在至少一个 status === `"passed"` 的 assertion（至少一个真绿 it），(ii) 不存在 status ∈ {`"skipped"`, `"pending"`, `"todo"`} 的 assertion（无 `.skip` / `.todo`），(iii) 不存在 status === `"failed"` 的 assertion（无失败）。任一违反即 exit 1。**注意：vitest 聚合字段 `numPassedTests` 会把 `.skip` 的 it 计入，不可靠；必须沿 `testResults[*].assertionResults[*].status` 走每条 assertion 实际状态**。配合 DoD 第 9 条同链 `.expect(200)`，运行时真实看到 "supertest 以 server.js 的 app 为入参 → `.get('/api/brain/time')` → `.expect(200)` 单链命中 + 至少一个真绿 it 通过 + 无 skip/todo 伪造"，不是 grep 字符串能欺骗的静态后门（禁止在 if(false) 块内假挂载、禁止路径笔误、禁止 import 但未注册、禁止 import 但另起 fake express、禁止链外 `toBe(200)` 伪装、**禁止 skip/todo 伪造 pending**）
  Test: bash -c "cd packages/brain && npx vitest run tests/time.test.js --no-coverage --reporter=json --outputFile=/tmp/time-vitest.json && node -e 'const r=JSON.parse(require(\"fs\").readFileSync(\"/tmp/time-vitest.json\",\"utf8\"));const st=(r.testResults||[]).flatMap(f=>(f.assertionResults||[]).map(a=>a.status));if(!st.includes(\"passed\")||st.some(s=>[\"skipped\",\"pending\",\"todo\"].includes(s))||st.includes(\"failed\"))process.exit(1)'"

## BEHAVIOR 索引（实际测试在 sprints/tests/ws1/）

见 `sprints/tests/ws1/time.test.js`，Round 4 / 5 覆盖 **8 个功能 it**（sprint 侧自 Round 4 起不变，Round 9 修订仅发生在 DoD 第 9 条的同链化；Round 10 修订仅发生在 DoD 第 9 条的源级剥离 + DoD 第 11 条的 JSON 摘要硬门）：
