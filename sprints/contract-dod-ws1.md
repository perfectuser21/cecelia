# Contract DoD — Workstream 1: build-info 端点路由 + 集成测试 + cascade 防护

**范围**: 新增 Express Router 模块 `packages/brain/src/routes/build-info.js`，在 `packages/brain/server.js` 用 try/catch 包裹挂载到 `/api/brain/build-info`，并在 `packages/brain/src/__tests__/build-info.test.js` 落地 supertest 集成测试。
**大小**: S
**依赖**: 无

> **路径裁定**：`packages/brain/server.js`（不是 `src/server.js`）。证据见 `contract-draft.md` §Reviewer 反馈 #4 事实证据小节（`ls`/`package.json main`/~50 个现存路由的 import 模式）。
...
- [ ] [ARTIFACT] `packages/brain/src/routes/build-info.js` 包含 module-level `const BUILT_AT = new Date().toISOString()` 一次性赋值（cascade risk #1：避免每请求重算 → 强制缓存语义）
  Test: node -e "const c=require('fs').readFileSync('packages/brain/src/routes/build-info.js','utf8');if(!/^const\s+BUILT_AT\s*=\s*new Date\(\)\.toISOString\(\)/m.test(c)){console.error('missing module-level const BUILT_AT = new Date().toISOString()');process.exit(1)}"
...
- [ ] [ARTIFACT] `packages/brain/server.js` 在挂载 build-info 路由处用 `try { ... } catch` 包裹（cascade risk #2：避免 router 模块 import/挂载失败让整个 server.js 启动崩溃）
  Test: node -e "const c=require('fs').readFileSync('packages/brain/server.js','utf8');if(!/try\s*\{[\s\S]{0,400}?\/api\/brain\/build-info[\s\S]{0,400}?\}\s*catch/.test(c)){console.error('missing try/catch wrap around build-info mount');process.exit(1)}"
...
见 `sprints/tests/ws1/build-info.test.js`，覆盖 8 个 it（Round 1 7 条 + Round 2 新增 1 条 N=3 idempotent），对应 PRD 的 4 个验收场景 + 显式三键约束 + git_sha 类型 + git_sha fallback 分支 + N=2/N=3 双 idempotent 强化：
...
- `returns identical built_at across three consecutive requests in the same process` → SC-006 强化（Round 2 新增；防"每偶数次刷新"型 mutation）
