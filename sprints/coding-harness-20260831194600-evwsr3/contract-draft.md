...
### Step 2: 操作者在每个端点说明内核对用途和鉴权边界
**来源**: `[FROM_PRD]` — PRD Golden Path 第 1-2 项及边界情况。

**可观测行为**: 文档以 `### POST /api/brain/harness/attempt-run` 与 `### GET /api/brain/harness/attempt-run/:id` 两个独立小节说明用途；每个小节各自明确 `internalAuthOrLoopback`、非生产且未配置 token 时 loopback 无需 Bearer，以及宿主/远端必须携带 `Authorization: Bearer CECELIA_INTERNAL_TOKEN`。不得只在公共前言笼统提一次鉴权。

**验证命令**:
```bash
npx vitest run --no-cache sprints/coding-harness-20260831194600-evwsr3/tests/attempt-run-bridge-guide.test.ts -t 'POST 端点独立说明提交用途与 internalAuthOrLoopback 鉴权边界|GET 端点独立说明按 id 查询用途与 internalAuthOrLoopback 鉴权边界'
```

**硬阈值**: POST 与 GET 两个独立小节分别命中用途、`internalAuthOrLoopback`、loopback 例外、宿主/远端和完整 Authorization 占位写法；测试 exit 0。
...
### Step 6: 中文主体与凭据安全可独立验证
**来源**: `[FROM_PRD]` — PRD NFR「文档语言：简体中文」与安全性「只展示凭据变量名，不写入真实 token」。

**可观测行为**: 去除代码块、行内代码和 URL 后，中文字符数大于四字母以上拉丁词数的两倍；全文只允许出现凭据变量名和 `Authorization: Bearer CECELIA_INTERNAL_TOKEN` 占位写法，拒绝其他 Bearer 值、JWT 形态或 token 变量赋真实值。

**验证命令**:
```bash
npx vitest run --no-cache sprints/coding-harness-20260831194600-evwsr3/tests/attempt-run-bridge-guide.test.ts -t '正文主体为中文且只展示凭据变量名不含真实凭据'
```

**硬阈值**: 中文主体比例断言通过且三类真实凭据模式匹配数均为 0；测试 exit 0。
...
|---|---|---|---|
| attempt-run 桥接说明 | `sprints/coding-harness-20260831194600-evwsr3/tests/attempt-run-bridge-guide.test.ts` | `POST 端点独立说明提交用途与 internalAuthOrLoopback 鉴权边界`、`GET 端点独立说明按 id 查询用途与 internalAuthOrLoopback 鉴权边界`、`角色白名单精确等于九个权威角色且无别名`、`payload 独立断言三个必填字段与 base_sha 可选自解析语义`、`派发失败回滚精确覆盖 run session task 三个终态`、`正文主体为中文且只展示凭据变量名不含真实凭据` | 目标文档尚不存在，6 个测试因 `ENOENT` 失败 |
