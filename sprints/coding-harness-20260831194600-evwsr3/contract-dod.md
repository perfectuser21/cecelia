...
- [ ] [BEHAVIOR] [L1] B-01: 阅读者能独立核对两个端点用途和鉴权边界
  动作: 打开说明页，阅读端点用途与鉴权章节
  预期观察: POST 被说明为提交、GET 被说明为按 id 查询；loopback 与宿主/远端 Bearer 要求无混淆
  等待预算: 0s
  留证: Vitest 详细输出
  Test: manual:bash -c 'npx vitest run --no-cache sprints/coding-harness-20260831194600-evwsr3/tests/attempt-run-bridge-guide.test.ts -t "说明两个 attempt-run 端点的独立用途与鉴权边界"'
