...
2. `radius.js`（不改，仅作为 reason_code 来源事实）：`resolveImpactRadius` 返回
   ...
   - 真新鲜度类（`status:'stale'|'unknown'` 且 reason_code ∈ `fact_snapshot_stale`/`projection_revision_missing`/`projection_revision_mismatch`/`manifest_projection_mismatch`/`graph_projection_revision_mismatch`）——radius.js:80-90,261-307。
...
3. `harness-gates.js` `gateReceipt(stage, result, extra)`（约 line 26-36）当前只透传 `stage/gate/reason/retryable/contract_id/contract_hash` + extra，**不含 `detail`、不含 `unclaimed_files`**。
4. `loop.js`：
   ...
   - 约 line 1662-1666：当前 `failure_class === 'impact_contract_invalid'` 分支**直接 `failRun('impact_gate_deterministic:...')` 并 return**，不经 derive 路由——本 sprint 要把它改成「记 BLOCKED 结果 + 由 derive 按 reason 路由 generator-fix / human_review」。
...
**验证命令**:
```bash
cd packages/brain && node ../../node_modules/.bin/vitest run --config vitest.config.js \
  src/impact-contract/__tests__/diff-gate-reason-code.test.js -t 'impact_anchor_missing'
...
```bash
cd packages/brain && node ../../node_modules/.bin/vitest run --config vitest.config.js \
  src/impact-contract/__tests__/diff-gate-reason-code.test.js -t 'capability_assertion_coverage_missing'
# 期望：PASS
```
**硬阈值**: 用例 PASS（exit 0）。
...
```bash
cd packages/brain && node ../../node_modules/.bin/vitest run --config vitest.config.js \
  src/impact-contract/__tests__/diff-gate-reason-code.test.js -t 'fact_snapshot_stale'
# 期望：PASS（回归保护，行为不变）
```
**硬阈值**: 用例 PASS（exit 0）。
...
```bash
cd packages/brain && node ../../node_modules/.bin/vitest run --config vitest.config.js \
  src/impact-contract/__tests__/diff-gate-reason-code.test.js -t 'mapper_contract_invalid'
# 期望：PASS
```
**硬阈值**: 用例 PASS（exit 0）。
...
```bash
cd packages/brain && node ../../node_modules/.bin/vitest run --config vitest.config.js \
  src/impact-contract/__tests__/harness-gates-reason-code.test.js
...
```bash
cd packages/brain && node ../../node_modules/.bin/vitest run --config vitest.config.js \
  src/orchestrator/__tests__/loop-impact-deterministic-route.test.js
...
- [回归] `src/orchestrator/__tests__/loop.test.js` → 「Diff Gate 未放行时不创建 evaluator attempt，并把裁决写入 decision log」（`mapper_stale` 路径必须保留）+「Impact schema 确定性错误精确终止且不进入基础设施重试」（`impact_gate_deterministic` 出口对**抛错类**仍成立）。本 sprint 改的是**返回类**确定性结论的路由，不得破坏这两条既有用例语义。
- [回归] `src/impact-contract/__tests__/diff-gate.test.js` → pass/extend/drift 三类既有裁决（`makeFreshMapperResult` fresh 路径）必须保持绿。
- [回归] `src/impact-contract/__tests__/harness-gates.test.js` → 既有 `legacy_exempt`/`declaration_missing` 等 receipt 语义不变。
- [回归] `src/impact-contract/__tests__/map-client.test.js` → `assertMapperContract` 不改（PRD 明示不动）。
- [累积FR] context-manifest: unavailable（journey `e6f803f2` 本 line 唯一 ability 状态=planned，无历史；Brain 不在本 proposer 环境运行，端点不可达，记一行按 PRD「累积 FR：本 line 暂无历史」）。
...
cd "$WORKSPACE_PATH"
SPRINT_DIR="sprints/08161030-kernel-f9f943fc"
...
#    以本 attempt 的 HARNESS_RUN_ID 落一行 spawn:evaluator intent，gate_verdict=deny:impact:impact_anchor_missing。
