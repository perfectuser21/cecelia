...
- [ ] [ARTIFACT] diff-gate 三类分流回归测试文件存在
  Test: node -e "require('fs').accessSync('packages/brain/src/impact-contract/__tests__/diff-gate-reason-code.test.js')"

- [ ] [ARTIFACT] harness-gates gateReceipt 透传测试文件存在
  Test: node -e "require('fs').accessSync('packages/brain/src/impact-contract/__tests__/harness-gates-reason-code.test.js')"

- [ ] [ARTIFACT] loop/derive 确定性出口路由测试文件存在
  Test: node -e "require('fs').accessSync('packages/brain/src/orchestrator/__tests__/loop-impact-deterministic-route.test.js')"

- [ ] [ARTIFACT] 回归夹具（run d1360a48 录制件）+ 夹具测试存在
  Test: node -e "require('fs').accessSync('sprints/08161030-kernel-f9f943fc/fixtures/radius-d1360a48-impact-anchor.json');require('fs').accessSync('packages/brain/src/impact-contract/__tests__/diff-gate-regression-d1360a48.test.js')"

- [ ] [ARTIFACT] Final E2E harness 存在（真 runLoop 一跳 + 真 appendHop 落 orchestrator_decision_log）
  Test: node -e "require('fs').accessSync('sprints/08161030-kernel-f9f943fc/e2e/impact-gate-e2e.mjs')"
...
- [ ] [ARTIFACT] Brain 版本四处同步 = 1.273.60（package.json / package-lock.json / .brain-versions / DEFINITION.md）
  Test: node -e "const v=require('./packages/brain/package.json').version;const l=require('./packages/brain/package-lock.json').version;const fs=require('fs');const bv=fs.readFileSync('.brain-versions','utf8').trim().split('\n').pop().trim();const def=/\*\*Brain 版本\*\*:\s*([0-9.]+)/.exec(fs.readFileSync('DEFINITION.md','utf8'))[1];if(!(v===l&&v===bv&&v===def&&v==='1.273.60'))process.exit(1)"
...
- [ ] [BEHAVIOR] [L1] B-05: harness-gates beforeEvaluate gateReceipt 透传 reason/retryable/detail/unclaimed_files（INV-3）
  动作: 跑 harness-gates-reason-code 测试（真 createHarnessImpactGates.beforeEvaluate + 注入 diffGate 确定性 blocked 结果）
  ...
  Test: manual:bash -c 'node -e "const {execFileSync}=require(\"child_process\");execFileSync(\"node\",[\"../../node_modules/.bin/vitest\",\"run\",\"--config\",\"vitest.config.js\",\"src/impact-contract/__tests__/harness-gates-reason-code.test.js\"],{cwd:\"packages/brain\",stdio:\"inherit\"})"'

- [ ] [BEHAVIOR] [L1] B-06: loop/derive impact_anchor_missing（retryable:false）→ intent 透传 + 路由 spawn:generator-fix（INV-1/INV-3）
  动作: 跑 loop-impact-deterministic-route 测试（真 runLoop + 真 derive，注入 beforeEvaluate 确定性 blocked receipt）
  ...
  等待预算: 0s
  留证: vitest 用例输出末 5 行（含 passed）
  Test: manual:bash -c 'node -e "const {execFileSync}=require(\"child_process\");execFileSync(\"node\",[\"../../node_modules/.bin/vitest\",\"run\",\"--config\",\"vitest.config.js\",\"src/orchestrator/__tests__/loop-impact-deterministic-route.test.js\",\"-t\",\"impact_anchor_missing\"],{cwd:\"packages/brain\",stdio:\"inherit\"})"'

- [ ] [BEHAVIOR] [L1] B-07: loop/derive capability_assertion_coverage_missing（retryable:false）→ 路由 wait:human_review
  动作: 跑 loop-impact-deterministic-route 测试的 coverage_missing 用例
  预期观察: 下一动作 dispatch 含 'wait:human_review'（Map 覆盖缺口需人补断言，不派 generator-fix）
  等待预算: 0s
  留证: vitest 用例输出末 5 行（含 passed）
  Test: manual:bash -c 'node -e "const {execFileSync}=require(\"child_process\");execFileSync(\"node\",[\"../../node_modules/.bin/vitest\",\"run\",\"--config\",\"vitest.config.js\",\"src/orchestrator/__tests__/loop-impact-deterministic-route.test.js\",\"-t\",\"capability_assertion_coverage_missing\"],{cwd:\"packages/brain\",stdio:\"inherit\"})"'

- [ ] [BEHAVIOR] [L1] B-08: 回归夹具 run d1360a48（真实 changed_files 含 DoD.md + radius 录制件）→ 新代码 blocked:impact_anchor_missing
  动作: 用录制件注入 evaluateDiffGate（changed_files 取自录制件 changed_files）
  ...
  等待预算: 0s
  留证: vitest 用例输出末 5 行（含 passed）
  Test: manual:bash -c 'node -e "const {execFileSync}=require(\"child_process\");execFileSync(\"node\",[\"../../node_modules/.bin/vitest\",\"run\",\"--config\",\"vitest.config.js\",\"src/impact-contract/__tests__/diff-gate-regression-d1360a48.test.js\"],{cwd:\"packages/brain\",stdio:\"inherit\"})"'
