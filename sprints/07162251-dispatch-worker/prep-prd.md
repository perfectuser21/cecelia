...
3. **吊 worker**（headless，命令模板 07-16 已实测）：
   - codex: `CODEX_HOME=~/.codex-teamX codex exec --cd <dir> --sandbox workspace-write '<brief>'`
...
5. **输出**：stdout 一行 JSON `{vendor, account, ok, quota_wall_hits, output_file}`；worker 完整输出落 `<dir>/.dispatch-worker-output-<ts>.log`
