# Sprint Contract Draft (Round 2) — generator 合同故障码保真透传（根除 provider_exit 语义埋没）[r76]

> **Round 2 修订说明（seal 打回，非 reviewer 打回）**：Round 1 合同已 APPROVED（7 维全 ≥7），但被封印机械闸拒绝：
> `FROZEN_CONTRACT_TEST_CONTRACT_UNRESOLVABLE:tests/gp/f1/step3-contract-fault-code-passthrough.test.js#behavior:同上（内容一致）`。
> 根因：下方 ## Test Contract 表**第 2 行「BEHAVIOR 覆盖」列**沿用占位串 `同上（内容一致）`——封印闸 `assertTestContractResolvable`
> 对 `tests/gp/f1/*`（非 `sprints/` 前缀）行走 repo 既有文件解析链（readRepoFile + it() 名双向子串匹配），占位串匹配不到任何 `it()` 名 → 拒封印。
> 本轮唯一实质修订：第 2 行「BEHAVIOR 覆盖」列改为逐字列出与第 1 行相同的真实 `it()` 名（可被封印闸/CI 覆盖检查解析）。
> 合同其余内容（Golden Path / DoD / 测试 / task-plan）与 Round 1 逐字一致，测试四件套已真实创建并 commit 进本 propose 分支。
...
| 合同故障码保真透传（tests/gp/f1 永久回归，PRD #5，补充行） | `tests/gp/f1/step3-contract-fault-code-passthrough.test.js` | preserves CONTRACT_SELF_CONTRADICTION structured BLOCKED on non-zero provider exit / preserves CONTRACT_CI_SCOPE_CONFLICT structured BLOCKED error_code faithfully / falls back to provider_exit on genuine crash without structured result / falls back to provider_exit when result file is invalid on non-zero exit / does not misroute non-blocked structured result on non-zero exit to passthrough / parses structured result unchanged on zero exit | → 并入上行 14 failures（`reconcileProviderCloseResult is not a function`，两文件各 7） |
