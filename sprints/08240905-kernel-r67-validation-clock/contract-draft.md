...
| 要素 | 本次答案 |
|---|---|
| FR（做什么） | 每个有 `effect:attempt_launched` 回执锚定的 `spawn:generator-fix`（最多六次）成为 validation clock 新原点。 |
| NFR（做得多好） | 默认 5400 秒不变；同一 decision log 重放结果完全一致。 |
| Invariant（永不违反） | 失败派发不顺延；第七次及以后不顺延；人审 deadline 不变；既有 fail-closed 语义不变。 |
| 判定点（怎么知道） | 以 decision log 的 action/hop 与 `detail.dispatch_hop` 反向锚定的 `effect:attempt_launched` 判定成功派发。 |
...
| 场景 | 失败行为 | 重试幂等？ | 降级策略 |
|---|---|---|---|
| fix 无匹配 `effect:attempt_launched` 回执 | 不将该 fix 作为原点 | 是，按同一日志重放 | 回到上一个有效原点 |
...
[进入验证时钟] → [按 dispatch_hop 识别成功 fix] → [按唯一 hop 六次内更新原点] → [返回可重放 deadline]

### Step 1: 读取同一 run 的 decision log
**来源**: `[FROM_PRD]` — PRD「Golden Path」第 1 点。

**可观测行为**: 输入相同但数组顺序不同、或存在同 hop 重复行，返回完全相同的 ISO 时钟且重复 hop 只计一次。

**验证命令**:
```bash
npx vitest run --no-cache tests/gp/f1/step3-validation-clock-fix-extension.test.js -t "乱序输入按 hop 重放得到同一时钟|同 hop 重复 fix 行只消耗一次顺延额度"
```
**硬阈值**: 两次结果严格相等；六个唯一成功 fix 加一个重复 hop 时原点仍为第六个唯一 fix；上方命令 exit 0。

### Step 2: 成功 fix 在六次内更新原点
**来源**: `[FROM_PRD]` — PRD「Golden Path」第 2 点及边界情况。

**可观测行为**: r50 类旧 deadline 已过、最新有效 fix deadline 未过时，resolver 只在 fix 有 `effect:attempt_launched.detail.dispatch_hop` 回执时返回该 fix 的时钟；裸 intent 不顺延。

**验证命令**:
```bash
npx vitest run --no-cache tests/gp/f1/step3-validation-clock-fix-extension.test.js -t "r50 类场景由最新成功 fix 重置时钟|没有 attempt_launched 成功回执的 fix 不顺延"
```
**硬阈值**: `pipeline_started_at` 等于最新且有成功回执的 fix `created_at`，deadline 精确增加 5400 秒；裸 fix intent 不改变原点；命令 exit 0。
...
|---|---|---|---|
| 冻结合同关键场景 | `sprints/08240905-kernel-r67-validation-clock/tests/validation-clock-contract.test.ts` | `r50 类场景由最新成功派发的 fix 重置时钟`; `同 hop 重复 fix 行只消耗一次顺延额度` | 旧实现返回 generator 原点，2 failures |
| 永久 GP 回归 | `tests/gp/f1/step3-validation-clock-fix-extension.test.js` | `r50 类场景由最新成功 fix 重置时钟`; `第七次成功 fix 不再顺延`; `零次 fix 保持 generator 原点语义`; `没有 attempt_launched 成功回执的 fix 不顺延`; `乱序输入按 hop 重放得到同一时钟`; `同 hop 重复 fix 行只消耗一次顺延额度` | 旧实现至少 4 failures |
