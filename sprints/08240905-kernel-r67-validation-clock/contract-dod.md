...
- [ ] [BEHAVIOR] [L2] B-01: r50 类健康 run 由最新成功 fix 重置时钟
  动作: 将 generator 与三次成功 generator-fix 的真实 decision-log shape 传给 resolver
...
- [ ] [BEHAVIOR] [L2] B-04: 失败派发与乱序输入不产生额外顺延
  动作: 输入关联 BLOCKED 的 fix、同 hop 重复行及乱序日志
  预期观察: BLOCKED fix 被排除，结果只由有效 hop 时序决定且可重放
  等待预算: 0s
  留证: 两个 Vitest 用例输出
  Test: manual:bash -c 'npx vitest run --no-cache tests/gp/f1/step3-validation-clock-fix-extension.test.js -t "失败派发不顺延|乱序输入按 hop 重放得到同一时钟"'
