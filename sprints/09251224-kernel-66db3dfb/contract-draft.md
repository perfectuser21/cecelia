...
## 案卷 Closure（Round 3 — 封印闸拒绝修复；无新增 reviewer blocker）

Round 2 合同 7 维全部 ≥7、reviewer VERDICT=APPROVED、唯一 blocker R1-1 已 closed（见下）。但批准后封印机械闸 `assertTestContractResolvable` 返回 `FROZEN_CONTRACT_TEST_CONTRACT_UNRESOLVABLE`——Test Contract 表所列测试文件在 propose 分支不可解析。Round 3 只做这一处机械修复，合同语义与 DoD/E2E/oracle 一字不改（净变化趋近 0，B50）。

**SEAL-R2-1**（封印闸 FROZEN_CONTRACT_TEST_CONTRACT_UNRESOLVABLE）— Test Contract 表三行中，(1) 第 3 行真 PG 集成测试文件 `packages/brain/src/__tests__/integration/task-run-primitive.pg.integration.test.js` 从未落盘；(2) 第 2 行冻结测试的「BEHAVIOR 覆盖」列文字非对应 `it()` 名的字面子串（`渲染含`/`空列表` vs 实际 `渲染出`/`空裸跑列表`），CI 解析链匹配不到 → 整行不可解析。

- **做了什么**：①真实创建第 3 行集成测试文件（含可执行断言，`import { startRun, finishRun, findBareRuns }` 自未建的 `lib/task-run.js` → 预期 Red；真 PG seed→断言落库/幂等/终态/已终态不覆盖/裸跑检出），其 5 个 `it()` 名逐字包含第 3 行覆盖名子串；②把第 2 行覆盖名改成实际 `it()` 名的字面子串（`非空裸跑列表渲染出 🟡 AMBER 标记与每个裸跑 task_id`／`空裸跑列表不含 AMBER 标记`／`单条裸跑正常渲染`）；③两个 `sprints/.../tests/` 冻结测试与 `checks/dod-checks.mjs` 原样落盘并 commit 进 propose 分支。
- **为什么足以关闭**：封印闸用与 CI 同一条解析链校验 Test Contract 每行「文件存在 + 覆盖名解析到 `it()` 子串」——三行文件现均落盘、每个覆盖名均为真实 `it()` 名字面子串，解析链不再返回 UNRESOLVABLE；runner finalizer 的 push 前 HEAD 树校验亦能查到两行 `sprints/.../tests/` 冻结测试。
- **quote**（本轮新增/修改原文片段，≥20 字）：Test Contract 第 2 行覆盖列改为 `非空裸跑列表渲染出 🟡 AMBER 标记与每个裸跑 task_id、空裸跑列表不含 AMBER 标记、单条裸跑正常渲染`；新建集成测试含 `it('startRun 写入恰好一行 running（含 context.source，真 PG）', ...)` 与 `it('findBareRuns 检出被派发但无 run 记录的 task（裸跑）且无误报', ...)`。

---

## 案卷 Closure（Round 2 — 对 Round 1 reviewer blocker 逐条关闭，历史留存）

**R1-1**（维度 verification_oracle_completeness=6 + scope_match_prd=6）— 晨报『裸跑检测 AMBER』出口无接线/渲染断言，PRD 需求④仅在 lib 层验 findBareRuns。

- **做了什么**：新增一条 [BEHAVIOR] `B-06`（contract-dod.md）镜像 B-05 双段，并新增 `checks/dod-checks.mjs` 的 `b06` oracle 与冻结测试 `tests/bare-run-report-render.test.js`。晨报出口收口到 `daily-report-generator.js` 新增导出 `renderBareRunSection(bareRuns)`（PRD 需求④ 出口的 SSOT 渲染器）；`generateDailyReport` 必须 `import { findBareRuns }` 并把其真实返回喂给 `renderBareRunSection`，AMBER 文本进入日报正文。b06 (a) 接线断言 grep `daily-report-generator.js` 确有 `import findBareRuns` + `findBareRuns(` 调用 + `renderBareRunSection` 被调用（≥2 次出现＝定义＋调用）；(b) 渲染断言以真 PG seed 一个裸跑 task（有 dispatch_events 无 task_runs）+ 一个有 run 的 task，取 `findBareRuns(pool)` 真实数据喂 `renderBareRunSection` 后断言输出文本含 `🟡 AMBER`＋裸跑 task_id、不含有 run 的 task_id、空数组时不含 AMBER 标记。同步把 Step 6『可观测行为』与『验证命令』改为渲染接线一致（消除观测行为↔命令矛盾）。
- **为什么足以关闭**：Generator 不接线晨报即 b06(a) grep 红、不渲染 AMBER 即 b06(b) 真 PG 渲染断言红、冻结测试 import `renderBareRunSection` 失败即全红——晨报出口无法在不实现的情况下转绿，PRD 人可见出口（裸跑 AMBER）由真实内容出现 oracle 兜底，不再是假绿。
- **quote**（本轮新增原文片段）：`node --input-type=module ... import { findBareRuns, renderBareRunSection } ... const text = renderBareRunSection(await findBareRuns(pool,{windowMinutes:60})); assert(/🟡 *AMBER/.test(text) && text.includes(bareId) && !text.includes(runId))`（见 contract-dod.md B-06 与 checks/dod-checks.mjs b06）。

**R1 非 block 提醒（一并据实修正，提升 internal_consistency）**：『已知约束』对 healing.js 读语义描述据实改写（task_runs 059 无 pid 列、读 `status='in_progress'`/`status='failed'` 非 `running`），并登记「写 failed 行激活 healing.js:800 隔离路径」二阶效应。
...
### 晨报裸跑渲染器（packages/brain/src/daily-report-generator.js，需求④ 出口 SSOT）

```
renderBareRunSection(bareRuns) -> string
// 入参 bareRuns: Array<{ task_id, dispatched_at }>（findBareRuns 返回 shape）
// 非空 → 返回含字面标记 "🟡 AMBER" + "裸跑" + 每个 task_id 的日报板块文本
// 空数组 → 返回不含 "AMBER" 标记的文本（如「== 裸跑检测 ==\n无裸跑执行」或空串）
```

`generateDailyReport` 必须 `import { findBareRuns } from './lib/task-run.js'` 并把其真实返回喂给 `renderBareRunSection`，使 AMBER 裸跑行进入日报正文（接线由 B-06(a) grep 验，渲染由 B-06(b) 真 PG 验）。
**禁用字段名/标记**：晨报裸跑行标记字面用 `🟡 AMBER`（不得用 `RED`/`WARN`/`yellow` 等同义替换顶替）。
...
### Step 6: 晨报「裸跑检测」AMBER 出口（daily-report-generator 接线 + 渲染 AMBER 裸跑行）
**来源**: `[FROM_PRD]` — Golden Path 第 6 步 + 范围「晨报（daily-report-generator / morning-cockpit-bark）增加『裸跑检测』AMBER 规则」

**可观测行为**: `daily-report-generator.js` 的 `generateDailyReport` 真实 `import { findBareRuns }` 并调用它拿到窗口内「有 dispatched 事件但无对应 task_runs」的 task，把真实结果喂给新增导出的 `renderBareRunSection(bareRuns)`；**日报正文出现 `🟡 AMBER` 裸跑行且列出裸跑 task_id**；无裸跑时正文不出现 AMBER 标记，且有 run 的执行不被误列。这是需求④ 人可见出口，非仅 lib 层检测原语。

**验证命令**（真 PG；seed 一个裸跑 task + 一个有 run 的 task → 取 findBareRuns 真实数据喂 renderBareRunSection → 断言 AMBER 文本真实出现；接线 grep 确保正文真调用它）:
```bash
# (a) 接线：daily-report-generator 真 import 并调用 findBareRuns，且接线 renderBareRunSection（定义+调用≥2 次出现）
node -e "const c=require('fs').readFileSync('packages/brain/src/daily-report-generator.js','utf8'); if(!/from ['\"][^'\"]*lib\/task-run(\.js)?['\"]/.test(c)||!/findBareRuns\s*\(/.test(c)){console.error('FAIL: 未 import/调用 findBareRuns');process.exit(1);} if((c.match(/renderBareRunSection\s*\(/g)||[]).length<2){console.error('FAIL: renderBareRunSection 未定义或未接线');process.exit(1);} console.log('OK: 晨报已接线 findBareRuns + renderBareRunSection')"
# (b) 渲染：真 PG 真实数据 → renderBareRunSection 产出 AMBER 文本（含裸跑 id、不含有 run 的 id、空数组不含 AMBER）
node sprints/09251224-kernel-66db3dfb/checks/dod-checks.mjs b06
```

**硬阈值**: 接线 grep 通过；renderBareRunSection 对真实裸跑数据产出含 `🟡 AMBER` 且含裸跑 task_id 的文本；有 run 的 task 不被误列；空数组文本不含 AMBER 标记。
...
- [回归测试] `src/alertness/healing.js` 两条 task_runs 读路径（据实核对 059 schema，修正 R1 描述不准确）：
  - `findOrphanProcesses`（healing.js:514）读 `SELECT pid FROM task_runs WHERE status='in_progress' AND pid IS NOT NULL` —— 但 migration 059 **无 `pid` 列**、状态枚举用 `running`（非 `in_progress`）。本 sprint 只写 `running`/终态、不设 `pid`、不用 `in_progress` → 该读路径保持休眠（对空表零行为回退，不新增 pid 列，不回退 059）。
  - `quarantineProblematicTasks`（healing.js:800）读 `SELECT task_id FROM task_runs WHERE status='failed' GROUP BY task_id HAVING COUNT(*)>=3` → 隔离 `queued`/`pending` 任务。**本 sprint 首次向 task_runs 写 `failed` 行会激活此前空表下休眠的隔离路径**（二阶效应，登记为实现期风险，非本合同 blocker：finishRun 写 failed 属真实终态，隔离逻辑符合「≥3 次失败任务自动隔离」预期语义）。
...
# 8. 晨报出口（需求④）：daily-report 接线 findBareRuns 且渲染 🟡 AMBER 裸跑行（真 PG 真实数据）
node -e "const c=require('fs').readFileSync('packages/brain/src/daily-report-generator.js','utf8'); if(!/from ['\"][^'\"]*lib\/task-run(\.js)?['\"]/.test(c)||!/findBareRuns\s*\(/.test(c)){console.error('FAIL: 晨报未接线 findBareRuns');process.exit(1);} if((c.match(/renderBareRunSection\s*\(/g)||[]).length<2){console.error('FAIL: renderBareRunSection 未接线');process.exit(1);} console.log('OK: 晨报已接线')"
node sprints/09251224-kernel-66db3dfb/checks/dod-checks.mjs b06

echo "✅ Golden Path 验证通过（run 原语落库/幂等/终态/单一写口/裸跑/投影/晨报 AMBER 出口）"
```
...
| 晨报裸跑 AMBER 渲染器（冻结测试，仓库根 vitest 跑；R1-1 修复） | `sprints/09251224-kernel-66db3dfb/tests/bare-run-report-render.test.js` | 非空裸跑列表渲染出 🟡 AMBER 标记与每个裸跑 task_id、空裸跑列表不含 AMBER 标记、单条裸跑正常渲染 | daily-report-generator.js 未导出 renderBareRunSection → 调用 undefined 抛错 → 全红 |
