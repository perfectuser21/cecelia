...
- [ ] [ARTIFACT] daily-report-generator 导出晨报裸跑渲染器 renderBareRunSection（需求④ 出口 SSOT）
  Test: node -e "const c=require('fs').readFileSync('packages/brain/src/daily-report-generator.js','utf8'); if(!/export (async )?function renderBareRunSection/.test(c)) process.exit(1); console.log('OK')"

- [ ] [ARTIFACT] 冻结测试（run 原语 + 晨报渲染）与真 PG 集成测试文件存在
  Test: node -e "const f=require('fs'); ['sprints/09251224-kernel-66db3dfb/tests/task-run-primitive.test.js','sprints/09251224-kernel-66db3dfb/tests/bare-run-report-render.test.js','packages/brain/src/__tests__/integration/task-run-primitive.pg.integration.test.js'].forEach(p=>f.accessSync(p)); console.log('OK')"
...
- [ ] [BEHAVIOR] [L2] B-06: 晨报裸跑 AMBER 出口 — daily-report 接线 findBareRuns 且渲染 🟡 AMBER 裸跑行（需求④ 人可见出口）[接缝×2]
  动作: (a) grep daily-report-generator.js 是否 import/调用 findBareRuns 并接线 renderBareRunSection；(b) 真 PG seed 一个裸跑 task（有 dispatch_events 无 task_runs）+ 一个有 run 的 task，取 findBareRuns 真实数据喂 renderBareRunSection
  预期观察: (a) daily-report-generator 真 import findBareRuns 并调用、renderBareRunSection 定义+调用（≥2 次出现）；(b) 渲染输出文本含 🟡 AMBER + 裸跑 task_id，不含有 run 的 task_id，空数组输出不含 AMBER 标记（无裸跑不误报）
  等待预算: 0s
  留证: dod-checks.mjs b06 stdout（OK: b06）
  Test: manual:bash -c 'node sprints/09251224-kernel-66db3dfb/checks/dod-checks.mjs b06'
