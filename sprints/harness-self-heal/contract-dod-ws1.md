...
- [ ] [ARTIFACT] `packages/brain/src/task-router.js` 的 VALID_TASK_TYPES 数组包含 `'harness_intervention'`
...
- [ ] [ARTIFACT] `packages/brain/src/task-router.js` 的 LOCATION_MAP 包含 `'harness_intervention': 'us'`
...
- [ ] [ARTIFACT] `packages/brain/.env` 存在并包含 `BARK_TOKEN=` 行
...
- [ ] [ARTIFACT] `packages/brain/.env` 包含 `FEISHU_WEBHOOK=` 行（飞书中间层降级预留）
...
- [ ] [BEHAVIOR] VALID_TASK_TYPES 包含 harness_intervention（写入前此项 FAIL）
...
- [ ] [BEHAVIOR] LOCATION_MAP 明确映射 harness_intervention → 'us'（不靠 DEFAULT_LOCATION 兜底）
...
- [ ] [BEHAVIOR] .env 包含 BARK_TOKEN=...行（非空值）
...
- [ ] [BEHAVIOR] error path — VALID_TASK_TYPES 变更不破坏现有路由（regression：dev → us 仍工作）
...
- [ ] [BEHAVIOR] .env 包含 FEISHU_WEBHOOK= 行（飞书中间层降级链可配置）
