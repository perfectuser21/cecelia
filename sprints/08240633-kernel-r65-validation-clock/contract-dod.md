...
- [ ] [BEHAVIOR] [L2] B-01: r50 型场景在成功 fix 新窗口内保持存活
  动作: 真实导入 `resolveValidationClock`，重放首次 generator、较晚 generator-fix intent 与匹配 launch effect
  预期观察: 仅因 `dispatch_hop`/`dispatch_action` 匹配的成功回执，原点切换到 fix intent 并重算 deadline
  等待预算: 5s
...
- [ ] [BEHAVIOR] [L2] B-02: 无匹配 launch effect 的 fix intent 不顺延
  动作: 重放 fix intent 与 `dispatch_hop` 不匹配的 `effect:attempt_launched`
  预期观察: 原点保持首次 generator，失败或未派发 intent 不获得续期
  等待预算: 5s
  留证: Vitest 精确 deadline 输出
  Test: manual:bash -c 'npx vitest run --no-cache sprints/08240633-kernel-r65-validation-clock/tests/validation-clock-fix-extension.test.ts -t "无匹配 attempt_launched 回执的 fix intent 不顺延"'

- [ ] [BEHAVIOR] [L2] B-03: 第六次成功 fix 仍允许顺延
...
- [ ] [BEHAVIOR] [L2] B-04: 第七次成功回执超限不再续期
...
- [ ] [BEHAVIOR] [L2] B-05: 无 fix 轮保持既有计时语义
...
- [验证时钟] 由 B-01 至 B-05 保持 fail-closed 正常时钟，不加旁路。
- [时间不变量] 由 B-02/B-03/B-04 验证只有匹配成功回执才计数，且 `deadline = 有效原点 + timeoutSeconds`、顺延次数 `<= 6`。
