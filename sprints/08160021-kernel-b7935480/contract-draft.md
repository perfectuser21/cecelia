...
## 案卷 Closure 声明（Round 2 — 关闭上一轮 Reviewer blocker）

上一轮（Round 1）Reviewer 判定 REVISION，2 条 blocker 属**同一根因的两个维度投影**（smoke 落点 vs ratchet 度量目录错配），本轮一次修复同时闭合：

- **R1-1**（internal_consistency 5→目标≥7）**closure**：Round-1 把新 smoke 落在 `packages/brain/scripts/smoke/` 却 bump `smoke_pool.watermark`（该 metric 由 `scripts/ratchet-guard.mjs:93` 只递归计数 `scripts/smoke/`，两目录无关），自相矛盾。本轮采用 Reviewer 修复选项 (b)：**保留 PRD 指定目录 `packages/brain/scripts/smoke/`，删除 `smoke_pool.watermark` bump（不再改 `scripts/ratchet-registry.json`），改由真正治理该目录的权威守卫 `run-smoke-ratchet.sh` + `smoke-allowlist.txt` 落实「永久接入/只增不减」**，并同步修订合同/PRD 措辞（§2、INV-3、Step 4、八要素、E2E）。
  `quote:` 「治理 `packages/brain/scripts/smoke/` 的权威 ratchet = `packages/quality/scripts/run-smoke-ratchet.sh` + `packages/quality/smoke-allowlist.txt`…本轮不再改 `scripts/ratchet-registry.json`（不 bump `smoke_pool.watermark`）——该 metric 与本 smoke 落点无关，改它属误导性动作。」（见 §2）
- **R1-2**（verification_oracle_completeness 5→目标≥7）**closure**：删除 `readdirSync('packages/brain/scripts/smoke').length(392) ≥ watermark(14)` 恒真伪 oracle；INV-3 与 E2E step6 改为**与 CI 守卫同源**——`SMOKE_DIR=<临时目录仅含新 smoke> bash packages/quality/scripts/run-smoke-ratchet.sh` exit 0（未登记则守卫 UNREGISTERED→exit 1），直接调用治理该目录的权威守卫证明纳管+通过。GET /api/brain/quality/ratchet 降级为仅台账端点健康检查、显式声明非纳管证据。
  `quote:` 「**同源纳管证据**：`SMOKE_DIR=<临时目录仅含新 smoke> bash packages/quality/scripts/run-smoke-ratchet.sh` exit 0——直接调用治理 `packages/brain/scripts/smoke/` 的权威守卫，exit 0 证明本 smoke 被识别为 allowlist 基线（非 UNREGISTERED）且通过；若未登记则守卫 `UNREGISTERED → exit 1`」（见 Step 4）

**精简纪律（B50）**：本轮先删后加、净变化聚焦单根因——删除 watermark bump/392≥14 伪 oracle，加入同源 run-smoke-ratchet oracle；未扩大 scope，其余 5 维（RED/scope/dod/risk/ci）保持不动。
...
### 2. 「永久接入 ratchet」的权威守卫（Round-2 修正：与 CI 守卫同源）

> **Round-2 根因修正（关闭 R1-1 / R1-2）**：Round-1 把新 smoke 落在 PRD 指定的 `packages/brain/scripts/smoke/`，却去 bump `smoke_pool.watermark` 并对 `packages/brain/scripts/smoke` 计数 392≥14 当「已接入」证据——但权威守卫 `scripts/ratchet-guard.mjs:93` 的 `smoke_pool` metric 只递归计数 **`scripts/smoke/`**（`walkSh`，现值 38 / watermark 13），与 `packages/brain/scripts/smoke/` 是两个无关目录。故 watermark bump 对本 smoke 无意义、392≥14 是恒真伪 oracle。本轮采用 Reviewer 修复选项 (b)：**保留 PRD 指定目录，改用真正治理该目录的守卫**。

**治理 `packages/brain/scripts/smoke/` 的权威 ratchet = `packages/quality/scripts/run-smoke-ratchet.sh` + `packages/quality/smoke-allowlist.txt`**（三档棘轮闸，CI 跑道 `.github/workflows/ci.yml` + `nightly-regression.yml` 调用）：

- `run-smoke-ratchet.sh` 遍历 `SMOKE_DIR`（默认 `packages/brain/scripts/smoke`）下每个 `*.sh`：
  - basename ∈ `smoke-allowlist.txt` → 必须通过，失败 → `FAIL_BASELINE` → **`exit 1`（CI 红）**；
  - basename 既不在 allowlist 也不在 debt/deny → `UNREGISTERED`（新债不许欠）→ **`exit 1`（CI 红）**。
  - 判定见脚本尾部「最终判定」：`FAIL_BASELINE>0 || UNREGISTERED>0 → exit 1`（`run-smoke-ratchet.sh:174-185`）。
- 因此「永久接入 / 每次 CI 都跑 / 只增不减」对本 smoke 的落地 = **basename 登记进 `smoke-allowlist.txt`**：一旦登记，该守卫每次 CI 都会跑它并要求通过；若把它从 allowlist 删除，则 `run-smoke-ratchet` 视之为 `UNREGISTERED` → CI 红（新债不许欠），构成「只增不减」护栏。
- **本轮不再改 `scripts/ratchet-registry.json`（不 bump `smoke_pool.watermark`）**——该 metric 与本 smoke 落点无关，改它属误导性动作。

### 2b. 既有端点 GET /api/brain/quality/ratchet（不改，仅作台账端点健康 oracle，**非**本 smoke 纳管证据）
...
```
- `available` (boolean, 必填): 来源——quality.js:/ratchet handler。该端点反映的是 `smoke_pool`（`scripts/smoke/`），**不枚举 `packages/brain/scripts/smoke/`**，故仅作台账端点健康检查，**不作为本 smoke「已接入 ratchet」的证据**（纳管证据见 §2 的 run-smoke-ratchet 同源 oracle）。
- 本回归当前拓扑若实测 `available:false(ENOENT)`——属正确降级（PRD 边界情况 + [local-api-gate5] 铁律），smoke 对该拓扑放行。
...
- [smoke铁律] → **INV-3**（新 smoke 登记进 `smoke-allowlist.txt`，由 `run-smoke-ratchet.sh` 基线保护：allowlist 脚本失败 **或** 新脚本未登记 → CI 红，构成「只增不减、每次 CI 都跑」；**不** bump `smoke_pool.watermark`——该 metric 度量 `scripts/smoke/`，与本 smoke 落点 `packages/brain/scripts/smoke/` 无关，Round-1 混淆已修正，见 §2）。
...
- [canonical-immutable] N/A：本 sprint 改动文件（attempt-runner.cjs 断言锚点 / attempt-runner.test.cjs / 新 smoke / smoke-allowlist.txt）经核对不在 canonical 不可变清单（Round-2 起不再改 ratchet-registry.json）。
...
本回归为 **local_api / 无 UI / 无真机** 形态：验证真相 = 「node 直调真实 attempt-runner.cjs 纯函数锚点收 exit code」+「bash 跑新 smoke 收 exit 0」+「`SMOKE_DIR=<临时目录仅含新 smoke> bash packages/quality/scripts/run-smoke-ratchet.sh` 收 exit 0——同源权威守卫证明新 smoke 已纳入 allowlist 且通过」+「curl GET /api/brain/quality/ratchet 仅作台账端点健康检查（available:true 或 available:false(ENOENT) 已知拓扑降级），非纳管证据」。**无浏览器截图、无真机 UIA**，judge 机械闸⑤（meta_verification_gap）对本合同放行。
...
### Step 4: 新 smoke 可跑通并永久接入 smoke ratchet（Round-2 同源守卫）
**来源**: `[FROM_PRD]` — PRD Golden Path 第 4 条（新 smoke 覆盖上述边界 + 永久登记进 ratchet 的 CI 跑道，只增不减）。

**可观测行为**: (a) `bash packages/brain/scripts/smoke/generator-publisher-runtime-boundary-smoke.sh` exit 0（内部真实执行 Step 1-3 的 node 锚点断言，无需 Brain/DB）；(b) smoke basename 已登记进 `packages/quality/smoke-allowlist.txt`；(c) **同源纳管证据**：`SMOKE_DIR=<临时目录仅含新 smoke> bash packages/quality/scripts/run-smoke-ratchet.sh` exit 0——直接调用治理 `packages/brain/scripts/smoke/` 的权威守卫，exit 0 证明本 smoke 被识别为 allowlist 基线（非 UNREGISTERED）且通过；若未登记则守卫 `UNREGISTERED → exit 1`（新债不许欠 → CI 红），构成「只增不减」。**不改 `scripts/ratchet-registry.json`**（`smoke_pool` 度量的是无关目录 `scripts/smoke/`）。

**验证命令**:
```bash
# (a) 新 smoke 自身真跑通
bash packages/brain/scripts/smoke/generator-publisher-runtime-boundary-smoke.sh
# (b)+(c) 同源守卫：临时 SMOKE_DIR 仅含新 smoke，跑真实 run-smoke-ratchet；exit 0 == 已登记 allowlist 且通过
TMP=$(mktemp -d); cp packages/brain/scripts/smoke/generator-publisher-runtime-boundary-smoke.sh "$TMP/"
SMOKE_DIR="$TMP" bash packages/quality/scripts/run-smoke-ratchet.sh; RC=$?; rm -rf "$TMP"
[ "$RC" = "0" ] || { echo "FAIL: run-smoke-ratchet 未纳管新 smoke（未登记 allowlist 或未通过）rc=$RC"; exit 1; }
# 期望：两条均 exit 0，末行 ✅ generator-publisher-runtime-boundary smoke 通过 / ✅ GATE PASSED
```
**硬阈值**: smoke exit 0；smoke basename ∈ `smoke-allowlist.txt`；`SMOKE_DIR=<临时目录仅含新 smoke> run-smoke-ratchet.sh` exit 0（同源守卫纳管+通过）。
...
| **Invariant（永不违反）** | 不变量 | `generator ∉ GITHUB_CREDENTIAL_ROLES`；caller 不能把 server `postgres:true` 降为 `false`；新 smoke 恒登记进 `smoke-allowlist.txt`（未登记/失败 → run-smoke-ratchet CI 红，只增不减） |
| **判定点（怎么知道）** | 判断假设 | 见判定点登记表 |
| **保质期（何时过期）** | 失效 | 无过期——回归护栏长期在库；GITHUB_CREDENTIAL_ROLES 定义变更时须同步更新锚点与测试 |
| **死亡告警（停了谁知道）** | 告警 | smoke 失败 → run-smoke-ratchet 基线 FAIL → CI 红（brain-ci）；smoke 被删/从 allowlist 移除 → run-smoke-ratchet UNREGISTERED（新债不许欠）→ CI 红 |
| **失败语义（挂了怎么办）** | 故障 | 见失败语义声明（fail-closed：授权不一致=拒绝，不放行） |
| **效果确认（已发≠已生效）** | 回执 | smoke exit 0 + `SMOKE_DIR=<临时>run-smoke-ratchet.sh` exit 0（同源守卫纳管+通过）+ 台账端点 200 健康；最终 real-harness 全链 PR/CI 绿 |
...
# 6. Step 4 — 永久接入 ratchet（同源守卫）：allowlist 登记 + run-smoke-ratchet 作用域跑纳管+通过
grep -qxF "generator-publisher-runtime-boundary-smoke.sh" packages/quality/smoke-allowlist.txt \
  || { echo "FAIL: smoke 未登记进 smoke-allowlist.txt"; exit 1; }
TMP="$(mktemp -d)"; cp "$SMOKE" "$TMP/"
if SMOKE_DIR="$TMP" bash packages/quality/scripts/run-smoke-ratchet.sh; then
  echo "OK step4 run-smoke-ratchet 纳管+通过（同源 CI 守卫，非 UNREGISTERED、非 BASELINE FAIL）"
else
  RC=$?; rm -rf "$TMP"; echo "FAIL step4: run-smoke-ratchet 未纳管新 smoke rc=$RC"; exit 1
fi
rm -rf "$TMP"

# 7. Step 4 — 台账端点健康检查（仅健康，非本 smoke 纳管证据；Brain 不可达则跳过，不阻塞）
RESP="$(curl -sS -m 8 -w $'\n%{http_code}' "$BRAIN/api/brain/quality/ratchet" 2>/dev/null || true)"
CODE="$(printf '%s' "$RESP" | tail -n1)"
if [ "$CODE" = "200" ]; then
  BODY="$(printf '%s' "$RESP" | sed '$d')"
  printf '%s' "$BODY" | node -e 'const d=JSON.parse(require("fs").readFileSync(0,"utf8"));if(d.available===true){if(!(d.registry||[]).some(e=>e.name==="smoke_pool"&&e.direction==="only_up"&&e.source)){console.error("FAIL: 台账缺 smoke_pool");process.exit(1)}console.log("OK step4 endpoint 健康 smoke_pool")}else if(d.available===false&&/ENOENT/.test(d.error||"")){console.log("OK step4 endpoint available:false(ENOENT) 已知拓扑降级")}else{console.error("FAIL: 非预期 "+JSON.stringify(d));process.exit(1)}'
else
  echo "SKIP step4 endpoint（Brain 台账端点不可达 HTTP=$CODE，健康检查跳过；纳管证据已由 step6 同源守卫给出）"
fi
