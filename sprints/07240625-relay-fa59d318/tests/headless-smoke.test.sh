...
# [BEHAVIOR] B1-B5: Brain 可达时运行完整 smoke（跳过条件：Brain 不可达）
...
if [ "$HEALTHZ_CODE" = "200" ]; then
  ...
  if bash "$SMOKE_SCRIPT"; then
    ok "B1-B5: 完整 smoke 脚本通过（A1-A5 全验）"
  else
    fail "B1-B5: smoke 脚本失败（见上方输出）"
  fi
else
  echo "  SKIP B1-B5: Brain 不可达（$BRAIN，HTTP $HEALTHZ_CODE），跳过运行时断言"
