...
```
- `owned_artifacts` (array, 必填, 无声明时为 `[]`): 按 OWNERS 声明确定性归属到本 capability 的照相层事实。每条含 `repo/fact_kind/stable_ref/method`（与 `findUnclaimedFacts` 的 factKey 口径逐字段一致，见 `map-read-service.js:33`）。来源——[NEW_PATTERN]
