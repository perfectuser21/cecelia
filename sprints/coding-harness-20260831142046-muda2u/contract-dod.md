...
- [ ] [BEHAVIOR] [L1] B-05: 实现范围不包含代码改动
  ...
  Test: manual:bash -c 'EXTRA=$(git diff --name-only 5c12d2af68e2b2e4b8dcaaa2c87e50efab743291...HEAD | grep -Ev '"'"'^(docs/current/attempt-run-bridge-guide\.md|sprints/coding-harness-20260831142046-muda2u/)'"'"' || true); [ -z "$EXTRA" ] || { echo "$EXTRA"; exit 1; }'

## 铁律映射

- 语言铁律：B-01 至 B-04 验证中文文档及四节事实。
- 分支与提交铁律：在 `cp-harness-propose-r1-efba73f3-r1836c876-a1` 上产出，不操作 main。
