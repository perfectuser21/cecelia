# Sprint Contract Draft (Round 2) — 结构化上报保真透传，根除 provider_exit 语义埋没 [r81]
...
>
> Round 2 修订（closure S-1，封印 seal_rejection FROZEN_CONTRACT_TEST_CONTRACT_UNRESOLVABLE）：R1 表中 `埋没点② 非零退出 + 结构化 success`、`埋没点② 非零退出 + 结构化 BLOCKED` 两条 BEHAVIOR 覆盖名无法解析——`assertTestContractResolvable`（contract-test-paths-seal.js:135-142）只提取 `it()`/`test()` 名做双向小写子串匹配，`埋没点②` 前缀来自 `describe()` 而非任何 `it()` 名，故不可解析。修法：把 describe ② 下的 4 条 `it()` 标题各前置 `埋没点② `（测试体一字未改，RED 7/9 语义不变），使表中两条 `埋没点②` 覆盖名成为对应 `it()` 名的字面子串；其余 6 条覆盖名（① 侧含 `负向：`/`铁律：` 中缀，前缀会破坏连续子串）保持裸写，本就是 ① `it()` 名的连续子串，无需改动。
