...
> **路径修订（Round 1 Fix）**：Feature 5/6 验证命令从 `packages/brain/src/harness.js` 更正为实际路径 `packages/brain/src/routes/harness.js`。
...
node -e "
  const code = require('fs').readFileSync('packages/brain/src/routes/harness.js', 'utf8');
...
node -e "
  ...
  const fs = require('fs');
  const raw = execSync('find apps/dashboard/src -name \"*ipeline*\" -o -name \"*pipeline*\"').toString().trim().split('\n').filter(Boolean);
  const allFiles = raw.filter(f => fs.statSync(f).isFile());
...
node -e "
  const fs = require('fs');
  const { execSync } = require('child_process');
  const raw = execSync('find apps/dashboard/src -name \"*pipeline*\" -o -name \"*Pipeline*\"').toString().trim().split('\n').filter(Boolean);
  const files = raw.filter(f => fs.statSync(f).isFile());
...
**范围**: `packages/brain/src/routes/harness.js` pipeline-detail 步骤扩展 + stats 端点 + `packages/brain/src/health-monitor.js` callback_queue_stats + `apps/dashboard/src/` 前端适配
**大小**: L（>300行，跨后端+前端）
**依赖**: 无
