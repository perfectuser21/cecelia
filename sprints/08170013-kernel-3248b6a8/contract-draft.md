...
map-radius: `map_scope=["F1"]`、`map_repo=null`、`expected_files=null` → 半径不可计算，`must_run_assertions=[]`（标 `[MAP_NOT_CONFIGURED]`，回归约束改由 PRD Invariant 段 + 本 sprint vitest 套件承载，禁止回退领域硬编码）。
...
**验证命令**:
```bash
npx vitest run sprints/08170013-kernel-3248b6a8/tests/ground-truth-proposal-remote.test.js sprints/08170013-kernel-3248b6a8/tests/regression-run-7a8e5319-rn.test.js --reporter=basic
```
...
**验证命令**:
```bash
npx vitest run sprints/08170013-kernel-3248b6a8/tests/derive-observation-gate.test.js --reporter=basic
```
...
**验证命令**:
```bash
npx vitest run sprints/08170013-kernel-3248b6a8/tests/work-routing-base-repo-backfill.test.js --reporter=basic
```
...
npx vitest run sprints/08170013-kernel-3248b6a8/tests/ --reporter=basic 2>&1 | tee /tmp/e2e-sprint.log
