...
> 冻结测试位于 `sprints/08170944-kernel-4eb00a75/tests/`（kernel 采集冻结产物只认此目录）；永久回归测试由 Generator 复制到 `packages/brain/src/**/__tests__/`（进 CI 长期回归）。B-05~B-08 的 vitest 命令跑 Generator 复制到 `packages/brain/src/**/__tests__/` 的永久副本（vitest include 覆盖，避免 sprints/** 范围外绿态假绿）。

## ARTIFACT 条目

- [ ] [ARTIFACT] 冻结测试 diff-gate 三类分流存在（sprints/tests + 永久副本）
  Test: node -e "require('fs').accessSync('sprints/08170944-kernel-4eb00a75/tests/diff-gate-reason-code.test.js');require('fs').accessSync('packages/brain/src/impact-contract/__tests__/diff-gate-reason-code.test.js')"

- [ ] [ARTIFACT] 冻结测试 harness-gates gateReceipt 透传存在（sprints/tests + 永久副本）
  Test: node -e "require('fs').accessSync('sprints/08170944-kernel-4eb00a75/tests/harness-gates-reason-code.test.js');require('fs').accessSync('packages/brain/src/impact-contract/__tests__/harness-gates-reason-code.test.js')"

- [ ] [ARTIFACT] 冻结测试 loop/derive 确定性出口路由存在（sprints/tests + 永久副本）
  Test: node -e "require('fs').accessSync('sprints/08170944-kernel-4eb00a75/tests/loop-impact-deterministic-route.test.js');require('fs').accessSync('packages/brain/src/orchestrator/__tests__/loop-impact-deterministic-route.test.js')"

- [ ] [ARTIFACT] 回归夹具（run d1360a48 录制件）+ 夹具测试存在（sprints/tests + 永久副本）
  Test: node -e "require('fs').accessSync('sprints/08170944-kernel-4eb00a75/fixtures/radius-d1360a48-impact-anchor.json');require('fs').accessSync('sprints/08170944-kernel-4eb00a75/tests/diff-gate-regression-d1360a48.test.js');require('fs').accessSync('packages/brain/src/impact-contract/__tests__/diff-gate-regression-d1360a48.test.js')"

- [ ] [ARTIFACT] Final E2E harness 存在（真 runLoop 一跳 + 真 appendHop 落 orchestrator_decision_log）
  Test: node -e "require('fs').accessSync('sprints/08170944-kernel-4eb00a75/e2e/impact-gate-e2e.mjs')"
...
- [ ] [ARTIFACT] Brain 版本四处同步 = 1.273.69（package.json / package-lock.json / .brain-versions / DEFINITION.md）
  Test: node -e "const v=require('./packages/brain/package.json').version;const l=require('./packages/brain/package-lock.json').version;const fs=require('fs');const bv=fs.readFileSync('.brain-versions','utf8').trim().split('\n').pop().trim();const def=/\*\*Brain 版本\*\*:\s*([0-9.]+)/.exec(fs.readFileSync('DEFINITION.md','utf8'))[1];if(!(v===l&&v===bv&&v===def&&v==='1.273.69'))process.exit(1)"
...
- [ ] [BEHAVIOR] [L1] B-05: harness-gates beforeEvaluate gateReceipt 透传 reason/retryable/detail/unclaimed_files（INV-3）
  动作: 跑 harness-gates-reason-code 永久副本测试（真 createHarnessImpactGates.beforeEvaluate + 注入 diffGate 确定性 blocked 结果）
  预期观察: receipt.stage='diff'、reason='impact_anchor_missing'、retryable=false、detail.unclaimed_files=['DoD.md']、顶层 unclaimed_files=['DoD.md']
  等待预算: 0s
  留证: vitest 用例输出末 5 行（含 passed）
  Test: manual:bash -c 'node node_modules/.bin/vitest run packages/brain/src/impact-contract/__tests__/harness-gates-reason-code.test.js --no-coverage'

- [ ] [BEHAVIOR] [L1] B-06: loop/derive impact_anchor_missing（retryable:false）→ intent 透传 + 路由 spawn:generator-fix（INV-1/INV-3）
  动作: 跑 loop-impact-deterministic-route 永久副本（真 runLoop + 真 derive，注入 beforeEvaluate 确定性 blocked receipt）
  预期观察: 决策日志 intent 行 gate_verdict='deny:impact:impact_anchor_missing'、detail.impact_gate.retryable=false、unclaimed_files=['DoD.md']；下一动作 dispatch 含 'spawn:generator-fix'（不再 blanket failRun/无限重试）
  等待预算: 0s
  留证: vitest 用例输出末 5 行（含 passed）
  Test: manual:bash -c 'node node_modules/.bin/vitest run packages/brain/src/orchestrator/__tests__/loop-impact-deterministic-route.test.js -t "impact_anchor_missing" --no-coverage'

- [ ] [BEHAVIOR] [L1] B-07: loop/derive capability_assertion_coverage_missing（retryable:false）→ 路由 wait:human_review
  动作: 跑 loop-impact-deterministic-route 永久副本的 coverage_missing 用例
  预期观察: 下一动作 dispatch 含 'wait:human_review'（Map 覆盖缺口需人补断言，不派 generator-fix）
  等待预算: 0s
  留证: vitest 用例输出末 5 行（含 passed）
  Test: manual:bash -c 'node node_modules/.bin/vitest run packages/brain/src/orchestrator/__tests__/loop-impact-deterministic-route.test.js -t "capability_assertion_coverage_missing" --no-coverage'

- [ ] [BEHAVIOR] [L1] B-08: 回归夹具 run d1360a48（真实 changed_files 含 DoD.md + radius 录制件）→ 新代码 blocked:impact_anchor_missing
  动作: 用录制件注入 evaluateDiffGate（changed_files 取自录制件 changed_files），跑永久副本测试
  预期观察: 返回 gate='blocked'、reason='impact_anchor_missing'、retryable=false、detail.unclaimed_files 含 'DoD.md'（旧代码此处为 mapper_stale/retryable:true）
  等待预算: 0s
  留证: vitest 用例输出末 5 行（含 passed）
  Test: manual:bash -c 'node node_modules/.bin/vitest run packages/brain/src/impact-contract/__tests__/diff-gate-regression-d1360a48.test.js --no-coverage'

- [ ] [BEHAVIOR] [L2] B-09: Final E2E — scratch 库 orchestrator_decision_log 落确定性 blocked 行（数据写入类，带时间窗，INV-5）[接缝×2]
  动作: 跑 Final E2E harness（迁移 scratch 库 → 真 runLoop 一跳 + 真 impactGate + 真 appendHop，注入 mapper 录制件），返回本次 run_id
...
- INV-3 证据入决策日志：gateReceipt 透传 detail，决策日志含 unclaimed_files — 由 B-05/B-06/B-09 覆盖
...
|---|---|---|---|
| diff-gate 三类分流 | `sprints/08170944-kernel-4eb00a75/tests/diff-gate-reason-code.test.js` | impact_anchor_missing → blocked/retryable=false/detail.unclaimed_files ; capability_assertion_coverage_missing → blocked/retryable=false/detail.capability_ids ; 未知 reason_code → impact_unknown/mapper_contract_invalid/retryable=false | 已实测 3 failed + 1 passed（fact_snapshot_stale 回归保护绿） |
| 回归夹具 d1360a48 | `sprints/08170944-kernel-4eb00a75/tests/diff-gate-regression-d1360a48.test.js` | blocked:impact_anchor_missing/retryable=false | 已实测 1 failed（当前折叠成 impact_unknown） |
| gateReceipt 透传 | `sprints/08170944-kernel-4eb00a75/tests/harness-gates-reason-code.test.js` | 透传 detail 与 unclaimed_files | 已实测 1 failed（receipt.detail === undefined） |
| loop/derive 确定性出口 | `sprints/08170944-kernel-4eb00a75/tests/loop-impact-deterministic-route.test.js` | 路由 spawn:generator-fix ; 路由 wait:human_review | 已实测 2 failed（当前 blanket failRun，未 dispatch 路由） |
