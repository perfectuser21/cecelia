...
**sprint**: 08170944-kernel-4eb00a75
**journey_type**: autonomous
**target_environment**: local_api
**base_sha (implementation_baseline, frozen)**: 76e5840230d8a7712a2b160f00d238581dc2fc36
...
> [MAP_NOT_CONFIGURED（部分）]: task.payload 有 `map_scope=['F1']` 但缺 `map_repo` 与 `expected_files`（Step 1.0 radius 半径查询需 scope+repo 两者），本轮无 `must_run_assertions` 注入；不回退领域硬编码，按 PRD + 代码实测起草。PRD 已声明 Map manifest 升 v3（F1 认领仓库根 `DoD.md`），生产上 `DoD.md` 不再触发 `impact_anchor_missing`；本 sprint 回归夹具用 run d1360a48 录制件复现，不依赖 live manifest。
> **冻结测试位置（r2 硬要求）**：本轮合同冻结测试位于 `sprints/08170944-kernel-4eb00a75/tests/`（前一单 f9f943fc 因放 `packages/brain/src/**/__tests__/` 致 `force_approve_but_contract_artifacts_missing`）；永久回归测试由 Generator 在实现阶段复制到 `packages/brain/src/**/__tests__/`。
...
**实测关键事实（起草依据，已逐行核对当前代码 base_sha=76e5840）**：

1. `diff-gate.js` 步骤 3a（当前 `evaluateDiffGate` 内，约 line 201-207）：
   ```js
   ...
   ```
   —— 把一切非 fresh（含 `status:'unknown'` 的确定性结论）统一折叠成 `mapper_stale/retryable:true`，**丢掉 `mapperResult.freshness.reason_code` 与 `mapperResult.unclaimed_files`**。这是无限重试根因（本轮已用 `db:null`+注入 mapper 实测复现：`impact_anchor_missing` 当前返回 `{gate:'impact_unknown',reason:'mapper_stale',retryable:true}`）。
2. `radius.js`（不改，仅作为 reason_code 来源事实）：`resolveImpactRadius` 返回
   ...
   - 真新鲜度类（reason_code ∈ `fact_snapshot_stale`/`projection_revision_missing`/`projection_revision_mismatch`/`manifest_projection_mismatch`/`graph_projection_revision_mismatch`）——radius.js:80-90 `baseFreshness`。
...
3. `harness-gates.js` `gateReceipt(stage, result, extra)`（约 line 26-36）当前只透传 `stage/gate/reason/retryable/contract_id/contract_hash` + extra，**不含 `detail`、不含 `unclaimed_files`**（本轮实测：注入确定性 blocked 结果后 `receipt.detail === undefined`）。
4. `loop.js`：
   - 约 line 1454：`gateVerdict = deny:impact:${impactGateReceipt?.reason ?? 'unknown'}`。
   - 约 line 1514：决策日志 `detail.impact_gate = impactGateReceipt`（受 gateReceipt 输出限制）。
   - 约 line 1538-1544：`gateVerdict` deny 时 `failure_class = impactGateReceipt?.retryable === false ? 'impact_contract_invalid' : 'infrastructure_blocked'`（已按 retryable 分流，故 diff-gate 改 retryable 后此处天然改道）。
   - 约 line 1661-1664：当前 `failure_class === 'impact_contract_invalid'` 分支**直接 `failRun('impact_gate_deterministic:...')` 并 return**，不经 derive 路由——本 sprint 要把它改成「记 BLOCKED 结果 + 由 derive 按 reason 路由 generator-fix / human_review」。
   - 约 line 84-90：`DETERMINISTIC_IMPACT_ERROR_CODES` 集合（当前含 `impact_contract_schema_invalid`/`impact_assertion_authority_invalid`/`mapper_evidence_invalid`/`mapper_evidence_missing`/`task_not_found`）——本 sprint 补入确定性 reason。
...
```
命中条件：`freshness.status !== 'fresh'` 且 `reason_code` 不在 (a)(b) 两个白名单内（含未来新增枚举、`status:'unknown'` 但 `reason_code` 为 `null`/未定义）。
...
**验证命令**:
```bash
node node_modules/.bin/vitest run sprints/08170944-kernel-4eb00a75/tests/diff-gate-reason-code.test.js -t 'impact_anchor_missing'
...
**验证命令**:
```bash
node node_modules/.bin/vitest run sprints/08170944-kernel-4eb00a75/tests/diff-gate-reason-code.test.js -t 'capability_assertion_coverage_missing'
# 期望：PASS
```
**硬阈值**: 用例 PASS（exit 0）。
...
**验证命令**:
```bash
node node_modules/.bin/vitest run sprints/08170944-kernel-4eb00a75/tests/diff-gate-reason-code.test.js -t 'fact_snapshot_stale'
# 期望：PASS（回归保护，行为不变；本用例在当前未改代码上已绿）
```
**硬阈值**: 用例 PASS（exit 0）。
...
**说明**: `[AI_ADDED]`（分类骨架）——(c) 兜底分支确保未来新增枚举不被误当可重试或误放行；理由：防止 mapper 契约演进时静默漏判（fail-closed 原则）。

**可观测行为**: 未知 reason_code → `gate='impact_unknown'`, `reason='mapper_contract_invalid'`, `retryable=false`（不 blocked、不放行、不可重试）。

**验证命令**:
```bash
node node_modules/.bin/vitest run sprints/08170944-kernel-4eb00a75/tests/diff-gate-reason-code.test.js -t 'mapper_contract_invalid'
# 期望：PASS
```
**硬阈值**: 用例 PASS（exit 0）。
...
**验证命令**:
```bash
node node_modules/.bin/vitest run sprints/08170944-kernel-4eb00a75/tests/harness-gates-reason-code.test.js
...
**验证命令**:
```bash
node node_modules/.bin/vitest run sprints/08170944-kernel-4eb00a75/tests/loop-impact-deterministic-route.test.js
...
- [回归] `packages/brain/src/orchestrator/__tests__/loop.test.js` → 「Diff Gate 未放行时不创建 evaluator attempt，并把裁决写入 decision log」（`mapper_stale` 路径必须保留）+「Impact schema 确定性错误精确终止且不进入基础设施重试」（`impact_gate_deterministic` 出口对**抛错类** `DETERMINISTIC_IMPACT_ERROR_CODES` 仍成立）。本 sprint 改的是**返回类**确定性结论的路由，不得破坏这两条既有用例语义。
...
- [累积FR] `context-manifest: unavailable`（journey `e6f803f2` 本 line 唯一 ability 状态=planned，无历史；Brain 端点在本 proposer 环境部分可达但 task 无 initiative_id 字段，累积 FR 按 PRD「本 line 暂无历史」）。
...
> Final E2E harness `sprints/08170944-kernel-4eb00a75/e2e/impact-gate-e2e.mjs` 是 Generator 实现阶段交付物（seed run/task/active-impact-contract + 候选 changed_files 含 DoD.md，跑真 runLoop 一跳落行，stdout 打印本次 run_id）。
...
cd "$WORKSPACE_PATH"
SPRINT_DIR="sprints/08170944-kernel-4eb00a75"
