...
Cecelia = Brain (Node.js, port 5221)
        ...
        + PostgreSQL（数据存储）
        + Skills 外部能力库（数量见 Brain API /api/brain/skills/count）
```
...
- 不要跳过 DevGate（`packages/quality/scripts/devgate/` 与 `packages/engine/scripts/devgate/`）
