...
## PR #934 — L4 并发限制修复（2026-03-13）

### 问题根因

`ci-l4-runtime.yml` 用 `concurrency: group: l4-runtime-mac` 全局串行所有 L4 run，即使 xian-mac-m1 有 4 个 runner 全部空闲，同时也只有 1 个 L4 在跑。根本原因是 matrix 里 PostgreSQL 端口硬编码（5432/5433/5434），多个 L4 并发会端口冲突。

### 解法

用 `GITHUB_RUN_ID` 动态计算端口区间：`5500 + (run_id % 100) * 3 + (shard_num - 1)`，每个 workflow run 独占 3 个端口，pgdata 目录也按 run_id 隔离。去掉全局 mutex 后多个 L4 可以真正并发。

### runner 安装陷阱

- tar 文件损坏时（gzip truncated），不要重新解压，直接 `rsync` 从已解压的 runner-1 复制
- `.runner_migrated` 文件会让 `config.sh` 报"already configured"——删除后重新注册即可
- runner 进程用 crontab `@reboot nohup` 启动，不用 launchd，新增 runner 后必须手动加 crontab 条目
- `config.sh remove --token` 在没有 `.credentials` 时会报错但不影响结果（exit 1 但实际已 remove）

### DoD Test 陷阱

- CI 环境没有 `gh` CLI 认证，`gh api` 命令会失败。runner 数量验证改为检查 workflow 文件本身的关键词

### 机器性能与 runner 数量

Mac mini M1（16GB / 8核）：每个 L4 run = 3 shard 并行（各 1 PostgreSQL + 1 vitest，约 5GB / 4核），6 个 runner 支持 2 个 L4 并发，是最优配置。超过 8 个 runner 内存会紧张。
...
### 根本原因
DoD 中使用了 `contract:C2-001` 格式，但根目录 regression-contract.yaml 不包含此 RCI ID（C2-001 定义在 packages/engine/ 子目录），导致 DoD Verification Gate 失败。

### 下次预防
- [ ] Brain/engine 脚本改动的 DoD 禁止使用 `contract:` 格式，改用 `manual:bash -c "grep -c ..."` 命令验证
- [ ] 写 DoD 时只用三种格式：tests/路径、manual:bash、manual:curl，不用 contract:
