...
### [2026-03-07] coding domain initiative_plan 路由到 /architect（PR #646, Brain v1.210.1）
...
5. **版本冲突处理（反复多轮）**：Brain 24/7 合并频繁，导致版本冲突从 1.205→1.209→1.210，每次都需要检查 main 最新版本并设置为 +1 patch
...
5. **测试平台隔离策略**：Darwin-specific 函数直接导出供测试，mock execSync 即可测试。Linux /proc 测试用 `it.skipIf(IS_DARWIN)` 标记
6. **child_process mock 陷阱**：模块初始化时有 `execSync('getconf PAGE_SIZE')`，vi.mock('child_process') 时必须让工厂处理这个调用，否则 PAGE_SIZE = NaN
