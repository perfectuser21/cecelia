...
## 2026-03-10 — 快手 OAuth 图文发布 API 接通（KR1 自救任务）

### 根本原因

- branch-protect hook 的 find_prd_dod_dir() 从编辑文件向上遍历，遇到 packages/workflows/.prd.md 就停止，导致目标目录不是 kuaishou-publisher/
- CI check-prd.sh 要求根目录 .prd-{BRANCH}.md 用 ## 成功标准 heading（非 **成功标准** bold）
- CI check-dod-mapping.cjs 要求所有 DoD 条目已验证 [x]（不能有 [ ]）
- Worktree 可能在 30 分钟内被 janitor 清理，导致会话工作目录失效

### 下次预防

- [ ] 新 skill 开发时，PRD/DoD 放在 skill 目录 + 根目录各一份（两处都需要）
- [ ] 根目录 DoD 的所有条目在推 PR 前全部手动改为 [x]
- [ ] check-prd.sh 要求 ## 成功标准（二级标题），不是加粗文本
- [ ] 避免 Worktree 长时间存在（超过 30 分钟），或确认 janitor 不清理活跃 worktree
