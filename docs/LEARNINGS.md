...
## fix(brain): execution-callback 补写 error_details 修复 quarantine last_error=Unknown (#911)

**分支**: cp-03122038-159bcf98-c825-4ca4-be8b-bd1659
**日期**: 2026-03-12
**影响程度**: Medium

### 根本原因

27 个隔离任务的 `quarantine_info.details.last_error` 全部为 "Unknown"，
根本原因是 `routes.js` execution-callback 失败分类时，
只写入 `failure_class` + `failure_detail`，漏掉了 `error_details` 字段。

`shouldQuarantineOnFailure()` 读取 `task.payload?.error_details`，
因字段缺失返回 null → 默认 "Unknown"。

`tick.js`（超时）和 `executor.js`（孤儿进程）已正确写入 `error_details`，
只有 execution-callback 路径（最常见）漏写。

### 修复

在 `routes.js` execution-callback 失败分类 payload 更新中补充 `error_details`：
- `billing_cap` 类 → `'quota_exhausted'`
- 其他失败 → 真实 `errorMsg`（截断至 1000 字符）

### 下次预防

- [ ] 新增失败处理路径时，检查是否同时写入了 `error_details` 到 payload（供隔离诊断使用）
- [ ] DoD test 命令避免跨行 grep pattern（如 `error_details.*errorMsg`），改用匹配变量名
- [ ] 修改 `.github/workflows/` 文件时，PR 标题必须加 `[CONFIG]` 标签
