...
Engine 有 `feature-registry.yml` 作为能力登记 SSOT，Brain 没有对应的强制登记机制。

`.agent-knowledge/brain.md` 是 Claude 感知 Brain 状态的 SSOT，但没有 CI 门禁保障其时效性。

结果：`self-drive.js`、`dopamine.js`、`pipeline-patrol.js` 等模块上线后，brain.md 长期未更新，Claude 代理对这些模块的存在和行为一无所知，形成功能孤岛。
