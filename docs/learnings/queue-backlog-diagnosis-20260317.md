...
1. **Brain 重启将 in_progress 孤儿任务批量 quarantine**：executor.js 启动清理逻辑在找不到进程时直接 quarantine，3 个任务同批次触发，导致 in_progress 骤降。
2. **Codex 账号全部耗尽后任务长期占槽**：所有 5 个账号耗尽后，cecelia-run.sh 继续轮询直到 783 分钟后超时，2 个槽位被"幻影占用"。
3. **Burst limiter 低压阈值过于保守**：pressure=0.37 时降速至 dispatch=0，但未考虑队列积压，导致 queued 积压时仍不派发。

### 下次预防

- [ ] 修复 executor.js 启动清理逻辑：in_progress 孤儿任务应 requeue（保留重试次数）而非直接 quarantine
- [ ] cecelia-run.sh 检测到"所有账号耗尽"时立即 exit，不再等超时（预期：占槽时间从 783 分钟降至 < 5 分钟）
- [ ] tick.js pressure 计算加入队列积压率：`pressure = max(mem, cpu, queued/MAX_SEATS)`
- [ ] 诊断类任务的 DoD Test 命令应使用 `node -e "..."` 而非 `ls`/`grep`（CI 白名单不含 grep/ls）
