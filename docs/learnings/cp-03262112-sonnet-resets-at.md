...
### 根本原因
Anthropic API 对 five_hour、seven_day、seven_day_sonnet 三个 scope 各自返回独立的 `resets_at`，但 `upsertCache()` 只读取了前两个，丢弃了 `data.seven_day_sonnet.resets_at`。导致 `/llm-quota` skill 无法显示 Sonnet 独立的重置时间，只能拿 7d all 的凑数。
