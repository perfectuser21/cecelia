...
## 陷阱 1：vi.useFakeTimers() 导致 waitFor/findByText 永久超时（2026-03-15）
...
### 下次预防
- [ ] 看到 `vi.useFakeTimers()` 时，立即检查测试是否用了 `findByText/waitFor`
- [ ] 有 fake timers 的测试文件，统一用 `act(async)` 模式，不用 `findByText`
...
## 陷阱 2：CI vitest v4 recharts useRef crash（本地 v1 不复现）（2026-03-15）
...
### 下次预防
- [ ] 新增 `act(async)` 测试的文件中，先检查是否有 recharts 组件被渲染
- [ ] 凡是包含 recharts 组件的页面测试，在文件顶层加 `vi.mock('recharts', ...)`
...
## 陷阱 3：DoD Gate [BEHAVIOR] 条目格式限制（2026-03-15）
...
### 下次预防
- [ ] 写 DoD 时，先确定哪些条目是真正的"运行时行为"，用 npm test / curl 验证
- [ ] `[BEHAVIOR]` 条目绝对不用 `grep/ls`；展示静态内容的检测用 `[ARTIFACT]`
