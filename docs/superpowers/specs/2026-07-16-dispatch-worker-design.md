...
2. `pickAccount(accounts, {vendor})`：过滤 usable（used_percent < 90 且查询成功）→ 按 used_percent 升序 → 返回排好序的候选队列（不只返回一个，供轮换）。
...
4. `buildCommand(vendor, account, brief, dir)`：返回 {cmd, args, env}——三厂商实测模板：
   - codex: `codex exec --cd <dir> --sandbox workspace-write <brief>`，env CODEX_HOME
...
## 测试策略
- **unit（node --test，scripts/dispatch-worker.test.mjs）**：pickAccount 排序/过滤/vendor 偏好、detectQuotaWall 命中与不命中样本（含 07-16 实测 codex 错误原文）、buildCommand 三厂商快照、轮换逻辑（mock spawn 注入撞墙输出→断言换账号）、池耗尽路径。
