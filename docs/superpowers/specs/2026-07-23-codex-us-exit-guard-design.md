...
采用“美国 M4 固定为默认、SF 临时兜底、严格验证”的方案：

- 只检测后报错虽然安全，但每次需要人工切换，无法满足一条命令启动。
- 动态恢复进入前的任意设置会把 CMG 恢复成 `none` 或中国出口，不符合 CMG 必须长期使用美国出口的要求。
- CMG 的稳定默认出口固定为美国 M4；仅当美国 M4 不可用时临时使用 SF，后续退出或下次启动都再次尝试回到美国 M4。
...
2. 把美国 M4 作为期望的稳定默认出口，不把进入前的 `none`、中国节点或 SF 当作恢复目标。
3. 每次准备时先确保使用美国 M4；美国 M4 不可用时才临时使用 SF VPS。
4. 切换后验证 Tailscale 配置、实际公网国家码、DNS/hosts 与 HTTPS 连通性。
5. Codex 会话退出时，如果当前使用 SF，则再次尝试切回美国 M4；绝不恢复成 `none` 或非美国出口。
...
2. 调用出口守卫；确保优先使用美国 M4，必要时临时回退 SF，并注册回归美国 M4 的收尾动作。
3. 只有门禁全部通过后，才反向 SSH 美国 M4并拉取对应 `auth.json`。
4. 保持现有 token 剩余有效期检查和 mode 600 要求。
5. 以前台子进程运行 Codex，使守卫能在正常退出、非零退出、`Ctrl-C`/`TERM` 后把临时 SF 出口切回美国 M4，并原样保留 Codex 的退出码。
...
美国侧 launcher 在传输 token 前，把出口守卫同步到西安临时路径并远程进入守卫；门禁通过后才推送 token、创建 launcher 和 tmux。远程 Codex launcher 持有收尾责任，在 Codex 退出后把临时 SF 出口切回美国 M4。若 token 推送或 tmux 创建在中途失败，美国侧 launcher 必须调用远程清理路径执行同一收尾动作。
...
- 启动时已使用 SF 也要先尝试切到美国 M4；只有 M4 当前不可用时才允许继续使用 SF。
- 会话结束时目标始终是美国 M4。如果 M4 仍不可用，则保留 SF 美国出口、打印高可见错误并返回非零；绝不清除 exit node，也不恢复非美国出口。
- 如果 Codex 本身成功而回归 M4 失败，整体返回非零；如果 Codex 已非零，则保留 Codex 退出码并同时报告回归失败。
- 不自动编辑 `/etc/hosts`、Tailscale 管理策略或系统 DNS。

本版本按现有人工单会话使用模型设计，不新增跨多个并发 Codex 会话的全局路由租约管理。若未来同一台 CMG 并发运行多个交互会话，需另行增加引用计数/租约，避免一个会话先退出后切换路由影响另一个会话。

## 测试策略

遵循 TDD，在现有 shell mock 测试模式上新增守卫测试，并扩充两个入口测试：

- 当前已是美国 M4：不切换，门禁通过后才传输 token，退出后仍保持 M4。
- 当前已是 SF VPS：先尝试切美国 M4；M4 不可用才保持 SF，退出时再次尝试 M4。
- 当前无 exit node 或为香港/中国出口：优先切美国 M4，Codex 退出后保持 M4。
- 美国 M4 切换失败：回退 SF VPS；退出时 M4 已恢复则切回 M4。
- 退出时 M4 仍不可用：保留 SF、返回非零，且绝不变成 `none`。
- 公网国家码为 `CN`、Cloudflare 超时、hosts 命中回环、ChatGPT TLS/HTTP 探测失败：均非零退出，且 mock 日志证明没有调用 `scp` 或 Codex。
- Codex 正常退出、非零退出和信号中断：都尝试回归美国 M4；Codex 非零退出码保持不变。
- `codex-remote-launch` 的远端准备失败时不推 token；后续步骤失败时执行远端回归美国 M4。
...
4. 退出 Codex，确认 CMG 仍固定使用美国 M4，不恢复成 `none`。
5. 模拟美国 M4 不可用，确认临时切换 `sf-vps`；美国 M4 恢复后，确认收尾或下次启动切回美国 M4。
