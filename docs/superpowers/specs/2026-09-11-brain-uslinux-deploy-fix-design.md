...
1. **`docker-compose.us-vps.yml`（新文件）**：`docker-compose.yml` 里 `node-brain` 服务的 Linux 版本。裁剪掉约24条 macOS 专属挂载（`.claude-account1~3`、`.codex-team1~5`、`.grok` 等），只保留：`REPO_ROOT` 代码卷（指向 us-vps 的 `/root/cecelia`）、DB 凭据挂载、`packages/workflows/skills`（只读，kernel-v1 in-process 执行需要读 skill 文件）。这是决策「Brain us-vps(Linux)部署-compose策略」和「...凭据挂载裁剪」的落地。
...
2. 解析后的配置文本 grep 断言：**不含** `claude-account`、`codex-team`、`.grok`；**含** REPO_ROOT 对应的卷、DB 凭据挂载、`packages/workflows/skills`
