...
> ⚠️ 落地编号勘误（2026-07-17 晚）：本设计原编号 347/348 被并行容器抢注——main 直落 commit a04136323 已带一个 thin 版 `348_promise_map_schema.sql`（无 domain/cell_key/双 partial unique，FK 无 SET NULL，未删旧 UNIQUE）。最终落地为 **349_promise_map_reconcile.sql（和解补齐）+ 350_seed_promise_map_two_domains.sql**，语义与本文 §3/§4 一致，差异仅为"在 348 thin 版之上补齐"而非全新建列。
