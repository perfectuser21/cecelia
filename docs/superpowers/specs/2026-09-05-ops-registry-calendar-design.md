...
## 架构（三个独立单元 + 清晰接口）
```
[采集器 collector.sh]──POST /agent-ops/report──▶[Brain ops 投影层]──5min pull──▶[notion-push 两库]
 宿主 LaunchDaemon         fail-closed 鉴权         ops_agents            upsert(PATCH)
 launchd腿+ssh OpenClaw腿   per-source 全集快照      ops_schedule_entries
 单飞锁TTL+spool≤3轮        单调 collected_at        ops_source_heartbeats
                                                    GET /agents /calendar(现算)
```
- **单元1 采集器**：纯 shell，无状态除 spool/lock；接口=report JSON（按 source 分段，每段自证全集）。
- **单元2 Brain ops 层**：migration（三表）+ routes（report/agents/calendar）+ freshness 判定（服务端单一时钟，3×间隔阈值，per-source）。calendar 现算合并 tasks/dispatch_events（过去24h 实跑）+ ops_schedule_entries + recurring_tasks 直读（死排程标 ⚠️ 禁绿灯）。
- **单元3 Notion 推送**：create-dbs 一次性脚本（按 title 搜索幂等）+ 两个 upsert push 函数挂进现有 syncAll。

关键契约（全部来自对抗结论，违反即 bug）：空结果必须 source_status=ok 佐证；失败双写 reason_code+last_error；宁 stale 不假数据；next_run 采集端算绝对 UTC 或留空；OpenClaw 只读 docker exec 写死路径；launchd 只认已加载；meta 白名单禁凭据入库；写端点 401 就是 401。
...
- **integration**（本地 pg，照 internal-auth.test.js 范式）：report 端点鉴权三态（无 token=401/坏 token=401/好 token=200）；同 payload 双 POST 幂等；部分 source 快照不影响他 source；旧 collected_at 丢弃；表缺失 503 migration_pending；agent 消失标 offline 不删行。
...
## 交付与部署
单 PR（本分支 cp-09052227-ops-registry-calendar）；merge 后 brain-deploy.sh 重建镜像（容器跑镜像快照，不重建=改了没生效）；LaunchDaemon 安装 + MUST_LOAD_DAEMONS 已在 PR 内；Notion 库创建脚本 merge 后手跑一次（NOTION_KEY 走 1Password）。
