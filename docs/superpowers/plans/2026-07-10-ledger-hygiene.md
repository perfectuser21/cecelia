...
### Task 1: Migration 330 — design_docs.type 加 ledger_hygiene

**Files:**
- Create: `packages/brain/migrations/330_design_docs_ledger_hygiene.sql`
...
Run: `node -e "const s=require('fs').readFileSync('packages/brain/migrations/330_design_docs_ledger_hygiene.sql','utf8'); if(!s.includes('ledger_hygiene')) process.exit(1); console.log('ok')"`
Expected: `ok`

- [ ] **Step 3: Commit**

```bash
git add packages/brain/migrations/330_design_docs_ledger_hygiene.sql
git commit -m "feat(brain): migration 330 design_docs.type 加 ledger_hygiene"
```
...
- [x] migration 330 加 ledger_hygiene 类型 — Test: manual: `node -e "const s=require('fs').readFileSync('packages/brain/migrations/330_design_docs_ledger_hygiene.sql','utf8'); if(!s.includes('ledger_hygiene')) process.exit(1); console.log('ok')"`
