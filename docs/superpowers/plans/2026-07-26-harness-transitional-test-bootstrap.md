...
**Architecture:** The self-contained `scripts/extract-contract-e2e.cjs` owns canonical E2E parsing and exports it to the shared Test Contract path module, ESM evaluator, default evaluator Skill, and ESM pyramid guard so they cannot drift. The Skill's non-Windows Step B-1 carries a byte-identical quoted-here-doc copy of that runtime, so third-party workspaces need no Cecelia `scripts/`. One recognized H2+ E2E section may contain multiple bash fences, concatenated in order, and internal H3 subsections do not terminate the section before the next H2. The pyramid guard retains raw artifact reporting but applies the zero orphan baseline only to artifacts registered by the same sprint contract; the ratchet imports that exact classification. Canonical contract fallback is allowed only when `contract-draft.md` is absent (`ENOENT`), never after other read errors.
...
`e2e-verify.sh` only when normalized contents match. Missing, empty, ambiguous,
or drifted E2E evidence remains unregistered; multiple bash blocks in one E2E
section are concatenated in document order. Drive the real evaluator export and
default evaluator Skill CLI through the same shared parser, reject ambiguous
evaluator-recognized E2E headings, and
