...
**Architecture:** A shared CommonJS module owns Test Contract parsing, canonical E2E extraction, and safe path resolution so the CommonJS coverage checker, ESM evaluator, default evaluator Skill, and ESM pyramid guard cannot drift. A repository CLI exposes that parser to the Skill's non-Windows Step B-1; one recognized H2+ E2E section may contain multiple bash fences, concatenated in order, and internal H3 subsections do not terminate the section before the next H2. The pyramid guard retains raw artifact reporting but applies the zero orphan baseline only to artifacts registered by the same sprint contract; the ratchet imports that exact classification. Canonical contract fallback is allowed only when `contract-draft.md` is absent (`ENOENT`), never after other read errors.
