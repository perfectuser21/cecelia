# Alertness State Behavior Integration Test — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 为 `/api/brain/alertness` 端点写完整行为级 integration test，覆盖 GET 字段完整性、POST override 状态变更、clear-override 还原三条链路。

**Architecture:** supertest + express 挂载真实 tick.js 路由（含 alertness 端点），mock `db.js`、`event-bus.js`、`events/taskEvents.js` 及 alertness 四个子模块（metrics/diagnosis/escalation/healing），保留 alertness/index.js 内存状态逻辑做真实行为验证。vi.resetModules() 在 beforeEach 隔离模块级状态，afterAll 调用 clear-override 严格还原。

**Tech Stack:** vitest, supertest, express, vi.resetModules(), vi.mock()
...
| 文件 | 操作 | 说明 |
|------|------|------|
| `packages/brain/src/__tests__/integration/alertness-state-behavior.integration.test.js` | **Create** | 唯一改动文件，integration test 主体 |

---

### Task 1：写 alertness-state-behavior integration test（TDD：先写失败测试，再验证通过）

**Files:**
- Create: `packages/brain/src/__tests__/integration/alertness-state-behavior.integration.test.js`

---

- [ ] **Step 1：创建测试文件（骨架，会 fail — 因为 import 路径 / mock 尚未写完）**

在 `/Users/administrator/worktrees/cecelia/brain-test-pyramid-pr5-alertness-integration/packages/brain/src/__tests__/integration/` 创建文件 `alertness-state-behavior.integration.test.js`，内容如下：
...
describe('Alertness State Behavior Integration Test', () => {
  ...
  describe('POST /api/brain/alertness/clear-override — 覆盖清除', () => {
    ...
    test('clear-override 后 GET override 字段为 null', async () => {
      ...
      // override 字段应为 null（evaluateAlertness 在 clearManualOverride 内被调用，可能 throw 因 mock，
      // 但 getCurrentAlertness 会反映 override=null）
      const after = await request(app).get('/api/brain/alertness');
      expect(after.status).toBe(200);
...
- [ ] **Step 2：在 worktree 中运行测试，验证它会 FAIL（依赖未 mock 完整或逻辑问题）**

```bash
cd /Users/administrator/worktrees/cecelia/brain-test-pyramid-pr5-alertness-integration/packages/brain
NODE_OPTIONS="--max-old-space-size=3072" npx vitest run \
  src/__tests__/integration/alertness-state-behavior.integration.test.js \
  --reporter=verbose 2>&1 | head -80
```

预期：看到 import 错误、mock 缺失或断言失败（FAIL 状态）。

- [ ] **Step 3：修复测试直至全部 PASS**

根据 Step 2 的错误输出，常见问题及修复方向：

**问题 A：`clearManualOverride` 内部调用 `evaluateAlertness`，后者调用 mock 的 `collectMetrics`，但 `vi.resetModules()` 后 mock 可能失效**

解决：在 `clearManualOverride` 的测试场景中，mock `evaluateAlertness` 本身，或接受 clear-override 可能返回 500（此时 success 字段不存在），调整断言为检查 `res.status` 而非 `res.body.success`：

若 `clear-override` 在 `evaluateAlertness` 失败时仍返回 200 + `success: true/false`（取决于路由错误处理），则无需修改。若返回 500，则修改 Step 1 中 `clear-override` 相关测试为：

```javascript
// 改为只断言 override 字段被清除，不依赖 clear-override 的 success 字段
test('clear-override 后 GET override 字段为 null', async () => {
  await request(app)
    .post('/api/brain/alertness/override')
    .send({ level: 4, reason: 'clear-test' });
  ...
  const after = await request(app).get('/api/brain/alertness');
  expect(after.status).toBe(200);
  // 只断言 override 为 null 或 level 已是初始值
  expect(after.body.override).toBeNull();
});
```

**问题 B：`vi.resetModules()` 后 `makeApp()` 重新 import tick.js，但 `vi.mock()` 的 mock 注册在 `resetModules` 后仍有效（vitest 文档确认此行为），若失效则去掉 `vi.resetModules()`**

若去掉 `vi.resetModules()` 导致状态污染（连续测试间 `_manualOverride` 残留），则在每个测试末尾手动清除：
...
`setManualOverride` 调用 `transitionToLevel(level, \`Manual override: ${reason}\`)`，所以 `after.body.reason` 应为 `"Manual override: integration-test-aware"`。若断言 `toContain('override')` 仍失败，改为 `toContain('integration-test-aware')` 或 `toBeTruthy()`。

- [ ] **Step 4：再次运行测试，确认全部 PASS**
...
预期输出（示例）：
```
✓ Alertness State Behavior Integration Test > GET /api/brain/alertness — 响应结构 > 响应包含 level、levelName、reason、startedAt 必填字段
✓ Alertness State Behavior Integration Test > GET /api/brain/alertness — 响应结构 > 响应还包含 levels 枚举对象和 level_names 数组
✓ Alertness State Behavior Integration Test > POST /api/brain/alertness/override — 状态变更行为 > override level=2(AWARE) 后 GET 返回 level=2，reason 含 override 信息
...
Tests  11 passed (11)
```

- [ ] **Step 5：Commit（TDD commit-1 = 测试文件）**

```bash
cd /Users/administrator/worktrees/cecelia/brain-test-pyramid-pr5-alertness-integration
git add packages/brain/src/__tests__/integration/alertness-state-behavior.integration.test.js
git commit -m "test(brain): alertness-state-behavior integration test — GET 字段+override 状态变更+clear 还原"
```
...
**文件路径**：`docs/learnings/cp-0501215632-brain-test-pyramid-pr5-alertness-integration.md`

内容模板（实现完成后填入真实根本原因）：

```markdown
### 根本原因
alertness 路由嵌在 tick.js 中，通过 routes.js 合并挂载，直接挂载 tick router 可直接覆盖所有 alertness 端点；alertness/index.js 用模块级变量维护状态，vi.resetModules() 可确保每次测试隔离。

### 下次预防
- [ ] alertness 状态行为测试：用 supertest + tick router 直接挂载，mock db/event-bus/alertness 子模块
- [ ] 每次测试前 vi.resetModules() + 重新 import，保证内存状态干净
- [ ] afterAll 调用 clear-override，防止测试污染 Brain 全局 alertness 状态
```

---

## DoD Checklist（push 前必须全部满足）
