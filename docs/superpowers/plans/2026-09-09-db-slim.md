...
> 修正（审查发现）：apply 必须先全量归档再统一删除，否则 ckpt 组孤儿行会不归档直接删。
...
注意：`SLIM_RULES` 顺序即执行顺序，graph 组天然"先 edges 后 versions"，ckpt 组"先 checkpoints 后 writes/blobs"（deleteWhere 的 NOT EXISTS 在前序删除提交后成立）。归档与删除拆成两个独立阶段串行执行：阶段 1 遍历全部规则做归档（archiveWhere 在任何删除发生前评估，不受其他规则删除影响），阶段 2 再遍历全部规则做删除（deleteWhere，此时前序规则已删除的效果对孤儿谓词生效）。
