...
- [x] [BEHAVIOR] GET /subtract?a=5&b=3 返 `{result:2, operation:"subtract"}` 严 schema（字段值 + 类型 + operation 字面字符串）
...
- [x] [BEHAVIOR] GET /subtract 成功响应顶层 keys 完整性恰好等于 ["operation","result"]（不允许多余字段，不允许少字段）
...
- [x] [BEHAVIOR] GET /subtract 成功响应严禁出现 PRD 禁用字段名（difference/diff/minus/subtraction/sub/subtracted/delta/gap/value/input/output/data/payload/response/answer/out/meta/sum/product/quotient/power/remainder/factorial/negation/incremented/next/successor/a/b）
...
- [x] [BEHAVIOR] GET /subtract?a=0.3&b=0.1 返 result === Number("0.3")-Number("0.1") === 0.19999999999999998（浮点严等，禁容差比较）
...
- [x] [BEHAVIOR] GET /subtract 算术边界值复算严等（a=10,b=10 → 0；a=-5,b=3 → -8；a=5,b=-3 → 8；a=-5,b=-3 → -2；a=0,b=0 → 0）evaluator 独立验，不依赖 E2E 段
...
- [x] [BEHAVIOR] GET /subtract 缺参（无 query / 缺 a / 缺 b）全部返 400 + 顶层 keys 严格 ["error"]，body 不含 result/operation
...
- [x] [BEHAVIOR] GET /subtract strict-schema 拒 10 类非法输入（科学计数法 / Infinity / NaN / 前导 + / 缺整数部分 / 缺小数部分 / 十六进制 / 千分位 / 字母串 / 双重负号）全返 400
...
- [x] [BEHAVIOR] GET /subtract?x=5&y=3（错 query 名）返 400（按缺 a/b 分支拒）
...
- [x] [BEHAVIOR] 8 路由回归 — /health、/sum、/multiply、/divide、/power、/modulo、/factorial、/increment 全部 happy 用例仍通过
