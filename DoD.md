contract_branch: cp-harness-propose-r1-c001a441
workstream_index: 1
sprint_dir: sprints/w35-walking-skeleton-final
...
- [ ] [ARTIFACT] `playground/server.js` 内含 strict-schema 浮点正则 `^-?\d+(\.\d+)?$`（复用 STRICT_NUMBER 常量合法，与 W20~W23 同款）
...
- [ ] [ARTIFACT] `playground/server.js` 内 `/subtract` 路由使用 query 名 `minuend` 和 `subtrahend`（不复用 `a`/`b`/`n`/`value`/`x`/`y`）
...
- [ ] [ARTIFACT] `playground/server.js` 内 `/subtract` 路由含 `Number.isFinite` 结果兜底（与 W22 /power 同款 defensive 设计）
...
- [ ] [ARTIFACT] `playground/server.js` 内 `/subtract` 路由响应字面含 `operation: "subtract"` 字符串
...
- [ ] [ARTIFACT] `playground/server.js` 内 `/subtract` 路由响应字面含 `result` 字段（不漂到 `difference`/`diff`/`subtraction`/`minus`/`delta` 等禁用名）
...
- [ ] [ARTIFACT] `playground/tests/server.test.js` 新增 `describe('GET /subtract'` 块（独立 describe，与其他 endpoint 平级）
...
- [ ] [ARTIFACT] `playground/README.md` 端点列表含 `/subtract` 段
  Test: node -e "const c=require('fs').readFileSync('playground/README.md','utf8');if(!/\/subtract/.test(c))process.exit(1)"

- [ ] [ARTIFACT] `playground/server.js` 现存 8 路由（/health /sum /multiply /divide /power /modulo /increment /factorial）全部仍注册存在（不被破坏）
...
- [ ] [BEHAVIOR] GET /subtract?minuend=10&subtrahend=3 → 200 + result 字面等于 7（值复算严格相等）
...
- [ ] [BEHAVIOR] GET /subtract?minuend=10&subtrahend=3 → operation 字面字符串严格等于 "subtract"（禁用变体 sub/subtraction/minus/diff）
...
- [ ] [BEHAVIOR] GET /subtract?minuend=10&subtrahend=3 → result 类型为 number
...
- [ ] [BEHAVIOR] GET /subtract?minuend=10&subtrahend=3 → 顶层 keys 字面集合等于 ["operation","result"]（不允许多余字段，不允许少字段）
...
- [ ] [BEHAVIOR] GET /subtract?minuend=10&subtrahend=3 → 响应不含首要禁用字段（difference/diff/subtraction/subtraction_result/sub_result/minus_result/minus/delta）
...
- [ ] [BEHAVIOR] GET /subtract?minuend=10&subtrahend=3 → 响应不含 generic 禁用字段（value/input/output/data/payload/response/answer/out/meta）
...
- [ ] [BEHAVIOR] GET /subtract?minuend=10&subtrahend=3 → 响应不含其他 endpoint 字段名（sum/product/quotient/power/remainder/factorial/negation）
...
- [ ] [BEHAVIOR] GET /subtract?minuend=abc&subtrahend=3 → 400（strict 拒字母）
...
- [ ] [BEHAVIOR] 错误响应顶层 keys 字面集合等于 ["error"]，且 error 是非空字符串，body 不含 result 也不含 operation
...
- [ ] [BEHAVIOR] GET /subtract?minuend=0&subtrahend=0 → 200 + result==0（零边界）
...
- [ ] [BEHAVIOR] GET /subtract?minuend=5&subtrahend=5 → 200 + result==0（minuend===subtrahend 反 off-by-one；不是 ±1）
...
- [ ] [BEHAVIOR] GET /subtract?minuend=5&subtrahend=10 → 200 + result==-5（负结果，防参数颠倒）
...
- [ ] [BEHAVIOR] GET /subtract?minuend=-5&subtrahend=-3 → 200 + result==-2（双负输入）
...
- [ ] [BEHAVIOR] GET /subtract?minuend=1.5&subtrahend=0.5 → 200 + result==1（浮点合法，IEEE 754 精确）
...
- [ ] [BEHAVIOR] GET /subtract?minuend=1e2&subtrahend=1 → 400（strict 拒科学计数法）
...
- [ ] [BEHAVIOR] GET /subtract?minuend=0xff&subtrahend=1 → 400（strict 拒十六进制）
...
- [ ] [BEHAVIOR] GET /subtract?minuend=Infinity&subtrahend=1 → 400（strict 拒 Infinity 字面）
...
- [ ] [BEHAVIOR] GET /subtract?minuend=NaN&subtrahend=1 → 400（strict 拒 NaN 字面）
...
- [ ] [BEHAVIOR] GET /subtract?minuend=&subtrahend=3 → 400（strict 拒空串）
...
- [ ] [BEHAVIOR] GET /subtract?minuend=%2B5&subtrahend=3 → 400（strict 拒前导 +）
...
- [ ] [BEHAVIOR] GET /subtract?minuend=--5&subtrahend=3 → 400（strict 拒双重负号）
...
- [ ] [BEHAVIOR] GET /subtract?minuend=10（缺 subtrahend）→ 400
...
- [ ] [BEHAVIOR] GET /subtract?subtrahend=3（缺 minuend）→ 400
...
- [ ] [BEHAVIOR] GET /subtract?a=10&b=3（错 query 名 a/b — W22 强约束）→ 400
...
- [ ] [BEHAVIOR] GET /subtract?x=10&y=3（错 query 名 x/y）→ 400；GET /subtract?lhs=10&rhs=3（错 query 名 lhs/rhs）→ 400
...
- [ ] [BEHAVIOR] 已有 8 路由 (/health /sum /multiply /divide /power /modulo /increment /factorial) happy 用例回归全通过
