contract_branch: cp-harness-contract-922fc33e
workstream_index: 1
sprint_dir: sprints/harness-planner-upgrade-v1

- [x] [ARTIFACT] `packages/workflows/skills/harness-planner/SKILL.md` 存在且 frontmatter version 为 `5.0.0`
...
- [x] [BEHAVIOR] Step 0 包含 `curl localhost:5221/api/brain/context` Brain API 上下文采集指令
...
- [x] [BEHAVIOR] PRD 模板包含 User Stories + Given-When-Then + FR-编号 + SC-编号 + 假设列表 + 边界情况 6 个结构化章节
...
- [x] [BEHAVIOR] 包含 9 类歧义自检列表且无法推断项标记为 `[ASSUMPTION: ...]`
...
- [x] [BEHAVIOR] PRD 模板包含 `## OKR 对齐` 章节，含 KR 编号、进度、预期推进字段
