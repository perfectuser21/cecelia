...
- [x] [ARTIFACT] 冻结合同测试文件存在且含 6 条 test()
...
- [x] [BEHAVIOR] [L2] B-01: 确定性 reason_code 走 fail-closed
...
- [x] [BEHAVIOR] [L2] B-02: 瞬时白名单 fact_snapshot_stale 保留重试
...
- [x] [BEHAVIOR] [L2] B-03: 瞬时白名单 projection_revision_missing 保留重试
...
- [x] [BEHAVIOR] [L2] B-04: freshness 缺失（null）保留重试
...
- [x] [BEHAVIOR] [L2] B-05: 未知/未来 reason_code 默认 fail-closed
...
- [x] [BEHAVIOR] [L2] B-06: gateReceipt 透传具体 reason_code（deny 标签非裸 mapper_stale）
...
- [x] [BEHAVIOR] [L2] INV-1: 既有 fail-closed 分支不回退（3a 仍 impact_unknown，无新增放行）
...
- [x] [BEHAVIOR] [L2] 冻结合同测试 6 条全绿（TDD 由红转绿）
