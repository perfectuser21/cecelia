contract_branch: cp-06111244-ws-65bf727b-ws1
sprint_dir: sprints/06111220-skill-drift-patrol
...
- [x] [ARTIFACT] `packages/brain/scripts/smoke/skill-drift-smoke.sh` 存在且含 `snapshot_version` 非 null 断言
...
- [x] [ARTIFACT] `packages/brain/src/cron/skill-drift-patrol.js` 存在且含 `raise` 调用
...
- [x] [ARTIFACT] `packages/brain/src/tick-runner.js` 含 skill-drift-patrol 模块导入
...
- [x] [ARTIFACT] `packages/brain/src/routes/harness.js` 含 `/patrol-history` 路由注册
...
- [x] [ARTIFACT] DB migration 文件存在（`packages/brain/migrations/301_skill_drift_alerts.sql` 或更高编号）
...
- [x] [BEHAVIOR] `GET /api/brain/harness/skill-drift/patrol-history` 返回 HTTP 200 + alerts 数组
...
- [x] [BEHAVIOR] `GET /api/brain/harness/skill-drift/patrol-history` schema 完整性（id + skill_name + ssot_version + snapshot_version + drift_date + detected_at，空列表时跳过字段检查）
...
- [x] [BEHAVIOR] `patrol-history` 禁用字段不出现（`alert_id`, `name`, `date`, `time` — 全部 4 个禁用名）
...
- [x] [BEHAVIOR] 制造漂移 → 触发巡检 → `skill_drift_alerts` 写入记录（带时间窗口防造假）
...
- [x] [BEHAVIOR] 同一 skill + drift_date 重复触发不新增记录（按日去重）
...
- [x] [BEHAVIOR] error path — `patrol-history` 非 GET 方法返回 4xx
