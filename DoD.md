contract_branch: cp-harness-propose-r1-8650eec6-r4ee28e74-a18
sprint_dir: sprints/08251745-kernel-r74-commander-retry

---
skeleton: false
journey_type: autonomous
---
# Contract DoD — Sprint: commander lease 过期有界自动重派 [r74]
...
- [ ] [ARTIFACT] 冻结合同测试存在且真 import derive.js
  Test: node -e "const c=require('fs').readFileSync('sprints/08251745-kernel-r74-commander-retry/tests/commander-infra-retry-bounded.test.ts','utf8');if(!c.includes(\"orchestrator/derive.js\"))process.exit(1)"
...
- [ ] [BEHAVIOR] [L2] B-01: 单条 commander infra 过期（<上限）不再挂人审
  ...
  等待预算: 0s
  留证: vitest -t 单用例绿输出 + /tmp/r74-e2e.log
  Test: manual:bash -c 'cd "${WORKSPACE_PATH:-/workspace}" && npx vitest run tests/gp/f1/step3-commander-infra-retry-bounded.test.js -t "单条 commander" --reporter=dot 2>&1 | grep -qE "Tests +1 passed"'
...
- [ ] [BEHAVIOR] [L2] B-02: 边界 累计 4 条（第 5 条前）仍不挂人审
  ...
  等待预算: 0s
  留证: vitest -t 单用例绿输出
  Test: manual:bash -c 'cd "${WORKSPACE_PATH:-/workspace}" && npx vitest run tests/gp/f1/step3-commander-infra-retry-bounded.test.js -t "边界 累计4条" --reporter=dot 2>&1 | grep -qE "Tests +1 passed"'
...
- [ ] [BEHAVIOR] [L2] B-03: 达上限 第 5 条 expired → fail-closed wait:human_review + callbackHop 锚
  ...
  Test: manual:bash -c 'cd "${WORKSPACE_PATH:-/workspace}" && npx vitest run tests/gp/f1/step3-commander-infra-retry-bounded.test.js -t "达上限 第5条" --reporter=dot 2>&1 | grep -qE "Tests +1 passed"'
...
- [ ] [BEHAVIOR] [L2] B-04: 负向 非 commander 角色（planner）infra 过期语义不变
  ...
  等待预算: 0s
  留证: vitest -t 单用例绿输出
  Test: manual:bash -c 'cd "${WORKSPACE_PATH:-/workspace}" && npx vitest run tests/gp/f1/step3-commander-infra-retry-bounded.test.js -t "非commander角色" --reporter=dot 2>&1 | grep -qE "Tests +1 passed"'

- [ ] [BEHAVIOR] [L2] B-05: 既有 #5058 消费闭环在「达上限」场景下 6 用例全绿
  动作: 跑既有（本 sprint 更新后）#5058 测试文件全量
  预期观察: 达上限 wait+callbackHop=112、approve 后消费出口（不再 wait）、<5 未达上限不挂人审、三条负向 fail-closed 全部成立
  等待预算: 0s
...
- [ ] [BEHAVIOR] [L2] INV-1 纯函数可重放：derive 只依赖 decisionLog 行时序，无时钟/随机/外部 IO
  动作: 同一 decisionLog 连续两次调 derive
  预期观察: 两次返回对象逐字段相等（无时间/随机漂移）；实现里不出现 Date/Math.random/fetch/pg
  等待预算: 0s
  留证: 冻结+ gp/f1 测试全绿（真 import 纯函数）+ grep 反向扫描输出
...
- [ ] [BEHAVIOR] [L2] INV-2 fail-closed 带锚：达上限必 wait:human_review 且带 callbackHop，禁静默放行/丢锚
  动作: 构造达上限（≥5）decisionLog 调 derive
  预期观察: action=wait:human_review 且 callbackHop 为整数（= 末条 expired 行 hop）
  等待预算: 0s
  留证: B-03 + 超上限用例绿输出
  Test: manual:bash -c 'cd "${WORKSPACE_PATH:-/workspace}" && npx vitest run tests/gp/f1/step3-commander-infra-retry-bounded.test.js -t "达上限 第5条" -t "超上限 第6条" --reporter=dot 2>&1 | grep -qE "Tests +2 passed"'
...
- [ ] [BEHAVIOR] [L2] INV-3 禁 mock 被改的边：合同测试真 import derive.js，无 vi.mock/stub 该边
  ...
  等待预算: 0s
  留证: grep 反向扫描输出（无命中）
  Test: manual:bash -c 'cd "${WORKSPACE_PATH:-/workspace}" && ! grep -rEn "vi\.mock|jest\.mock|infrastructureRetryForCallback\s*=|stub\(" tests/gp/f1/step3-commander-infra-retry-bounded.test.js tests/gp/f1/step3-route-unknown-review-approve-consume.test.js sprints/08251745-kernel-r74-commander-retry/tests/commander-infra-retry-bounded.test.ts'
