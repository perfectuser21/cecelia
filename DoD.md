...
- [ ] [ARTIFACT] validation-clock.js 落地 fix 轮顺延分支（含 spawn:generator-fix 过滤 + 有界常量 6）
...
- [ ] [BEHAVIOR] [L2] B-01: 复刻 r50 场景多次 fix 后原点顺延到最后一次 fix（旧判死→新存活）
...
- [ ] [BEHAVIOR] [L2] B-02: 顺延有界——满 6 次后原点冻结第 6 次 fix，第 7 次不再顺延照常判死
...
- [ ] [BEHAVIOR] [L2] B-03: 纯函数可重放——fix 行乱序/重复 hop 以 hop 升序取最后合法 fix
...
- [ ] [BEHAVIOR] [L2] B-04: sprint 冻结合同测试全绿（replay 顺延 + 有界 + 无 fix 回归）
...
- [ ] [BEHAVIOR] [L2] B-05: 无 fix 轮语义不变——既有 11 条 brain 单测不回归
