...
## [BEHAVIOR] 动态验证（manual:bash 命令，evaluator 直接执行）

**[BEHAVIOR 1]** `git diff --name-only origin/main HEAD -- sprints/` 输出含 `sprints/w49-b37-validation/` 路径
...
**[BEHAVIOR 2]** `bash verify-b37.sh` exit 0 且 stdout 含 "B37 验证全部通过"
  Test: manual:bash sprints/w49-b37-validation/verify-b37.sh
  期望: exit 0

**[BEHAVIOR 3]** `bash verify-b37.sh` 输出 ≥4 条 `✅ PASS` 断言
...
**[BEHAVIOR 4]** Brain Docker 日志无 ENOENT 关联 w49-b37-validation
