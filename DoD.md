...
- [x] [BEHAVIOR] 缺陷 B 精确性：isCaptureNegativeThenAssert 导出；捕获内 || true + 同名断言放行、捕获外 swallow 不放行
  Test: manual:node -e "import('./packages/brain/src/lib/contract-gate.js').then(m=>{const f=m.isCaptureNegativeThenAssert;if(typeof f!=='function')process.exit(2);const D=String.fromCharCode(36);const Q=String.fromCharCode(34);const cap='LOG='+D+'(run 2>&1 || true)';const ll=[{content:cap},{content:'echo '+Q+D+'LOG'+Q+' | grep -q err'}];if(f(cap,{logicalLines:ll,index:0})!==true)process.exit(3);const out='BAR='+D+'(baz); foo || true';const ol=[{content:out},{content:'echo '+Q+D+'BAR'+Q+' | grep -q ok'}];if(f(out,{logicalLines:ol,index:0})!==false)process.exit(4);console.log('OK')}).catch(e=>{console.error(e);process.exit(1)})"
