...
- [x] [ARTIFACT] generator 补 sprint-local e2e wrapper
  Test: node -e "const fs=require('fs');const p='scripts/smoke/e2e/relay-4bb31ef5.sh';const c=fs.readFileSync(p,'utf8');if(!c.includes('claude-headed-dispatch-smoke.sh')||!c.includes('4bb31ef5-e140-41f4-9daf-9ca4a9e51216'))process.exit(1)"

- [x] [ARTIFACT] 新增 claude headed smoke 并登记 allowlist
  Test: grep -Fxq "claude-headed-dispatch-smoke.sh" packages/quality/smoke-allowlist.txt

- [x] [ARTIFACT] 既有 codex headed smoke 仍在 allowlist（不回归）
  Test: grep -Fxq "codex-headed-dispatch-smoke.sh" packages/quality/smoke-allowlist.txt

- [x] [ARTIFACT] DoD.md 已记录本 sprint 的 claude-headed relay DoD
...
- [x] [BEHAVIOR] e2e wrapper 调用 claude-headed-dispatch-smoke.sh
  Test: manual:bash -c 'set -euo pipefail; SCRIPT="scripts/smoke/e2e/relay-4bb31ef5.sh"; [ -f "$SCRIPT" ] || { echo "FAIL: missing $SCRIPT"; exit 1; }; grep -F "packages/brain/scripts/smoke/claude-headed-dispatch-smoke.sh" "$SCRIPT" >/dev/null || { echo "FAIL: wrapper 未调用新 headed smoke"; exit 1; }; grep -F "packages/quality/smoke-allowlist.txt" "$SCRIPT" >/dev/null || { echo "FAIL: wrapper 未验证 allowlist"; exit 1; }; echo OK'
  期望: OK

- [x] [BEHAVIOR] payload 包含 mode=headed、executor=claude、orchestrator=skill-relay 且禁用 token/github_token/anthropic_token/thin_prd
...
- [x] [BEHAVIOR] initiative_runs 含 skill-relay-claude-headed 且 phase 拒绝 failed/unknown
...
- [x] [BEHAVIOR] ci.yml DoD 动态 seed 步骤补齐 claude-headed 分支，且 codex-headed 既有分支不回归（顺序性静态断言，Plan A）
...
- [x] [BEHAVIOR] tui.log 存在则验真，缺失则验留痕机制且不伪造
  Test: manual:bash -c 'set -euo pipefail; SCRIPT="scripts/smoke/e2e/relay-4bb31ef5.sh"; [ -f "$SCRIPT" ] || { echo "FAIL: missing $SCRIPT"; exit 1; }; grep -F "tui.log" "$SCRIPT" >/dev/null || { echo "FAIL: wrapper 未验证 tui.log"; exit 1; }; grep -F "WARN:" "$SCRIPT" >/dev/null || { echo "FAIL: 缺日志时必须输出 WARN/evidence"; exit 1; }; grep -F "packages/brain/src/harness-skill-relay.js" "$SCRIPT" >/dev/null || { echo "FAIL: 缺日志时必须验证 relay 源码"; exit 1; }; grep -F "appendFileSync" "$SCRIPT" >/dev/null || { echo "FAIL: 缺日志分支未验证 appendFileSync"; exit 1; }; grep -F "headed spawn" "$SCRIPT" >/dev/null || { echo "FAIL: 缺日志分支未验证 headed spawn"; exit 1; }; ! grep -E "touch[[:space:]]+\\\"?\\$LOG_PATH|>>[[:space:]]*\\\"?\\$LOG_PATH|appendFileSync\\([^)]*LOG_PATH" "$SCRIPT" >/dev/null || { echo "FAIL: wrapper 不得 touch/append tui.log"; exit 1; }; grep -E "FAIL: tui\\.log 缺失|tui\\.log 缺失或为空" "$SCRIPT" >/dev/null && { echo "FAIL: 缺日志不得硬失败"; exit 1; }; echo OK'
  期望: OK

- [x] [BEHAVIOR] local_api E2E wrapper 完整验证当前 task/run/log/ci-seed 外部真相
  Test: manual:bash -c 'set -euo pipefail; TASK_ID="${TASK_ID:-4bb31ef5-e140-41f4-9daf-9ca4a9e51216}" BRAIN_URL="${BRAIN_URL:-http://localhost:5221}" DATABASE_URL="${DATABASE_URL:-postgresql://cecelia:cecelia@localhost:5432/cecelia}" bash scripts/smoke/e2e/relay-4bb31ef5.sh'
  期望: OK
