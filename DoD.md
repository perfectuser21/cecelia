contract_branch: cp-harness-propose-r1-77f19b77-r8783807c-a32
sprint_dir: sprints/08131104-kernel-77f19b77
...
- [x] [ARTIFACT] migration 413 存在且加 controller_session_id + controller_lease_expires_at 到 initiative_runs（幂等 ADD COLUMN IF NOT EXISTS + schema_version 写 413）
...
- [x] [ARTIFACT] migration 413 rollback 存在（回滚 DROP COLUMN 两列）
...
- [x] [ARTIFACT] selfcheck EXPECTED_SCHEMA_VERSION 升到 413（DevGate facts 同步）
...
- [x] [ARTIFACT] change-kind.js 头注释同步四档正向默认映射 + 决策 29ae54ae + 禁反向推导降档
...
- [x] [ARTIFACT] 三个新回归测试文件登记进 vitest.config.js 的 POSTGRES_INTEGRATION_TESTS（两个 .pg.integration.test.js 永久进 brain-integration CI）
...
- [x] [BEHAVIOR] [L2] B-01: 四档 change_kind 驱动 derive 执行 Profile（bugfix/parameter_only 跳 planning）
...
- [x] [BEHAVIOR] [L2] B-02: createKernelRun 无 Controller identity → fail-closed（真 PG，拒绝创建不写半态）[接缝×2]
...
- [x] [BEHAVIOR] [L2] B-03: migration 413 真跑后 initiative_runs 有 controller_session_id + controller_lease_expires_at 列（真 PG information_schema）
...
- [x] [BEHAVIOR] [L2] B-04: 启动链收敛 — harness_runtime=kernel-v1 直打经 relay 不产生无 Controller run [接缝×2]
...
- [x] [BEHAVIOR] [L2] B-05: Kernel fatal 只结束 Kernel process，Controller 存活并回传结构化 failure_reason [接缝×2]
...
- [x] [BEHAVIOR] [L2] B-06: Controller fatal（lease 过期）后 Kernel 不成为无主 run，无主历史 run fail-closed 进恢复 [接缝×2]
...
- [x] [BEHAVIOR] [L2] B-07: 四档全部保留 Generate→Evaluate→Judge 与 merge fence（回归：gear default/hotfix 逐字节不变）
...
- [x] [BEHAVIOR] [L2] INV-1 [真环境done]+[禁mock被改边]: 被改的边（createKernelRun↔initiative_runs、relay 启动链、derive 状态机）在真 PG / 真代码路径验证，测试不 mock 被改的边
...
- [x] [BEHAVIOR] [L2] INV-2 [日志脱敏]+[凭据安全]: Kernel/Controller fatal 的 failure_reason 与 Brain log 结构化且脱敏，不落 controller_session_id 以外凭据
...
- [x] [BEHAVIOR] [L2] INV-3 [evaluator时钟]: Kernel 复用既有 PR 时采纳 evaluator validation clock（Controller 守护不改 validation identity 归属，late-bound）
