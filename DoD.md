...
- [ ] [ARTIFACT] m2 回归测试永留 CI：合同 tests/ledger-hygiene-m2-noise.test.js 已复制到 packages/brain/src/__tests__/。落位改写点**仅以下两处**（CONTRACT-IS-LAW 授权边界）：①静态 import 语句路径 `'../../../packages/brain/src/ledger-hygiene.js'` → `'../ledger-hygiene.js'`；②`MODULE_PATH` 常量值同步改为 `'../ledger-hygiene.js'`（供动态 import 在落位处正确解析）。其余逐字同源，关键 it 名齐全
