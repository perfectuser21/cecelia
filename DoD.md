...
- [x] [BEHAVIOR] POST /api/brain/notes（带 initiative_id）→ 201 + warnings array 非空
...
- [x] [BEHAVIOR] POST /api/brain/notes 响应 schema 完整性
...
- [x] [BEHAVIOR] POST /api/brain/notion/task → 201 + id/url/title/warnings
...
- [x] [BEHAVIOR] notion-property-map.js 导出 stripUnknownProperties 函数语义正确
...
- [x] [BEHAVIOR] error path — POST /api/brain/notes 缺 title 返 400
...
- [x] [BEHAVIOR] Brain DB notes 表在 5 分钟内有 initiative_id 对应记录
