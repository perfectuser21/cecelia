contract_branch: cp-harness-propose-r4-a69c58e4
workstream_index: 1
sprint_dir: sprints/w30-walking-skeleton-p1-v2
...
**Round 2 SSOT 引用**: 禁用字段清单单源在 `sprints/w30-walking-skeleton-p1-v2/banned-keys.sh`（详见 contract-draft.md `## Stable IDs` 段）。本 DoD 文件 BEHAVIOR-12 与 BEHAVIOR-13 严格 source SSOT 文件并通过 `${BANNED_RESPONSE_KEYS[@]}` / `${BANNED_ERROR_KEYS[@]}` 引用，不再 inline 粘贴 34/10 字段名清单。

**Round 3 风险登记联动**: contract-draft.md `## Risk Register` 段登记 12 类风险（R1~R12），其中 R12 数字边界对应本 DoD 新增 BEHAVIOR-11（`value=-0 → result=-1`），R1~R10 已映射到现有 BEHAVIOR-1 ~ BEHAVIOR-14 + ARTIFACT-1~9，R11 基础设施侧（P1 B1~B8 终验）属 out-of-contract 观测维度不在 DoD 范围。

**Round 4 显式编号**: 本 DoD 文件所有 BEHAVIOR 条目显式带 `[BEHAVIOR-N]` 序号标签（N=1..14，按文件出现顺序）。contract-draft.md 的 Risk Register 引用全部按此序号校齐。round 3 漂移问题（R9 引用 BEHAVIOR-13 实际应 14、R12 引用 Step 9.5 应明确为 BEHAVIOR-11）一并修复；R1/R7/R8 因 round 3 中段插入 BEHAVIOR-11（value=-0）导致的尾部 +1 偏移也同步对齐。
...
- [ ] [BEHAVIOR-1] GET /decrement?value=5 → 200 + {result:4, operation:"decrement"} + 顶层 keys 字面 ["operation","result"]（schema 完整性 + 字段名字面相等 oracle）
...
- [ ] [BEHAVIOR-2] GET /decrement?value=0 → 200 + {result:-1, operation:"decrement"}（off-by-one 零边界，防 generator 漂成 result=0）
...
- [ ] [BEHAVIOR-3] GET /decrement?value=1 → 200 + {result:0, operation:"decrement"}（off-by-one 1 边界，防 generator 漂成 result=1）
...
- [ ] [BEHAVIOR-4] GET /decrement?value=-9007199254740990 → 200 + {result:-9007199254740991, operation:"decrement"}（精度下界 happy，===Number.MIN_SAFE_INTEGER）
...
- [ ] [BEHAVIOR-5] GET /decrement?value=9007199254740990 → 200 + {result:9007199254740989, operation:"decrement"}（精度上界 happy）
...
- [ ] [BEHAVIOR-6] GET /decrement?value=9007199254740991 → 400（上界 +1 拒）；body 不含 result 也不含 operation；含 error 字段非空字符串
...
- [ ] [BEHAVIOR-7] GET /decrement?value=-9007199254740991 → 400（下界 -1 拒）
...
- [ ] [BEHAVIOR-8] strict-schema 拒非法输入（1.5 / 1e2 / 0xff / abc / 空串 / +5 / --5 / Infinity / NaN）全返 400
...
- [ ] [BEHAVIOR-9] query 唯一性约束：缺 value / 错 query 名 n / 多余 query extra 全返 400
...
- [ ] [BEHAVIOR-10] GET /decrement?value=01 → 200 + {result:0, operation:"decrement"}（前导 0 happy；禁 generator 错用 parseInt(value, 8) 八进制）
...
- [ ] [BEHAVIOR-11] GET /decrement?value=-0 → 200 + {result:-1, operation:"decrement"}（PRD ASSUMPTION 可选择项；round 3 R12 数字边界登记，挡 generator 把 strict regex 错改成 `^(0|-?[1-9]\d*)$` 拒掉 `-0` 字符串的漂移）
...
- [ ] [BEHAVIOR-12] 禁用响应字段反向：response 不含 SSOT BANNED_RESPONSE_KEYS 任一（PR-G 死规则继承；引用 sprints/w30-walking-skeleton-p1-v2/banned-keys.sh 单源，不 inline 粘贴 34 字段名清单）
...
- [ ] [BEHAVIOR-13] 错误体 schema 完整性：keys 字面 ["error"]，不含 SSOT BANNED_ERROR_KEYS 任一（引用 sprints/w30-walking-skeleton-p1-v2/banned-keys.sh 单源，不 inline 粘贴 10 字段名清单）
...
- [ ] [BEHAVIOR-14] 8 路由回归 happy：/health /sum /multiply /divide /power /modulo /factorial /increment 全过
