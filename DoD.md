contract_branch: cp-06120545-e2e-exec-judge-r3
...
- [x] [ARTIFACT] `packages/brain/src/harness-shared.js` 导出 `ExecutionRecordSchema`（新增 Zod schema）
...
- [x] [ARTIFACT] `packages/brain/src/workflows/harness-task.graph.js` 的 `evaluateContractNode` 函数体含代码执行段调用（非直接 spawn LLM）
...
- [x] [ARTIFACT] 单测文件存在且含 ExecutionRecordSchema + coverage 断言
...
- [x] [BEHAVIOR] Step 2 — `ExecutionRecordSchema` 已从 `harness-shared.js` 导出且可 parse 正常执行记录
...
- [x] [BEHAVIOR] Step 3 — `EvaluatorOutputSchema.shape` 含 `coverage` 字段，PASS verdict + coverage 数组 parse 成功
...
- [x] [BEHAVIOR] Step 4+5+6 — 单测全量通过（coverage 完整性 + 落盘 + 失败型 fixture）
...
- [x] [BEHAVIOR] Step 6 — `EvaluatorOutputSchema` 接受 FAIL verdict + failed_step + coverage 含失败步，parse 成功
...
- [x] [BEHAVIOR] Step 7 — legacy 回退开关（`EVALUATE_PATH=legacy` 可走旧 spawn 路径），源码含对应分支
...
- [x] [BEHAVIOR] Step 2 追加 — execution-record.json 写入逻辑存在于实现代码
