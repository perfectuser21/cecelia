...
- [x] [ARTIFACT] `resolveValidationClock` 顺延实现落在受影响文件（不越界改 runner）
...
- [x] [ARTIFACT] 冻结 sprint 测试存在并含顺延断言
...
- [x] [BEHAVIOR] [L2] B-01: 冻结 sprint 测试全绿（从仓库根跑，命中根 vitest include sprints/**）
...
- [x] [BEHAVIOR] [L2] B-02: 2 次 generator-fix → 窗口顺延两个 timeout（one timeout per generator-fix）
...
- [x] [BEHAVIOR] [L2] B-03: 1 次 generator-fix → 窗口顺延一个 timeout（exactly one timeout for a single generator-fix）
...
- [x] [BEHAVIOR] [L2] B-04: persisted 锚 detail 已顺延值不误判 invalid（tolerates a persisted anchor clock already advanced）
...
- [x] [BEHAVIOR] [L2] INV-1 [有界运行] B-05: 顺延窗口精确线性有界（finite and exactly linear for a bounded fix count）
...
- [x] [BEHAVIOR] [L2] INV-2 [零回归] B-06: 零 fix 窗口逐字节不变（byte-for-byte unchanged when no generator-fix）
...
- [x] [BEHAVIOR] [L2] INV-2 [零回归] B-07: 既有 kernel validation-clock 测试全绿（子 shell 用包自身 vitest 配置）
