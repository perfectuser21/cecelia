...
- [x] [ARTIFACT] apps/dashboard/package.json 声明 mind-elixir 依赖（MIT）
...
- [x] [ARTIFACT] view-model 纯函数落在 planning 内部并导出 buildMindmapTree
...
- [x] [ARTIFACT] MapPage.tsx 挂载 mind-elixir 容器且引用 view-model（不删既有语义 DOM）
...
- [x] [BEHAVIOR] [L2] B-01: mind-elixir view-model 从 contains 边推导三层脑图
...
- [x] [BEHAVIOR] [L2] B-02: 权威回归测试保持 4/4 全绿（mind-elixir additive 不回归）
...
- [x] [BEHAVIOR] [L2] B-03: /map 页渲染 mind-elixir 容器且非 fresh 出现可见提示（stale mock 单测）
...
- [x] [BEHAVIOR] [L2] B-04: cecelia scope 现算 summary == 2 价值流 / 11 能力（页面读数与 API 一致） [接缝×2]
  动作: 现算读取 GET /api/brain/map?scope=cecelia 的 summary
  预期观察: 端点契约成立——有投影时 summary 数值字段为 number；空库（CI）返回结构化 MAP_* 错误不假绿。真值 2 价值流/11 能力与页面 DOM 同值由 E2E 截图 01-initial 实证（2026-08-25 人工 controller 已在本机真 Brain 验证 2/11 通过）
  等待预算: 10s
  留证: curl+jq 输出（summary JSON）
  Test: manual:bash -c 'S=$(curl -s --max-time 10 "localhost:5221/api/brain/map?scope=cecelia"); echo "$S" | jq -e "(.summary and (.summary.value_streams|type==\"number\") and (.summary.capabilities|type==\"number\")) or (.error.code|tostring|startswith(\"MAP_\"))" || { echo FAIL; exit 1; }; echo OK'

- [x] [BEHAVIOR] [L2] B-05: error path — 缺 scope 参数返回 4xx（不静默兜底）
...
- [x] [BEHAVIOR:E2E] 用户完整走完 Golden Path（打开 /#/map → 语义全景+mind-elixir 脑图 → scope 切换重渲染 → freshness 徽标），截图可视化验证
  Screenshots:
