...
- [x] [ARTIFACT] TDD Red 阶段验证 — Generator commit 1（仅测试文件，无实现）时 vitest 必须 exit≠0（防止 Generator 写空实现绕过 Red 阶段）
  ...
  说明: Generator 在 commit 1 后必须运行 `cd /workspace/playground && npm install && npx vitest run tests/echo.test.js > /workspace/playground/.ws1-red.log 2>&1`，exit code 非 0；Evaluator 核查此日志确认测试真红（路径改为 /workspace/playground/.ws1-red.log 确保 Docker 环境可达）
