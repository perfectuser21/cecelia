contract_branch: cp-harness-review-approved-42434c49
workstream_index: 1
...
- [x] [BEHAVIOR] 执行流程区域内 Step 0 包含 `curl localhost:5221/api/brain/context` 且包含"不读代码实现"边界声明
...
- [x] [BEHAVIOR] 执行流程区域内包含"范围限定"和"受影响文件"章节
...
- [x] [BEHAVIOR] 包含 9 类歧义自检列表 + `[ASSUMPTION: ...]` 标记 + "方向性"决策提问原则
...
- [x] [BEHAVIOR] PRD 模板包含 `## OKR 对齐` 章节，含 KR + 进度 + 推进三字段
