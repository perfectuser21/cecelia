contract_branch: cp-harness-contract-4b9fdf1b
workstream_index: 1
...
- [x] [BEHAVIOR] Step 0 区域内包含 `curl localhost:5221/api/brain/context` 且包含"不读代码实现"边界声明 [R3-1: 限定到 Step 0]
...
- [x] [BEHAVIOR] 执行流程区域内包含 User Stories + Given-When-Then + FR-编号 + SC-编号 + 假设 + 边界 6 个结构化章节
...
- [x] [BEHAVIOR] 执行流程区域内包含 9 类歧义自检列表 + `[ASSUMPTION: ...]` 标记 + "方向性"决策提问原则 [R3-2: 限定到执行流程]
...
- [x] [BEHAVIOR] 执行流程区域内 PRD 模板包含 `## OKR 对齐` 章节，含 KR + 进度 + 推进三字段 [R3-3: 限定到执行流程]
...
- [x] [BEHAVIOR] PRD 模板不包含任何用户交互占位符（`请用户确认`/`待用户回答`/`等待用户`）
