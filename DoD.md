...
- [x] [BEHAVIOR] [L2] B1 Golden Path Step 1 只识别原 Cecelia Harness Pipeline
  ...
  Test: manual:bash -c 'HARNESS_TEST_DATABASE_URL="${HARNESS_TEST_DATABASE_URL:-${TEST_DATABASE_URL:-${DATABASE_URL:-}}}"; export HARNESS_TEST_DATABASE_URL; : "${HARNESS_TEST_DATABASE_URL:?}"; timeout 180 bash packages/brain/scripts/smoke/kernel-harness-f1-baseline-smoke.sh unique-journey'
...
- [x] [BEHAVIOR] [L2] B2 Golden Path Step 2 保留历史并补齐 S0-S12
  ...
  Test: manual:bash -c 'HARNESS_TEST_DATABASE_URL="${HARNESS_TEST_DATABASE_URL:-${TEST_DATABASE_URL:-${DATABASE_URL:-}}}"; export HARNESS_TEST_DATABASE_URL; : "${HARNESS_TEST_DATABASE_URL:?}"; timeout 180 bash packages/brain/scripts/smoke/kernel-harness-f1-baseline-smoke.sh history-and-backbone'
...
- [x] [BEHAVIOR] [L2] B3 Golden Path Step 3 每个 backbone Step 恰有精确 11 个 element cells
  ...
  Test: manual:bash -c 'HARNESS_TEST_DATABASE_URL="${HARNESS_TEST_DATABASE_URL:-${TEST_DATABASE_URL:-${DATABASE_URL:-}}}"; export HARNESS_TEST_DATABASE_URL; : "${HARNESS_TEST_DATABASE_URL:?}"; timeout 180 bash packages/brain/scripts/smoke/kernel-harness-f1-baseline-smoke.sh cells-and-evidence'
...
- [x] [BEHAVIOR] [L2] B5 Golden Path Step 4 P0/P1 筛选与五态证据逐项机检
  ...
  Test: manual:bash -c 'HARNESS_TEST_DATABASE_URL="${HARNESS_TEST_DATABASE_URL:-${TEST_DATABASE_URL:-${DATABASE_URL:-}}}"; export HARNESS_TEST_DATABASE_URL; : "${HARNESS_TEST_DATABASE_URL:?}"; timeout 180 bash packages/brain/scripts/smoke/kernel-harness-f1-baseline-smoke.sh legacy-baseline'
...
- [x] [BEHAVIOR] [L2] B6 Golden Path Step 5 非空 assertion_ref 全部可追到唯一根合同或真实测试
  ...
  Test: manual:bash -c 'HARNESS_TEST_DATABASE_URL="${HARNESS_TEST_DATABASE_URL:-${TEST_DATABASE_URL:-${DATABASE_URL:-}}}"; export HARNESS_TEST_DATABASE_URL; : "${HARNESS_TEST_DATABASE_URL:?}"; timeout 180 bash packages/brain/scripts/smoke/kernel-harness-f1-baseline-smoke.sh assertion-refs'
...
- [x] [BEHAVIOR] [L2] B7 Golden Path Step 6 真实 Brain endpoint 显示 merge 不等于 completed
  ...
  Test: manual:bash -c 'HARNESS_TEST_DATABASE_URL="${HARNESS_TEST_DATABASE_URL:-${TEST_DATABASE_URL:-${DATABASE_URL:-}}}"; export HARNESS_TEST_DATABASE_URL; : "${HARNESS_TEST_DATABASE_URL:?}"; timeout 180 bash packages/brain/scripts/smoke/kernel-harness-f1-baseline-smoke.sh endpoint-semantics'
...
- [x] [BEHAVIOR] [L2] B8 Golden Path Step 7 既有 merge staging production 行为零回退
  ...
  Test: manual:bash -c 'HARNESS_TEST_DATABASE_URL="${HARNESS_TEST_DATABASE_URL:-${TEST_DATABASE_URL:-${DATABASE_URL:-}}}"; export HARNESS_TEST_DATABASE_URL; : "${HARNESS_TEST_DATABASE_URL:?}"; timeout 240 bash packages/brain/scripts/smoke/kernel-harness-f1-baseline-smoke.sh runtime-nonregression'
...
- [x] [BEHAVIOR] [L2] INV-31 真 worktree 测试必须隔离生产资源
  ...
  Test: manual:bash -c 'HARNESS_TEST_DATABASE_URL="${HARNESS_TEST_DATABASE_URL:-${TEST_DATABASE_URL:-${DATABASE_URL:-}}}"; export HARNESS_TEST_DATABASE_URL; : "${HARNESS_TEST_DATABASE_URL:?}"; DB=$(psql -X -qAt "$HARNESS_TEST_DATABASE_URL" -c "SELECT current_database()"); case "$DB" in *_test|preview_*) exit 0;; *) echo "FAIL: non-isolated db=$DB"; exit 1;; esac'
