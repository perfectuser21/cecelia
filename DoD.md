contract_branch: cp-harness-propose-r2-aa069d30-re06fc474-a48
sprint_dir: sprints/09060615-kernel-aa069d30
...
- [x] [ARTIFACT] 新增门禁模块 capability-gate.js 导出 runCapabilityGate
...
- [x] [ARTIFACT] work-router 侧接线：new_capability 在选 pipeline 前调用门禁（createRoutedTask 生产接线）
...
- [x] [BEHAVIOR] [L2] B-01: new_capability 过闸后 decisions 落一句 postcondition + NFR 三数（category=nfr, level=step, target_type=journey_step）
...
- [x] [BEHAVIOR] [L2] B-02: context.nfr 三数均为 number（能力验收锚点完整）
...
- [x] [BEHAVIOR] [L2] B-03: 三镜头判 reject → fail-closed 拦截，拒绝原因可查，不写 nfr
...
- [x] [BEHAVIOR] [L2] B-04: postcondition/NFR 三数不完整 → fail-closed（capability_gate_contract_incomplete），不写 nfr
...
- [x] [BEHAVIOR] [L2] B-05: 非 new_capability（capability_change/bugfix/parameter_only）不触发门禁，四格路由行为不变
...
- [x] [BEHAVIOR] [L2] B-06: 冻结控制逻辑测试转绿（短路/拦截/落库编排单元验证）
