...
- [x] [ARTIFACT] 本 sprint 冻结守卫存在且真 import real validation-clock.js，断言有界顺延语义
...
- [x] [ARTIFACT] validation-clock.js 硬编码顺延上限常量 VALIDATION_CLOCK_EXTENSION_LIMIT = 6（有界续命铁律）
...
- [x] [BEHAVIOR] [L2] B-01: 2 轮 generator-fix 后 deadline 顺延到最新 fix 原点 created_at + timeout（RED→GREEN 核心）
...
- [x] [BEHAVIOR] [L2] B-02: 顺延超上限 —— 7 轮 generator-fix deadline 冻结在第 6 次顺延原点（防无限续命）
...
- [x] [BEHAVIOR] [L2] B-03: 边界 —— 恰好 6 轮 generator-fix 仍顺延到第 6 次原点（上限内不冻结）
...
- [x] [BEHAVIOR] [L2] B-04: 回归守恒 —— 无 generator-fix 行时窗口仍以首 generator 原点算（语义不变）
...
- [x] [BEHAVIOR] [L2] B-05: fail-closed 守恒 —— 非 generator 系且无有效 origin 仍抛 validation_clock_required
...
- [x] [BEHAVIOR] [L2] B-06: 既有 validation-clock 回归零回退（11 条 it 全绿，子 shell 用 packages/brain vitest 配置）
