...
- [x] [ARTIFACT] `packages/brain/scripts/ci/changed-test-router.mjs` 文件存在，包含 `readFileSync` 扫描逻辑
...
- [x] [ARTIFACT] `packages/brain/scripts/ci/check-contract-exists.mjs` 文件存在，含 `contract-draft.md` 检查逻辑
...
- [x] [ARTIFACT] `packages/engine/tests/skills/harness-evaluator.test.ts` 文件存在
  Test: node -e "require('fs').accessSync('packages/engine/tests/skills/harness-evaluator.test.ts')"

- [x] [ARTIFACT] `.github/workflows/ci.yml` 包含 skills 变更触发的 changed-test-router 调用
...
- [x] [BEHAVIOR] changed-test-router.mjs 对 harness-evaluator SKILL.md 输出含 harness-evaluator.test.ts 路径的清单（Step1）
...
- [x] [BEHAVIOR] harness-evaluator.test.ts 含 env_missing 不变量断言（B-1.6 关键守卫，文件内容检查）
...
- [x] [BEHAVIOR] check-contract-exists.mjs 对缺 contract-draft.md 的文件清单非零退出（Step4 负向）
...
- [x] [BEHAVIOR] check-contract-exists.mjs 对含 contract-draft.md 的文件清单退出码 0（Step4 正向）
...
- [x] [BEHAVIOR] harness-contract-reviewer.test.ts 含 7 维度名逐字断言（在活跃 it 块中，不在 describe.skip 内）
...
- [x] [BEHAVIOR] ci.yml 包含 packages/workflows/skills/** 路径触发的 changed-test-router 调用（Step5）
