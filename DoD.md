...
- [x] [BEHAVIOR] [L2] B-06: gear 列 round-trip + observed.gear 注入（真 Postgres）[接缝×2]
  ...
  Test: manual:bash -c 'cd "${REPO_ROOT:-/workspace}/packages/brain"; export DATABASE_URL="${DB_URL:?}"; L=$(npx vitest run src/__tests__/integration/kernel-gear-dispatch.pg.integration.test.js -t "round-trip" --reporter=dot 2>&1); echo "$L" | grep -qE "Tests +[0-9]+ passed" && ! echo "$L" | grep -qE "[1-9][0-9]* failed"'
...
- [x] [BEHAVIOR] [L2] B-07: hotfix run harness_attempts 角色分布 planner/proposer/reviewer=0 且 generator≥1（真 PG，时间窗防伪）[接缝×2]
  ...
  Test: manual:bash -c 'cd "${REPO_ROOT:-/workspace}/packages/brain"; export DATABASE_URL="${DB_URL:?}"; L=$(npx vitest run src/__tests__/integration/kernel-gear-dispatch.pg.integration.test.js -t "hotfix 首角色" --reporter=dot 2>&1); echo "$L" | grep -qE "Tests +[0-9]+ passed" && ! echo "$L" | grep -qE "[1-9][0-9]* failed"'
