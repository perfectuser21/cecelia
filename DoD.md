...
---
contract_branch: cp-harness-propose-r26-f09c9e31
...
- [x] [ARTIFACT] 合同草案含 Golden Path、禁 mock 边清单、E2E 验收脚本。
...
- [x] [ARTIFACT] Sprint 红测文件存在且测试名可被 Test Contract 字面匹配。
...
- [x] [ARTIFACT] Commander Phase 1/2 可复用稳定恢复原语，且未复制第二状态机。
...
- [x] [BEHAVIOR] [L2] 后续 run 继承 latest approved contract id/version/branch，且 derive 不再派 proposer/reviewer
  ...
  Test: manual:bash -c 'DB_NAME="${DB_NAME:-cecelia_test}" NODE_ENV=test npx vitest run sprints/07251915-kernel-f09c9e31/tests/kernel-durable-resume.test.ts -t "后续 run 继承 latest approved contract" --reporter=verbose'
  期望: exit 0

- [x] [BEHAVIOR] [L2] 首个 run 无 approved contract 时维持首次 GAN 路径并隔离其他 task
  ...
  Test: manual:bash -c 'DB_NAME="${DB_NAME:-cecelia_test}" NODE_ENV=test npx vitest run sprints/07251915-kernel-f09c9e31/tests/kernel-durable-resume.test.ts -t "首个 run 无 approved contract" --reporter=verbose'
  期望: exit 0

- [x] [BEHAVIOR] [L2] run bootstrap 中途失败时生产事务回滚且不留半写
  ...
  Test: manual:bash -c 'DB_NAME="${DB_NAME:-cecelia_test}" NODE_ENV=test npx vitest run sprints/07251915-kernel-f09c9e31/tests/kernel-durable-resume.test.ts -t "run bootstrap 中途失败" --reporter=verbose'
  期望: exit 0

- [x] [BEHAVIOR] [L2] ground truth 从历史 approved contract 恢复当前 run，已确认合同里程碑不降级
  ...
  Test: manual:bash -c 'DB_NAME="${DB_NAME:-cecelia_test}" NODE_ENV=test npx vitest run sprints/07251915-kernel-f09c9e31/tests/kernel-durable-resume.test.ts -t "ground truth 从历史 approved contract 恢复当前 run" --reporter=verbose'
  期望: exit 0

- [x] [BEHAVIOR] [L2] Brain restart 后 PRD/PR/合同里程碑单调恢复，下一角色与不中断基线一致
  ...
  Test: manual:bash -c 'DB_NAME="${DB_NAME:-cecelia_test}" NODE_ENV=test npx vitest run sprints/07251915-kernel-f09c9e31/tests/kernel-durable-resume.test.ts -t "Brain restart 后 PRD/PR/合同里程碑单调恢复" --reporter=verbose'
  期望: exit 0

- [x] [BEHAVIOR] [L2] 稳定恢复原语：expired lease 有 provider session 时调用真实 provider resume 并恢复原 attempt
  ...
  Test: manual:bash -c 'DB_NAME="${DB_NAME:-cecelia_test}" NODE_ENV=test npx vitest run sprints/07251915-kernel-f09c9e31/tests/kernel-durable-resume.test.ts -t "稳定恢复原语：expired lease 有 provider session" --reporter=verbose'
  期望: exit 0

- [x] [BEHAVIOR] [L2] 稳定恢复原语：未过期 running attempt 不被本 worker reclaim 或 resume
  ...
  Test: manual:bash -c 'DB_NAME="${DB_NAME:-cecelia_test}" NODE_ENV=test npx vitest run sprints/07251915-kernel-f09c9e31/tests/kernel-durable-resume.test.ts -t "稳定恢复原语：未过期 running attempt" --reporter=verbose'
  期望: exit 0

- [x] [BEHAVIOR] [L2] 稳定恢复原语：无 provider session 时自动终结 orphan 后从 DB/GitHub 真相推导
  ...
  Test: manual:bash -c 'DB_NAME="${DB_NAME:-cecelia_test}" NODE_ENV=test npx vitest run sprints/07251915-kernel-f09c9e31/tests/kernel-durable-resume.test.ts -t "稳定恢复原语：无 provider session" --reporter=verbose'
  期望: exit 0

- [x] [BEHAVIOR] [L2] 跨 run 同结构化 failure signature 重现时不再派 generator
  ...
  Test: manual:bash -c 'DB_NAME="${DB_NAME:-cecelia_test}" NODE_ENV=test npx vitest run sprints/07251915-kernel-f09c9e31/tests/kernel-durable-resume.test.ts -t "跨 run 同结构化 failure signature" --reporter=verbose'
  期望: exit 0

- [x] [BEHAVIOR] [L2] 跨 run 不同 failure signature 首次出现时仍派 generator-fix
  ...
  Test: manual:bash -c 'DB_NAME="${DB_NAME:-cecelia_test}" NODE_ENV=test npx vitest run sprints/07251915-kernel-f09c9e31/tests/kernel-durable-resume.test.ts -t "跨 run 不同 failure signature 首次出现" --reporter=verbose'
  期望: exit 0

- [x] [BEHAVIOR] [L2] Brain restart 回归池全量通过，既有合同测试未被削弱
  动作: 运行 sprint 红测与现有 orchestrator 合同回归池。
  预期观察: 测试命令退出时，所有相关 vitest exit 0。
  Test: manual:bash -c 'DB_NAME="${DB_NAME:-cecelia_test}" NODE_ENV=test bash -c "npx vitest run sprints/07251915-kernel-f09c9e31/tests/kernel-durable-resume.test.ts --reporter=verbose && cd packages/brain && npx vitest run src/orchestrator/__tests__/contract-store.test.js src/orchestrator/__tests__/attempt-store.test.js src/orchestrator/__tests__/ground-truth.test.js src/orchestrator/__tests__/derive.test.js src/orchestrator/__tests__/counters.test.js src/orchestrator/__tests__/loop.test.js src/__tests__/harness-kernel-resume-secret.test.js --reporter=verbose"'
  期望: exit 0

- [x] [BEHAVIOR] [L2] Final E2E 对实现 PR 真调 GitHub PR state/head_sha/statusCheckRollup
  ...
  Test: manual:bash -c 'CURRENT_BRANCH=$(git branch --show-current); PR_JSON=$(gh pr view "$CURRENT_BRANCH" --json url,state,headRefOid,statusCheckRollup); echo "$PR_JSON" | jq -e ".state == \"OPEN\" and (.url | type == \"string\") and (.headRefOid | type == \"string\" and length >= 7) and (.statusCheckRollup | type == \"array\")"'
  期望: exit 0

- [x] [BEHAVIOR] [L2] INV-01 不新增第二账本，branch 只在 initiative_contracts.branch
  动作: 扫描本 PR diff 与源码。
  预期观察: 源码扫描命令退出时，不出现新增 `contract_branch` 存储或新 ledger 表。
  Test: manual:bash -c 'node -e "const fs=require(\"fs\");const files=[\"packages/brain/src/orchestrator/contract-store.js\",\"packages/brain/src/orchestrator/ground-truth.js\",\"packages/brain/src/harness-skill-relay.js\"];const s=files.filter(fs.existsSync).map(f=>fs.readFileSync(f,\"utf8\")).join(\"\\n\");if(s.includes(\"contract_branch\"))process.exit(1);console.log(\"OK\")"'
  期望: OK

- [x] [BEHAVIOR] [L2] INV-02 禁 mock 被改 DB 边，sprint 红测使用真实 PostgreSQL temp schema
  动作: 运行 sprint 红测。
  ...
  Test: manual:bash -c 'node -e "const fs=require(\"fs\");const c=fs.readFileSync(\"sprints/07251915-kernel-f09c9e31/tests/kernel-durable-resume.test.ts\",\"utf8\");if(!c.includes(\"new Pool(DB_DEFAULTS)\"))process.exit(1);if(/vi\\.mock\\([^\\n]*(contract-store|ground-truth|attempt-store|derive|counters)/.test(c))process.exit(1);console.log(\"OK\")"'
  期望: OK
