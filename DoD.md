...
- [x] [ARTIFACT] 合同 Red 测试存在并覆盖 smoke 脚本防调度约束
...
- [x] [ARTIFACT] 实现 PR 只能触碰 smoke 脚本本身
...
- [x] [BEHAVIOR] 合法 headless/codex POST 校验必须保留
...
- [x] [BEHAVIOR] valid headless smoke task 创建后必须被取消或创建为非 queued
...
- [x] [BEHAVIOR] executor=codex 缺少 orchestrator=skill-relay 不得被当作本合法路径
...
- [x] [BEHAVIOR:E2E] local_api final-e2e 按 `contract-draft.md` 的 `## E2E 验收` 脚本执行，确认合法 headless/codex、非法 mode、valid task 非 queued 三项同时成立
