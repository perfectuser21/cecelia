...
- [x] [ARTIFACT] migration 395 新增 `initiative_runs.gear` 列（nullable）
...
- [x] [BEHAVIOR] [L2] B-01: gear=hotfix 初始态跳过 planner 直进 generate
  ...
  Test: manual:bash -c 'cd "${REPO_ROOT:-/workspace}/packages/brain"; L=$(npx vitest run src/orchestrator/__tests__/derive.test.js -t "不等于 spawn:planner" --reporter=dot 2>&1); echo "$L"; echo "$L" | grep -qE "Tests +[0-9]+ passed" && ! echo "$L" | grep -qE "[1-9][0-9]* failed"'
...
- [x] [BEHAVIOR] [L2] B-02: gear=hotfix 全程不派 planner/proposer/reviewer
  ...
  等待预算: 0s
  留证: vitest 输出末 5 行
  Test: manual:bash -c 'cd "${REPO_ROOT:-/workspace}/packages/brain"; L=$(npx vitest run src/orchestrator/__tests__/derive.test.js -t "全程不派" --reporter=dot 2>&1); echo "$L"; echo "$L" | grep -qE "Tests +[0-9]+ passed" && ! echo "$L" | grep -qE "[1-9][0-9]* failed"'
...
- [x] [BEHAVIOR] [L2] B-03: INV-1 gear=default 初始态零回归返回 spawn:planner
  ...
  Test: manual:bash -c 'cd "${REPO_ROOT:-/workspace}/packages/brain"; L=$(npx vitest run src/orchestrator/__tests__/derive.test.js --reporter=dot 2>&1); echo "$L"; echo "$L" | grep -qE "Tests +[0-9]+ passed" && ! echo "$L" | grep -qE "[1-9][0-9]* failed"'
...
- [x] [BEHAVIOR] [L2] B-04: gear=segmented 初始态照跑 planner（≠hotfix，对齐 controller segmented）
  ...
  等待预算: 0s
  留证: vitest 输出末 5 行
  Test: manual:bash -c 'cd "${REPO_ROOT:-/workspace}/packages/brain"; L=$(npx vitest run src/orchestrator/__tests__/derive.test.js -t "segmented 初始态 照跑 planner" --reporter=dot 2>&1); echo "$L"; echo "$L" | grep -qE "Tests +[0-9]+ passed" && ! echo "$L" | grep -qE "[1-9][0-9]* failed"'
...
- [x] [BEHAVIOR] [L2] B-05: INV-2 非法 gear（turbo）kernel 侧 fail-closed → mark_failed invalid_gear
  ...
  等待预算: 0s
  留证: vitest 输出末 5 行
  Test: manual:bash -c 'cd "${REPO_ROOT:-/workspace}/packages/brain"; L=$(npx vitest run src/orchestrator/__tests__/derive.test.js -t "fail-closed" --reporter=dot 2>&1); echo "$L"; echo "$L" | grep -qE "Tests +[0-9]+ passed" && ! echo "$L" | grep -qE "[1-9][0-9]* failed"'
