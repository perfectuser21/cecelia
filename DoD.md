contract_branch: cp-harness-propose-r1-6b8ad6ed-r2e593e10-a105
sprint_dir: sprints/09052209-kernel-6b8ad6ed
...
- [x] [ARTIFACT] scheduler-jobs.js 注册每日 `crystal-judge` 定时任务（走同一 runCrystalJudge）
...
- [x] [ARTIFACT] 判官模块 `packages/brain/src/crystal-judge.js` 只写 crystal_* 表（NFR 数据完整性：源只读）
...
- [x] [BEHAVIOR] [L2] B-01: 触发判官后结晶台账八格落库（真 Postgres 时间窗）
...
- [x] [BEHAVIOR] [L2] B-02: 每日结晶报告端点返回八格建议（三态 + 依据 + 六项指标）
...
- [x] [BEHAVIOR] [L2] B-03: N<20 的格判决必为 keep_llm（数据不足不晋升）
...
- [x] [BEHAVIOR] [L2] B-04: 每格有且仅有 1 条判决（无重复判决）
...
- [x] [BEHAVIOR] [L2] B-05: registry 回写复合键 model|app_version|density（真 Postgres 读回）
...
- [x] [BEHAVIOR] [L2] B-06: 证据文件名缺 trial+timestamp → 400
...
- [x] [BEHAVIOR] [L2] B-07: 合规证据文件名（带 trial+timestamp）→ 200 并解析出 trial
...
- [x] [BEHAVIOR] [L1] INV-1 判定层不蒸馏：判定层技能永不出 promote（即使全部晋升条件满足）
...
- [x] [BEHAVIOR] [L1] INV-2 探针强制：无 postcondition 的技能不许入库晋升
...
- [x] [BEHAVIOR] [L2] INV-3 registry 是数据：定位 registry 复合键 model|app_version|density 缺一即拒
...
- [x] [BEHAVIOR] [L2] INV-4 证据留痕：复用已存在文件名 → 409（禁覆盖）
...
- [x] [BEHAVIOR] [L1] INV-5 固化优先级 = 频率 × 失败率
...
- [x] 冻结单测 `sprints/09052209-kernel-6b8ad6ed/tests/crystal-verdict.test.ts` 先 Red 后 Green（TDD 顺序）
- [x] 全部 [ARTIFACT] / [BEHAVIOR] 在真实 Brain(localhost:5221) + 真 Postgres 上通过
- [x] DevGate 三项（facts-check / version-sync / dod-mapping）通过；Brain version bump
