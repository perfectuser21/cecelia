...
- [x] [ARTIFACT] 采集器新文件存在且导出 parser/枚举/collector（含 refresh_token 禁止注释）
...
- [x] [ARTIFACT] migration 449 新建 ops_model_accounts 表（含 status/last_error/forward_targets 列 + account_id 唯一）
...
- [x] [ARTIFACT] collector 源码无 refresh_token 调用路径（只允许出现在注释里）
...
- [x] [BEHAVIOR] [L2] B-01: 调 model-accounts 端点返回 8 条账号快照，11 字段齐全
...
- [x] [BEHAVIOR] [L2] B-02: 单账号失败只标该条，其余正常，HTTP 200
  ...
  等待预算: 0s
  留证: HTTP code + jq 输出
  Test: manual:bash -c 'bash packages/brain/scripts/model-accounts-seed-e2e.sh; CODE=$(curl -s -o /tmp/ma.json -w "%{http_code}" localhost:5221/api/brain/agent-ops/model-accounts); [ "$CODE" = "200" ] || { echo "FAIL HTTP $CODE"; exit 1; }; jq -e "[.data.accounts[]|select(.status==\"unknown\")]|length>=1 and (.[0].last_error|type==\"string\")" /tmp/ma.json'

- [x] [BEHAVIOR] [L2] B-03: Grok key 过期只标 key_expired，代码零 refresh_token 调用路径 [接缝×2]
  ...
  Test: manual:bash -c 'bash packages/brain/scripts/model-accounts-seed-e2e.sh && curl -s -o /tmp/ma.json localhost:5221/api/brain/agent-ops/model-accounts && jq -e "[.data.accounts[]|select(.status==\"key_expired\")]|length>=1" /tmp/ma.json && { HITS=$(grep -nE "refresh_token" packages/brain/src/ops-model-accounts-collector.js | grep -vE "^[0-9]+:[[:space:]]*(//|\*|#)" || true); [ -z "$HITS" ] || { echo "FAIL 非注释 refresh_token: $HITS"; exit 1; }; echo OK; }'

- [x] [BEHAVIOR] [L2] B-04: agents 端点每条追加真实 model_role（三字段）
...
- [x] [BEHAVIOR] [L2] B-05: 冻结合同测试全绿（parser 同 schema + key_expired 分类 + refresh spy 零调用 + collector→表→端点真 PG + model_role 聚合）
...
- [x] [BEHAVIOR] [L2] B-06: Notion「Agents&机器」库 schema 含配额列（FiveHourPct/SevenDayPct/QuotaUpdatedAt）
...
- [x] [BEHAVIOR] INV-3 [枚举单份] status 枚举只一份，collector 与 route builder 同源 import（无手抄副本）
...
- [x] [BEHAVIOR] INV-4 [幂等 upsert] collector 真跑写路径 + 同 account_id 重复采集不产生重复行
