...
## Brain 1.325.3 — skill-sync-to-runners 慢链路修复：超时可配 + --partial + 失败重试（任务 4950ccf3，棒 8 遗留缺口）
...
## Brain 1.325.2 — Skill 分发：清单哈希 + 漂移检测 + 同步脚本入库（链 bf5088a3 棒8，任务 1141f101，决策 105a5868）
...
## Brain 1.325.1 — executor=script 一等任务类型·契约与安全闸（任务 5cdbd52a，链 bf5088a3 棒 3 PR A，决策 105a5868）
...
42 批（#5085，assertion npm 预算保底 600s + assertion-budget.mjs 单一事实源）改了 entrypoint 与 Dockerfile，重建镜像后按 verify-digest-pin 清单一次性 repin b3ff98ff → 720c9c7b，worker 版本 pin 同步 bump 1.273.144 → 1.273.146，随 fleet-rollout 分发。
...
## Brain 1.273.146 — fleet runner digest repin b3ff98ff（40 批信任断言留证镜像上产）

40 批（#5079）修 entrypoint 信任断言失败留证后重建 runner 镜像，canonical pin 从 895f25f0 一次性替换为 b3ff98ff（verify-digest-pin 清单），worker 版本 pin 同步 bump 1.273.136 → 1.273.146，随 fleet-rollout 分发。
...
1.273.112 的 entrypoint 修复构建进 runner 镜像（build head 2c3341a6e），canonical Runner digest 重钉为 `sha256:720c9c7bf02826301bd9b89246c20bc276e3c2b1d5b1991feb00b582832a7288`，全部 pin 位置同步（历史条目保留旧 digest 原文）。
...
run 40f00669 实证 LLM 对抗性适应：9.26 给的「repo 既有测试不受封印闸约束」逃生口被 proposer
用到极致——全部 RED 写进 packages/ 既有测试、sprint tests/ 零产出（盘上有/树里无，finalizer
查盘不查树 + Provider 自 commit 时 diff --cached quiet 跳过 finalizer commit）→ propose 树无
冻结测试 → 封印 requireTests 在 APPROVED 后 failRun（太晚太贵）。修：finalizer push 前
git ls-tree 校验 HEAD 树中 sprint tests/ 非空；proposer skill 9.27 死规则（至少一行冻结测试，
repo 测试仅补充）。教训：给 LLM 的豁免条款必须写清边界，机械闸校验产物而非盘面。
canonical Runner digest 重钉为 `sha256:eda14cf3706a9072422219c8bd7f169a0cffb0e77899917cd5a0b2acee5aa9a5`（docker/build.sh 构建），11 处 pin 同步。
...
1.273.102 的 c8133468 镜像在容量清理（docker image prune -af）中被误删——build 全缓存命中
使镜像 Created 继承旧时间戳，逃过 until=24h 保护。从 main（d8381434）重建同源镜像重钉
（entrypoint sha256 与 102 完全一致 bf46fc7e）。教训：pin 型镜像机器禁用 prune -a。
...
canonical Runner digest 重钉为 `sha256:eda14cf3706a9072422219c8bd7f169a0cffb0e77899917cd5a0b2acee5aa9a5`
...
run ee2f9ff9 实证：合同文档（contract-draft/dod/sprint-prd）由 Provider 落盘随 (Red) 混入
→ TDD 顺序闸红；fix 重排历史 → append-only 血统闸 fail-closed 死锁。修法（fix 陈词方案 b）：
...
run 425c5279 实证：manifest 扫描器把 HARNESS_DEADLINE_AT 并进自己的死线——期限一过，
post-provider 复核进门即 throw，把 evaluator 的 **PASS 判词**错报成「installed dependencies
...
容器 --rm 后明细全丢）。canonical Runner digest 重钉为 `sha256:eda14cf3706a9072422219c8bd7f169a0cffb0e77899917cd5a0b2acee5aa9a5`。
...
- canonical runner digest 重钉为 `sha256:eda14cf3706a9072422219c8bd7f169a0cffb0e77899917cd5a0b2acee5aa9a5`；11 处 pin 同步。
...
- canonical Runner digest 重钉为 `sha256:eda14cf3706a9072422219c8bd7f169a0cffb0e77899917cd5a0b2acee5aa9a5`（`docker/build.sh` 构建，label `cecelia.build.head=e1e2be0af`），11 处 pin 同步；Fleet Worker 版本 pin 不变。
...
- canonical Runner digest 重钉为 `sha256:eda14cf3706a9072422219c8bd7f169a0cffb0e77899917cd5a0b2acee5aa9a5`（`docker/build.sh` 构建，label `cecelia.build.head=65a6c3266`），11 处 pin 同步；Fleet Worker 版本 pin 不变。
...
- canonical Runner digest 重钉为 `sha256:eda14cf3706a9072422219c8bd7f169a0cffb0e77899917cd5a0b2acee5aa9a5`（`docker/build.sh` 构建，label `cecelia.build.head=87079e056`），11 处 pin 同步；Fleet Worker 版本 pin 不变。
...
- canonical Runner digest 重钉为 `sha256:eda14cf3706a9072422219c8bd7f169a0cffb0e77899917cd5a0b2acee5aa9a5`（`docker/build.sh` 构建，label `cecelia.build.head=e0b4a0c93`），11 处 pin 同步；Fleet Worker 版本 pin 不变。
...
- canonical Runner digest 重钉为 `sha256:eda14cf3706a9072422219c8bd7f169a0cffb0e77899917cd5a0b2acee5aa9a5`（`docker/build.sh` 构建，label `cecelia.build.head=fab5b118d`），NodeProfile / rollout / reconciler / installer / smoke 全部同步；Fleet Worker 版本 pin 不变。
