...
### 2.3 单 Brain 原则（Single-Brain Rule）

**核心铁律**：

1. **唯一调度源**：所有任务状态推进（queued→in_progress→completed）、资源分配、熔断/警觉评估，**只有 US Brain 有权执行**。
2. **唯一真相源**：PostgreSQL 是唯一真相源；Brain 是唯一写入者（执行回调通过 Brain API 接收后落库，本质仍是 Brain 写）。
3. **执行端无状态**：HK 执行端只"接任务→执行→回调"，不保存长期调度状态，不运行 tick/planner。

**架构原则**：

```
单 Brain（US）+ 多执行端（US/HK）

US Brain:
  ├─ Tick Loop（唯一）
  ├─ Planner（唯一）
  ├─ Executor Dispatcher（唯一）
  └─ PostgreSQL Writer（唯一）

Execution Runtimes:
  ├─ US Runtime: Claude Code (headed/daemon), Opus/Sonnet
  └─ HK Runtime: MiniMax executor, N8N workflows
```

**不是什么**：
- ❌ 不是"双 Brain"（HK 没有独立的 Brain）
- ❌ Brain 不会"迁移"或"漂移"到 HK
- ❌ HK 不参与调度决策，只执行已分配的任务

**是什么**：
- ✅ Brain 在 US 做决策，选择调用哪个执行端
- ✅ HK 作为执行运行时（Execution Runtime），接受 Brain 派发的任务
- ✅ 所有状态变更由 Brain 在 US 写入唯一的 PostgreSQL
...
### 4.6 任务类型与执行端映射（Executor Routing）

**执行端选择（不是 Brain 路由）**：Brain 在 US 做决策后，决定把任务交给哪个执行端运行。

| task_type | 执行端 | 运行区域 | 说明 |
|-----------|--------|----------|------|
| dev | Claude Code headed/daemon | US | 代码仓库在 US、CI 在 US、Opus 模型 |
| review | Claude Code daemon | US | 只读代码审查、Sonnet 模型 |
| qa | Claude Code daemon | US | QA 总控、Sonnet 模型 |
| audit | Claude Code daemon | US | 代码审计、Sonnet 模型 |
| talk | MiniMax executor | HK | 对话/文档任务、MiniMax LLM |
| research | MiniMax executor | HK | 研究/只读任务、MiniMax LLM |
| data | N8N workflow | HK | 纯工作流/数据管线、无 LLM |

**重点**：
- **路由发生在 US Brain 的 `dispatchNextTask()` 中**，HK 不参与决策
- Brain 通过 `task-router.js` 的 `LOCATION_MAP` 决定执行端
- 执行端接收任务 → 执行 → 回调 US Brain API → Brain 更新状态
...
### 8.1 US Brain + HK Execution Runtime

**架构原则**：单 Brain（US）+ 多执行端（US/HK）

```
┌──────────────────────────────────┐     ┌──────────────────────────────────┐
│  🇺🇸 美国 VPS (Brain + US 执行端)  │     │  🇭🇰 香港 VPS (HK 执行端)          │
│  146.190.52.84                   │     │  43.154.85.217                   │
│                                  │     │                                  │
│  ╔══════════════════════════╗   │     │  ┌────────────────────────────┐ │
│  ║ Brain (唯一调度源)        ║   │     │  │  Execution Runtime (HK)    │ │
│  ║ ├─ Tick Loop (5s/5min)   ║   │◄────┼─►│  ├─ MiniMax executor       │ │
│  ║ ├─ Planner               ║   │HTTP/│  │  ├─ N8N workflows          │ │
│  ║ ├─ Executor Dispatcher   ║───┼─┐   │  │  └─ Publishing workers     │ │
│  ║ └─ Decision Engine       ║   │ │   │  └────────────────────────────┘ │
│  ╚══════════════════════════╝   │ │   │                                  │
│                                  │ │   │  接收任务：talk, research, data  │
│  ╔══════════════════════════╗   │ │   │  回调：POST /api/brain/callback  │
│  ║ PostgreSQL (唯一真相源)   ║◄──┼─┘   │                                  │
│  ║ - 单主写入（Brain only）  ║   │     │  不运行: Brain, Tick, Planner    │
│  ╚══════════════════════════╝   │     │  ENV_REGION=hk (标识，非 Brain)  │
│                                  │     │                                  │
│  ┌────────────────────────────┐ │     └──────────────────────────────────┘
│  │  Execution Runtime (US)    │ │
│  │  ├─ Claude Code (headed)   │ │
│  │  ├─ Claude Code (daemon)   │ │
│  │  └─ Opus/Sonnet LLM        │ │
│  └────────────────────────────┘ │
│                                  │
│  执行任务：dev, review, qa, audit│
│  ENV_REGION=us                   │
└──────────────────────────────────┘
```
...
- **US Brain 是唯一的调度点**：所有 tick、planner、decision-executor 只在 US 运行
- **PostgreSQL 单主架构**：只在 US，Brain 是唯一写入者
- **HK 是纯执行端**：不运行 Brain，不参与调度，只执行已分配的任务
- **跨区域调用**：Brain 通过 HTTP/webhook 调用 HK Runtime，HK 完成后回调 Brain API
- **ENV_REGION 的含义**：标识执行端所在区域，**不代表有独立的 Brain**

### 8.2 容器化

**Brain 容器**：
- 镜像：`cecelia-brain:1.22.4`（多阶段构建，163MB）
...
| `/route-task` | POST | 执行端选择（US/HK Runtime） |
