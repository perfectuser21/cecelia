...
- [x] [BEHAVIOR] 活跃 container 的 worktree 不被 cleanup
  Test: manual:node -e "const c=require('fs').readFileSync('packages/brain/src/__tests__/startup-recovery-active-container-protect.test.js','utf8');if(!c.includes('skipped_active_container'))process.exit(1)"

- [x] [BEHAVIOR] docker probe 失败时安全降级（保守跳过删除，记 warn）
  Test: manual:node -e "const c=require('fs').readFileSync('packages/brain/src/__tests__/startup-recovery-active-container-protect.test.js','utf8');if(!c.includes('skipped_docker_probe'))process.exit(1)"

- [x] [BEHAVIOR] docker probe 返回空 / 路径不匹配仍能删除真正 orphan dir（不破坏既有能力）
  Test: manual:node -e "const c=require('fs').readFileSync('packages/brain/src/__tests__/startup-recovery-active-container-protect.test.js','utf8');if(!c.includes('truly-orphan'))process.exit(1)"
