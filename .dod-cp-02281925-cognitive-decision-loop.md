...
changelog:
  ...
  - 1.3.0: 修复 test 文件引用为 manual:npm 格式
---
...
- [x] 新增测试：emotion-dispatch.test.js（情绪门禁 + 修正系数）
  Test: manual:npm run test --workspace packages/brain -- --run src/__tests__/emotion-dispatch.test.js
- [x] 新增测试：planner-insight.test.js（buildInsightAdjustments 得分调整）
  Test: manual:npm run test --workspace packages/brain -- --run src/__tests__/planner-insight.test.js
