...
describe('GET /abs', () => {
  test('value=-5 → 200 + {result:5, operation:"abs"}（字面严等）', async () => {
  ...
  test('success 响应反向不含 PRD 完整 23 个禁用字段名', async () => {
    const res = await request(app).get('/abs').query({ value: '-5' });
    expect(res.status).toBe(200);
  ...
  test('success 响应 operation 字面 "abs"，PRD 禁用 8 变体一律不等', async () => {
    ...
    for (const v of ['absolute', 'absoluteValue', 'abs_value', 'magnitude', 'modulus', 'positive', 'absval', 'abs_val']) {
      expect(res.body.operation).not.toBe(v);
    }
  });
  ...
  test('strict-schema 非法输入全 400 (1.5 / 1e2 / abc / +5 / 空串 / 缺 value / 0x10 / 千分位)', async () => {
    for (const bad of ['1.5', '1e2', 'abc', '+5', '', '0x10', '1,000']) {
      const res = await request(app).get('/abs').query({ value: bad });
      expect(res.status).toBe(400);
    }
    const missing = await request(app).get('/abs');
    expect(missing.status).toBe(400);
  });
