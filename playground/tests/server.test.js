...
describe('GET /negate — int-only strict + 上下界拒 + -0 双保险规范化 + scope 锁死 [BEHAVIOR]', () => {
  ...
  test('success operation 字面 "negate"，PRD 禁用 8 变体一律不等', async () => {
    const res = await request(app).get('/negate').query({ value: '5' });
    expect(res.status).toBe(200);
  ...
  test('scope 锁死: value=5 + 任意额外 query (extra/foo/neg/重复 value) 全 400', async () => {
  ...
  test('error 路径 value=foo → 400 + body keys 严等 [error]', async () => {
  ...
  test('error body 反向不含 4 个 PRD 禁用替代错误名 (message/msg/reason/detail)', async () => {
    const res = await request(app).get('/negate').query({ value: 'foo' });
    expect(res.status).toBe(400);
    for (const k of ['message', 'msg', 'reason', 'detail']) {
      expect(Object.prototype.hasOwnProperty.call(res.body, k)).toBe(false);
