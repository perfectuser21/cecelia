...
describe('GET /negate — 浮点 strict-schema + 一元负号 + 跨调用自反不变量 oracle [BEHAVIOR]', () => {
  // === Happy 路径（覆盖 n=5/-5/0/-0/3.14/-3.14/100/-100）===
  // ARTIFACT 规约 query 字面量清单: n:'5' n:'-5' n:'0' n:'-0' n:'3.14' n:'-3.14' n:'100' n:'-100'
  ...
  // === 值 oracle 独立复算（正整数 / 负整数 / 正小数 / 负小数 各 ≥ 1）===
  ...
  test('成功响应 schema 完整性: Object.keys 严等 ["negation"]', async () => {
    const res = await request(app).get('/negate').query({ n: '5' });
    expect(res.status).toBe(200);
    expect(Object.keys(res.body)).toEqual(['negation']);
  });
  ...
  test('GET /negate?n=-3.14 响应 negation 必须是 number 类型（非字符串）', async () => {
    const res = await request(app).get('/negate').query({ n: '-3.14' });
    expect(res.status).toBe(200);
    expect(typeof res.body.negation).toBe('number');
  });
  ...
  test('回归 W24 /factorial?n=5 → 200 {factorial: 120}', async () => {
    const res = await request(app).get('/factorial').query({ n: '5' });
    expect(res.status).toBe(200);
    expect(res.body).toEqual({ factorial: 120 });
  });
