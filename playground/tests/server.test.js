...
describe('GET /subtract — strict-schema + minuend/subtrahend query 锁死 + 值/schema oracle [BEHAVIOR]', () => {
  ...
  test('GET /subtract?minuend=0&subtrahend=0 → 200 + result===0（零边界）', async () => {
  ...
  test('GET /subtract?minuend=0&subtrahend=1 → 200 + result===-1（off-by-one 负侧）', async () => {
    const res = await request(app).get('/subtract').query({ minuend: '0', subtrahend: '1' });
    expect(res.status).toBe(200);
    expect(res.body.result).toBe(-1);
  });

  test('GET /subtract?minuend=-5&subtrahend=-3 → 200 + result===-2（双负输入）', async () => {
...
describe('GET /subtract — schema 完整性 + 禁用字段反向 [BEHAVIOR]', () => {
  test('顶层 keys 字面集合 == ["operation","result"]', async () => {
    const res = await request(app).get('/subtract').query({ minuend: '10', subtrahend: '3' });
    expect(res.status).toBe(200);
    expect(Object.keys(res.body).sort()).toEqual(['operation', 'result']);
  });

  test('operation 严格字面字符串 "subtract"（非变体 sub/subtraction/minus/diff）', async () => {
    const res = await request(app).get('/subtract').query({ minuend: '10', subtrahend: '3' });
    expect(res.status).toBe(200);
    expect(res.body.operation).toBe('subtract');
  ...
  test('成功响应不含首要禁用字段（difference/diff/subtraction/subtraction_result/sub_result/minus_result/minus/delta）', async () => {
    const res = await request(app).get('/subtract').query({ minuend: '10', subtrahend: '3' });
    expect(res.status).toBe(200);
...
describe('GET /subtract — strict-schema 拒（10+ 类非法输入）[BEHAVIOR]', () => {
  test('minuend=1e2&subtrahend=1 → 400（拒科学计数法）', async () => {
...
describe('GET /subtract — 缺参 + 错 query 名拒 [BEHAVIOR]', () => {
  ...
  test('GET /subtract?a=10&b=3 → 400（错 query 名 a/b）', async () => {
...
describe('GET /subtract — 错误体 schema 完整性 [BEHAVIOR]', () => {
...
describe('GET /subtract — 已有 8 路由不被破坏 [BEHAVIOR]', () => {
  test('GET /health → 200 + {ok:true}', async () => {
    const res = await request(app).get('/health');
    expect(res.status).toBe(200);
    expect(res.body).toEqual({ ok: true });
