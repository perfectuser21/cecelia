...
describe('GET /decrement — happy 值复算 + schema 完整性 [BEHAVIOR]', () => {
  ...
  test('GET /decrement?value=-5 → 200 + {result:-6}', async () => {
    const res = await request(app).get('/decrement').query({ value: '-5' });
    expect(res.status).toBe(200);
    expect(res.body.result).toBe(-6);
    expect(res.body.operation).toBe('decrement');
...
describe('GET /decrement — off-by-one 防盲抄 W26 increment [BEHAVIOR]', () => {
  test('GET /decrement?value=0 → 200 + {result:-1}（防 generator 抄 W26 +1）', async () => {
  ...
  test('GET /decrement?value=1 → 200 + {result:0}（off-by-one 正侧 2）', async () => {
    const res = await request(app).get('/decrement').query({ value: '1' });
    expect(res.status).toBe(200);
    expect(res.body.result).toBe(0);
  });

  test('GET /decrement?value=-1 → 200 + {result:-2}（负侧）', async () => {
    const res = await request(app).get('/decrement').query({ value: '-1' });
    expect(res.status).toBe(200);
    expect(res.body.result).toBe(-2);
  });
...
describe('GET /decrement — 精度上下界 happy [BEHAVIOR]', () => {
  test('GET /decrement?value=9007199254740990 → 200 + {result:9007199254740989}', async () => {
  ...
  test('GET /decrement?value=-9007199254740990 → 200 + {result:-9007199254740991}（=== -Number.MAX_SAFE_INTEGER）', async () => {
    const res = await request(app).get('/decrement').query({ value: '-9007199254740990' });
    expect(res.status).toBe(200);
    expect(res.body.result).toBe(-9007199254740991);
  });
...
describe('GET /decrement — strict-schema 拒 [BEHAVIOR]', () => {
  ...
  for (const [v, label] of cases) {
    test(`GET /decrement?value=${JSON.stringify(v)} (${label}) → 400`, async () => {
...
describe('GET /decrement — 错 query 名 + 缺参 [BEHAVIOR]', () => {
  test('GET /decrement（无参） → 400', async () => {
  ...
  test('GET /decrement?x=5（错 query 名 x） → 400', async () => {
    const res = await request(app).get('/decrement').query({ x: '5' });
    expect(res.status).toBe(400);
  });

  test('GET /decrement?value=5&extra=1（多余 query 字段） → 400', async () => {
...
describe('GET /decrement — 前导 0 happy（十进制归一化，非八进制） [BEHAVIOR]', () => {
  test('GET /decrement?value=01 → 200 + {result:0}', async () => {
  ...
  test('GET /decrement?value=007 → 200 + {result:6}（不许错用八进制）', async () => {
