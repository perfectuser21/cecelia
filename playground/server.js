...
app.get('/decrement', (req, res) => {
  // 成功 schema 字面: { result: <number>, operation: "decrement" }；strict ^-?\d+$；双侧上界 |value| ≤ 9007199254740990；query 名 req.query.value
  const DEC_STRICT_INT = /^-?\d+$/;
  const keys = Object.keys(req.query);
  const v = req.query.value;
  const n = Number(v);
  if (keys.length !== 1 || keys[0] !== 'value' || typeof v !== 'string' || !DEC_STRICT_INT.test(v) || Math.abs(n) > 9007199254740990) {
    return res.status(400).json({ error: 'value 必须是唯一 query 名 + 匹配 ^-?\\d+$（仅整数；禁小数、前导 +、双重负号、科学计数法、十六进制、千分位、空格、Infinity、NaN、空串）+ |value| ≤ 9007199254740990' });
  }
