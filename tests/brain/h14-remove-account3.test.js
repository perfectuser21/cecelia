...
//   2026-07-02（PR #3506）：account1 加回凭据监控名单——credentials-health-scheduler.js 与
//     credential-expiry-checker.js 均 = [account1, account2]。该 PR 未同步本测试（vitest --changed
//     漏跑 fs 读取型测试，known issue），main 上断言 stale，随 #3511 对齐。
//
// 本回归测试守护的不变量是【account3 永不出现在任何池里】（org 禁用）：
//   - account-usage.js（调度池）= [account1, account2]（account1 凭据已恢复，与 account2 轮换）
//   - credentials-health-scheduler.js / credential-expiry-checker.js = [account1, account2]（#3506 起监控 account1）
//   - 三处一律禁 account3。
...
describe('账号池守护 — account3 永不出现在任何池里（org 禁用）', () => {
  ...
  test('credentials-health-scheduler.js CLAUDE_ACCOUNTS = [account1, account2]（#3506 起监控 account1；禁 account3）', () => {
  ...
  test('credential-expiry-checker.js ACCOUNTS = [account1, account2]（#3506 起监控 account1；禁 account3）', () => {
