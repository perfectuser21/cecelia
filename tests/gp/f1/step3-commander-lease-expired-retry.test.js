// F1「工厂 · 开发闭环」步骤 3「造完真验」—— 边：commander lease 过期有界自动重派 [r75]
//
// 与 sprints/08252100-kernel-r75-commander-retry/tests/commander-lease-expired-retry-bounded.test.ts
// 是同一改动的两个 CI 闸产物：那份满足封印闸「sprints/<sprint_dir>/tests/ 至少一行冻结测试」+
// finalizer HEAD 树校验；本文件满足 F1 gp-anchor 闸。两者都真 import 被改文件 derive.js，
// 禁 mock 被改的边（commander infra 路由 ↔ decisionLog 行）。
//
// 背景（本主题第四次点火，r70–r74 案卷）：kernel 编排器 derive.js 的 attemptCallbackRoute 对
// infrastructure_blocked 失败按角色查 INFRA_RETRY_ACTION_BY_ROLE 重派，该表独缺 commander。故
// commander attempt lease 过期被收割器 reconcile（effect:expired_attempt_reconciled,
// role=commander, failure_class=infrastructure_blocked）后 infrastructureRetryForCallback 返回
// undefined → 直接 wait:human_review（callback_infrastructure_route_unknown），每轮都要人审，
// 破坏 zero-human-gate。
//
// 修法（本批，纯函数）：commander 纳入「有界」infrastructure 重试——
//  · 同 run 内 commander infrastructure 类 expired 行累计 < 5：attemptCallbackRoute 不再挂人审
//    （返回不阻塞路由，主链继续；commander 是监理角色，其重派由 commanderCoordinator 在下一 tick
//     独立负责，重派安全无副作用）。可观测契约：derive 结果 action ≠ wait:human_review 且
