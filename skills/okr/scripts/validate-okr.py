#!/usr/bin/env python3
"""
OKR Validation Script with Anti-Cheating (v9.0.0)
- Information Gate: validates field sources before DB write (NEW)
...
PLACEHOLDER_VALUES = {None, "", "未知", "待定", "TBD", "TODO", "unknown", "tbd", "todo"}

REQUIRED_FIELDS_BY_LAYER = {
    "kr": [
        ("metric_from", "KR 基线值（当前是多少）"),
        ("metric_to", "KR 目标值（要到多少）"),
        ("measurement_method", "如何度量"),
        ("repos_involved", "涉及哪些仓库"),
    ],
    "project": [
        ("current_state", "当前实现状态（至少50字描述）"),
        ("repos", "涉及的仓库路径列表"),
        ("out_of_scope", "明确说了什么不做"),
    ],
    "initiative": [
        ("target_repo", "目标仓库路径"),
        ("current_implementation", "当前代码实现状态"),
    ],
}


def check_information_gate(data):
    """Information Gate: validate field completeness and source legitimacy.

    Ensures every required field for the given layer:
    1. Exists and has a real value (not a placeholder)
    2. Has a corresponding _source field
    3. The _source is NOT 'assumed'

    Returns:
        dict with keys:
            - violated (bool): True if any violation found
            - violations (list[str]): Human-readable violation messages
            - layer (str): The identified_layer value
            - fields_checked (int): How many fields were checked
    """
    layer = data.get("identified_layer", "")
    required_fields = REQUIRED_FIELDS_BY_LAYER.get(layer, [])

    # If no identified_layer or no required fields for this layer, pass through
    if not required_fields:
        return {"violated": False, "violations": [], "layer": layer, "fields_checked": 0}
    ...
    for field_name, field_desc in required_fields:
        ...
        if field_name not in data:
            violations.append(f"缺少必填字段 '{field_name}'（{field_desc}）")
            continue

        # 2. Value must not be a placeholder
        # Note: lists are not hashable, so check isinstance before 'in set'
        if isinstance(value, list):
            # Lists handled separately below; skip placeholder check
            pass
        elif value in PLACEHOLDER_VALUES:
            violations.append(
                f"字段 '{field_name}' 值无效（当前: {repr(value)}）— 需要真实值"
            )
            continue

        # 3. Lists must not be empty
        if isinstance(value, list) and len(value) == 0:
            violations.append(f"字段 '{field_name}' 不能是空列表")
            continue

        # 4. current_state must be at least 50 characters
        if field_name == "current_state" and isinstance(value, str) and len(value) < 50:
            violations.append(
                f"字段 'current_state' 太短（{len(value)}字），需至少50字描述现状"
            )
            continue

        # 5. _source field must exist
        if source_key not in data:
            violations.append(
                f"缺少来源字段 '{source_key}'（必须是 user_confirmed / ai_explored）"
            )
            continue

        # 6. _source must not be 'assumed' (the most critical check)
        if source == "assumed":
            violations.append(
                f"字段 '{field_name}' 来源是 'assumed'（禁止猜测）"
                f" — 请向用户确认或探索代码获取"
            )
            continue

        # 7. _source must be a valid value
        valid_sources = {"user_confirmed", "ai_explored"}
        if source not in valid_sources:
            violations.append(
                f"字段 '{source_key}' 值无效（当前: '{source}'）"
                f" — 必须是 user_confirmed 或 ai_explored"
            )
...
def main():
    ...
    # Information Gate: check before form validation
    gate_result = check_information_gate(data)
    if gate_result['violated']:
        print(f"\n{'='*60}")
        print(f"  ⛔ INFORMATION GATE VIOLATED")
        print(f"{'='*60}")
        for violation in gate_result['violations']:
            print(f"  ✗ {violation}")
        print(f"\n  必须先收集完整信息才能继续拆解。")
        print(f"  请回去补充缺失的字段，或向用户确认 assumed 的字段。")
        sys.exit(2)
