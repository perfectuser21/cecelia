...
## CI L4 runner 切换至西安 Mac Mini M1（2026-03-12）

### 根本原因
L4 brain-integration job 原配置为 `us-mac-m4`（Mac Mini US）runner，但用户明确要求只使用 HK VPS（Linux）+ 西安 Mac Mini M1（macOS），不使用 Mac Mini US。runner 标签配置与实际使用策略不一致。
