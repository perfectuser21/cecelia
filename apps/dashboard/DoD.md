...
- [x] [ARTIFACT] `apps/dashboard/src/pages/harness-pipeline/HarnessPipelineDetailPage.tsx` 包含步骤列表和三栏视图组件
  Test: node -e "require('fs').accessSync('apps/dashboard/src/pages/harness-pipeline/HarnessPipelineDetailPage.tsx');console.log('OK')"
- [x] [BEHAVIOR] 组件实际代码（排除注释）引用 steps 数组并渲染 Input/Prompt/Output 三栏，无内容时显示"暂无数据"
